...
require_config "$config" 'SILIECO_APP_URL: http://localhost:3100'
...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "SILIECO_APP_URL=${SILIECO_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "SILIECO_SERVER_URL=${SILIECO_SERVER_URL}" \
...
require_env "$local_env" 'SILIECO_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'SILIECO_SERVER_URL=ws://localhost:9100/ws'
