...
GUARD_DIR=$(mktemp -d "${TMPDIR:-/tmp}/silieco-agent-cli-guard.XXXXXX")
...
printf '%s\n' "$line" >>"$SILIECO_AGENT_CLI_GUARD_MARKER"
exit 126
EOF
  chmod 755 "$sentinel"
done <"$NAMES_FILE"
...
set +e
PATH="$BIN_DIR:$PATH" SILIECO_AGENT_CLI_GUARD_MARKER="$MARKER_FILE" "$@"
command_status=$?
set -e
