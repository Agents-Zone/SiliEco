...
_setup_sandbox() {
  ...
  cat >"$payload_dir/silieco" <<'STUB'
#!/usr/bin/env bash
echo "silieco v0.3.2 (commit: test)"
STUB
  chmod +x "$payload_dir/silieco"
  tar -czf "$tmp/silieco.tar.gz" -C "$payload_dir" silieco

  cat >"$stub_bin/curl" <<'STUB'
#!/usr/bin/env bash
if [[ "$*" == *"-sI"* ]]; then
  printf 'HTTP/2 302\r\nlocation: https://github.com/silieco-ai/silieco/releases/tag/v0.3.2\r\n'
  exit 0
fi
...
_run_installer() {
  ...
  if ! PATH="$tmp/stub-bin:$tmp/install-bin:/usr/bin:/bin" \
    SILIECO_BIN_DIR="$tmp/install-bin" \
    SILIECO_TEST_ARCHIVE="$tmp/silieco.tar.gz" \
  ...
  if [[ ! -x "$tmp/install-bin/silieco" ]]; then
    echo "expected fallback binary at $tmp/install-bin/silieco" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    return 1
  fi
...
STUB
  ...
  if ! grep -q "https://silieco.ai/settings?tab=tokens" "$tmp/install.out"; then
  ...
  if ! grep -q "silieco login --token <YOUR_TOKEN>" "$tmp/install.out"; then
    echo "expected token login command in installer output" >&2
    cat "$tmp/install.out" >&2 || true
    return 1
  fi
  if grep -q "silieco config set server_url" "$tmp/install.out"; then
    echo "did not expect default cloud server config command in installer output" >&2
    cat "$tmp/install.out" >&2 || true
    return 1
  fi
  if grep -q "silieco config set app_url" "$tmp/install.out"; then
