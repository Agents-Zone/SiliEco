...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/silieco-test-go.XXXXXX")
...
mkdir -p "$BIN_DIR"
export SILIECO_TEST_GO_CALLS="$CALLS_FILE"
...
case "${1:-}" in
  list)
    ...
    printf '%s\n' \
      github.com/silieco-ai/silieco/core \
      github.com/silieco-ai/silieco/core/internal/daemon \
      github.com/silieco-ai/silieco/core/pkg/agent \
      github.com/silieco-ai/silieco/core/pkg/agent/internal/testutil
    ;;
  test)
    printf '%s\n' "$*" >>"$SILIECO_TEST_GO_CALLS"
    ;;
  *)
...
expected_calls='test -race github.com/silieco-ai/silieco/core github.com/silieco-ai/silieco/core/internal/daemon
