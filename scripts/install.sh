#!/usr/bin/env bash
# Silieco installer — installs the CLI and optionally provisions a self-host server.
#
...
#
# After installation, run `silieco setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/silieco-ai/silieco.git"
REPO_WEB_URL="https://github.com/silieco-ai/silieco"  # without .git, for GitHub web APIs
INSTALL_DIR="${SILIECO_INSTALL_DIR:-$HOME/.silieco/server}"
BREW_PACKAGE="silieco-ai/tap/silieco"
...
print_remote_server_token_hint() {
  ...
  printf "     1. On your local computer, open ${CYAN}https://silieco.ai/settings?tab=tokens${RESET}\n"
  printf "        and create a token under ${BOLD}Settings > API Tokens${RESET}.\n"
  printf "     2. On this server, run:\n"
  printf "        ${CYAN}silieco login --token <YOUR_TOKEN>${RESET}\n"
  printf "        ${CYAN}silieco daemon start${RESET}\n"
  printf "\n"
}
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). Silieco supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing Silieco CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap silieco-ai/tap >"$brew_log" 2>&1; then
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "Silieco CLI already installed via Homebrew"
    else
      warn "Failed to install silieco via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "Silieco CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing Silieco CLI from GitHub Releases..."
  ...
  local url="https://github.com/silieco-ai/silieco/releases/download/${latest}/silieco-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/silieco.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/silieco.tar.gz" -C "$tmp_dir" silieco

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with SILIECO_BIN_DIR.
  local bin_dir="${SILIECO_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/silieco" "$bin_dir/silieco"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/silieco" "$bin_dir/silieco"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/silieco" "$bin_dir/silieco"
    chmod +x "$bin_dir/silieco"
  ...
  rm -rf "$tmp_dir"
  ok "Silieco CLI installed to $bin_dir/silieco"
}
...
get_selfhost_ref() {
  if [ -n "${SILIECO_SELFHOST_REF:-}" ]; then
    printf '%s' "$SILIECO_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading Silieco CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "Silieco CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "Silieco CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists silieco; then
    local current_ver
    # `silieco version` outputs "silieco 0.3.23 (commit: f46b929eb, built: 2026-06-16T10:11:56Z)" — extract just the version
    current_ver=$(silieco version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "Silieco CLI is up to date ($current_ver)"
      return 0
    fi

    info "Silieco CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(silieco version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ok "Silieco CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists silieco; then
    fail "CLI installed but 'silieco' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. Silieco self-hosting requires Docker and Docker Compose.
...
# ---------------------------------------------------------------------------
setup_server() {
  info "Setting up Silieco server..."
  local server_ref
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning Silieco repository..."
  ...
  info "Pulling official Silieco images..."
  pull_official_selfhost_images
  info "Starting Silieco services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  Silieco — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ Silieco CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}silieco setup${RESET}                # Connect to Silieco Cloud (silieco.ai)\n"
  printf "     ${CYAN}silieco setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  print_remote_server_token_hint
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  Silieco — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ Silieco server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}silieco setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  printf "\n"
  info "Stopping Silieco services..."
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No Silieco installation found at $INSTALL_DIR"
  fi

  if command_exists silieco; then
    silieco daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the Silieco CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  SILIECO_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.silieco/server)"
        echo "  SILIECO_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  SILIECO_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'silieco setup' to configure your environment."
        exit 0
        ;;
