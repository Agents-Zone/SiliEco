# Silieco installer for Windows — one command to get started.
#
# Install CLI (default): connects to silieco.ai
#   irm https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Silieco server + installs CLI + configures
#   $env:SILIECO_MODE="local"; irm https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/silieco-ai/silieco.git"
$RepoWebUrl    = "https://github.com/silieco-ai/silieco"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".silieco\server"
$InstallDir    = if ($env:SILIECO_INSTALL_DIR) { $env:SILIECO_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/silieco-ai/silieco/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:SILIECO_SELFHOST_REF) {
        return $env:SILIECO_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = silieco version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Silieco CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Silieco requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/silieco-ai/silieco/releases/download/$latest/silieco-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "silieco-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "silieco.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/silieco-ai/silieco/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "silieco.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "silieco-cli-$version-windows-$arch.zip"
        $legacyAsset = "silieco_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "silieco.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".silieco\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "silieco.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "silieco.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "silieco.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "silieco.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Silieco CLI installed to $binDir\silieco.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "silieco") {
        ...
        if ($isUpToDate) {
            Write-Ok "Silieco CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Silieco CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Silieco CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "silieco")) {
        Write-Fail "CLI installed but 'silieco' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Silieco self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:SILIECO_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Silieco server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Silieco repository..."
    ...
    Write-Info "Pulling official Silieco images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Silieco services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Silieco server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Silieco - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Silieco CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     silieco setup               " -NoNewline; Write-Host "# Connect to Silieco Cloud (silieco.ai)" -ForegroundColor DarkGray
    Write-Host "     silieco setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:SILIECO_MODE="with-server"; irm https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Silieco - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Silieco server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     silieco setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:SILIECO_MODE="stop"; irm https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Silieco services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Silieco installation found at $InstallDir"
    }

    if (Test-CommandExists "silieco") {
        try {
            silieco daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:SILIECO_MODE) { $env:SILIECO_MODE.ToLower() } else { "default" }
