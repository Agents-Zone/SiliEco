...
(cd core && go run ./cmd/migrate up) || { EXIT_CODE=1; exit 1; }
...
if curl -sf "http://localhost:${PORT}/health" > /dev/null 2>&1; then
  ...
  (cd core && go run ./cmd/server) > /tmp/silieco-check-backend.log 2>&1 &
  BACKEND_PID=$!
...
if curl -sf "http://localhost:${FRONTEND_PORT}" > /dev/null 2>&1; then
  ...
  pnpm dev:web > /tmp/silieco-check-frontend.log 2>&1 &
  FRONTEND_PID=$!
