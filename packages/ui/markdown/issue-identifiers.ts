 ...
 *
 * Rewrites bare issue identifiers like `SILI-123` / `TES-1` into canonical
 * mention links `[SILI-123](mention://issue/SILI-123)`, so the shared markdown
...
/**
 * True when `value` is a bare issue identifier (e.g. "SILI-123"). A UUID never
...
export function preprocessIssueIdentifiers(text: string): string {
  ...
  while ((match = IDENTIFIER_RE.exec(text)) !== null) {
    ...
    // end ("see SILI-1.") and stays linkable.
    ...
    // Embedded in a dotted name on the left (`file.SILI-1`).
