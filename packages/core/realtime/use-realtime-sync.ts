...
//
// SECURITY (review on PR #5018 / SILI-4159): this is deliberately an
...
export function applyChatDoneToCache(
  ...
  if (messageId && content !== undefined) {
    const assistant: ChatMessage = {
      ...
      sender_agent_id: payload.sender_agent_id ?? null,
      elapsed_ms: payload.elapsed_ms ?? null,
      // Carry the kind so a no_response turn renders its placeholder inline
      // without waiting for the reconciling refetch (SILI-4351). Missing →
 ...
 * unread badge lit forever — the same SILI-4360 stuck-badge bug, one surface over.
 ...
 * identifier (`SILI-123`) is recomputed from the workspace prefix at read time.
 *
...
export async function handleInboxNew(
  ...
  // the user is already looking at Silieco, the inbox sidebar's unread
  ...
  useEffect(() => {
    ...
    const refreshMap: Record<string, () => void> = {
      ...
      task: () => {
        ...
        // (signature), mirroring its query design (SILI-3375).
      },
    };
    ...
    // threads, SILI-1941). Inactive observers don't refetch either way;
    ...
    const unsubCommentCreated = ws.on("comment:created", (p) => {
      ...
      // (SILI-5009), so any open board/list sorted by "Updated date" has
    ...
    //
    // SECURITY (review on PR #5018 / SILI-4159): chat `task:*` events are a
    ...
    // message and would re-create the request storm SILI-4159 fixed); the
    ...
    const unsubChatMessage = ws.on("chat:message", (p) => {
      ...
      // Group chat messages can be ordinary human messages with no task
      // lifecycle event. Refresh list order, preview and each participant's
      // unread count directly from the server-owned read cursor.
      ...
      // by the task lifecycle handlers below (SILI-4159).
    });
    ...
    const unsubChatDone = ws.on("chat:done", (p) => {
      ...
      // the aggregate on every turn (SILI-4159).
    ...
    const unsubChatSessionUpdated = ws.on("chat:session_updated", (p) => {
      ...
      const id = getCurrentWsId();
      if (!id) return;
      if (payload.participants_changed) {
