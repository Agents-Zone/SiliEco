...
export const issueKeys = {
  ...
  /** Resolve a bare issue identifier (e.g. "SILI-123") to an issue. */
 ...
 * (SILI-4290), so it lives in the cache and renders like any other column;
...
/**
 * Resolve a bare issue identifier ("SILI-123") to its issue, or `null`.
 *
 * Backs the Linear-style autolink: the backend `q` search matches an
 * identifier on issue NUMBER only (prefix-agnostic — `SILI-123` and `TES-123`
 ...
 * match `listChildrenByParentsLimit` in core/internal/handler/issue.go.
