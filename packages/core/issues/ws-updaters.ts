...
export function onIssueUpdated(
  ...
  // and keeps a new frontend on an old backend from regressing (SILI-3669 /
...
export function onIssueMetadataChanged(
  ...
  // event is server-committed, so refetch those keys to re-sort (SILI-5016).
...
export function onIssuePropertiesChanged(
  ...
  // mutation onSuccess); the optimistic leg uses patchIssueProperties (SILI-5016).
  invalidateUpdatedAtSortedIssueLists(qc, wsId);
}
