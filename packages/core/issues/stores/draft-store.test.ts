...
describe("issue draft store — legacy rehydrate", () => {
  ...
  it("migrates a pre-SILI-5181 flat draft into the shared/manual slots", async () => {
    localStorage.setItem(
      "silieco_issue_draft:acme",
      JSON.stringify({
        state: {
          draft: {
            title: "legacy",
...
describe("issue draft store — logout cleanup", () => {
  ...
  it("logout sequence (reset in-memory, then clear storage) leaves no persisted key", async () => {
    ...
    setLastAssignee("member", "alice");
    expect(localStorage.getItem("silieco_issue_draft:acme")).not.toBeNull();
    ...
    expect(localStorage.getItem("silieco_issue_draft:acme")).toBeNull();
  });
});
