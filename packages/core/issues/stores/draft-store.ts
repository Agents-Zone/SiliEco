...
// One logical Issue-Create draft (SILI-5181), split so switching between the
...
export interface IssueCreateShared {
   ...
   *  only the ids its own content references. Coordinator-owned (SILI-5181 L2):
...
// use the pre-SILI-5181 flat shape. Backfill defaults so every read site can
...
// in that store — it lived in `silieco_quick_create` and is not carried over).
...
registerDraftCleanup({
  storageKey: "silieco_issue_draft",
