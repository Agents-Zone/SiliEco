...
describe("ApiClient server Table query", () => {
  it("posts the canonical query to the group and branch endpoints", async () => {
    ...
    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual([
      "https://api.example.test/api/tasks/table/groups",
      "https://api.example.test/api/tasks/table/rows",
      "https://api.example.test/api/tasks/table/facets",
    ]);
...
describe("ApiClient issue move intent", () => {
  it("posts relative anchors without a client-authored position", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.example.test/api/tasks/issue-1/move",
...
describe("ApiClient", () => {
  ...
  it("uses the expected HTTP contract for comment trigger preview and suppress", async () => {
    ...
    expect(fetchMock.mock.calls.map(([url, init]) => ({
      ...
      {
        url: "https://api.example.test/api/tasks/issue-1/comments/trigger-preview",
        method: "POST",
        body: JSON.stringify({ content: "hello", parent_id: "parent-1", editing_comment_id: "comment-1" }),
      },
      {
        url: "https://api.example.test/api/tasks/issue-1/comments",
        method: "POST",
        body: JSON.stringify({
          content: "hello",
          type: "comment",
  ...
  describe("cancelTaskById response parsing", () => {
    ...
    it("parses the cancelled chat message payload", async () => {
      ...
      expect(fetchMock.mock.calls[0]).toMatchObject([
        "https://api.example.test/api/runs/task-1/cancel",
        { method: "POST" },
      ]);
  ...
  describe("chat attachment wiring", () => {
    ...
    it("threads an AbortSignal into fetch so the coordinator can cancel it (SILI-5181)", async () => {
      const fetchMock = vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ id: "att-1", url: "https://cdn/x" }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
