...
export interface AppConfigResponse {
  ...
  // (SILI-3254). Older servers omit the field; treat that as false.
  cdn_signed?: boolean;
...
// SILI-3192 omits the field, in which case the schema defaults it to "".
...
// Per-target outcome of an explicit @agent / @squad mention (SILI-4525 §2).
...
export const CommentTriggerPreviewSchema = z.object({
  ...
  // Drop malformed blocked entries INDIVIDUALLY (SILI-4525): a single bad item
...
export const IssueSchema = z.object({
  ...
  // Older servers do not know about optional SOP assignment. Missing values
  // therefore degrade to a plain Task rather than invalidating the response.
  workflow_instance_id: z.string().nullable().default(null),
  workflow_stage_id: z.string().nullable().default(null),
  position: z.number(),
...
export const WorkflowSchema = z.object({
  ...
  current_version: WorkflowVersionSchema.optional(),
  created_by: z.string(),
  created_at: z.string(),
  updated_at: z.string(),
}).loose();
...
export const WorkflowGateDecisionSchema = z.object({
  ...
  to_stage_id: z.string().nullable().default(null),
  outcome: z.string(),
  actor_type: z.string(),
  actor_id: z.string(),
...
export const ListWorkflowInstancesResponseSchema = z.object({
  instances: z.array(WorkflowInstanceSchema).default([]),
  total: z.number().default(0),
}).loose();

export const ListWorkflowTasksResponseSchema = z.object({
  tasks: z.array(IssueSchema).default([]),
  total: z.number().default(0),
}).loose();
...
export const EMPTY_WORKFLOW_VERSION: WorkflowVersion = {
  id: "",
  workspace_id: "",
  workflow_id: "",
  version: 0,
...
export const EMPTY_LIST_WORKFLOWS_RESPONSE: ListWorkflowsResponse = {
  workflows: [],
  total: 0,
};

export const EMPTY_LIST_WORKFLOW_VERSIONS_RESPONSE: ListWorkflowVersionsResponse = {
  versions: [],
  total: 0,
};

export const EMPTY_LIST_WORKFLOW_INSTANCES_RESPONSE: ListWorkflowInstancesResponse = {
  instances: [],
  total: 0,
};

export const EMPTY_LIST_WORKFLOW_TASKS_RESPONSE: ListWorkflowTasksResponse = {
  tasks: [],
  total: 0,
};
...
// (core/pkg/taskfailure). Pinning it to a z.enum would make an installed
...
// Human attribution (SILI-4302 §9): who an agent run is accountable to, and how
...
// ---------------------------------------------------------------------------
// Agent invocation permissions (SILI-3963)
//
...
const AutopilotListItemSchema = z.object({
  ...
  // Older servers (pre-SILI-2429) omit assignee_type; "agent" is the
  // documented default.
...
// which branches on `status` to avoid a false-success toast (SILI-4525), so the
...
// SILI-2406; the server emits `""` when unset (NOT NULL DEFAULT ''), so
...
//
// All billing JSON we receive comes from silieco-cloud verbatim — we proxy
...
// server-side catalog cache (SILI-5444); an older backend omits them.
// ---------------------------------------------------------------------------
