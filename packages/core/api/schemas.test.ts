...
describe("IssueSchema (via ListIssuesResponseSchema)", () => {
  it("defaults optional workflow coordinates to null for older servers", () => {
    const parsed = ListIssuesResponseSchema.parse({
      issues: [baseIssue],
      total: 1,
    });
    expect(parsed.issues[0]?.workflow_instance_id).toBeNull();
    expect(parsed.issues[0]?.workflow_stage_id).toBeNull();
  });
...
describe("Workflow schemas", () => {
  ...
  it("parses a published SOP with immutable stages", () => {
    ...
    expect(parsed.current_version?.stages[0]?.stable_key).toBe("intake");
  });

  it("parses one workflow run with Task and shared gate decisions", () => {
    const parsed = WorkflowInstanceSchema.parse({
      id: "instance-1",
      workspace_id: "ws-1",
      workflow_id: "workflow-1",
      workflow_version_id: "version-1",
      ...
      updated_at: "2026-01-01T00:00:00Z",
      stages: [stage],
      tasks: [{ ...baseIssue, workflow_instance_id: "instance-1", workflow_stage_id: "stage-1" }],
      decisions: [],
    });
...
// (SILI-3375). The four entry points read it to decide "will this start a run",
...
describe("DuplicateIssueErrorBodySchema", () => {
  const valid = {
    ...
    error: "An active issue with this title already exists: SILI-12 – Login bug",
    issue: {
      id: "11111111-1111-1111-1111-111111111111",
      identifier: "SILI-12",
      title: "Login bug",
...
describe("dashboard + runtime usage schema drift", () => {
  ...
  it("coerces a missing failure row field without dropping the array", () => {
    ...
    // (core/internal/handler/dashboard_test.go) guards the other side by
...
describe("AppConfigSchema cdn_signed drift", () => {
  it("defaults cdn_signed to false when the server omits it (pre-SILI-3254 servers)", () => {
...
// toast (SILI-4525), so the trigger response must survive backend drift.
...
// (SILI-4525 §2), so the additive field must parse and degrade gracefully.
...
describe("RuntimeModelListRequestSchema", () => {
  ...
  // A backend that predates SILI-5444 sends neither marker; an even older one
