...
describe("authStore.initialize — token mode", () => {
  it("keeps the stored token when getMe fails with a non-401 ApiError (e.g. 500)", async () => {
    const storage = makeStorage({ silieco_token: "t" });
    const api = makeApi(() =>
    ...
    expect(storage.snapshot().silieco_token).toBe("t");
  });
  ...
  it("on 401, leaves storage cleanup to ApiClient.onUnauthorized and resets state", async () => {
    ...
    const storage = makeStorage({ silieco_token: "t" });
    const api = makeApi(() => {
      storage.removeItem("silieco_token"); // stand-in for onUnauthorized
    ...
    expect(store.getState().user).toBeNull();
    expect(storage.snapshot().silieco_token).toBeUndefined();
  });
