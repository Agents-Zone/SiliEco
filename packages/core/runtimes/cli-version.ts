 ...
 * silieco CLI to be at least this version — older daemons either
 * double-create issues on partial CLI failures, drop quick-create attachment
 * bindings, or mishandle pasted screenshot URLs (see PR #1851 / SILI-1496).
 *
 ...
 * (`core/pkg/agent/version.go`). The assignment handoff note is only rendered
 * into the run's opening prompt by daemons at or above this silieco CLI version
 * (SILI-3375); older daemons silently drop it. Unlike the quick-create gate this
