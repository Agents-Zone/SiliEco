...
// The pre-SILI-4864 scheme kept one new-chat draft per agent, in `__new__:<id>`
...
describe("chat store — legacy per-agent new-chat draft migration", () => {
  const DRAFTS_KEY = "silieco:chat:drafts";
  const ATTACHMENTS_KEY = "silieco:chat:draft-attachments";
  const AGENT_KEY = "silieco:chat:selectedAgentId";
  ...
  it("migrates the matching attachments with the text, not another agent's", () => {
    ...
    // Legacy bare Attachment rows load as `uploaded` entries (SILI-5181 L2).
    expect(
      store
        .getState()
...
describe("chat store — open/closed default", () => {
  ...
  it("honours an explicit stored 'open' preference", () => {
    const storage = memStorage();
    storage.setItem("silieco:chat:isOpen", "true");
    const store = createChatStore({ storage });
  ...
  it("persists a toggle so the choice survives reload", () => {
    ...
    expect(storage.getItem("silieco:chat:isOpen")).toBe("true");
...
describe("chat store — selected project", () => {
  it("persists and clears the next chat's project per workspace", () => {
    ...
    expect(storage.getItem("silieco:chat:selectedProjectId")).toBe("project-1");
    expect(createChatStore({ storage }).getState().selectedProjectId).toBe("project-1");

    store.getState().setSelectedProjectId(null);
    expect(storage.getItem("silieco:chat:selectedProjectId")).toBeNull();
  });
});
...
describe("chat store — floating window preference", () => {
  ...
  it("honours an explicit stored 'false' preference (opt-out)", () => {
    const storage = memStorage();
    storage.setItem("silieco:chat:floatingChatEnabled", "false");
    const store = createChatStore({ storage });
  ...
  it("honours an explicit stored 'true' preference", () => {
    const storage = memStorage();
    storage.setItem("silieco:chat:floatingChatEnabled", "true");
    const store = createChatStore({ storage });
    expect(store.getState().floatingChatEnabled).toBe(true);
  });
  ...
  it("persists an enable, then collapses an open overlay when disabled again", () => {
    ...
    expect(store.getState().floatingChatEnabled).toBe(false);
    expect(store.getState().isOpen).toBe(false);
    expect(storage.getItem("silieco:chat:floatingChatEnabled")).toBe("false");
    ...
    expect(store.getState().floatingChatEnabled).toBe(true);
    expect(storage.getItem("silieco:chat:floatingChatEnabled")).toBe("true");
  });
});
...
// Coordinator-owned upload lifecycle in the draft slots (SILI-5181 L2).
