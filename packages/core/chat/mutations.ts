...
export function useCreateGroupChatSession() {
  const qc = useQueryClient();
  const wsId = useWorkspaceId();
  ...
  return useMutation({
    ...
    }) => api.createGroupChatSession(data),
    onSuccess: (session) => {
      logger.info("createGroupChatSession.success", { sessionId: session.id });
    ...
    onError: (err) => logger.error("createGroupChatSession.error", err),
    onSettled: () => qc.invalidateQueries({ queryKey: chatKeys.sessions(wsId) }),
  });
}

export function useUpdateGroupChatParticipants() {
  const qc = useQueryClient();
  const wsId = useWorkspaceId();

  return useMutation({
    mutationFn: (data: { sessionId: string; member_ids: string[]; agent_ids: string[] }) =>
      api.updateGroupChatParticipants(data.sessionId, {
        member_ids: data.member_ids,
        agent_ids: data.agent_ids,
      }),
    onSuccess: (session) => {
      qc.setQueryData<ChatSession[]>(chatKeys.sessions(wsId), (current) =>
        current?.map((item) => item.id === session.id
          ? {
              ...item,
              ...session,
 ...
 * ListAllChatSessionsByCreator / SILI-4360); this is the optimistic half so there
