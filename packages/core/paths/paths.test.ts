...
describe("paths.workspace(slug)", () => {
  ...
  it("builds workspace paths with slug prefix", () => {
    ...
    expect(ws.taskDetail("abc-123")).toBe("/acme/tasks/abc-123");
    ...
    expect(ws.projectResources("p1")).toBe(
      "/acme/projects/p1?section=resources",
    );
    expect(ws.projectSop("p1")).toBe("/acme/projects/p1?section=sop");
    expect(ws.projectWorkflowRun("p1", "run/1")).toBe(
      "/acme/projects/p1?run=run%2F1",
    );
    ...
    expect(ws.myTasks()).toBe("/acme/my-tasks");
  ...
  it("URL-encodes special characters in ids", () => {
    expect(ws.taskDetail("id with space")).toBe("/acme/tasks/id%20with%20space");
  });
});
