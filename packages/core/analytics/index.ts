...
const SIGNUP_SOURCE_COOKIE = "silieco_signup_source";
...
export function initAnalytics(config: AnalyticsConfig | null | undefined): boolean {
  ...
  posthog.init(config.key, {
    ...
    // identical `$exception` events per session (SILI-3331). The fingerprint is
 ...
 * (see `core/internal/analytics`); this wrapper is reserved for the
...
export interface CaptureEventOptions {
   ...
   * matters (SILI-4115: three deterministic hangs, zero events delivered).
 ...
 * `@silieco/ui` ErrorBoundary can opt in by passing `onError={captureException}`
