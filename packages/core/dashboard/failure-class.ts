...
//
// The backend taxonomy (core/pkg/taskfailure) has 22 reasons, which is far
...
// pre-SILI-1949 coarse values that still sit in historical rows.
//
...
const REASON_CLASS: Record<string, FailureClass> = {
  ...
  // Silieco-side execution substrate: daemon offline / restarted, task never
  ...
  // operator response is "check the daemon's link to Silieco", the same as a
