...
// SILI-3192 — verifies that the URL chosen for markdown persistence is
...
describe("useFileUpload — markdownLink picks the durable URL with three-layer fallback", () => {
  it("prefers att.markdown_url when the server populates it (modern deployment)", async () => {
    const att = makeAttachment({
      markdown_url: "https://cdn.silieco.test/uploads/abc.png",
    });
    const upload = await runUpload(makeApi(att));
    expect(upload?.markdownLink).toBe("https://cdn.silieco.test/uploads/abc.png");
  ...
  it("falls back to att.url when there's no attachment-row id (no-workspace avatar branch)", async () => {
    const att = makeAttachment({
      id: "",
      markdown_url: "",
      url: "https://cdn.silieco.test/avatars/u-1.png",
    });
    const upload = await runUpload(makeApi(att));
    expect(upload?.markdownLink).toBe("https://cdn.silieco.test/avatars/u-1.png");
    expect(upload?.link).toBe("https://cdn.silieco.test/avatars/u-1.png");
  });
...
// SILI-3339 — `uploading` is an in-flight counter, not a single boolean.
...
// is in flight.
describe("useFileUpload — concurrent uploads (SILI-3339 regression)", () => {
