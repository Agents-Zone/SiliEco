...
//                    pre-SILI-3130 contract for non-markdown callers
...
//                    `<SILIECO_PUBLIC_URL>/api/attachments/<id>/download`).
...
//
// SILI-3130 introduced the persisted-image regression by collapsing
// these two semantics into a single `link` field; SILI-3192 followed up
...
//      modern contract introduced in SILI-3192; the server (`buildMarkdownURL`)
//      decides whether to emit a public CDN URL or an absolute API
//      endpoint pinned to `SILIECO_PUBLIC_URL` based on the deployment.
//   2. `attachmentDownloadPath(att.id)` — site-relative legacy shape,
//      retained for compatibility with backends old enough to predate
//      SILI-3192. Web's Next rewrite makes this load; desktop / mobile
...
// coordinated-upload path (SILI-5181), which runs the request through the
...
export function useFileUpload(
  ...
  // their attachment ids never to be bound (SILI-3339).
  //
