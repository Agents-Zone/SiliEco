 ...
 * left `silieco_comment_drafts`, `silieco_quick_create`, `silieco_project_draft`,
 * `silieco_feedback_draft`, and the chat draft-attachment / restore keys
...
export function resetAllRegisteredDrafts(): void {
  ...
  // must never bind an attachment under the next session (SILI-5181).
  abortAllUploads();
  for (const entry of entries.values()) {
    entry.resetInMemory();
  }
