...
/**
 * Kind of a chat message. Additive (SILI-4351): the server always sends a
...
export interface ChatSession {
  ...
  workspace_id: string;
  agent_id: string;
  /** Direct chats bind one agent; group chats bind a participant roster. */
  kind?: "direct" | "group";
...
export interface ChatParticipant {
  type: "member" | "agent";
  /** User id for members, agent id for agents. */
  id: string;
}
...
export interface ChatMessage {
  ...
  task_id: string | null;
  created_at: string;
  /** Group-chat author identity. Optional for legacy/direct-chat rows. */
  sender_user_id?: string | null;
  sender_agent_id?: string | null;
  /** Agents explicitly mentioned by the human message and therefore run. */
   ...
   * produced no text reply (SILI-4351). Optional/additive: absent on older
...
export interface SendChatMessageResponse {
  message_id: string;
  /** Empty/absent for an ordinary group message with no @Agent trigger. */
  task_id?: string;
  /** One id per explicitly mentioned agent in a group message. */
  task_ids?: string[];
  /**
