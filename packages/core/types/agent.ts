...
// haven't shipped SILI-2062 omit the field; the consumer must default to
...
export interface RuntimeDevice {
  ...
  name: string;
  /**
   * Optional user-set display name override (SILI-4217). Overrides `name` for
...
// ---------------------------------------------------------------------------
// Custom runtime profiles (SILI-3284)
//
 ...
 * when only `id` is available. See SILI-4302 §9.
 */
...
/**
 * The resolved accountable-human provenance of an agent run (SILI-4302 §9). Free-text
...
export interface AgentTask {
  ...
  /**
   * Handoff instruction the assigner attached when starting this run (SILI-3375).
  ...
  /**
   * Resolved accountable-human provenance of this run (SILI-4302 §9): who it ran
...
export interface Agent {
   ...
   * endpoint (agent owner or workspace owner/admin, audited). SILI-2600.
   *
   * Optional in the type so older backends (pre-SILI-2600) that omit
   ...
   * alongside `has_custom_env`. Treat `undefined` as zero. SILI-2600.
   */
   ...
   * agent owner (SILI-3869 / SILI-3721). `null`/`[]`/omitted all mean "no
  ...
  /**
   * Authoritative invocation permission mode (SILI-3963). The `visibility`
...
export interface CreateAgentRequest {
  ...
  visibility?: AgentVisibility;
  /**
   * Invocation permission mode (SILI-3963). When present it is authoritative;
...
export interface UpdateAgentRequest {
  ...
  custom_args?: string[];
  /**
   * MCP server configuration. Tri-state semantics (SILI-2764):
  ...
  model?: string;
  /**
   * Runtime-native reasoning/effort token. Tri-state semantics (SILI-2339):
   *   - field omitted → no change
 ...
 * accidentally surface env values. SILI-2600.
 */
 ...
 * silieco-ai/silieco#2174). Use `Skill` from a detail endpoint when you need
...
export interface RuntimeModel {
   ...
   * picker for this model". See SILI-2339.
   */
...
export interface RuntimeModelThinking {
   ...
   *  but not actively rendered today — Silieco's empty `thinking_level`
...
export interface RuntimeModelListRequest {
   ...
   * daemon round trip (SILI-5444). Informational only: such a response already
...
export interface RuntimeModelsResult {
   ...
   * daemon round trip (SILI-5444). Drives the query's freshness policy: a
