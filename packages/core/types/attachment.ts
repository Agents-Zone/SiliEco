export interface Attachment {
   ...
   *
   * The server (`buildMarkdownURL` in core/internal/handler/file.go)
   * computes this per deployment policy:
   *   - public CDN path when storage URL is itself absolute and unsigned;
   *   - otherwise `<SILIECO_PUBLIC_URL>/api/attachments/<id>/download`,
   ...
   * supported client (web / desktop / mobile webview). SILI-3192.
   *
