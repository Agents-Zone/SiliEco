...
import { CurrencyNumberFlow } from "@silieco/ui/components/ui/number-flow";
import type {
  Agent,
  AgentRuntime,
  AgentTask,
...
import { agentTaskSnapshotOptions } from "@silieco/core/agents";
import {
  deriveRuntimeHealth,
...
import {
  ...
  ListGridHeaderCell,
  ListGridRow,
} from "@silieco/ui/components/ui/list-grid";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
...
function RuntimeNameCell({
   ...
   * (SILI-5248). Omitted when the row has no machine context (orphan custom
...
export function CliCell({ runtime }: { runtime: AgentRuntime }) {
  ...
  // The separate `cli_version` is the shared silieco daemon CLI, identical
...
export function RuntimeRowMenu({
  ...
  // re-registers within seconds), but SILI-3352 surfaced that owners read
...
export function RuntimeList({
   ...
   * while a per-runtime alias still shows (SILI-5248).
   */
  machineTitle?: string;
}) {
  const { t } = useT("runtimes");
