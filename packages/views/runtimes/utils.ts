...
import { getCustomPricing } from "@silieco/core/runtimes/custom-pricing-store";
...
// `core/pkg/agent/models.go` so the catalog and pricing stay in sync.
//
...
> = {
  ...
  //    `core/pkg/agent/models.go` (Codex provider list).
  ...
  //    (core/pkg/agent/models.go) but is absent from the price sheet; it
...
// in-range buckets the user asked for (SILI-2382 weekly window scoping).
...
// shift the boundary by a day at the edges (#SILI-2382 sliceWindow tz bug).
