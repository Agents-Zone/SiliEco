...
// ApiError is re-exported from @silieco/core/api; we mock the api module
...
// row uses it to render the Silieco agent's name in place of the raw
...
describe("LarkAgentBindButton (CTA gate)", () => {
  ...
  it("shows the Feishu bind CTA but hides the Lark CTA for an owner (SILI-3083)", () => {
    ...
    // install→inbound pipeline is stabilized (SILI-3083).
  ...
  it("shows the Feishu bind CTA but hides the Lark CTA for an admin (SILI-3083)", () => {
  ...
  it("shows the Feishu bind CTA for a non-admin agent owner (agentOwnerId matches the user, SILI-4213)", () => {
  ...
  // NOTE (SILI-3083): the "clicking Bind to Lark begins an install with
  ...
  it("shows the Feishu CTA (Lark hidden) for an agent without its own installation, per-agent scoping (SILI-3083)", () => {
    installationsRef.current.installations = [
      {
        id: "inst-other",
        workspace_id: "ws-1",
  ...
  it("keeps the Connected + Manage badge for an already-installed agent even when new installs are unavailable (install_supported=false)", () => {
    ...
    // (core/internal/handler/lark.go: "already-installed bots still appear
  ...
  it("shows the Feishu CTA (Lark hidden) when this agent's only installation is revoked (SILI-3083)", () => {
    installationsRef.current.installations = [
      {
        id: "inst-revoked",
        workspace_id: "ws-1",
...
describe("LarkInstallDialog (polling terminal errors)", () => {
  ...
  async function openDialog() {
    ...
    // The Lark CTA is hidden (SILI-3083); open the dialog via the Feishu CTA
  ...
  it("renders the QR after a React StrictMode double-mount (regression for empty dialog body)", async () => {
    ...
    // The Lark CTA is hidden (SILI-3083); the StrictMode regression is about
...
// renders the Silieco agent's avatar + name (joined via inst.agent_id),
...
describe("LarkTab connected bots list (agent identity rendering)", () => {
  ...
  it("renders the Silieco agent's name and avatar instead of the raw Lark app_id / bot_open_id", () => {
