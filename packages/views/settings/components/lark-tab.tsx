...
import { Card, CardContent } from "@silieco/ui/components/ui/card";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { larkInstallationsOptions, larkKeys } from "@silieco/core/lark";
import { api, ApiError } from "@silieco/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@silieco/core/types";
import { ActorAvatar } from "../../common/actor-avatar";
import { useT } from "../../i18n";

// SILI-3083: the Lark (international, open.larksuite.com) "connect a Bot"
...
// install path is per-agent (each Silieco Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Silieco Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Silieco agent's
...
//      owner/admin; see core/internal/handler/lark.go, SILI-4213), so
...
//      core/internal/handler/lark.go — "already-installed bots still
...
export function LarkAgentBindButton({
   ...
   * canManageAgent authorization (SILI-4213). Omit it to keep the button
   * workspace owner/admin-only.
   */
  ...
  // workspace owner/admin may bind/manage the bot (SILI-4213).
  ...
  // unavailable (core/internal/handler/lark.go). Surfacing the badge here
  ...
  // which has surfaced as confusing for international users (SILI-3083
  ...
  return (
    <>
      ...
      >
        ...
        {/* SILI-3083: Lark (international) bind entry is temporarily hidden —
...
// backend authorizes DELETE with the same canManageAgent check (SILI-4213)
  ...
  return (
    ...
    >
      {/* Row 1: connection status (left) and the destructive unbind
          ...
          zombie-bot trap (core/internal/handler/lark.go). Confirmation
