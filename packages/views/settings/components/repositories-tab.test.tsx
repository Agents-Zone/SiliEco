...
const workspaceRef = vi.hoisted(() => ({
  current: {
    ...
    name: "Test Workspace",
    slug: "test-workspace",
    repos: [{ url: "https://github.com/silieco-ai/silieco" }] as {
      url: string;
      description?: string;
    }[],
  },
...
describe("RepositoriesTab — automatic updates", () => {
  beforeEach(() => {
    ...
    workspaceRef.current = {
      ...
      name: "Test Workspace",
      slug: "test-workspace",
      repos: [{ url: "https://github.com/silieco-ai/silieco" }],
    };
  ...
  it("renders persisted repositories as the same shared input controls used for editing", () => {
    ...
    expect(inputs[0]!.value).toBe("https://github.com/silieco-ai/silieco");
  ...
  it("updates a changed URL automatically on blur", async () => {
    ...
    await user.type(urlInput, "https://github.com/silieco-ai/edited");
    await user.tab();

    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [{ url: "https://github.com/silieco-ai/edited" }],
      });
  ...
  it("does not persist a new row until its URL is non-empty", async () => {
    ...
    await user.type(newUrlInput, "git@github.com:silieco-ai/second.git");
    await user.tab();

    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [
          { url: "https://github.com/silieco-ai/silieco" },
          { url: "git@github.com:silieco-ai/second.git" },
        ],
      });
    });
  });
  ...
  it("accepts scp-like repository shorthand", async () => {
    ...
    await user.clear(urlInput);
    await user.type(urlInput, "git@github.com:silieco-ai/silieco.git");
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [{ url: "git@github.com:silieco-ai/silieco.git" }],
      });
    });
  });
  ...
  it("includes the description in the automatic update payload", async () => {
    workspaceRef.current = {
      ...workspaceRef.current,
      repos: [{ url: "https://github.com/silieco-ai/silieco", description: "Main app" }],
    };
    const user = setupUser();
    render(<RepositoriesTab />, { wrapper: I18nWrapper });
  ...
  it("starts GitHub connection with the signed repository return target", async () => {
    ...
    mockGetGitHubConnectURL.mockResolvedValue({
      configured: true,
      url: "https://github.com/apps/silieco/installations/new",
    });
    ...
    await waitFor(() => {
      ...
      expect(open).toHaveBeenCalledWith(
        "https://github.com/apps/silieco/installations/new",
        "_blank",
        "noopener",
      );
    });
  ...
  it("imports selected GitHub repositories and deduplicates HTTPS against SSH", async () => {
    ...
    githubRef.current = {
      installations: [{ id: "installation-row-1", account_login: "silieco-ai" }],
      configured: true,
      repository_browse_configured: true,
      can_manage: true,
    };
    githubRepositoriesRef.current = [
      {
        id: 1,
        full_name: "silieco-ai/silieco",
        html_url: "https://github.com/silieco-ai/silieco",
        clone_url: "https://github.com/silieco-ai/silieco.git",
      ...
      {
        id: 2,
        full_name: "silieco-ai/console",
        html_url: "https://github.com/silieco-ai/console",
        clone_url: "https://github.com/silieco-ai/console.git",
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [
          { url: "git@github.com:silieco-ai/silieco.git" },
          {
            url: "https://github.com/silieco-ai/console.git",
            description: "Console app",
          },
        ],
      });
