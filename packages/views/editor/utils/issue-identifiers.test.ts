...
import {
  ...
  isIssueIdentifier,
} from "@silieco/ui/markdown";

/**
 * Pure detector for the Linear-style issue-identifier autolink. Lives in
 * @silieco/ui/markdown (no test runner there), exercised here where views'
 * vitest can reach it.
 */
describe("preprocessIssueIdentifiers", () => {
  it("rewrites a bare identifier into a canonical mention link", () => {
    expect(preprocessIssueIdentifiers("Related to SILI-1745")).toBe(
      "Related to [SILI-1745](mention://issue/SILI-1745)",
    );
  });

  it("rewrites multiple identifiers in one string", () => {
    expect(preprocessIssueIdentifiers("Created TES-1 and SILI-2")).toBe(
      "Created [TES-1](mention://issue/TES-1) and [SILI-2](mention://issue/SILI-2)",
    );
  });

  it("links an identifier at a sentence end (trailing dot + space)", () => {
    expect(preprocessIssueIdentifiers("See SILI-1. Done.")).toBe(
      "See [SILI-1](mention://issue/SILI-1). Done.",
    );
  });

  it("links identifiers wrapped in prose punctuation", () => {
    expect(preprocessIssueIdentifiers("(SILI-1) and [SILI-2]")).toContain(
      "([SILI-1](mention://issue/SILI-1))",
    );
  });

  // --- skip: code -------------------------------------------------------
  it("skips identifiers inside inline code", () => {
    expect(preprocessIssueIdentifiers("use `SILI-1` here")).toBe(
      "use `SILI-1` here",
    );
  });
  ...
  it("does not double-process an existing mention link", () => {
    const input = "[SILI-1](mention://issue/00000000-0000-0000-0000-000000000001)";
    expect(preprocessIssueIdentifiers(input)).toBe(input);
  });

  it("skips an identifier used as a markdown link label", () => {
    const input = "[SILI-1](https://example.com/x)";
    expect(preprocessIssueIdentifiers(input)).toBe(input);
  });

  // --- skip: urls / filenames / paths -----------------------------------
  it("skips an identifier inside a URL", () => {
    const input = "https://example.com/board/SILI-1";
    expect(preprocessIssueIdentifiers(input)).toBe(input);
  });
  ...
  it("ignores lowercase tokens", () => {
    const input = "some-word-1 and sili-1";
    expect(preprocessIssueIdentifiers(input)).toBe(input);
  });

  it("ignores a token embedded in a larger word", () => {
    const input = "XSILI-1A stays";
    expect(preprocessIssueIdentifiers(input)).toBe(input);
  });
...
describe("isIssueIdentifier", () => {
  it("accepts a bare identifier", () => {
    expect(isIssueIdentifier("SILI-1745")).toBe(true);
  ...
  it("rejects lowercase and malformed tokens", () => {
    expect(isIssueIdentifier("sili-1")).toBe(false);
    expect(isIssueIdentifier("SILI-")).toBe(false);
