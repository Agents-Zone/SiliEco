...
import { preprocessLinks } from "@silieco/ui/markdown/linkify";
...
describe("preprocessLinks — CJK punctuation boundary", () => {
  ...
  it("handles the original bug report (PR link then 。 then more text)", () => {
    const out = preprocessLinks(
      "已合并 PR #1623：https://github.com/silieco-ai/silieco/pull/1623。merge commit",
    );
    expect(out).toBe(
      "已合并 PR #1623：[https://github.com/silieco-ai/silieco/pull/1623](https://github.com/silieco-ai/silieco/pull/1623)。merge commit",
    );
  });
...
// swallow was the SILI-4242 render bug. Mirrors GFM's own autolink trailing trim.
describe("preprocessLinks — trailing markdown delimiter is not part of the URL", () => {
  ...
  it("keeps ** outside when a CJK punctuation immediately follows (variant B)", () => {
    expect(preprocessLinks("**https://example.com/x**（SILI-4277）")).toBe(
      "**[https://example.com/x](https://example.com/x)**（SILI-4277）",
    );
  });
