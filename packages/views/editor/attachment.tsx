...
import { attachmentIdFromDownloadURL } from "@silieco/core/types/attachment-url";
...
function normalize(
  ...
  return {
    ...
    // markdown stays on its existing path. See SILI-3130 review.
    //
    ...
    // See SILI-3192 — quick-create modal regressed because the freshly-
...
// `/uploads/<key>` URL.
//
// The current (post-SILI-3192) write path persists an absolute URL chosen
// server-side by `buildMarkdownURL` (see core/internal/handler/file.go),
// so new content already loads natively on every client. This helper only
// matters for content written BEFORE SILI-3192 — those bodies still carry
...
//                             persistence (SILI-3192 / `buildMarkdownURL`):
//                             public CDN passthrough when the storage is
//                             public-readable, or `SILIECO_PUBLIC_URL +
...
//     bucket (must-fix 2 from SILI-3192 review), except for the explicit
//     site-relative local upload path above.
//  5. `record.url` — legacy fallback for responses that omit
//     `markdown_url` (a backend old enough to predate SILI-3192).
...
// ---------------------------------------------------------------------------
// Inline media re-sign (SILI-3254)
// ---------------------------------------------------------------------------

// Keep refetches well inside the server's signed-URL TTL (30 min default,
// core/internal/handler/file.go) so a re-render never serves an expired
...
// auth-gated API path again, and before SILI-5445 the renderer simply kept the
