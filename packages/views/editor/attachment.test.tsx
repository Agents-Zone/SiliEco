...
import type { Attachment as AttachmentRecord } from "@silieco/core/types";
...
import { configStore } from "@silieco/core/config";
...
describe("Attachment — image dispatch", () => {
  it("record image renders <img> with hover toolbar (View/Download/Copy)", () => {
    ...
    // attachment.tsx for the rationale (SILI-3130 review follow-up).
  ...
  it("url-only image resolves to a record via context and uses its id for download", () => {
    ...
    // (SILI-3130 review). The raw stored url is the fallback for
  ...
  it("renders the configured CDN URL when description markdown stores the stable API URL", () => {
    ...
    const markdownUrl = `https://silieco-api.copilothub.ai/api/attachments/${id}/download`;
    const att = makeRecord({
      id,
  ...
  it("does not pick the raw CDN url when the server reports cdn_signed (SILI-3254)", () => {
  ...
  it("re-signs the inline media URL through getAttachment on token-mode clients (SILI-3254)", async () => {
    ...
    getBaseUrlMock.mockReturnValue("https://silieco-api.copilothub.ai");
    configStore.setState({ cdnDomain: "cdn.example.test", cdnSigned: true });
    const id = "11111111-2222-3333-4444-555555555555";
    const markdownUrl = `https://silieco-api.copilothub.ai/api/attachments/${id}/download`;
    const signed =
  ...
  it("re-signs URL-only inline media when no resolver record is available (SILI-3254)", async () => {
  ...
  it("falls back to an authenticated byte fetch when the deployment has no signed URL (SILI-5445)", async () => {
    ...
    const att = makeRecord({
      id,
      url: "https://minio:9000/silieco/uploads/ws/shot.png",
      markdown_url: markdownUrl,
      download_url: "",
    });
    configStore.setState({ cdnDomain: "", cdnSigned: false });
  ...
  it("copies the durable URL, not the session-local object URL (SILI-5445)", async () => {
  ...
  it("does not pull bytes for non-image attachments (SILI-5445)", async () => {
    ...
    resolverState.attachments = [
      makeRecord({
        ...
        url: "https://minio:9000/silieco/uploads/ws/archive.zip",
        markdown_url: markdownUrl,
        download_url: "",
      }),
    ];
  ...
  it("prefers the signed URL over a byte fetch when the server can presign (SILI-5445)", async () => {
  ...
  it("stable /api/attachments/<id>/download download_url falls through to the durable record.markdown_url for native <img> loadability (SILI-3192)", () => {
    // After SILI-3192, pickInlineMediaURL prefers `record.markdown_url`
    ...
    const att = makeRecord({
      ...
      markdown_url: "https://api.silieco.test/api/attachments/att-1/download",
    ...
    const img = document.querySelector("img");
    expect(img?.getAttribute("src")).toBe(
      "https://api.silieco.test/api/attachments/att-1/download",
    );
  ...
  it("legacy backend (no markdown_url on record) still falls back to record.url", () => {
    // A backend old enough to predate SILI-3192 omits markdown_url; the
...
// SILI-3192 — Desktop quick-create: site-relative `/api/attachments/<id>/
...
describe("Attachment — absolutize site-relative URLs (SILI-3192)", () => {
  ...
  it("absolutizes the legacy site-relative /uploads/<key> when record.markdown_url is empty (legacy backend)", () => {
    // Legacy compat: a backend old enough to predate SILI-3192 omits
