...
import type { Agent, MemberWithUser } from "@silieco/core/types";
...
> {
  ...
  return {
    ...
    // (`/usr/local/bin`) never opens the skill picker (SILI-5429).
 ...
 * `noteCommentPrefix` in core/internal/handler/comment.go.
 */
...
> {
  ...
  return {
    ...
    pluginKey,
    // Only open over a `/` the user actually typed, so a pasted path
    // (`/usr/local/bin`) never opens the command menu (SILI-5429).
    shouldShow: ({ editor, range }) => isTriggerArmedAt(editor, range.from),
