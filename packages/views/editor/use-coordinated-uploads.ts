...
/**
 * The coordinated-upload engine shared by every composer surface (SILI-5181, L2).
 *
 * Ownership inversion: an upload is owned by the module-level upload
 * coordinator (`@silieco/core/drafts`), not by the React component that
...
import {
  ...
  type DraftUpload,
} from "@silieco/core/drafts";
import { createSafeId } from "@silieco/core/utils";
import { contentReferencesAttachment, type Attachment } from "@silieco/core/types";
import {
  toUploadResult,
  type UploadContext,
  type UploadResult,
} from "@silieco/core/hooks/use-file-upload";
import { MAX_FILE_SIZE } from "@silieco/core/constants/upload";
...
export function useCoordinatedUploads(
  ...
  // a user who deletes the placeholder mid-upload means it, and SILI-5181's
            ...
            if (outcome.status === "uploaded") {
              if (target) {
                ...
                if (target.getUploads().some((u) => u.clientUploadId === clientUploadId)) {
                  ...
                  // Write-back (SILI-5181): the mount that started this upload
