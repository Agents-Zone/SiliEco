...
describe("ContentEditor", () => {
  ...
  // document under the new destination (SILI-4864).
  ...
  it("refreshes the live placeholder getter and repaints when the placeholder prop changes", () => {
    // Repro for SILI-4276: Tiptap's Placeholder snapshots a *string* option at
...
function makeAttachment(id: string, overrides: Partial<Attachment> = {}): Attachment {
  return {
    ...
    markdown_url: `https://api.silieco.test/api/attachments/${id}/download`,
...
// SILI-4808 — the document IS the upload queue, so hosts gate submit off it
// instead of each keeping a counter. These pin the publisher's contract.
describe("ContentEditor — onUploadingChange (SILI-4808)", () => {
...
// SILI-3192 — surfaces like the quick-create modal upload images through the
...
describe("ContentEditor — in-session attachment tracking (SILI-3192)", () => {
