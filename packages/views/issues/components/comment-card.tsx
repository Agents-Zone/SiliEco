...
import { Card } from "@silieco/ui/components/ui/card";
...
import type { TimelineEntry, Attachment } from "@silieco/core/types";
import { contentReferencesAttachment } from "@silieco/core/types";
import { useCommentCollapseStore, useCommentDraftStore } from "@silieco/core/issues/stores";
...
export function AttachmentList({
  ...
  // SILI-3130 follow-through — a comment can mix the new
...
function TaskCommentRetryButton({
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      // Rerun re-checks the operator's invoke permission (SILI-4525); a
...
function useEditAttachmentState(
  ...
  // Uploads for this edit session (SILI-5181) — coordinator-owned, persisted in
  ...
  // Await-then-render save (SILI-5181): shared submit contract, with the edit-
  ...
  // Stale-submit guard (SILI-5181 P0) — see CommentInput. The edit hook lives
  ...
  const { submitting: saving, submit: saveEdit } = useComposerSubmit({
    ...
    onSubmit: async (trimmed) => {
      ...
      const activeIds = collectActiveAttachmentIds(
        trimmed,
        [...(entry.attachments ?? []), ...pendingAttachments],
        // Body-referenced + retained-standalone only (SILI-5181): an upload the
  ...
  return (
    <div className="py-1.5">
      ...
      {edit.editing ? (
        ...
        <>
          <div className="comment-content pl-12 pr-4 pt-1 text-body leading-relaxed text-foreground/85">
        ...
        {open && (
          <div id={`comment-body-${entry.id}`} className="px-4 pt-1 pb-3">
            {edit.editing ? (
              ...
              <>
                <div className="comment-content pl-10 text-body leading-relaxed text-foreground/85">
                  <ReadonlyContent content={entry.content ?? ""} attachments={entry.attachments} />
                </div>
