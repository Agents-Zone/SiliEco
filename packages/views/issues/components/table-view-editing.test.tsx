 ...
 *
 * SILI-5108 regression coverage: an open cell editor popup must survive the
...
import { setApiInstance } from "@silieco/core/api";
import type { ApiClient } from "@silieco/core/api/client";
import { issueKeys } from "@silieco/core/issues/queries";
import { ViewStoreProvider } from "@silieco/core/issues/stores/view-store-context";
import { getIssueSurfaceViewStore } from "@silieco/core/issues/stores/surface-view-store";
import type {
  Issue,
  IssueTableQuerySpec,
...
describe("TableView cell editors under data refresh", () => {
  ...
  // strips the default real-timer gaps between events (SILI-5108 review R1#1).
  ...
  // (SILI-5326). It runs in ~1s in isolation, so the generous 60s ceiling
  ...
  it("keeps the status picker open and the row order frozen across a refresh, then catches up on close", async () => {
    ...
    const identifiers = () =>
      screen.getAllByText(/^SILI-/).map((node) => node.textContent);
    await screen.findByText("SILI-a");
    expect(identifiers()).toEqual(["SILI-a", "SILI-b"]);

    // Open the status picker on row A: its cell trigger shows "Todo".
    const rowA = screen.getByText("SILI-a").closest("tr")!;
    ...
    expect(identifiers()).toEqual(["SILI-b", "SILI-a"]);
  }, 60_000);
  ...
  it("opens creation with the row as parent and inherits its project", async () => {
    ...
    const row = (await screen.findByText("SILI-a")).closest("tr")!;
    await user.click(
    ...
    expect(onCreateIssue).toHaveBeenCalledWith({
      parent_issue_id: "a",
      parent_issue_identifier: "SILI-a",
      project_id: "project-1",
    });
  });
...
// explicit release when the owning cell leaves the DOM (SILI-5108 review R1#3).
