...
import { useCommentComposerStore, useCommentDraftStore } from "@silieco/core/issues/stores";
import { renderWithI18n } from "../../test/i18n";
import { CommentInput } from "./comment-input";
import { ReplyInput } from "./reply-input";

// Uploads now flow through the module-level coordinator, which calls
// `api.uploadFile(file, ctx, signal)` (SILI-5181). Tests drive uploads by
...
// Observability + failure control for the write-back insert path (SILI-5181):
...
vi.mock("@silieco/core/hooks/use-file-upload", async () => ({
  ...(await vi.importActual<typeof import("@silieco/core/hooks/use-file-upload")>(
    "@silieco/core/hooks/use-file-upload",
  )),
...
describe("comment composers", () => {
  ...
  // SILI-5181 P0: the editor stays interactive during a send — text typed
  ...
  // SILI-5181 P0: a submit that outlives its composer may only clear the draft
...
// SILI-4808 — posting mid-upload strips the pending image's blob URL out of the
...
describe("comment composers — upload submit gate", () => {
  ...
  it("does not redraw a placeholder the user deleted mid-upload", async () => {
    // SILI-5181's rule: a placeholder the user removed stays removed. The
  ...
  it("does NOT bind an upload the user deleted from the body", async () => {
    ...
    // is not referenced must not ship — deleting really unbinds (SILI-5181).
