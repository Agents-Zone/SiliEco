...
import type { Issue, Label, TimelineEntry } from "@silieco/core/types";
import { I18nProvider } from "@silieco/core/i18n/react";
import { useResolvedExpandStore } from "@silieco/core/issues/stores/resolved-expand-store";
import {
  DEFAULT_SUB_ISSUE_ROW_PROPERTIES,
  useSubIssueDisplayStore,
} from "@silieco/core/issues/stores/sub-issue-display-store";
...
vi.mock("@silieco/ui/hooks/use-mobile", () => ({
  useIsMobile: () => mockViewport.isMobile,
}));

// useWorkspaceId() derives from useCurrentWorkspace (relative import inside
// @silieco/core/hooks.tsx). vi.mock("@silieco/core/paths") only intercepts
...
// Mock @silieco/core/workspace/hooks
...
// Mock @silieco/core/paths — after the URL-driven workspace refactor,
...
vi.mock("../../navigation", () => ({
  ...
  useNavigation: () => ({
    ...
    getShareableUrl: (p: string) => `https://app.silieco.com${p}`,
  }),
...
// Mock issue config
vi.mock("@silieco/core/issues/config", () => ({
...
vi.mock("@silieco/core/issues/stores", async () => ({
  ...
  ...(await vi.importActual<
    typeof import("@silieco/core/issues/stores/resolved-expand-store")
  >("@silieco/core/issues/stores/resolved-expand-store")),
  ...
  ...(await vi.importActual<
    typeof import("@silieco/core/issues/stores/sub-issue-display-store")
  >("@silieco/core/issues/stores/sub-issue-display-store")),
...
// Mock react-resizable-panels (used by @silieco/ui/components/ui/resizable)
...
describe("IssueDetail (shared)", () => {
  ...
  it("opts the description editor into the unmount flush", async () => {
    ...
    // markdown and its attachment_ids bind (SILI-3254). The flush behavior
  ...
  it("renders the issue title leaf as a link to the issue detail page", async () => {
    ...
    expect(leaf.closest("a")).toHaveAttribute("href", "/test/tasks/issue-1");
  });
  ...
  it("does not show retry for child-done system comments", async () => {
    mockApiObj.listTimeline.mockResolvedValue([
      ...mockTimeline,
      {
        ...
        content: "Sub-issue SILI-123 is done.",
        parent_id: null,
        created_at: "2026-01-18T00:00:00Z",
        updated_at: "2026-01-18T00:00:00Z",
        comment_type: "system",
    ...
    await screen.findByText("Sub-issue SILI-123 is done.");
