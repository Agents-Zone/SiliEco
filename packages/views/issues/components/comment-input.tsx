...
function CommentInput({ issueId, workspaceId, projectId, onSubmit }: CommentInputProps) {
  ...
  // Await-then-render send (SILI-5181): the shared hook reads the markdown,
  ...
  // Stale-submit guard (SILI-5181 P0): if this composer unmounts mid-submit
  ...
  const { submitting, submit } = useComposerSubmit({
    ...
    onSubmit: (content) => {
      ...
      // Bind only uploads the BODY still references (SILI-5181): deleting an
    ...
    onAccepted: () => {
      // Success may only consume the entry it submitted (SILI-5181 P0): edits
