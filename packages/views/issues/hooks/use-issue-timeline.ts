...
} from "@silieco/core/issues/mutations";
import { sortTimelineEntriesAsc } from "@silieco/core/issues/timeline-sort";
import {
  unhandledCommentTriggerOutcomes,
  mentionLabelsByTarget,
} from "@silieco/core/issues/comment-trigger-outcomes";
import { useWSEvent, useWSReconnect } from "@silieco/core/realtime";
...
export function useIssueTimeline(issueId: string, userId?: string) {
  ...
  // silent no-op (SILI-4525 §2): the comment IS posted, but N explicitly-named
