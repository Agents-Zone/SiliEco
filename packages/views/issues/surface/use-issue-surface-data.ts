...
import type { Issue, IssueAssigneeGroup, Project } from "@silieco/core/types";
...
} from "@silieco/core/issues/surface/repository";
import type { IssueSurfaceQueryPlan } from "@silieco/core/issues/surface/query-plan";
import type { IssueStatus } from "@silieco/core/types";
import {
  applyIssueFilters,
 ...
 * first place (SILI-4884); duplicating the rules in both places would just
...
export function useIssueSurfaceData({
  ...
  // `cancelled` is a first-class default status (SILI-4290): it is fetched into
  ...
  // toggle, moved the list but not the chip (SILI-4884).
  //
