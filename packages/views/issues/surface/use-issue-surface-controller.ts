...
import { workspaceWorkingAgentsOptions } from "@silieco/core/agents";
import { useWorkspaceId } from "@silieco/core/hooks";
import { ALL_STATUSES } from "@silieco/core/issues/config";
import { dateOnlyToLocalDate } from "@silieco/core/issues/date";
import type {
  AssigneeGroupedIssuesFilter,
  IssueSortParam,
  MyIssuesFilter,
} from "@silieco/core/issues/queries";
import { issueTableFacetsOptions } from "@silieco/core/issues/queries";
import {
  buildIssueSurfaceQueryPlan,
  type IssueSurfaceQueryPlan,
} from "@silieco/core/issues/surface/query-plan";
import type { IssueScope } from "@silieco/core/issues/surface/scope";
import type { IssueDateFilter, SortField } from "@silieco/core/issues/stores/view-store";
import { propertyListOptions } from "@silieco/core/properties";
import { propertyIdFromViewKey } from "@silieco/core/issues/stores/view-store";
import { useViewStore } from "@silieco/core/issues/stores/view-store-context";
...
export interface IssueSurfaceController {
   ...
   *  header chip so its count IS the post-click row count (SILI-4884). */
