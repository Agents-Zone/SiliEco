/**
 * SILI-5208 — a link that points back at this deployment is an in-app
...
beforeEach(() => {
  navigatedPaths = [];
  window.addEventListener("silieco:navigate", captureNavigate);
  openSpy = vi.spyOn(window, "open").mockImplementation(() => null);
});

afterEach(() => {
  window.removeEventListener("silieco:navigate", captureNavigate);
  vi.restoreAllMocks();
});
...
describe("RichContent link routing", () => {
  it("routes a link to this deployment into the app instead of the browser", () => {
    renderContent(`[SILI-1](${APP_ORIGIN}/acme/issues/SILI-1)`);

    screen.getByText("SILI-1").click();
  ...
  it("still hands a genuinely external link to the browser", () => {
    const external = "https://github.com/silieco-ai/silieco/pull/1";
