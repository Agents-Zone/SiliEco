...
import type { SupportedLocale } from "@silieco/core/i18n";
...
vi.mock("@silieco/core/paths", async () => {
  const actual = await vi.importActual<typeof import("@silieco/core/paths")>(
    "@silieco/core/paths",
  );
  return {
    ...actual,
...
describe("WelcomeAfterOnboarding", () => {
  ...
  describe("runtime path", () => {
    it("creates Sili Agent then opens a blocking modal with starter cards", async () => {
      ...
      expect(screen.getByText(/Preparing Sili Agent/i)).toBeInTheDocument();

      await waitFor(() => {
        expect(screen.getByText(/welcome to Silieco/i)).toBeInTheDocument();
      });

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.runtime_id).toBe("rt-1");
      expect(agentArgs.name).toBe("Sili Agent");
      expect(agentArgs.instructions).toContain("Sili Agent");
      expect(agentArgs.avatar_url).toContain("data:image/svg+xml");
      expect(agentArgs.avatar_url).toContain("%233978f6");

      // 3 starter card titles come from HELPER_STARTER_PROMPTS (TS const,
      // EN under the test's en locale).
      expect(
        screen.getByText("Introduce Silieco to me"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Walk me through the core features"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Show me what Silieco can do for me — as slides"),
      ).toBeInTheDocument();
    });

    it("migrates an existing Silieco Helper to Sili Agent instead of creating a duplicate", async () => {
      mockListAgents.mockResolvedValueOnce([
        {
          id: "agent-existing",
          name: "Silieco Helper",
      ...
      mockUpdateAgent.mockResolvedValueOnce({
        ...
        name: "Sili Agent",
        description: "",
        avatar_url: "data:image/svg+xml,updated",
        visibility: "workspace",
        archived_at: null,
      });
      useWelcomeStore.getState().set({
      ...
      expect(mockUpdateAgent).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          name: "Sili Agent",
          avatar_url: expect.stringContaining("%233978f6"),
        }),
      );
    });
    ...
    it("selecting cards then clicking Assign creates one issue per pick and navigates to the first", async () => {
      ...
      // Toggle two cards.
      fireEvent.click(screen.getByText("Introduce Silieco to me"));
      fireEvent.click(
        screen.getByText("Show me what Silieco can do for me — as slides"),
      );
      ...
      expect(titles).toEqual([
        "Introduce Silieco to me",
        "Show me what Silieco can do for me — as slides",
      ]);
    ...
    it("uses Korean persisted Helper and starter issue artifacts under ko locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("Silieco를 간단히 소개해 주세요"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("Silieco 사용 어시스턴트");
      expect(agentArgs.instructions).toContain(
        "당신은 이 Silieco 워크스페이스에 내장된 AI 어시스턴트",
      );

      fireEvent.click(screen.getByText("Silieco를 간단히 소개해 주세요"));
      fireEvent.click(
      ...
      expect(issueArgs.title).toBe("Silieco를 간단히 소개해 주세요");
      expect(issueArgs.description).toContain(
        "Silieco를 1-2문단으로 간단히 소개해 주세요",
      );
    });
    ...
    it("uses Japanese persisted Helper and starter issue artifacts under ja locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("Silieco を簡単に紹介してください"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("Silieco の使い方アシスタント");
      expect(agentArgs.instructions).toContain(
        "あなたは Sili Agent、この Silieco ワークスペースに組み込まれた AI アシスタント",
      );

      fireEvent.click(screen.getByText("Silieco を簡単に紹介してください"));
      fireEvent.click(
      ...
      await waitFor(() => expect(mockCreateIssue).toHaveBeenCalledTimes(1));
      const [issueArgs] = mockCreateIssue.mock.calls[0]!;
      expect(issueArgs.title).toBe("Silieco を簡単に紹介してください");
      expect(issueArgs.description).toContain(
        "Silieco を1〜2段落で簡単に紹介してください",
      );
    });
  });
  ...
  describe("skip path", () => {
    it("provisions install-runtime → agent-guide → follow-up comment, then opens the celebration Modal", async () => {
      ...
      expect(commentContent).toContain("[SILI-2](mention://issue/issue-agent)");
    });
    ...
    it("silently dismisses without showing the Modal when provisioning fails", async () => {
      ...
      expect(screen.queryByText(/Welcome to Silieco/i)).not.toBeInTheDocument();
    });
    ...
    it("uses Korean persisted skip-path issue and comment artifacts under ko locale", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Silieco에 오신 것을 환영합니다/i)).toBeInTheDocument();
      });
      ...
      expect(installCall![0].description).toContain(
        "Silieco에 오신 것을 환영합니다.",
      );
      expect(guideCall![0].title).toBe(
        "2단계 — 첫 Silieco Agent 만들기",
      );
      ...
      expect(commentContent).toContain(
        "[SILI-2](mention://issue/issue-agent)",
      );
    });
    ...
    it("uses Japanese persisted skip-path issue and comment artifacts under ja locale", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Silieco へようこそ/),
        ).toBeInTheDocument();
      });
      ...
      expect(installCall![0].description).toContain("Silieco へようこそ。");
      expect(guideCall![0].title).toBe(
        "ステップ2 — 最初の Silieco Agent を作成する",
      );
