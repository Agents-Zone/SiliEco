...
import {
  ...
  CardContent,
  CardFooter,
} from "@silieco/ui/components/ui/card";
...
import {
  ...
  InputOTPGroup,
  InputOTPSlot,
} from "@silieco/ui/components/ui/input-otp";
...
import type { User } from "@silieco/core/types";
...
interface LoginPageProps {
   ...
   *  would be absurd). */
  extra?: ReactNode;
  /** Use direct email/password registration and login instead of email codes. */
...
export function LoginPage({
    ...
    api
      ...
      .catch(() => {
        ...
        api
          ...
          .catch(() => {
            api.setToken(null);
            localStorage.removeItem("silieco_token");
          });
      });
  }, [cliCallback]);
    ...
    async (value: string) => {
      ...
      try {
        if (cliCallback) {
          ...
          localStorage.setItem("silieco_token", token);
          api.setToken(token);
          onTokenObtained?.();
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (cliCallback) {
        const { token } = await api.passwordAuth(passwordMode, email, password);
        localStorage.setItem("silieco_token", token);
        api.setToken(token);
        onTokenObtained?.();
        redirectToCliCallback(cliCallback.url, token, cliCallback.state);
      ...
      await useAuthStore.getState().passwordAuth(passwordMode, email, password);
      const wsList = await api.listWorkspaces();
      qc.setQueryData(workspaceKeys.list(), wsList);
      onTokenObtained?.();
      onSuccess();
      ...
      setError(
        err instanceof Error ? err.message : t(($) => $.errors.password_failed),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCliAuthorize = async () => {
    ...
    try {
      ...
      if (authSourceRef.current === "localStorage") {
        ...
        const stored = localStorage.getItem("silieco_token");
    ...
    <div className="flex min-h-svh items-center justify-center">
      <Card className="w-full max-w-sm">
        ...
        <CardContent>
          <form
            id="login-form"
            onSubmit={passwordAuth ? handlePasswordSubmit : handleSendCode}
            className="space-y-4"
          >
        ...
        <CardFooter className="flex flex-col gap-3">
          <Button
            ...
            className="w-full"
            size="lg"
            disabled={!email || loading || (passwordAuth && password.length < 6)}
          >
            {loading
              ? t(($) => $.signin.sending)
              : passwordAuth
