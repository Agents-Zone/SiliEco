...
describe("LoginPage", () => {
  ...
  it("renders the Silieco account form", () => {
    renderWithI18n(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/create or sign in to silieco/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /sign in with email and password, or create a new account/i,
      ),
    ).toBeInTheDocument();
  ...
  it("shows the working state while submitting", async () => {
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("silieco_token", "existing-jwt");
  ...
  it("'Use a different account' returns to email step", async () => {
    ...
    expect(
      screen.getByText(/sign in to silieco/i),
    ).toBeInTheDocument();
  });
