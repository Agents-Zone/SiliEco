...
import type { Agent, RuntimeDevice } from "@silieco/core/types";
import {
  AgentNameField,
  ModeChooser,
  StudioFooter,
...
// SILI-5390: thinking_level / service_tier are runtime + model scoped. The create
...
describe("Agent creation studio execution overrides", () => {
  ...
  it("drops the execution config when the duplicate falls back to another runtime", () => {
    ...
    // is what persisted a cross-provider model before SILI-5390.
    const duplicate = buildDuplicateDraft(sourceAgent(), {
