 ...
 * disagree with what actually ran (SILI-5390).
 */
...
const BUILDER_INPUT_PREFIX = "SILIECO_AGENT_BUILDER_INPUT\n";
...
export function AgentCreationStudio() {
  ...
  // SILI-5163: the picker showed runtime B while every subsequent message still
  ...
  const sendBuilderMessage = async (content: string): Promise<boolean> => {
    ...
    try {
      ...
      if (!result.task_id) throw new Error("builder chat send returned no task id");
 ...
 * for the user to pick again — the same rule `silieco agent copy` already
 * enforces server-side (cmd_agent_copy.go). Before SILI-5390 the fallback kept
 ...
 * concurrency) — mirroring `silieco agent copy`.
 */
