...
import { useFeatureEnabled } from "@silieco/core/config";
import { COMPOSIO_MCP_APPS_FLAG } from "@silieco/core/feature-flags";
...
import { isImeComposing } from "@silieco/core/utils";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function CreateAgentDialog({
  ...
  // SILI-4010: rolls out the private / public_to access model in the create
  // flow to match the AccessPicker on the agent detail page. Shares the
  // `composio_mcp_apps` switch with the Composio rollout — the SILI-3963
  ...
  // New access state (SILI-3963 aligned). When duplicating, seed from the
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (accessPickerEnabled) {
        // New SILI-3963 shape: send the authoritative permission fields and
      ...
      if (template) {
        ...
        // SILI-2600 the agent resource shape no longer carries
      ...
      // SILI-2178) — a partial failure surfaces a warning toast and
 ...
 * `permission_mode` + `invocation_targets` (SILI-3963), not the legacy
 * `visibility`.
 *
