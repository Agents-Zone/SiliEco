...
// borrow this entry's levels — that's the SILI-4347 fix.
...
describe("ThinkingPropRow", () => {
  ...
  it("renders the row with \"Follow CLI config\" when value is empty and the model exposes levels", async () => {
    ...
    await screen.findByText("Thinking");
    // Empty value means Silieco omits --effort, so the local CLI's
  ...
  it("hides the picker for an empty codex model — it must not borrow the Default's catalog (SILI-4347)", async () => {
