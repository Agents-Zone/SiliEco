...
import type { Agent, AgentRuntime } from "@silieco/core/types";
...
vi.mock("@silieco/core/runtimes", async () => {
  const actual =
    await vi.importActual<typeof import("@silieco/core/runtimes")>(
      "@silieco/core/runtimes",
    );
  return {
    ...actual,
...
describe("McpConfigTab", () => {
  ...
  it("projects historical aggregate config into individually managed rows", () => {
    ...
    expect(screen.getByRole("heading", { name: /managed by silieco/i })).toBeInTheDocument();
  ...
  it("adds one stdio server through the form and preserves historical top-level data", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /add core/i }));
  ...
  it("deletes the last managed server only after confirmation", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /delete core/i }));
  ...
  it("blocks invalid single-server JSON", async () => {
    ...
    expect(screen.getByRole("button", { name: /add core/i })).toBeDisabled();
