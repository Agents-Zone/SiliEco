...
import type { Agent, ComposioToolkit } from "@silieco/core/types";
import { useUpdateAgentAllowlist } from "@silieco/core/agents";
import { useFeatureEnabled } from "@silieco/core/config";
import {
  composioConnectionsOptions,
  composioToolkitsOptions,
} from "@silieco/core/composio";
...
/**
 * Creator-only MCP tab on the agent detail page (SILI-3870). Lets the agent
 ...
 * agent OWNER's Composio connection (SILI-3963) — it is no longer gated on the
...
export function AgentMcpTab({ agent }: { agent: Agent }) {
  ...
  // Composio access warning (SILI-3963). Once an agent is shared, anyone who
