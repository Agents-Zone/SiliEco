...
vi.mock("@silieco/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@silieco/core/onboarding")>(
      "@silieco/core/onboarding",
    );
...
vi.mock("@silieco/core/paths", async () => {
  const actual =
    await vi.importActual<typeof import("@silieco/core/paths")>(
      "@silieco/core/paths",
    );
...
vi.mock("@silieco/core/api", async () => {
  const actual =
    await vi.importActual<typeof import("@silieco/core/api")>(
      "@silieco/core/api",
    );
  return {
    ...actual,
...
function wipeDismissCounters() {
  for (let i = window.localStorage.length - 1; i >= 0; i--) {
    ...
    if (k && k.startsWith("silieco.source_backfill.dismiss.")) {
...
describe("SourceBackfillModal", () => {
  it("does not render when there is no user", () => {
    renderModal();
    expect(
      screen.queryByText(/How did you hear about Silieco/i),
  ...
  it("opens for an onboarded user with empty source once agents completed enough issues", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about Silieco/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("does not open once the per-user dismiss cap is reached on this browser", () => {
    window.localStorage.setItem("silieco.source_backfill.dismiss.u1", "3");
    setUser({
      id: "u1",
      onboarded_at: "2026-01-01T00:00:00Z",
      onboarding_questionnaire: { source: [] },
