 ...
 * Sili Agent is created. Each card maps to ONE task being created
...
export const HELPER_STARTER_PROMPTS: Record<StarterCardId, StarterPrompt> = {
  intro: {
    title: {
      en: "Introduce Silieco to me",
      zh: "简单介绍一下 Silieco",
      ko: "Silieco를 간단히 소개해 주세요",
      ja: "Silieco を簡単に紹介してください",
    },
    prompt: {
      en: "Introduce Silieco to me in 1–2 paragraphs. Cover what it is, the core concepts (workspace / task / agent / runtime), and how it differs from tools like Linear or Jira.",
      zh: "用 1-2 段话简单介绍 Silieco 给我。讲清楚它是什么、核心概念有哪些(workspace / task / agent / runtime)、和 Linear / Jira 之类的工具核心区别在哪。",
      ko: "Silieco를 1-2문단으로 간단히 소개해 주세요. 무엇인지, 핵심 개념(workspace / task / agent / runtime)이 무엇인지, Linear나 Jira 같은 도구와 핵심적으로 어떻게 다른지 설명해 주세요.",
      ja: "Silieco を1〜2段落で簡単に紹介してください。何であるか、中心となる概念(workspace / task / agent / runtime)、そして Linear や Jira のようなツールと根本的にどう違うのかを説明してください。",
    },
  },
  tour: {
    title: {
    ...
    prompt: {
      en: "Walk me through Silieco's core features — task, agent, squad, autopilot, chat. Pick one realistic scenario I might run into and explain how all these pieces fit together.",
      zh: "陪我熟悉 Silieco 的每个核心功能 —— task、agent、squad、autopilot、chat。挑一个我可能用得上的真实场景,讲讲这几个东西是怎么配合的。",
      ko: "Silieco의 핵심 기능인 task, agent, squad, autopilot, chat을 안내해 주세요. 제가 실제로 겪을 만한 상황 하나를 골라 이 요소들이 어떻게 함께 작동하는지 설명해 주세요.",
      ja: "Silieco の主要な機能 — task、agent、squad、autopilot、chat を案内してください。私が実際に遭遇しそうな現実的なシナリオを1つ選び、これらの要素がどう連携するのかを説明してください。",
    },
  },
  welcome_page: {
    title: {
      en: "Show me what Silieco can do for me — as slides",
      zh: "用 slides 介绍 Silieco 能为我做什么",
      ko: "Silieco가 저에게 무엇을 해줄 수 있는지 슬라이드로 보여 주세요",
      ja: "Silieco が私に何をしてくれるのかをスライドで見せてください",
    },
    prompt: {
      en: `Build me a single-file HTML slide deck that shows what Silieco can do for me. Tailor it to my role and use case (see "About me" below). Paste the FULL HTML in a fenced \`\`\`html block in a comment on this task so I can copy it straight out, save as \`silieco-intro.html\`, and double-click to open it in a browser.
...
- 5–8 slides total:
  1. Title — "What Silieco can do for [my role]"
  2. Four core concepts — workspace / task / agent / runtime, one slide
  3–6. 3–4 concrete scenarios tailored to my use case, each in the form "When you want X → here's how Silieco handles it"
...
When done, also reply with a one-sentence summary of which scenarios you picked for me and why.`,
      zh: `给我做一份单文件 HTML 演示稿,介绍 Silieco 能为我做什么。根据我的角色和使用场景定制(见下面"关于我")。把完整 HTML 贴到这条 task 的评论里的 \`\`\`html 代码块中,我直接复制下来存成 \`silieco-intro.html\` 双击就能在浏览器里打开。

**产出格式**
- 一个自包含 .html,CSS / JS 全部 inline。零依赖、不用打包、不引外部图片(视觉用纯 CSS 生成 —— 渐变、几何形状、内联 SVG)。
- 5-8 张 slide:
  1. 标题页 —— "Silieco 能为 [我的角色] 做什么"
  2. 四个核心概念 —— workspace / task / agent / runtime,一张
  3-6. 3-4 个针对我使用场景的具体例子,形如"当你想做 X → Silieco 是这样处理的"
  7. 收尾页 —— 一个具体的下一步动作
...
做完后再用一句话告诉我你为我挑了哪几个场景以及为什么。`,
      ko: `Silieco가 저에게 무엇을 해줄 수 있는지 보여주는 단일 파일 HTML 슬라이드 덱을 만들어 주세요. 제 역할과 사용 사례에 맞춰 주세요(아래 "내 정보" 참고). 전체 HTML을 이 task의 댓글에 fenced \`\`\`html 코드 블록으로 붙여 주세요. 그대로 복사해 \`silieco-intro.html\`로 저장하고 브라우저에서 더블클릭해 열 수 있어야 합니다.
...
- 전체 5-8장:
  1. 제목 — "[내 역할]에게 Silieco가 해줄 수 있는 일"
  2. 네 가지 핵심 개념 — workspace / task / agent / runtime, 한 장
  3-6. 제 사용 사례에 맞춘 구체적인 시나리오 3-4개. 각 시나리오는 "X를 하고 싶을 때 → Silieco는 이렇게 처리합니다" 형식
  7. 마무리 — 구체적인 다음 액션 하나
...
완료 후, 어떤 시나리오를 골랐고 왜 골랐는지 한 문장으로 요약해 주세요.`,
      ja: `Silieco が私に何をしてくれるのかを示す、単一ファイルの HTML スライドデッキを作ってください。私の役割とユースケースに合わせてください(下の「私について」を参照)。完全な HTML を、この task のコメントに fenced \`\`\`html コードブロックで貼り付けてください。そのままコピーして \`silieco-intro.html\` として保存し、ブラウザでダブルクリックして開けるようにしてください。

**出力フォーマット**
- 1つの self-contained な .html。CSS / JS はすべて inline。依存関係なし、ビルドツールなし、外部画像なし(視覚要素は CSS で生成 — グラデーション、幾何学的な図形、inline SVG を使用)。
- 全体で5〜8枚のスライド:
  1. タイトル — 「[私の役割] に Silieco ができること」
  2. 4つの中心概念 — workspace / task / agent / runtime を1枚で
  3〜6. 私のユースケースに合わせた具体的なシナリオを3〜4個。それぞれ「X をしたいとき → Silieco はこう処理します」の形で
  7. 締め — 具体的な次の一歩のアクション
