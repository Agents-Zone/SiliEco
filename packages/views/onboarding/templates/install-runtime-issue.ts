/**
 * Skip path, task 1/2: "Connect a runtime to start using agents".
 *
 * Written to a new task (assigned to the user themselves) by the welcome
...
That gives you the project-management layer first. Once a runtime is connected, agents can start working from the same tasks.
...
4. Confirm your terminal can find it:
   which codex
   codex --version
5. Wait for Silieco to pick it up. A running daemon re-checks for newly
...
7. Create your first agent from that runtime, then assign an task to the agent and set status to todo.
...
1. 为当前工作创建一个项目。
2. 新建几个 task,并在 backlog、todo、in_progress、done 之间流转。
3. 给 task 加优先级、标签、评论和订阅。
4. 用收件箱追踪分配给你的事项和 @mention。

这样你先熟悉项目管理层。连上运行时后,智能体会直接在这些 task 上开始工作。
...
5. 等 Silieco 识别到它。运行中的守护进程每隔几分钟会重新检查一次新装的 CLI,通常不需要重启。
   想立刻生效:
   silieco daemon restart
   桌面端请打开任意一个本机 runtime 并点 Restart。退出再打开 app 是不够的 —— 守护进程会继续在后台运行。
6. 回到 Runtimes 页面刷新。你应该能看到一个在线的 Kimi 运行时。
7. 用这个运行时创建第一个智能体,再把一个 task 分配给它,并把状态切到 todo。

Kimi CLI 官方文档:https://moonshotai.github.io/kimi-cli/zh/guides/getting-started.html

运行时连上后，你就可以创建 Sili Agent，开始一次有智能体参与的上手引导。`;

const ko = `Silieco에 오신 것을 환영합니다.

agent가 작업을 실행하려면 먼저 runtime이 필요합니다. runtime을 설치하는 동안에도 Silieco를 가벼운 프로젝트 관리 워크스페이스로 먼저 사용할 수 있습니다.
...
2. task 몇 개를 만들고 backlog, todo, in_progress, done 사이에서 이동해 봅니다.
3. priority, label, comment, subscription을 추가합니다.
4. Inbox에서 나에게 배정된 작업과 mention을 확인합니다.

이렇게 프로젝트 관리 계층을 먼저 익힐 수 있습니다. runtime이 연결되면 agent가 같은 task에서 바로 작업을 시작합니다.

## 첫 agent runtime 설치하기

전체 가이드: https://silieco.ai/docs/install-agent-runtime
...
4. 터미널에서 찾을 수 있는지 확인합니다:
   which codex
   codex --version
5. Silieco가 인식할 때까지 기다립니다. 실행 중인 daemon은 몇 분마다 새로 설치된 CLI를
...
7. 해당 runtime으로 첫 agent를 만든 뒤 task를 agent에게 배정하고 status를 todo로 바꿉니다.

Codex 참고 문서: https://developers.openai.com/codex/cli

runtime이 연결되면 Sili Agent를 만들어 안내를 받으며 첫 실행을 시작할 수 있습니다.`;

const ja = `Silieco へようこそ。

agent が作業を実行するには、まず runtime が必要です。runtime をインストールしている間も、Silieco を軽量なプロジェクト管理ワークスペースとして先に使うことができます。
...
2. task をいくつか作り、backlog、todo、in_progress、done の間で動かしてみる。
3. priority、label、comment、subscription を追加する。
4. Inbox で自分への割り当てや mention を確認する。

これでまずプロジェクト管理のレイヤーに慣れることができます。runtime を接続すると、agent が同じ task から作業を始められます。

## 最初の agent runtime をインストールする

詳しいガイド: https://silieco.ai/docs/install-agent-runtime
...
4. ターミナルから見つけられるか確認します:
   which codex
   codex --version
5. Silieco が認識するまで待ちます。動作中の daemon は数分ごとに新しくインストールされた
...
7. その runtime から最初の agent を作り、task を agent に割り当てて status を todo にします。

Codex のリファレンス: https://developers.openai.com/codex/cli

runtime が接続されたら、Sili Agent を作成して、案内付きの最初の実行を始められます。`;

export const INSTALL_RUNTIME_ISSUE_BODY = { en, zh, ko, ja } as const;

/**
 * Prefix sentence for the follow-up comment posted on this task (the one
 * that links to the create-agent-guide task via a mention chip). Kept
