...
const HELPER_AGENT_NAME = "Sili Agent";

/**
 * Skip path, task 2/2: "Create your first Silieco Agent".
 *
 * Companion to install-runtime-task.ts. The body is a FUNCTION (not a
 * static const) because it needs to embed:
 *   - A mention chip pointing at the install-runtime task (so the user
 *     can jump to it from this task) — requires the install-runtime
 *     task's identifier + uuid, only known after that task is created.
...
export const CREATE_AGENT_GUIDE_ISSUE_TITLE = {
  en: "Step 2 — Create your first Silieco Agent",
  zh: "第 2 步 —— 创建你的第一个 Silieco Agent",
  ko: "2단계 — 첫 Silieco Agent 만들기",
  ja: "ステップ2 — 最初の Silieco Agent を作成する",
} as const;
...
function enBody(installRuntimeMention: string): string {
  return `Once your runtime is online (see ${installRuntimeMention}), build your first agent, Sili Agent. The prompt below is pre-written; just copy.
...
Now create an task (or reassign an existing one) → set assignee = Sili Agent → set status to **todo**. The runtime picks the task up within a few seconds and starts working. Watch progress in the task's Run panel.
...
- **Docs** — https://silieco.ai/docs.`;
}

function zhBody(installRuntimeMention: string): string {
  return `等运行时上线（见 ${installRuntimeMention}）之后，创建你的第一个 Agent：Sili Agent。下面的提示词已经写好，直接复制即可。
...
接着创建一个 task（或把已有 task 重新分派）→ 把 assignee 设成 Sili Agent → 状态切到 **todo**。运行时会在几秒内接走任务并开始工作。在 task 的任务面板里看进度。
...
- **文档** —— https://silieco.ai/docs。`;
}

function koBody(installRuntimeMention: string): string {
  return `runtime이 online 상태가 되면(${installRuntimeMention} 참고), 첫 agent인 Sili Agent를 만드세요. 아래 prompt는 미리 작성되어 있으니 그대로 복사하면 됩니다.
...
이제 task를 만들거나 기존 task를 다시 배정한 뒤 assignee를 Sili Agent로 설정하고 status를 **todo**로 바꾸세요. runtime이 몇 초 안에 작업을 가져가 실행을 시작합니다. 진행 상황은 task의 Run panel에서 볼 수 있습니다.
...
function jaBody(installRuntimeMention: string): string {
  return `runtime が online になったら(${installRuntimeMention} を参照)、最初の agent である Sili Agent を作りましょう。下の prompt はあらかじめ書いてあるので、そのままコピーするだけです。
...
次に task を作る(または既存の task を割り当て直す)→ assignee を Sili Agent にする → status を **todo** にします。runtime が数秒以内にタスクを受け取って作業を始めます。進捗は task の Run panel で確認できます。
