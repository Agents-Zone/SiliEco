...
// self-hosted instances instead of the hardcoded `silieco.ai`.
describe("StepWorkspace — workspace URL prefix", () => {
  it("shows the brand host when no app URL is configured", () => {
    renderStep({ existing: null, disabled: false });
    expect(screen.getByText("silieco.ai/")).toBeInTheDocument();
  });
  ...
  it("shows the deployment host for self-hosted instances", () => {
    renderStep({
      existing: null,
      disabled: false,
      daemonAppUrl: "https://silieco.example.com",
    });
    expect(screen.getByText("silieco.example.com/")).toBeInTheDocument();
    expect(screen.queryByText("silieco.ai/")).not.toBeInTheDocument();
  });
});
