...
import { Archive, ArchiveRestore, MoreHorizontal, Pencil, Trash2, UserRound, UsersRound } from "lucide-react";
...
export function ChatSessionHeader({
  ...
  return (
    <div className="flex h-12 shrink-0 items-center gap-3 border-b px-4">
      {session.kind === "group" ? (
        <span className="flex size-[30px] shrink-0 items-center justify-center rounded-full bg-brand/10 text-brand ring-1 ring-brand/15">
      ...
      <div className="min-w-0 flex-1">
        ...
        {session.kind === "group" ? (
          <div className="truncate text-caption text-muted-foreground">
            {t(($) => $.group.header_summary, { people: memberCount, agents: agentCount })}
          </div>
        ) : agent && (
          <div className="truncate text-caption text-muted-foreground">
            {agent.name}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-auto">
          ...
          {session.kind === "group" && onManageParticipants && (
            <DropdownMenuItem onClick={onManageParticipants}>
              <UsersRound className="h-4 w-4" />
              {t(($) => $.group.manage_action)}
            </DropdownMenuItem>
          )}
          {agent && (
