...
import { SubmitButton } from "@silieco/ui/components/common/submit-button";
import { ChatAddMenu } from "./chat-add-menu";
import { CHAT_COLUMN, CHAT_GUTTER } from "./chat-column";
import { useChatStore, DRAFT_NEW_SESSION } from "@silieco/core/chat";
import { attachmentToDraftUpload, type DraftUpload } from "@silieco/core/drafts";
import { createLogger } from "@silieco/core/logger";
import { formatShortcut, useShortcut } from "@silieco/core/shortcuts";
import type { MentionItem } from "../../editor/extensions/mention-suggestion";
import type { Attachment, Project } from "@silieco/core/types";
...
interface ChatInputProps {
   ...
   *  The transport itself is the coordinated-upload engine (SILI-5181 L2) —
  ...
  agentName?: string;
  /** Surface-specific placeholder, used by participant-based group chat. */
...
export function ChatInput({
  ...
  // where the first send goes (SILI-4864). This is a STORAGE key, not a React
  // identity.
  //
  ...
  // image would leave stuck (SILI-4808).
  ...
  // Coordinator-owned uploads (SILI-5181 L2): survive window close, abort on
  ...
  // The unified await-then-render send contract (SILI-5181). `useComposerSubmit`
  ...
  const { submitting, submit } = useComposerSubmit({
    ...
    onSubmit: async (content: string): Promise<boolean> => {
      ...
      // Stale-submit guard (SILI-5181 P0): snapshot the sent draft's persisted
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // porthole (SILI-5196). `max-h-[50%]` resolves against the chat
