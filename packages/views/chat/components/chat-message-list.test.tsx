...
import { chatKeys } from "@silieco/core/chat/queries";
import type { TaskMessagePayload } from "@silieco/core/types";
import type { ReactElement } from "react";
import enChat from "../../locales/en/chat.json";

// The live timeline is a real list row rather than Virtuoso chrome (SILI-4922),
...
describe("ChatMessageList live timeline (SILI-3960 regression)", () => {
...
describe("ChatMessageList failure copy (SILI-5370 regression)", () => {
  // The backend moved to the refined taxonomy (agent_error.*) in SILI-2946 but
...
describe("ChatMessageList group authors", () => {
  it("labels messages from other humans and the replying agent", async () => {
    render(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <QueryClientProvider client={new QueryClient()}>
          <ChatMessageList
            ...
            members={[
              { user_id: "user-other", name: "Alex", email: "alex@example.com" },
            ] as never}
            agents={[{ id: "agent-review", name: "Review Agent" }] as never}
            messages={[
              {
                id: "human-message",
                chat_session_id: "group-1",
                role: "user",
              ...
              },
              {
                ...
                chat_session_id: "group-1",
                role: "assistant",
                content: "The plan looks good.",
                task_id: null,
                created_at: new Date(1).toISOString(),
                sender_agent_id: "agent-review",
              },
            ]}
            pendingTask={undefined}
            availability="online"
    ...
    expect(screen.getByText("The plan looks good.")).toBeInTheDocument();
  });
});
