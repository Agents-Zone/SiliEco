...
import { Minus, Maximize2, Minimize2, ChevronDown, Plus, Check, Archive, Pencil, Loader2, Square, UsersRound } from "lucide-react";
...
import { agentListOptions, memberListOptions } from "@silieco/core/workspace/queries";
import { projectListOptions } from "@silieco/core/projects/queries";
import { canAssignAgent } from "@silieco/views/issues/components";
import { api } from "@silieco/core/api";
import { useAgentPresenceDetail, useWorkspaceAgentAvailability } from "@silieco/core/agents";
...
import {
  ...
  chatKeys,
  isTaskMessageTaskId,
} from "@silieco/core/chat/queries";
import {
...
import { removeChatMessageFromCaches } from "@silieco/core/realtime";
...
import { createLogger } from "@silieco/core/logger";
import type { Agent, Attachment, ChatMessage, ChatMessagesPage, ChatPendingTask, ChatSession, PendingChatTasksResponse } from "@silieco/core/types";
...
export function ChatWindow() {
  ...
  const currentSession = activeSessionId
    ...
    : null;
  const isGroupSession = currentSession?.kind === "group";
  ...
  const sessionAgent = currentSession && !isGroupSession
  ...
  const activeAgent = isGroupSession
    ? null
    : sessionAgent ??
  ...
  const groupParticipantMembers = isGroupSession
    ? members.filter((member) => currentSession?.participants?.some(
        (participant) => participant.type === "member" && participant.id === member.user_id,
      ))
    : [];
  const groupParticipantAgents = isGroupSession
    ? availableAgents.filter((agent) => currentSession?.participants?.some(
        (participant) => participant.type === "agent" && participant.id === agent.id,
      ))
    : [];
  ...
  const noAgent = !isGroupSession && agentAvailability === "none";
  ...
  // backgrounded must stay unread so the sidebar badges it (SILI-4485), then
  ...
  // (SILI-5181 L2); the host only says whether the affordance exists. Uploads
  ...
  const handleSend = useCallback(
    async (
      ...
      if (!isGroupSession && isAgentArchived) {
      ...
      if (!isGroupSession && !result.task_id) {
        apiLogger.error("direct chat send returned no task id", { sessionId });
        toast.error(t(($) => $.input.send_failed_toast));
        return false;
      }
      ...
      if (result.task_id && stopRequestedBeforeTaskRef.current) {
  ...
  const handleSelectSession = useCallback(
    (session: ChatSession) => {
      ...
      if (session.kind !== "group" && activeAgent && session.agent_id !== activeAgent.id) {
  ...
  const defaultContextItems = useChatContextItems(wsId);
  const contextItems = isGroupSession
    ? [
        ...groupParticipantMembers.map((member) => ({
          id: member.user_id,
          label: member.name || member.email,
          type: "member" as const,
        })),
        ...groupParticipantAgents.map((agent) => ({
          id: agent.id,
          label: agent.name,
          type: "agent" as const,
        })),
    ...
    >
      ...
      {showSkeleton ? (
        ...
        <ChatMessageList
          ...
          groupMode={isGroupSession}
          currentUserId={user?.id}
          members={groupParticipantMembers}
          agents={groupParticipantAgents}
        />
      ) : isGroupSession ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center">
          ...
          <div className="text-body font-medium">{t(($) => $.group.empty_title)}</div>
          <div className="max-w-sm text-caption text-muted-foreground">
      ...
      <ChatInput
        ...
        uploadEnabled={isGroupSession || !!activeAgent}
        onStop={handleStop}
        isRunning={!isGroupSession && !!pendingTaskId}
        disabled={isSessionArchived || isAgentArchived}
        noAgent={noAgent}
        agentArchived={isAgentArchived}
        agentName={activeAgent?.name}
        placeholderOverride={isGroupSession ? t(($) => $.group.input_placeholder) : undefined}
        projects={projects}
        ...
        leftAdornment={isGroupSession ? undefined :
          <AgentDropdown
          ...
          isConfirmingStop && pendingTask ? (
            ...
            <div className="flex shrink-0 items-center">
              ...
              <div className="hidden h-7 items-center gap-0.5 group-hover/history-row:flex">
                {isRunning && pendingTask && session.kind !== "group" && (
                  <button
                    type="button"
                    onPointerDown={(e) => {
                      e.preventDefault();
