...
import type { DraftUpload } from "@silieco/core/drafts";
...
// DraftUpload entries (SILI-5181 L2).
...
function renderInput(props: Partial<React.ComponentProps<typeof ChatInput>> = {}) {
  ...
  render(
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <ChatInput onSend={onSend} uploadEnabled agentName="Silieco" {...props} />
    </I18nProvider>,
  );
  return { onSend };
}
...
function element(props: Partial<React.ComponentProps<typeof ChatInput>>) {
  return (
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <ChatInput onSend={vi.fn()} uploadEnabled agentName="Silieco" {...props} />
    </I18nProvider>
  );
}

// SILI-4864: an uncreated chat has ONE draft per workspace. `selectedAgentId`
...
describe("ChatInput focusRequest", () => {
  it("focuses the editor when focusRequest becomes a non-zero value (new chat)", () => {
    const { rerender } = render(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Silieco" focusRequest={0} />
      </I18nProvider>,
    );
    ...
    rerender(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Silieco" focusRequest={1} />
      </I18nProvider>,
    );
    ...
    rerender(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Silieco" focusRequest={2} />
      </I18nProvider>,
    );
...
describe("ChatInput @ context wiring", () => {
  it("configures chat @ with current/recent issue/project context", () => {
    const contextItems = [
      { id: "issue-1", label: "SILI-1", type: "issue" as const, group: "current" as const },
    ];
...
describe("ChatInput project context", () => {
  ...
  it("locks the project control while a send is in flight so a mid-send switch cannot retarget the session", async () => {
    ...
    // (SILI-5150). The control must stay locked for the whole send, not only
...
describe("ChatInput attachment wiring", () => {
  ...
  it("binds attachment_ids when the upload's markdownLink differs from its link (SILI-3130 regression)", async () => {
...
describe("ChatInput commit handoff", () => {
  ...
  it("scrubs the editor and clears the draft on a normal commit", async () => {
    ...
    // rather than being dropped (SILI-5181 follow-up).
