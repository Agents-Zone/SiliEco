...
import type { Agent, ChatSession, Project } from "@silieco/core/types";
...
vi.mock("@silieco/core/hooks", () => ({ useWorkspaceId: () => "ws-1" }));
...
vi.mock("@silieco/views/issues/components", () => ({ canAssignAgent: () => true }));
vi.mock("@silieco/core/api", () => ({
  api: { sendChatMessage: vi.fn(), cancelTaskById: vi.fn() },
  // Names the 403 that a revoked invoke permission raises (SILI-4525); plain
  // failures have no reason code.
  dispatchReasonCode: () => undefined,
}));
vi.mock("@silieco/core/agents", () => ({
...
vi.mock("@silieco/core/chat/mutations", () => ({
...
describe("useChatController project context", () => {
  ...
  it("pins the fresh chat to the open session's agent when selectedAgentId is stale", () => {
    ...
    // the lazily-created chat to the wrong agent (SILI-5150 regression). The
...
// SILI-4360 mount race: `activeSessionId` is persisted, so on a bare `/chat`
...
// Foreground gating (SILI-4485): a reply that lands while the app is backgrounded
// must stay unread and clear once the user returns. This composes with the
// SILI-4360 mount-race defer above — the read is scheduled a tick after the
// effect runs — so each assertion advances fake timers to let it (or not) fire.
describe("useChatController auto mark-read — foreground gating (SILI-4485)", () => {
...
// draft and clearing it would wipe visible input. SILI-4864 changes what
