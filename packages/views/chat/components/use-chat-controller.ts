...
import { api, dispatchReasonCode } from "@silieco/core/api";
 ...
 * the draft slot — that is not "navigating away" (SILI-4864). Counting it as
...
export function useChatController(opts?: { isActive?: boolean }) {
  ...
  // (SILI-4485); it clears the moment the user returns and this effect re-runs.
  //
  ...
  // SILI-4360's mount race: `activeSessionId` is persisted, so on a bare `/chat`
  ...
  // (SILI-5181 L2); surfaces only forward whether the affordance exists.
  const uploadEnabled = isGroupSession || !!activeAgent;
  ...
  const handleSend = useCallback(
    async (
      ...
      try {
        ...
        // 403 (SILI-4525) — name the cause instead of a generic failure.
        toast.error(
      ...
      try {
        ...
        // a structured 403 before anything persists (SILI-4525). Surface the
  ...
  const handleSelectSession = useCallback(
    (session: { id: string; agent_id: string; kind?: "direct" | "group"; project_id?: string | null }) => {
