...
import { ArrowLeft, MessageSquare, UsersRound } from "lucide-react";
...
import {
  ...
  ResizablePanel,
  ResizableHandle,
} from "@silieco/ui/components/ui/resizable";
...
import type { Agent, ChatSession } from "@silieco/core/types";
...
export function ChatPage() {
  ...
  const groupParticipantAgents = useMemo(() => {
    if (c.currentSession?.kind !== "group") return [];
    const ids = new Set(
      (c.currentSession.participants ?? [])
        .filter((participant) => participant.type === "agent")
        .map((participant) => participant.id),
    );
    return c.availableAgents.filter((agent) => ids.has(agent.id));
  }, [c.currentSession, c.availableAgents]);
  const groupParticipantMembers = useMemo(() => {
    if (c.currentSession?.kind !== "group") return [];
    const ids = new Set(
      (c.currentSession.participants ?? [])
    ...
    return c.members.filter((member) => ids.has(member.user_id));
  }, [c.currentSession, c.members]);
  const groupMentionItems = useMemo(
    () => c.isGroupSession
      ? [
      ...
      : undefined,
    [c.isGroupSession, groupParticipantMembers, groupParticipantAgents],
  );
  const canManageCurrentGroup = c.currentSession?.kind === "group" && (
    c.currentSession.creator_id === c.user?.id ||
    ["owner", "admin"].includes(c.members.find((member) => member.user_id === c.user?.id)?.role ?? "")
  );
    ...
    <PageHeader className="justify-between">
      ...
      <div className="flex items-center gap-1">
        <Button
          variant="ghost"
          size="icon-sm"
          title={t(($) => $.group.new_tooltip)}
          aria-label={t(($) => $.group.new_tooltip)}
    ...
    <div className="flex flex-1 flex-col min-h-0 @container">
      {c.currentSession && (
        <ChatSessionHeader
          ...
          agent={c.activeAgent}
          memberCount={groupParticipantMembers.length}
          agentCount={groupParticipantAgents.length}
          onManageParticipants={canManageCurrentGroup ? () => setManagingGroup(true) : undefined}
          onArchive={handleArchive}
        />
      )}
      {c.showSkeleton ? (
      ...
      {c.isGroupSession ? null : c.noAgent ? (
        <NoAgentBanner />
      ) : c.isAgentArchived ? (
      ...
      <ChatInput
        ...
        onStop={c.handleStop}
        isRunning={!c.isGroupSession && !!c.pendingTaskId}
        disabled={c.isSessionArchived || c.isAgentArchived}
        noAgent={c.noAgent}
        agentArchived={c.isAgentArchived}
        agentName={c.activeAgent?.name}
        placeholderOverride={c.isGroupSession ? t(($) => $.group.input_placeholder) : undefined}
        contextItems={groupMentionItems}
        projects={c.projects}
  ...
  const groupDialog = creatingGroup ? (
  ...
  const manageGroupDialog = managingGroup && c.currentSession?.kind === "group" ? (
    <CreateGroupChatDialog
      open
      session={c.currentSession}
      onOpenChange={setManagingGroup}
      members={c.members}
      agents={c.availableAgents}
      currentUserId={c.user?.id}
      onCreated={(session) => {
