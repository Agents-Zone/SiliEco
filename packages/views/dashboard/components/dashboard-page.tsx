...
import {
  ...
  NumberFlow,
  NumberFlowGroup,
} from "@silieco/ui/components/ui/number-flow";
import {
  Select,
  SelectContent,
  SelectItem,
...
import type { Agent } from "@silieco/core/types";
import { agentListOptions } from "@silieco/core/workspace/queries";
...
import {
  ...
  FAILURE_CLASSES,
  type FailureClass,
} from "@silieco/core/dashboard";
import { useWorkspacePaths } from "@silieco/core/paths";
import { useCustomPricingStore } from "@silieco/core/runtimes/custom-pricing-store";
...
const EMPTY_DAILY: import("@silieco/core/types").DashboardUsageDaily[] = [];
const EMPTY_BY_AGENT: import("@silieco/core/types").DashboardUsageByAgent[] = [];
const EMPTY_RUNTIME: import("@silieco/core/types").DashboardAgentRunTime[] = [];
const EMPTY_RUNTIME_DAILY: import("@silieco/core/types").DashboardRunTimeDaily[] = [];
const EMPTY_FAILURE_DAILY: import("@silieco/core/types").DashboardFailureDaily[] = [];
const EMPTY_FAILURE_BY_AGENT: import("@silieco/core/types").DashboardFailureByAgent[] =
  [];
...
export function DashboardPage() {
  ...
  // (see sinceFromDays in core/internal/handler/runtime.go), and only a
  ...
  // instead of being dropped (SILI-2382 weekly window scoping). Week
  ...
  // agents" row instead of showing them as a bare UUID (SILI-3771) or dropping
  // them outright — dropping made the per-agent breakdown stop reconciling
  // with the top-line Cost/Tokens KPIs, which still count that spend (SILI-3776,
...
// full screen or more below the fold (SILI-5388). Ten answers "who is spending
        ...
        <>
          ...
          <ul aria-label={t(($) => $.leaderboard.title)} className="divide-y">
            {visibleRows.map((row) => {
              ...
              // merged them (SILI-5409), so zeroing their columns would
