...
// refuses to name (SILI-5409): agents the viewer may not see, plus the hidden
...
// core/internal/handler/dashboard.go — the two strings must stay in sync.
//
...
//
// SILI-3771 (PR #4637) originally *dropped* these rows so they'd stop rendering
...
// per-agent breakdown no longer reconciled with the totals (SILI-3776, #4640).
...
// `@silieco/core/dashboard`; the raw reason survives only in
...
// serializing (SILI-5409), because client-side filtering is decoration — one
