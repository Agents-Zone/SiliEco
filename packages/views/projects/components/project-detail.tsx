...
import type { ProjectStatus, ProjectPriority } from "@silieco/core/types";
import { useAuthStore } from "@silieco/core/auth";
import { projectDetailOptions } from "@silieco/core/projects/queries";
import { useUpdateProject, useDeleteProject } from "@silieco/core/projects/mutations";
import { pinListOptions } from "@silieco/core/pins";
import { useCreatePin, useDeletePin } from "@silieco/core/pins";
...
import { PROJECT_STATUS_ORDER, PROJECT_STATUS_CONFIG, PROJECT_PRIORITY_ORDER } from "@silieco/core/projects/config";
...
import { EmojiPicker } from "@silieco/ui/components/common/emoji-picker";
...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  const { defaultLayout, onLayoutChanged } = useDefaultLayout({
    id: "silieco_project_detail_layout",
  });
      ...
      <ResizablePanel id="content" minSize="50%">
        <div className="flex h-full flex-col">
          <BreadcrumbHeader
            ...
            actions={
              <>
              <div className="mr-2 flex items-center rounded-lg bg-muted p-0.5">
                <Button
                  variant={activeSection === "tasks" ? "secondary" : "ghost"}
                  size="sm"
                  className="h-7 gap-1.5 px-2.5 text-caption"
                  onClick={() => router.replace(wsPaths.projectDetail(projectId))}
                >
