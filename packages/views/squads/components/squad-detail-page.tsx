...
import { getShortcut, shortcutMatchesEvent } from "@silieco/core/shortcuts";
import { useTimeAgo } from "../../i18n";
import { agentListOptions, memberListOptions, squadMemberStatusOptions, workspaceKeys } from "@silieco/core/workspace/queries";
...
import { ActorAvatar as ActorAvatarBase } from "@silieco/ui/components/common/actor-avatar";
...
import type { Squad, SquadMember, SquadMemberStatus, SquadMemberStatusValue, Agent, MemberWithUser } from "@silieco/core/types";
...
export function SquadDetailPage() {
  ...
  // core/internal/handler/squad.go so editable controls appear exactly when
  // the API will accept the write, and everyone else gets a read-only view
  // instead of controls that 403 (SILI-4223).
  const canManage =
...
function SquadOverviewPane({
  ...
  // Save). See canManageSquad in core/internal/handler/squad.go.
  canManage: boolean;
...
// (core/internal/handler/daemon.go).
