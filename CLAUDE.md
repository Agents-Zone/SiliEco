...
Silieco is an AI-native task management platform for small teams, with agents as first-class assignees that can own issues, comment, and change status.

- `core/`: Go backend, Chi router, sqlc, gorilla/websocket.
...
- `packages/ui/`: no `@silieco/core` imports and no business logic.
...
Mobile is independent. It may import types and pure functions from `@silieco/core`, with `import type` for types, but owns its UI, state, hooks, providers, i18n, React version, build pipeline, and release cadence.
...
- Reserved slugs live in `core/internal/handler/reserved_slugs.json`. Edit it, run `pnpm generate:reserved-slugs`, and commit the generated `packages/core/paths/reserved-slugs.ts`.
- When changing CLI commands/flags, API fields, or product behavior documented by built-in skills under `core/internal/service/builtin_skills/*`, update the relevant `SKILL.md` and `references/*-source-map.md` in the same PR.
...
In `core/internal/handler/`, always know where a UUID came from before using it in write queries.
...
- `setCurrentWorkspace(slug, uuid)` from `@silieco/core/platform` mirrors the active route for headers, storage namespaces, and reconnects; workspace route layouts own setting it.
...
- Full-window desktop views outside the dashboard shell must mount `<DragStrip />` from `@silieco/views/platform` as the first flex child. Interactive controls in the top 48px need `WebkitAppRegion: "no-drag"`.
...
- Mobile shares only `@silieco/core` types and pure functions.
...
- Mock `@silieco/core` stores with the Zustand callable-store shape (`selectorFn` plus `getState`).
- Mock `@silieco/core/api` for API calls.
...
- Real-agent smoke tests belong behind the `agentintegration` build tag and must check `SILIECO_RUN_REAL_AGENT_SMOKE=1` before executable lookup or account access.
- Run an explicitly authorized real-agent smoke test with `(cd core && SILIECO_RUN_REAL_AGENT_SMOKE=1 go test -tags=agentintegration ./pkg/agent -run '<test-name>' -count=1 -v)`. This command may access an authenticated account and consume quota.
