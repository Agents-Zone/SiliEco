...
Deploy Silieco on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`silieco` CLI** and runs the **agent daemon** on their own machine.
...
curl -fsSL https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.sh | bash -s -- --with-server
...
```powershell
# 1. Install CLI + provision the self-host server
$env:SILIECO_MODE="with-server"; irm https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.ps1 | iex
...
This installs the `silieco` CLI, checks out the latest self-host assets, pulls the official Silieco images from GHCR, and configures everything for localhost.
...
```bash
git clone https://github.com/silieco-ai/silieco.git
cd silieco
make selfhost
```
...
`make selfhost-build` uses local `silieco-backend:dev` / `silieco-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `SILIECO_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `SILIECO_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
4. Create a Task and assign it to your Agent — the Daemon will pick up the execution automatically

---

## Kubernetes Deployment (Alternative)

If you already run a Kubernetes cluster, you can deploy Silieco there instead of Docker Compose using the released OCI Helm chart at `oci://ghcr.io/silieco-ai/charts/silieco` or the source chart at [`deploy/helm/silieco/`](deploy/helm/silieco/). It targets a typical k3s / k8s setup with an Ingress controller and a default `ReadWriteOnce` StorageClass — authored against k3s + Traefik + `local-path`, and should work on any cluster with minor tweaks.

The chart creates the following resources in the target namespace:

- `silieco-postgres` — `pgvector/pgvector:pg17` backed by a 10Gi PVC
- `silieco-backend` — Go API/WS server. Backed by a 5Gi `ReadWriteOnce` uploads PVC by default; set `backend.uploads.persistence.enabled=false` when you have configured S3 (`backend.config.s3Bucket`) and don't want the chart to declare the PVC at all.
- `silieco-frontend` — Next.js standalone server
- Two `Ingress` resources: one for the web host, one for the backend host
- `silieco-config` ConfigMap (rendered from `values.yaml`)

The `silieco-secrets` Secret is **not** managed by the chart — you create it once with `kubectl` so real values never need to land in git.

> **Runtime frontend upstreams:** current `silieco-web` images read `REMOTE_API_URL` and `DOCS_URL` when the Next.js server runs, so API/docs upstream changes do not require a web rebuild. The chart defaults `REMOTE_API_URL` to this release's backend Service. `frontend.compatibility.backendAlias` exists only for legacy images that still baked `REMOTE_API_URL=http://backend:8080` at build time.
...
The chart defaults to `silieco.dev.lan` (web) and `api.silieco.dev.lan` (backend). Pick one of:

- **`/etc/hosts`** on every machine that needs access (developer laptops + the machine running the daemon):

  ```text
  192.168.1.206  silieco.dev.lan api.silieco.dev.lan
  ```
...
```bash
kubectl create namespace silieco
```

### Step 3 — Create the `silieco-secrets` Secret

The chart references this Secret by name. Create it once with random values:

```bash
kubectl -n silieco create secret generic silieco-secrets \
  --from-literal=JWT_SECRET="$(openssl rand -hex 32)" \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -hex 16)" \
  --from-literal=RESEND_API_KEY="" \
  --from-literal=GOOGLE_CLIENT_SECRET="" \
  --from-literal=CLOUDFRONT_PRIVATE_KEY="" \
  --from-literal=SILIECO_DEV_VERIFICATION_CODE=""
```
...
```bash
helm install silieco oci://ghcr.io/silieco-ai/charts/silieco \
...
```bash
helm show values oci://ghcr.io/silieco-ai/charts/silieco \
...
```bash
helm install silieco deploy/helm/silieco -n silieco
```

Watch the pods come up:

```bash
kubectl -n silieco get pods -w
```
...
```bash
curl -H "Host: api.silieco.dev.lan" http://<ingress-ip>/healthz
# {"status":"ok","checks":{"db":"ok","migrations":"ok"}}
```

Then open http://silieco.dev.lan in your browser.
...
- **Recommended (production):** patch the Secret with a real Resend key, then restart the backend:
  ...
  ```bash
  kubectl -n silieco patch secret silieco-secrets --type=merge \
    -p '{"stringData":{"RESEND_API_KEY":"re_xxx"}}'
  kubectl -n silieco rollout restart deploy/silieco-backend
  ```
...
- **Without email configured:** the verification code is generated server-side and printed to the backend pod logs (look for `[DEV] Verification code for ...:`). Useful for one-off testing.
  ...
  ```bash
  kubectl -n silieco logs -f deploy/silieco-backend | grep "Verification code"
  ```

- **Deterministic local/private testing:** set `backend.config.appEnv: development` in your values file and `SILIECO_DEV_VERIFICATION_CODE=888888` in the Secret, then `helm upgrade` and restart. This fixed code is ignored when `APP_ENV=production`.

  ```bash
  helm upgrade silieco oci://ghcr.io/silieco-ai/charts/silieco \
    --version <chart-version> \
    -n silieco \
    -f my-values.yaml --set backend.config.appEnv=development
  kubectl -n silieco patch secret silieco-secrets --type=merge \
    -p '{"stringData":{"SILIECO_DEV_VERIFICATION_CODE":"888888"}}'
  kubectl -n silieco rollout restart deploy/silieco-backend
  ```
...
```bash
silieco setup self-host \
  --server-url http://api.silieco.dev.lan \
  --app-url http://silieco.dev.lan
```
...
```bash
kubectl -n silieco rollout restart deploy/silieco-backend deploy/silieco-frontend
```

To upgrade to a specific Silieco release, upgrade to the matching chart version. The released chart defaults its app images to the matching Git tag:
...
```bash
helm -n silieco rollback silieco
```

> **Upgrading from `v0.3.4` to `v0.3.5+` fails with `refusing to drop legacy daily rollups: ...`?** As of SILI-2957 the `migrate up` command runs an idempotent monthly-slice backfill automatically before applying migration `103`, so a clean upgrade is a single `helm upgrade` + backend rollout. If you are still on a pre-SILI-2957 binary or the auto-hook fails, run the standalone backfill against the same database the chart is using (`kubectl -n silieco exec deploy/silieco-backend -- ./backfill_task_usage_hourly --sleep-between-slices=2s`), then restart the backend deployment to re-apply migrations. See [Advanced Configuration → Usage Dashboard Rollup](SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup) for the full recovery flow.

### Tearing down

```bash
# Remove the workloads but keep the PVCs and the Secret
helm -n silieco uninstall silieco

# Wipe everything, including PostgreSQL data and uploads
kubectl delete namespace silieco
```
...
The Usage / Runtime dashboards read from a derived `task_usage_hourly` table populated by `rollup_task_usage_hourly()`. As of SILI-2957 the backend runs this rollup **in-process** on every replica via a DB-backed scheduler (`sys_cron_executions`); a fresh self-host install needs no operator action and the bundled `pgvector/pgvector:pg17` image works without changes — you do **not** need to swap it for an image that ships `pg_cron`, register an external cron job, set up a systemd timer, or run a Kubernetes `CronJob`.
...
> **Upgrading from `v0.3.4` to `v0.3.5+`?** As of SILI-2957 the `migrate up` command runs an idempotent monthly-slice backfill automatically right before applying migration `103`, so the upgrade completes in a single invocation — no operator step required. If you are still on a pre-SILI-2957 binary or the auto-hook fails for an environmental reason, run `backfill_task_usage_hourly` against the same database and re-run the upgrade. See [Advanced Configuration → Usage Dashboard Rollup](SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup) for the recovery flow.

### Compatibility paths (existing deployments only)

External schedulers — **`pg_cron` registered on the database, an external cron job, a systemd timer, or a Kubernetes `CronJob`** — that call `SELECT rollup_task_usage_hourly()` directly were the only option before SILI-2957 and remain a supported compatibility path. They are no longer the recommended setup; new deployments should rely on the in-process scheduler instead. The SQL function holds advisory lock 4246 internally, so the in-process scheduler and any pre-existing external schedule can coexist without ever double-writing the rollup.
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in Silieco's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
```bash
curl -fsSL https://raw.githubusercontent.com/silieco-ai/silieco/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to Silieco Cloud

If you've been self-hosting and want to switch your CLI to [Silieco Cloud](https://silieco.ai):

```bash
silieco setup
```

This reconfigures the CLI for silieco.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `SILIECO_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
If the selected GHCR tag has not been published yet, fall back to `make selfhost-build` or `docker compose -f docker-compose.selfhost.yml -f docker-compose.selfhost.build.yml up -d --build`.

> **Upgrading from `v0.3.4` to `v0.3.5+` fails with `refusing to drop legacy daily rollups: ...`?** That's migration `103`'s fail-closed guard: it requires `task_usage_hourly` to be seeded before the legacy daily rollups are dropped. As of SILI-2957 `migrate up` runs that backfill automatically right before applying `103`, so the upgrade completes in a single invocation. If you are still on a pre-SILI-2957 binary or the auto-hook fails, run `backfill_task_usage_hourly` manually first, then re-run the upgrade. Full instructions in [Advanced Configuration → Usage Dashboard Rollup](SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
...
If you prefer configuring the CLI step by step instead of `silieco setup`:
