...
const DATABASE_URL = process.env.DATABASE_URL ?? "postgres://silieco:silieco@localhost:5432/silieco?sslmode=disable";
...
export class TestApiClient {
  ...
  async login(email: string, name: string) {
    ...
    try {
      ...
      const configuredDevCode = process.env.SILIECO_DEV_VERIFICATION_CODE?.trim();
