...
// prompt (SILI-5159). Captures screenshots for review. Uses a unique
...
test("onboarding — welcome → about you (answer path)", async ({ page }) => {
  ...
  await page.addInitScript((t) => {
    localStorage.setItem("silieco_token", t);
  }, token);
  ...
  await expect(page.getByText("What do you want to use Silieco for?")).toBeVisible();
  await expect(page.getByText(/Step 1 of 3/)).toBeVisible();
  await expect(page.getByText("How did you hear about Silieco?")).toHaveCount(0);
...
test("onboarding — one skip clears the whole questionnaire step", async ({ page }) => {
  ...
  await page.addInitScript((t) => localStorage.setItem("silieco_token", t), token);
  await page.goto("/onboarding", { waitUntil: "domcontentloaded" });
  await waitForPageText(page, "Continue on web");
...
test("onboarding — zh-Hans renders Chinese labels", async ({ page, context }) => {
  await context.addCookies([
    { name: "silieco-locale", value: "zh-Hans", url: "http://localhost:13442" },
  ]);
  const api = new TestApiClient();
