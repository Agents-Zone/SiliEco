...
// Stage 3.2 (SILI-3870): the creator-only MCP tab on the agent detail page.
//
...
const EMAIL = `e2e-mcp-${E2E_WORKER}-${E2E_RUN_ID}@silieco.ai`;
...
async function loginCapturingUser(page: Page): Promise<SetupResult> {
  ...
  await page.addInitScript((t) => {
    ...
    localStorage.setItem("silieco:chat:isOpen", "false");
  }, token);
