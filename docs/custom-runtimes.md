...
Silieco's supported protocol families but is launched through a team-specific
command.
...
```sh
silieco runtime profile set-path <profile-id> --path /abs/path/to/agent
```
