...
两次错位重构(SILI-2438 + da0ecb6a)叠加产出了一个味道很重的 onboarding:
...
<WelcomeAfterOnboarding /> consumes welcome-store:
  phase 1: full-screen loading "Preparing your Helper…"
           api.listAgents → 查重名 Sili Agent（workspace 可见的）
           → 找到 → 复用
...
<WelcomeAfterOnboarding /> consumes signal:
  ...
  后台并发 api.createIssue × 2:
    ...
    - Create your first Silieco Agent
...
### 删
- `core/internal/service/onboarding.go`
- `core/internal/service/workspace_content.go`
- `core/internal/handler/onboarding.go`:`BootstrapOnboardingRuntime`、`BootstrapOnboardingNoRuntime` handler 整体 + 相关常量(`onboardingAssistantInstructions` 94 行 markdown、`onboardingAssistantName`、`onboardingIssueTitle`、`onboardingIssueDescription`、`onboardingAssistantAvatarURL`、`onboardingAssistantDescription`、`onboardingAgentTemplate`)
- `core/internal/handler/workspace.go`:`EnsureOnboardingContent` handler
- `core/cmd/core/router.go`:3 条路由(`runtime-bootstrap` / `no-runtime-bootstrap` / `ensure-onboarding-content`)
- `core/internal/handler/handler.go`:`OnboardingService` / `WorkspaceContent` 字段 + 实例化
- `core/internal/handler/onboarding_test.go`:5 个 Bootstrap* 测试 + 2 个 PatchOnboarding 测试(只留 JoinCloudWaitlist 测试)
...
### 改
- `core/migrations/098_user_onboarding_runtime_choice.up.sql`:反转为 DROP CONSTRAINT/COLUMN IF EXISTS(同名文件、保留 098 序号)
- `core/migrations/098_user_onboarding_runtime_choice.down.sql`:no-op + 注释说明不可逆
- `core/pkg/db/queries/user.sql`:`PatchUserOnboarding` 回退到只接 questionnaire
- `core/internal/handler/onboarding.go`:`CompleteOnboarding` 简化为直接 `qtx.MarkUserOnboarded`(无 service 包装)+ `PatchOnboarding` 只接 questionnaire
- `core/internal/handler/invitation.go`:`AcceptInvitation` 改回直接 `qtx.MarkUserOnboarded`(保留 mark 调用 — 注释强调 layout gate 依赖)
- `core/internal/handler/workspace.go`:`CreateWorkspace` 删 `ClaimStarterContentStateIfUnset` 调用(后端不再触碰 `starter_content_state`)
- `core/internal/handler/auth.go`:`UserResponse` / `userToResponse` 删两字段
- `core/internal/handler/workspace_test.go`:测试断言保留,注释更新引用 layout gate 而非 OnboardingHelperModal
...
1. `onboarded_at` 的写入收敛到 3 个 handler:`CompleteOnboarding`(v3 主路径)、`AcceptInvitation`(invitee 必须)、`BootstrapOnboarding*`(**shim,见 §10**)。全部直接调 `qtx.MarkUserOnboarded`,无 service 包装。`grep -rln "qtx.MarkUserOnboarded\|h.Queries.MarkUserOnboarded" core/internal/handler/ | grep -v _test.go`
...
**位置**:`core/internal/handler/onboarding_shim.go`(独立文件,所有 deprecated 代码都在这里;v3 主路径 `onboarding.go` 不含一行 shim 代码)。
...
- `core/internal/handler/onboarding_shim.go` 整文件
- `core/cmd/core/router.go` 里 2 条 deprecated 注释 + 路由
- `core/internal/handler/onboarding_test.go` 末尾 5 个 `TestBootstrapOnboarding*`
