...
This doc explains the failure mode that caused [SILI-963][sili-963] and the
matrix the daemon now follows when writing Codex's per-task `config.toml`.

[sili-963]: https://silieco-api.copilothub.ai/issues/28c34ad2-102a-4f46-91ac-336ed78c5859
...
If you see `no such host` *inside a Codex session on macOS* but `curl https://silieco-api.copilothub.ai` from a plain shell on the same machine works, you are hitting the Seatbelt bug below.
...
The daemon writes a *silieco-managed* block into each task's
`$CODEX_HOME/config.toml`, delimited by `# BEGIN silieco-managed` /
`# END silieco-managed` markers. Anything outside the markers is left
untouched so users can still tune Codex behavior.

Decision matrix (see [`core/internal/daemon/execenv/codex_sandbox.go`](../core/internal/daemon/execenv/codex_sandbox.go)):
...
`[permissions.silieco]`, a bare `sandbox_mode = "..."` line would be parsed
as `permissions.silieco.sandbox_mode` and Codex would silently ignore it.
...
```bash
# Is the Silieco API reachable at all?
curl -sSf https://silieco-api.copilothub.ai/healthz
```

From inside a Codex session (after the daemon writes its config):

```bash
silieco issue list --limit 1 --output json >/dev/null && echo OK
```
...
- **A. Domain-scoped `permissions` profile** (tight): when the upstream
  `network_access` fix is available, prefer writing a `permissions.silieco`
  profile that allows only `silieco-api.copilothub.ai` and
  `silieco-static.copilothub.ai`. Keeps filesystem sandbox intact.
...
sed -n '/# BEGIN silieco-managed/,/# END silieco-managed/p' \
  ~/silieco_workspaces/$WORKSPACE_ID/$TASK_SHORT/codex-home/config.toml
```
