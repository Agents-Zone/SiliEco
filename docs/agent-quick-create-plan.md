...
3. Daemon 算工作目录:
   core/internal/daemon/execenv/execenv.go:114, 124
   workDir = {WorkspacesRoot}/{wsID}/{shortTaskID}/workdir

4. Daemon 按 runtime 算 skill 目录:
   core/internal/daemon/execenv/context.go:121-158 (resolveSkillsDir)
...
**核心 invariant**:Silieco 不教 runtime 怎么用 skill,只把文件摆到 runtime 已经会扫的位置。
...
- skill 内容跟 GitHub 同步,不需要 vendoring 进 silieco 仓库
...
3. Daemon 构造 prompt:
   ...
   - 加上"调用 silieco issue create CLI 命令"的指令

4. Agent 跑 LLM + tool calling:
   - LLM 输出形如 `silieco issue create --title="..." --description="..."` 的命令
   - daemon 执行 CLI 命令,CLI 调 POST /api/issues 创建 issue
   - CLI 自动在请求里带上 SILIECO_QUICK_CREATE_TASK_ID env(daemon/daemon.go:2081)
...
- **Template 定义存放**:静态 JSON,commit 在 `core/internal/agenttmpl/templates/*.json`
- **Template JSON 形态**:
  ```json
  {
    "slug": "code-reviewer",
...
| 文件 / 位置 | 改动 |
|---|---|
| `core/internal/agenttmpl/`(新包) | 加载 JSON 模板的代码 |
| `core/internal/agenttmpl/templates/*.json`(新文件) | 5 个起步模板 |
| `core/internal/handler/agent.go` | 新 handler `CreateAgentFromTemplate` |
| `core/internal/handler/skill_create.go` | **重构**:拆出 `createSkillWithFilesInTx` 变体(见 §4) |
| `core/pkg/db/queries/skill.sql` | 加 `GetSkillByWorkspaceAndName`(见 §4) |
| `core/cmd/core/router.go` | 注册新 endpoint |
...
- **Daemon prompt builder**:`daemon/prompt.go` 加 `buildSkillFindPrompt`(类比 buildQuickCreatePrompt)
  - 喂给 agent 的 prompt 大致:
    ...
    输出:调用 `silieco skill find --output-results '<JSON>'` 命令,
    JSON 形态为 [{name, description, source_url, reason}, ...]
    ```
- **CLI 命令**(新):`silieco skill find --output-results <JSON>`
  - 不发起 HTTP 请求,只把 JSON 写到 daemon 通过 env 指定的临时文件
  - daemon 读这个文件,把内容塞进 inbox notification 的 payload
- **Curated skill 索引**:`core/internal/agenttmpl/skill_index.json`(新文件)
...
| 文件 / 位置 | 改动 |
|---|---|
| `core/internal/handler/skill.go` | 新 handler `FindSkill`(enqueue task) |
| `core/internal/service/task.go` | 加 `EnqueueSkillFindTask` + 完成检测 + inbox 通知 |
| `core/internal/daemon/prompt.go` | 加 `buildSkillFindPrompt` |
| `core/internal/daemon/daemon.go` | 加 `SkillFindContext` 识别 + env 注入 |
| `core/cmd/silieco/cmd_skill.go` | 加 `find --output-results` 子命令 |
| `core/internal/agenttmpl/skill_index.json`(新文件) | curated 清单 |
...
- **Daemon prompt builder**:`buildAgentCreatePrompt` 指挥 agent 三步走:
  ```
  1. 调用 `silieco skill find --output-results ...` 选 skill
     (或直接看 curated 清单选)
  2. 基于选定 skill 写 instructions
  3. 调用 `silieco agent create --name ... --instructions ... --skill-ids ...`
     创建 agent 并关联 skill
  ```
- **CLI 命令**(新):`silieco agent create`
  - 后端 handler 已存在(handler/agent.go:CreateAgent),只需要绑 CLI(~50 行)
  - 创建时带 `SILIECO_AI_DRAFT_TASK_ID` env,服务端用它做 origin 标记 + LinkTaskToAgent
...
| 文件 / 位置 | 改动 |
|---|---|
| `core/internal/handler/agent.go` | 新 handler `AIDraftAgent`(enqueue task) |
| `core/internal/service/task.go` | 加 `EnqueueAgentDraftTask` + 完成检测 + inbox 通知 |
| `core/internal/daemon/prompt.go` | 加 `buildAgentCreatePrompt` |
| `core/cmd/silieco/cmd_agent.go` | 加 `create` 子命令(handler 已有) |
...
**问题**:`core/internal/handler/skill_create.go:21-71` 这个函数自己 `Begin()` 一个事务,执行完 `Commit()`。Phase 1 需要在外层事务里**多次**调用它(import N 个 skill + createAgent + setAgentSkills 都在一个 TX),但现在没法这么用。
...
1. 新 query `GetSkillByWorkspaceAndName`(`core/pkg/db/queries/skill.sql`)
2. Phase 1 流程改成:
...
### 4.3 [SOFT] 缺 `silieco skill find` CLI

**影响范围**:Phase 2

**方案**:加一个 CLI 子命令,模仿 `silieco skill import` 的实现(`core/cmd/silieco/cmd_skill.go:55-60, 323-357`)。**注意**:这个命令不发 HTTP 请求,只是 LLM agent 用来"输出推荐结果"的 channel——它把 LLM 推荐的 JSON 写到 daemon 指定的临时文件,daemon 读完塞进 inbox notification。

**工作量**:小(~80 行)

### 4.4 [SOFT] 缺 `silieco agent create` CLI

**影响范围**:Phase 3

**方案**:后端 handler 已有(`handler/agent.go:CreateAgent`),只需在 `core/cmd/silieco/cmd_agent.go` 加 `create` 子命令。
...
Silieco 只有**单体 skill**(SKILL.md + skill_file),没有 plugin / bundle 概念。要接入得新写 plugin parser + 拆分逻辑,工作量大,而 skills.sh 已经覆盖了同一批高质量内容(skills.sh 后端就是 GitHub raw,绝大多数 skill 作者就在 GitHub 上,Anthropic plugin 体系里的 skill 通常也在作者的 GitHub repo 里有单体副本)。

### 5.2 为什么走 quick-create 模式而不是后端直接调 LLM?

代码事实:`core/` 目前**完全没有任何 LLM SDK**(grep `anthropic-sdk-go` / `openai-go` / 任何 LLM provider 都是 0 命中)。所有 LLM 调用都通过 daemon → runtime → CLI 这条路。
...
- skill 教 agent 调什么?调 `npx skills`(装到本地,目标错)?调 Silieco API(那要写 tool channel,绕一大圈)
...
**判断**(详见之前讨论):
- ClawHub 服务的是 OpenClaw 平台(Silieco 同生态位竞品的内容生态)
...
| 主题 | 位置 |
|---|---|
| Skill DB 模型 | `core/migrations/008_structured_skills.up.sql:4-32` |
| Skill 创建 handler + 事务 | `core/internal/handler/skill.go:143-162` + `skill_create.go:21-71` |
| Skill import 入口(支持 3 个 source) | `core/internal/handler/skill.go:1538` |
| Skill import source 分发 | `core/internal/handler/skill.go:586-617` (`detectImportSource`) |
| Skills.sh fetcher | `core/internal/handler/skill.go:757-879` (`fetchFromSkillsSh`) |
| GitHub fetcher | `core/internal/handler/skill.go:1363-1463` (`fetchFromGitHub`) |
| ClawHub fetcher | `core/internal/handler/skill.go:642-744` (`fetchFromClawHub`) |
| Agent 创建 handler | `core/internal/handler/agent.go:380-399` (request) + `:422-564` (CreateAgent) |
| Agent 创建 sqlc | `core/pkg/db/queries/agent.sql:19-25` |
| Agent-Skill 关联 sqlc | `core/pkg/db/queries/agent.sql:86-103` |
...
| Quick-create Issue handler | `core/internal/handler/issue.go:877-982` (`QuickCreateIssue`) |
| Quick-create task enqueue | `core/internal/service/task.go:488+` (`EnqueueQuickCreateTask`) |
| Daemon claim + load skills | `core/internal/handler/daemon.go:1018-1098` + `service/task.go:1447-1463` |
| Daemon prompt build | `core/internal/daemon/prompt.go:17-36` (dispatch) + `:45-106` (`buildQuickCreatePrompt`) |
| Daemon execenv prepare | `core/internal/daemon/execenv/execenv.go:103-176` |
| Skill 目录约定(runtime mapping) | `core/internal/daemon/execenv/context.go:121-158` (`resolveSkillsDir`) |
| Skill 文件落盘 | `core/internal/daemon/execenv/context.go:175-204` (`writeSkillFiles`) |
| Quick-create 完成检测 + inbox | `core/internal/service/task.go:1810-1949` |
| LinkTaskToIssue | `core/internal/handler/agent.go:97-105` |
| Quick-create Issue 前端 modal | `packages/views/modals/quick-create-issue.tsx:48-570+` |
| Silieco CLI 入口 | `core/cmd/silieco/main.go:62-79` |
| Skill CLI 命令 | `core/cmd/silieco/cmd_skill.go:17-96`(已有 import,无 find) |
| Agent CLI 命令 | `core/cmd/silieco/cmd_agent.go:101-112`(已有 list/get,无 create) |
