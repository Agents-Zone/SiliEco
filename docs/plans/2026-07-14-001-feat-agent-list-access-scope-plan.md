...
- Permission semantics: `core/internal/handler/agent_access.go` (`canInvokeAgent`); derivation comment in `packages/core/types/agent.ts`.
- List payload: `core/internal/handler/agent.go` (`ListAgents` attaches `invocation_targets`); `core/pkg/db/queries/agent.sql` (`ListAgents`).
...
- **Verification:** Render the page with `@silieco/core/api` mocked and a callable store (per the repo test rules); assert the label per agent shape; `pnpm typecheck`.
...
- **Verification:** Page-level test mocking `@silieco/core/api` (`api.updateAgent` spy) with a callable store; assert per-id calls and query invalidation; `pnpm typecheck`.
...
- `pnpm test` — Vitest: U1 helper unit tests; `view-store.test.ts` for the new filter and column; page-level tests for the column label, the filter predicate, and the bulk action (mock `@silieco/core/api` and use the callable-store pattern per the repo test rules).
