...
5. **服务于产品定位**：Silieco 的核心差异化是 **"BYO-agent 的 Linear—— agent 跑在你自己的机器，你掌控计算和 provider 选择"**。每篇的语气和深度都应该为这个叙事服务。
...
---
title: Issues
description: Issue 是 Silieco 的核心工作对象——人和 agent 都能被分配、评论、改状态。
---
...
Issue 是 Silieco 的核心工作对象……（1-2 段话说清楚"是什么"）

## 关键概念

### Polymorphic Assignee

Issue 的 assignee 可以是 member（人）或 agent。这是 Silieco 和传统 task manager
...
```bash
silieco issue assign <issue-id> --agent <agent-slug>
```

分配给 agent 时，Silieco 会立刻入队一个 task 到对应 runtime。详见
...
- **目标读者**: P0 新用户 / evaluator（第一次听说 Silieco）
...
### 1.2 How Silieco Works — ⬜ Not started [v1]

- **Source files**: `core/cmd/silieco-daemon/`, `packages/core/`, 战略 plan 的"产品定位"段
...
- **Source files**: `apps/docs/content/docs/cloud-quickstart.mdx`（现有）, `core/cmd/silieco/cmd_setup.go`, `cmd_login.go`, `cmd_agent.go`, `cmd_issue.go`
- **目标读者**: P0 新用户（想 5 分钟跑起来）
- **叙事位置**: 第三页。
- **写什么**（600-1000 字）:
  - Signup → install CLI → `silieco login` → `silieco setup cloud` → 创建第一个 agent → 创建第一个 issue → 分配给 agent → 看它工作
  - 每步命令可独立复制运行
...
- **Source files**: `Makefile`（selfhost target）, `docker-compose.selfhost.yml`, `.env.example`, `core/cmd/silieco/cmd_setup.go`
...
- **写什么**（800-1200 字）:
  ...
  - 第一次启动后 `silieco setup self-host`
  - 最小可行配置（必填 env）
...
- **Source files**: `core/internal/handler/workspace.go`, `core/pkg/db/queries/workspace.sql`, `core/migrations/001/006/020`, `core/internal/validation/workspace.go`（slug + reserved 列表）
...
- **Source files**: `core/internal/handler/invitation.go`, `workspace.go`（member 部分）, `core/pkg/db/queries/invitation.sql`, `core/migrations/041`
- **目标读者**: P1 团队管理员
- **叙事位置**: Workspace 之后。
- **写什么**（1000-1500 字）:
...
- **Source files**:
  - Issues: `core/internal/handler/issue.go`, `core/pkg/db/queries/issue.sql`, migrations 001/015/017/018/020/050
  - Projects: `core/internal/handler/project.go`, `core/pkg/db/queries/project.sql`
- **目标读者**: P0 新用户、P1 团队管理员
- **叙事位置**: 核心工作对象。
- **写什么**（1500-2000 字）:
  - **Issues 部分**:
...
- **Source files**: `core/internal/handler/comment.go`, `core/pkg/db/queries/comment.sql`, migrations 017/018, `core/cmd/core/notification_listeners.go`（mention 解析）
- **目标读者**: P0 新用户、P1 重度用户
...
- **Source files**: `core/internal/handler/agent.go`, `core/pkg/db/queries/agent.sql`, migrations 001, `core/cmd/core/notification_listeners.go`（"agent 不收 inbox" 过滤）
...
- **Source files**: `core/internal/handler/agent.go`, `core/pkg/db/queries/agent.sql`, `packages/views/agents/`, `core/cmd/silieco/cmd_agent.go`
- **目标读者**: P1 团队管理员
- **叙事位置**: 怎么创建一个 agent。
- **写什么**（1200-1800 字）:
...
- **Source files**: `core/internal/handler/skill.go`, `core/pkg/db/queries/skill.sql`, `core/internal/daemon/execenv/context.go`（各 provider skill 注入路径）, `core/cmd/silieco/cmd_skill.go`
...
- **写什么**（1200-1800 字）:
  ...
  - **兼容性宣示**：
    > "Silieco Skill 采用 [Anthropic Agent Skills 开放标准](https://agentskills.io) 的 `SKILL.md` 格式。所有符合该规范的 skill（包括 Anthropic 官方仓库、ClawHub、skills.sh 上发布的包）都可以直接导入使用。"
  ...
  - **末尾一段"Skills vs MCP"**（v2 再开 MCP 独立页）:
    > MCP（Model Context Protocol）是另一层概念——让 agent 连外部工具（数据库、文件系统、第三方 API）。Silieco 支持 `mcp_config` 字段，但目前**仅 Claude Code 真实消费**，其他 provider 接收但未传递。详见 v2 的 MCP 专页（开发中）。
...
> **板块叙事**：agent 怎么真的动起来——分布式执行、用户自己跑 daemon。这是 Silieco 结构上区别于 Linear/Jira 的关键部分。
...
- **Source files**:
  - Daemon: `core/internal/daemon/daemon.go`, `core/cmd/silieco-daemon/main.go`, `core/cmd/silieco/cmd_daemon.go`, `core/pkg/db/queries/daemon.sql`
  - Runtime: `core/pkg/db/queries/runtime.sql`, `core/internal/handler/runtime.go`, `core/migrations/004`, `core/cmd/core/runtime_sweeper.go`
- **目标读者**: P0 运维、P1 开发者
...
- **写什么**（1500-2000 字）:
  - **Daemon 部分**:
    ...
    - CLI：`silieco daemon install/login/start/stop/status/logs`
  - **Runtime 部分**:
...
- **Source files**: `core/internal/service/task.go`, `core/pkg/db/queries/task.sql`, `core/internal/handler/task.go`, `task_lifecycle.go`, `runtime_sweeper.go`（timeout）
- **目标读者**: P0 新用户、P1 开发者
...
- **Source files**: `core/pkg/agent/*.go`（10 个 provider 文件）, `core/internal/daemon/execenv/context.go`（skill 路径）
...
- **Source files**: `core/internal/handler/issue.go`（UpdateIssue assign）, `core/internal/service/task.go`（`EnqueueTaskForIssue`）
- **目标读者**: P0 新用户
- **叙事位置**: 最常见触发方式。
- **写什么**（600-1000 字）:
  - UI：issue 详情页选 agent as assignee
  - CLI：`silieco issue assign <id> --agent <agent-slug>`
  - 分配后立刻入队 task
...
- **Source files**: `core/cmd/core/notification_listeners.go`（mention 解析）, `core/internal/service/task.go`（`EnqueueTaskForMention`）, `subscriber_listeners.go`（防自触发）
- **目标读者**: P0 新用户、P1 重度用户
...
- **Source files**: `core/internal/handler/chat.go`, `core/pkg/db/queries/chat.sql`, `core/internal/service/task.go`（`EnqueueChatTask`）, `packages/views/chat/`
- **目标读者**: P1 重度用户
...
- **Source files**: `core/internal/handler/autopilot.go`, `core/pkg/db/queries/autopilot.sql`, `core/internal/service/autopilot.go`, `service/task.go`（`CreateAutopilotTask`）
- **目标读者**: P1 管理员
...
- **Source files**:
  - Inbox: `core/cmd/core/notification_listeners.go`, `core/pkg/db/queries/inbox.sql`, `packages/core/inbox/`, `packages/views/inbox/`
  - Subscriptions: `core/cmd/core/subscriber_listeners.go`, `core/pkg/db/queries/issue_subscriber.sql`, migrations 015
...
- **Source files**: `.env.example`, `core/internal/config/`
- **目标读者**: self-host 运维
...
- **Source files**: `core/internal/handler/auth.go`（固定测试验证码 + checkSignupAllowed）, `.env.example`（auth 相关注释）
- **目标读者**: self-host 运维
- **叙事位置**: self-host 的 auth 配置。
- **写什么**（1500-2000 字）:
  - **🚨 超醒目 warning block**：生产环境必须保持 `SILIECO_DEV_VERIFICATION_CODE` 为空；固定测试验证码只用于非 production 私有测试
...
- **写什么**（1200-2000 字，v1 先覆盖 Top 6-8 问题）:
  ...
  - 固定测试验证码不工作（APP_ENV / SILIECO_DEV_VERIFICATION_CODE 检查）
  - Port 冲突
...
- **Source files**: `core/cmd/silieco/main.go`（命令树）, 各 `cmd_*.go`
- **目标读者**: P1 开发者、P2 agent
- **叙事位置**: Reference 板块首篇。
- **写什么**（2000-2500 字）:
  - **认证入口**（开头一段）:
    - `silieco login` → 拿 PAT（`sili_` 前缀）
    - PAT 存 `~/.silieco/config.json`
...
- **Source files**: `core/internal/handler/auth.go`, `core/internal/middleware/auth.go`, `core/internal/middleware/daemon_auth.go`, `core/cmd/silieco/cmd_auth.go`
- **目标读者**: P1 管理员、P1 开发者（用 API / CLI / daemon）
- **叙事位置**: Reference 板块。讲"三种身份证"。
- **写什么**（1200-1800 字）:
  - **3 种 token**:
    - **JWT Cookie**（`silieco_auth`，HttpOnly，30 天）—— 浏览器
    - **PAT**（`sili_` 前缀）—— CLI / 脚本
  ...
  - PAT 创建 / 撤销 / 管理（UI 在 Settings，CLI 通过 `silieco login`）
  - Daemon token 生成时机（`silieco daemon login`）
...
- **写什么**（1000-1500 字）:
  ...
  - **Daemon 不内置**：desktop 只是窗口，daemon 要单独 `silieco daemon start`，desktop package 里 bundle 了 CLI
...
- **板块 8**（CLI 详细 reference，从 8.1 cheatsheet 拆开）:
  - 8.4 silieco auth / login 详细
  - 8.5 silieco setup 详细
  - 8.6 silieco workspace 详细
  - 8.7 silieco issue（+comment / subscriber）详细
  - 8.8 silieco project 详细
  - 8.9 silieco agent（+skills）详细
  - 8.10 silieco skill 详细
  - 8.11 silieco autopilot 详细
  - 8.12 silieco repo 详细
  - 8.13 silieco daemon 详细
  - 8.14 silieco runtime 详细
  - 8.15 silieco config / version / update / attachment 详细
