...
Silieco's product analytics live in the **operational database** and in
**Prometheus / Grafana**. This document is the catalogue of instrumentation and
its history.

See [SILI-1122](https://github.com/silieco-ai/silieco) for the original design
context and [SILI-4127](https://github.com/silieco-ai/silieco) for the PostHog
retirement below.

> **SILI-4127 — PostHog retired for product analytics.** PostHog had become a
...
>   those.
> - The `silieco_signup_source` attribution cookie (`captureSignupSource`,
...
Self-hosters should **never inherit a Silieco-issued `POSTHOG_API_KEY`** —
...
- `Close()` drains remaining events during graceful shutdown. Called from
  `core/cmd/core/main.go` via `defer`.
...
- **Person properties (`$set`)** — use for mutable cohort signals
  ...
  `setPersonProperties()` in `@silieco/core/analytics`. Use
...
SILI-4127 those dashboards are retired: server events are Prometheus-only (DB is
the source of truth) and the frontend funnel events were deleted. The `Status`
column records where each event stands now.

| Category | Events | Status after SILI-4127 |
|---|---|---|
...
Grafana via `silieco_*` business counters (see `core/internal/metrics`).
...
| `signup_source` | string | Opaque attribution bundle from the frontend cookie `silieco_signup_source` (UTM + referrer). Empty when the cookie is absent. |
...
SILI-4127, because `signup` is now Prometheus-only and never reaches PostHog:

| Property | Type | Description |
|---|---|---|
| `email` | string | Full email. Was never broadcast per-event. |
| `signup_source` | string | Attribution bundle. Today only its bucketed form survives, as the `silieco_signup_total{signup_source}` Prometheus label (see `NormalizeSignupSource`); it is no longer set as a person property for segmentation. |
...
| `cli_version` | string | Version of the `silieco` CLI that registered it. |
...
> `BusinessMetrics.RecordTask*` methods in `core/internal/service/task.go`.
...
The actual metrics (defined in `core/internal/metrics/business.go`; label
sets in `core/internal/metrics/labels.go`):

| Metric | Type | Labels |
|---|---|---|
| `silieco_agent_task_enqueued_total` | counter | `source`, `runtime_mode` |
| `silieco_agent_task_dispatched_total` | counter | `source`, `runtime_mode` |
| `silieco_agent_task_started_total` | counter | `source`, `runtime_mode`, `provider` |
| `silieco_agent_task_terminal_total` | counter | `source`, `runtime_mode`, `terminal_status` |
| `silieco_agent_task_failed_total` | counter | `source`, `runtime_mode`, `failure_reason` |
| `silieco_agent_task_queue_wait_seconds` | histogram | `source`, `runtime_mode` |
| `silieco_agent_task_run_seconds` | histogram | `source`, `runtime_mode`, `terminal_status` |
| `silieco_agent_task_total_seconds` | histogram | `source`, `runtime_mode`, `terminal_status` |
...
- `terminal_status` is the task's final `agent_task_queue.status` —
  ...
  `silieco_agent_task_terminal_total{terminal_status=…}`. Failures
  additionally increment `silieco_agent_task_failed_total` carrying the coarse
...
`issue_executed` is the canonical core-loop success signal. Since SILI-4127 it is
metrics-only like every server event: recorded to Prometheus as
`silieco_issue_executed_total{source}` (not PostHog) and backed in the DB by
`issue.first_executed_at`. Per-task completion counts live in Grafana via
`BusinessMetrics.RecordTaskTerminal`; use `silieco_issue_executed_total` for the
...
> **Removed in SILI-4127**, except `$exception` (unchanged) and the
...
- `$pageview` — fired by the web tracker
  ...
  **pathname** changes, and by the desktop tracker
  ...
  segments are collapsed, so `/acme/issues/8d5c…` and `/acme/issues/SILI-12`
...
- Attribution is NOT a separate event; UTM + referrer origin are written
  to the `silieco_signup_source` cookie on the first anonymous pageview
...
`BusinessMetrics.RecordTaskTerminal` counter (exported as a `silieco_*` task
...
issue). Since SILI-4127 it is Prometheus-only, so reconcile
`silieco_issue_executed_total` against `issue.first_executed_at` rather than a
PostHog event.
...
2. Update `core/internal/analytics/events.go` constants and helpers to
   match.
