# Silieco Docs 站重写规划（v2）

> **本规划是什么**：Silieco 对外 doc 站（`apps/docs/`，Fumadocs + Next.js）的从零重写方案。它替换 v1 规划——v1 之前在代码调研之前写的，很多对概念的切分现在看是错的。
>
...
>
> **风格目标**：排版/布局对标 Anthropic docs（奶油底、serif heading、宽松行距、窄行宽、深色代码块的灵魂），但**色板继续用 Silieco 自己的 tokens**（冷蓝 brand）——visual 上是"Silieco 色 + Anthropic 排版语法"。

---

## 一、产品定位（文档的落脚点）

Silieco = **人 + AI agent 在同一个看板上协作的任务管理平台**。
...
> **关键**：P2 的 agent 不会逛导航，只会被人类用 `Fetch https://docs.silieco.ai/...` 指向某一页。所以每一页都要**自包含**。
...
| **How Silieco works** | 一张大图把 User / Issue / Agent / Runtime / Daemon / Skill / Task / Trigger 之间的关系串起来——目标是建立**正确心智模型**，而不是记名词 |

### 板块 2. Getting Started（3 篇）

| 篇目 | 核心内容 |
|---|---|
| **Cloud Quickstart** | 5 分钟：signup → install CLI → `silieco setup` → 第一个 agent → 第一个 issue |
| **Self-Host Quickstart** | 10 分钟：`install.sh --with-server` → `silieco setup self-host` |
...
| 16 | **Authentication & Tokens** | 3 种凭证 + signup flow + OAuth | JWT cookie（30 天）/ PAT（`sili_` 前缀）/ Daemon Token（`mdt_` 前缀）；Daemon Token **不能命中 user-scoped 路由**；PAT 几乎什么都能命中；signup 白名单优先级：`ALLOWED_EMAILS` > `ALLOWED_EMAIL_DOMAINS` > `ALLOW_SIGNUP` |
...
| Authentication setup | **🚨 固定测试验证码必须显式设置 `SILIECO_DEV_VERIFICATION_CODE`，生产保持为空**；Google OAuth 配置；signup 白名单 |
...
| 8 | 固定本地测试验证码默认关闭；`SILIECO_DEV_VERIFICATION_CODE` 仅用于非 production 私有测试 | Self-Hosting → Auth |
...
- `apps/docs/app/global.css` 里 `@import "@silieco/ui/styles/tokens.css"`，覆盖 Fumadocs 的 `neutral.css`
...
当前 `.github/workflows/ci.yml:33` 用 `--filter='!@silieco/docs'` 排除了 docs build。**在 Phase 0 做一个独立小 PR 把它加回来**——否则 MDX 语法错 CI 不拦，只有 Vercel 部署时才发现。
