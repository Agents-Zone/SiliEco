...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each Silieco Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Silieco user, per-installation. open_id is
...
--
--   2. The composite FK on (workspace_id, silieco_user_id) targets
--      member(workspace_id, user_id) with ON DELETE CASCADE, so when a
--      Silieco user is removed from the workspace the stale Lark
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one Silieco `chat_session`. The Lark side
...
-- =====================
-- Maps a Silieco task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a Silieco user. The
