-- Audit `admin`, `silieco`, `new`, `www` against the workspace.slug column.
--
...
-- was deemed unacceptable. SILI-972 closed that loop on prd:
--
--   * `admin`   (99cd10e4-…) → renamed to `legacy-admin-99cd10e4`
--   * `silieco` (dcd796aa-…) → renamed to `legacy-silieco-dcd796aa`
...
--  - core/internal/handler/workspace_reserved_slugs.go
...
BEGIN
  ...
  IF conflict_count > 0 THEN
    RAISE EXCEPTION 'Found % workspace(s) with slugs that collide with the legacy reserved-slug audit set: %. Rename or delete before deploying (see SILI-972 for the playbook).', conflict_count, conflict_list;
  END IF;
END $$;
