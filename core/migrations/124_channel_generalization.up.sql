...
-- platform-agnostic channel_* tables (SILI-3515, parent SILI-3506). Each
...
--
--   * NO foreign keys and NO cascades (SILI-3515 §4). The lark_* tables
...
--
--     PRD (rolling silieco-api, maxUnavailable:0) overlapped old and new pods.
--     A one-time SILIECO_LARK_HUB_DISABLED park-switch existed during the
...
--     since been removed (SILI-3515); this note is kept as history. Rollback to
...
CREATE TABLE channel_installation (
    ...
    -- that is an application-layer rule (SILI-3515 §4), not a DB constraint.
...
CREATE INDEX idx_channel_user_binding_user
    ON channel_user_binding(silieco_user_id, workspace_id);
...
INSERT INTO channel_user_binding (
    id, workspace_id, silieco_user_id, installation_id,
