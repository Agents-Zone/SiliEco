...
--
-- No foreign keys / cascades by design: Silieco enforces cross-table
...
--
-- composio_user_id always equals the Silieco user_id.String() — the
-- application keeps that mapping as an invariant so a Composio session can be
-- created from the Silieco user id alone. It is stored explicitly so a future
