...
// TestComposioCallbackIsPublic_NoCookieNot401 locks in the SILI-3843 fix: the
...
func TestComposioCallbackIsPublic_NoCookieNot401(t *testing.T) {
	...
	if resp.StatusCode == http.StatusUnauthorized {
		t.Fatalf("callback returned 401 without a session — it is still behind the Auth group (regression of SILI-3843). body=%s", body)
	}
}
