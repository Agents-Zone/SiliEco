...
import (
	...
	"github.com/silieco-ai/silieco/core/internal/auth"
	"github.com/silieco-ai/silieco/core/internal/cloudruntime"
	"github.com/silieco-ai/silieco/core/internal/daemonws"
	"github.com/silieco-ai/silieco/core/internal/events"
	"github.com/silieco-ai/silieco/core/internal/featureflags"
	"github.com/silieco-ai/silieco/core/internal/handler"
	"github.com/silieco-ai/silieco/core/internal/integrations/channel"
	"github.com/silieco-ai/silieco/core/internal/integrations/channel/engine"
	composiointeg "github.com/silieco-ai/silieco/core/internal/integrations/composio"
	"github.com/silieco-ai/silieco/core/internal/integrations/lark"
	"github.com/silieco-ai/silieco/core/internal/integrations/slack"
	obsmetrics "github.com/silieco-ai/silieco/core/internal/metrics"
	"github.com/silieco-ai/silieco/core/internal/middleware"
	"github.com/silieco-ai/silieco/core/internal/realtime"
	"github.com/silieco-ai/silieco/core/internal/service"
	"github.com/silieco-ai/silieco/core/internal/storage"
	"github.com/silieco-ai/silieco/core/internal/util"
	"github.com/silieco-ai/silieco/core/internal/util/secretbox"
	composiosdk "github.com/silieco-ai/silieco/core/pkg/composio"
	db "github.com/silieco-ai/silieco/core/pkg/db/generated"
	"github.com/silieco-ai/silieco/core/pkg/featureflag"
)
...
// SILIECO_APP_URL and falls back to FRONTEND_ORIGIN, matching how the backend
// resolves the app URL elsewhere (handler.daemonSetupURLsFromEnv) and the CLI
// login flow (cmd/silieco tryResolveAppURL). Empty when neither is set.
func appURLFromEnv() string {
	if v := strings.TrimRight(strings.TrimSpace(os.Getenv("SILIECO_APP_URL")), "/"); v != "" {
		return v
	}
...
// SILIECO_TRUSTED_PROXIES env var. Invalid entries are dropped with a single
...
func parseTrustedProxies(raw string) []netip.Prefix {
	...
	for _, part := range strings.Split(raw, ",") {
		...
		if err != nil {
			slog.Warn("SILIECO_TRUSTED_PROXIES: ignoring invalid CIDR",
				"value", s, "error", err)
			continue
		}
		out = append(out, p)
...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	signupConfig := handler.Config{
		...
		VCSIntegrationEnabled:    os.Getenv("SILIECO_VCS_INTEGRATION_ENABLED") == "true",
		PublicURL:                strings.TrimRight(strings.TrimSpace(os.Getenv("SILIECO_PUBLIC_URL")), "/"),
		TrustedProxies:           parseTrustedProxies(os.Getenv("SILIECO_TRUSTED_PROXIES")),
		CloudRuntimeFleetURL:     cloudRuntimeFleetURLFromEnv(),
		CloudRuntimeFleetTimeout: envDuration("SILIECO_CLOUD_FLEET_TIMEOUT", 35*time.Second),
		AttachmentDownloadMode:   os.Getenv("ATTACHMENT_DOWNLOAD_MODE"),
		AttachmentDownloadURLTTL: envDuration("ATTACHMENT_DOWNLOAD_URL_TTL", 30*time.Minute),
		AttachmentFrameAncestors: origins,
		LLMAPIKey:                strings.TrimSpace(os.Getenv("SILIECO_LLM_API_KEY")),
		LLMBaseURL:               strings.TrimSpace(os.Getenv("SILIECO_LLM_BASE_URL")),
		LLMDefaultModel:          strings.TrimSpace(os.Getenv("SILIECO_LLM_DEFAULT_MODEL")),
	...
	if opts.BusinessMetrics != nil {
		...
		// silieco_cloudruntime_request_* histograms.
	...
	// Channel engine (SILI-3620): the platform-agnostic inbound runtime.
	...
	// into one agent run instead of one per message (SILI-2968).
	...
	// Lark integration. Only wired when SILIECO_LARK_SECRET_KEY is set:
	...
	if larkKey, err := secretbox.LoadKey("SILIECO_LARK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				// SILIECO_LARK_SECRET_KEY is the operator's opt-in for
				...
				// real Lark traffic can point SILIECO_LARK_HTTP_BASE_URL
				// at a mock server.
				//
				// SILIECO_LARK_HTTP_BASE_URL is an OPTIONAL deployment-wide
				...
				larkClient := lark.NewHTTPAPIClient(lark.HTTPClientConfig{
					BaseURL: strings.TrimSpace(os.Getenv("SILIECO_LARK_HTTP_BASE_URL")),
					Logger:  slog.Default(),
				})
				h.LarkAPIClient = larkClient

				// Channel-backed store: routes the lark package's DB seams
				// onto the channel_* tables (SILI-3515). Interface-wired
				...
				// Feishu adapter (SILI-3620): the WSLongConnConnector talks
				...
				// to initialize (bad SILIECO_LARK_CALLBACK_BASE_URL or
				...
				// on a fresh deployment and exit. SILI-2671.
				...
				go lark.BackfillRegionFromLegacyOverride(context.Background(), cs,
					strings.TrimSpace(os.Getenv("SILIECO_LARK_HTTP_BASE_URL")),
					strings.TrimSpace(os.Getenv("SILIECO_LARK_CALLBACK_BASE_URL")),
					slog.Default())
				...
				// SILIECO_LARK_REGISTRATION_DOMAIN / _LARK_DOMAIN env
				// vars override the protocol hosts for staging / dev.
				regCfg := lark.RegistrationConfig{
					Domain:     strings.TrimSpace(os.Getenv("SILIECO_LARK_REGISTRATION_DOMAIN")),
					LarkDomain: strings.TrimSpace(os.Getenv("SILIECO_LARK_REGISTRATION_LARK_DOMAIN")),
				}
		...
		slog.Info("lark integration disabled (SILIECO_LARK_SECRET_KEY not set)")
	}

	// Slack integration. Multi-tenant B2 model (SILI-3666): Silieco hosts ONE
	...
	// stage-3 per-installation connection model (SILI-3516).
	//
	// Two deployment-level env vars gate the two halves:
	//   - SILIECO_SLACK_SECRET_KEY decrypts the per-installation bot token
	...
	//     Slack at all.
	//   - SILIECO_SLACK_APP_TOKEN is the app-level token (xapp-) authorizing the
	...
	if slackKey, err := secretbox.LoadKey("SILIECO_SLACK_SECRET_KEY"); err == nil {
		box, err := secretbox.New(slackKey)
		if err != nil {
			slog.Error("slack: secretbox.New failed; slack integration disabled", "error", err)
		} else {
			// Outbound replier (SILI-3666): delivers NeedsBinding prompt /
			...
			// the Slack user to their Silieco account.
			...
			slackReplier := slack.NewOutboundReplier(slack.OutboundReplierConfig{
				...
				// app URL (SILIECO_APP_URL ?? FRONTEND_ORIGIN), NOT SILIECO_PUBLIC_URL
				// (the backend/API URL). Mirrors the Lark replier (appURLFromEnv).
				AppURL: appURLFromEnv(),
				Logger: slog.Default(),
			})
			// Typing indicator (SILI-3874): a 👀 reaction on the user's message
			...
			// On-demand history reader behind the unified `silieco chat history`
			// command (SILI-3871): pull the session's Slack conversation when the
			// agent asks, instead of force-assembling it on every inbound.
			h.SlackHistory = slack.NewHistory(queries, box.Open, slog.Default())

			// `/issue` slash command (SILI-3908): a real Slack slash command,
			...
			// and a Silieco notification when the issue lands.
		...
		slog.Info("slack integration disabled (SILIECO_SLACK_SECRET_KEY not set)")
	}

	// Composio integration (SILI-3720). Gated by COMPOSIO_API_KEY plus the
	...
	// (COMPOSIO_CALLBACK_BASE_URL, falling back to SILIECO_PUBLIC_URL). The
	...
	if composioAPIKey := strings.TrimSpace(os.Getenv("COMPOSIO_API_KEY")); composioAPIKey != "" {
		if !featureflags.ComposioMCPAppsEnabled(context.Background(), opts.FeatureFlags) {
			...
			if err != nil {
				...
				case callbackBase == "":
					slog.Error("composio: no callback base url (set COMPOSIO_CALLBACK_BASE_URL or SILIECO_PUBLIC_URL); composio integration disabled")
				default:
					...
					if serr != nil {
						...
						h.Composio = svc
						// Stage 3 (SILI-3721) hook: feed the per-task MCP
	...
	if vcsKey, err := secretbox.LoadKey("SILIECO_VCS_SECRET_KEY"); err == nil {
		...
		slog.Info("vcs integration disabled (SILIECO_VCS_SECRET_KEY not set)")
	}
	...
	// the DaemonAuth fallback (sili_) path, and the revoke handler
	...
	// Cloud PAT verifier: validates mcn_ tokens against Silieco Cloud
	// Fleet. Returns nil when no Fleet URL is configured — the Auth /
	// DaemonAuth middlewares treat nil as "mcn_ not supported" and
	// reject with 401, instead of falling through to sili_/JWT paths.
	// Reuses SILIECO_CLOUD_FLEET_URL (the same URL the cloud-runtime
	...
	// WS-first claim (SILI-4257): route daemon:rpc_request frames (e.g.
	...
	// Share the same trusted-proxy CIDRs (SILIECO_TRUSTED_PROXIES) so the
	...
	// dependency. See SILI-1138 (Phase 0).
	//
	// Access is restricted (SILI-1342): when REALTIME_METRICS_TOKEN is set,
	...
	// Capability-authenticated attachment download (SILI-5292). Public by
	...
	// be avatar-class — see core/internal/handler/avatar.go (SILI-5393 /
	// #6024).
	...
	r.With(authRL).Post("/auth/password/register", h.RegisterPassword)
	r.With(authVerifyRL).Post("/auth/password/login", h.LoginPassword)
	...
	// GitHub App webhook (no Silieco auth — requests are authenticated via
	// HMAC-SHA256 signature in the handler) and post-install setup callback.
	r.Post("/api/webhooks/github", h.HandleGitHubWebhook)
	r.Get("/api/github/setup", h.GitHubSetupCallback)
	// Slack OAuth callback (no Silieco auth in the path — it is hit by Slack's
	...
	// VCS webhook for token-based providers (Forgejo / Gitea / GitLab). No Silieco
	...
	// Stripe webhook (no Silieco auth — Stripe signs the raw body
	// with a shared secret, the silieco-cloud upstream verifies. We
	// only forward the bytes + the Stripe-Signature header; see
	// HandleCloudBillingStripeWebhook for the rationale).
	r.Post("/api/webhooks/stripe", h.HandleCloudBillingStripeWebhook)

	// Composio OAuth callback (SILI-3843). NOT under the Auth group on purpose:
	...
	r.Route("/api/daemon", func(r chi.Router) {
		...
		// Canonical machine-level batch claim (SILI-4257). `/claim` is a
	...
	r.Group(func(r chi.Router) {
		...
		// core/internal/handler/onboarding_shim.go.
		...
		// Note (SILI-4309): the generic OpenAI-compatible passthrough endpoints
		...
		// comment markdown depends on this — see SILI-3130. The
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				// being a workspace admin (SILI-4213). The router can't make
				...
				// Slack integration (SILI-3666). Same admin/member split as
		...
		// Composio integration (SILI-3720). User-scoped (no workspace context):
		...
		// see SILI-3843). All return 503 when COMPOSIO_API_KEY is unset.
		...
		// cloud-runtime — silieco-cloud's Fleet and Billing share
		...
		r.Group(func(r chi.Router) {
			...
			// Tasks are Silieco's canonical work item. `/api/issues` remains a
			// wire-compatible alias for older clients and existing integrations.
			mountTaskRoutes := func(r chi.Router, canonical bool) {
				...
				r.Get("/", h.ListIssues)
				// POST twin of GET list for oversized filter sets
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					if canonical {
						r.Get("/active-run", h.GetActiveTaskForIssue)
						r.Post("/runs/{taskId}/cancel", h.CancelTask)
						r.Get("/runs", h.ListTasksByIssue)
					} else {
			...
			r.Get("/api/runs/{taskId}/messages", h.ListTaskMessagesByUser)
			...
			// Workflows are optional, versioned SOPs. Tasks without a workflow
			// keep using the existing status lifecycle unchanged.
			r.Route("/api/workflows", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					r.Post("/versions", h.CreateWorkflowVersion)
					r.Post("/versions/{versionId}/publish", h.PublishWorkflowVersion)
					r.Post("/instances", h.CreateWorkflowInstance)
				})
			})
			r.Route("/api/workflow-instances", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					r.Put("/tasks/{taskId}", h.AttachWorkflowTask)
					r.Delete("/tasks/{taskId}", h.DetachWorkflowTask)
				})
			})
			...
			r.Route("/api/agents", func(r chi.Router) {
				...
				r.Route("/{id}", func(r chi.Router) {
					...
					// activity_log. See SILI-2600, SILI-5438 and
			...
			r.Post("/api/runs/{taskId}/cancel", h.CancelTaskByUser)
			...
			r.Route("/api/chat/sessions", func(r chi.Router) {
				...
				r.Route("/{sessionId}", func(r chi.Router) {
					...
					r.Patch("/participants", h.UpdateGroupChatParticipants)
			...
			// Agent-facing channel reads (SILI-3871). The caller's task-scoped token
...
// SILIECO_LARK_CALLBACK_BASE_URL), we log and fall back to the
...
func buildLarkConnector(installSvc *lark.InstallationService, apiClient lark.APIClient) (lark.EventConnector, string) {
	endpointFetcher, err := lark.NewHTTPConnectionTokenFetcher(lark.HTTPConnectionTokenConfig{
		BaseURL: strings.TrimSpace(os.Getenv("SILIECO_LARK_CALLBACK_BASE_URL")),
		Logger:  slog.Default(),
	})
	if err != nil {
	...
	if proxyURL := strings.TrimSpace(os.Getenv("SILIECO_LARK_WS_PROXY_URL")); proxyURL != "" {
	...
	// prefetches a window of surrounding group history (SILI-3084) into the
...
func cloudRuntimeFleetURLFromEnv() string {
	if url := strings.TrimSpace(os.Getenv("SILIECO_CLOUD_FLEET_URL")); url != "" {
		return url
	}
	return strings.TrimSpace(os.Getenv("SILIECO_FLEET_URL"))
}
...
// already-resolved SILIECO_PUBLIC_URL, then the app URL.
