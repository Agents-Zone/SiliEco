...
import (
	...
	"github.com/silieco-ai/silieco/core/internal/scheduler"
...
func main() {
	...
	if os.Getenv("SILIECO_DEV_VERIFICATION_CODE") != "" {
		if strings.EqualFold(strings.TrimSpace(os.Getenv("APP_ENV")), "production") {
			slog.Warn("SILIECO_DEV_VERIFICATION_CODE is set but ignored because APP_ENV=production.")
		} else {
			slog.Warn("SILIECO_DEV_VERIFICATION_CODE is enabled. Use it only for local development or private test instances.")
		}
	}
	...
	shutdownHoldDuration := envNonNegativeDuration("SILIECO_SHUTDOWN_HOLD_DURATION", 0)

	// Feature flags: loaded once at startup from SILIECO_FEATURE_FLAGS_FILE
	...
	if dbURL == "" {
		dbURL = "postgres://silieco:silieco@localhost:5432/silieco?sslmode=disable"
	}
	...
	// SILI-1138: when REDIS_URL is set, route fanout through a Redis relay so
	...
	// GitHub PR-card API snapshot pipeline (SILI-5265): worker pool + TTL sweeper.
	// No-op when unconfigured (no App private key).
	h.PRRefresh.Start(sweepCtx)

	// Channel inbound supervisor (SILI-3620): holds the §4.4 WS lease per
	...
	// SILI-2957: DB-backed execution scheduler. The scheduler turns the
	...
	// SILI-3551: scheduled-Autopilot dispatch runs on the same DB-backed
