...
func TestRedisClientName(t *testing.T) {
	tests := []struct {
		...
		{"empty_suffix_returns_existing", "silieco-api:store", "", "silieco-api:store"},
		{"empty_existing_uses_default_prefix", "", "store", "silieco-api:store"},
...
func TestNewNamedRedisClient_SetsClientName(t *testing.T) {
	...
	opts := client.Options()
	if opts.ClientName != "silieco-api:store" {
		t.Errorf("ClientName = %q, want %q", opts.ClientName, "silieco-api:store")
	}
}
...
func TestNewNamedRedisClient_DisableClientName_InvalidValue(t *testing.T) {
	...
	if opts.ClientName != "silieco-api:store" {
		t.Errorf("ClientName = %q, want %q (invalid env should fall back to naming enabled)", opts.ClientName, "silieco-api:store")
	}
}
