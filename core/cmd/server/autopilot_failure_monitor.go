...
// monitor. Defaults match the proposal in SILI-1336 §6 action item #2:
...
// `Registro de ls cada 5 min` case in SILI-1336: 1,475 / 1,476 runs failed
...
func tickAutopilotFailureMonitor(ctx context.Context, queries *db.Queries, bus *events.Bus, cfg failureMonitorConfig) {
	...
	for _, c := range candidates {
		...
		// A system auto-pause is a substantive status change (SILI-4302 §3.4).
