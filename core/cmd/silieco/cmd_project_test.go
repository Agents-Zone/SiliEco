...
// TestBuildResourceRefFromFlagsGithubMergesHint pins the nit fix from SILI-2662
// review round 2: `silieco project resource update <p> <r> --default-branch-hint x`
...
func TestBuildResourceRefFromFlagsGithubMergesHint(t *testing.T) {
	t.Run("hint-only edit preserves existing url", func(t *testing.T) {
		...
		existing := map[string]any{"url": "https://github.com/silieco-ai/silieco"}
	...
	t.Run("url override survives merge", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/silieco-ai/new-repo")
		existing := map[string]any{
			"url":                 "https://github.com/silieco-ai/silieco",
		...
		if ref["url"] != "https://github.com/silieco-ai/new-repo" {
...
func TestBuildResourceRefFromRefFlagKeepsJSONEscapeHatch(t *testing.T) {
	t.Run("json payload wins over github shortcuts", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/silieco-ai/ignored")
		_ = cmd.Flags().Set("ref", `{"url":"https://github.com/silieco-ai/silieco","ref":"release/v2"}`)
		...
		if payload["url"] != "https://github.com/silieco-ai/silieco" {
	...
	t.Run("broken json is still rejected", func(t *testing.T) {
		cmd := newProjectResourceUpdateTestCmd()
		_ = cmd.Flags().Set("url", "https://github.com/silieco-ai/silieco")
