...
func TestResolveTextFlag(t *testing.T) {
	...
	t.Run("file body is preserved verbatim with non-ASCII content", func(t *testing.T) {
		// The workdir guardrail (SILI-4252) requires the file to live inside the
	...
	// SILI-4252: a --<name>-file path that resolves outside the working
...
// TestEnsureAttachmentWithinWorkdir covers the SILI-4252 guardrail extended to
...
// TestRunIssueCommentAddRejectsExternalAttachmentWithZeroUploads is the SILI-4252
...
func TestRunIssueCommentAddRejectsExternalAttachmentWithZeroUploads(t *testing.T) {
	...
	t.Setenv("SILIECO_TOKEN", "mat_test-token")
...
func TestRunIssueCreateSendsExistingAttachmentIDs(t *testing.T) {
	...
	t.Setenv("SILIECO_WORKSPACE_ID", "ws-1")
	t.Setenv("SILIECO_TOKEN", "test-token")
	t.Setenv("SILIECO_QUICK_CREATE_ATTACHMENT_IDS", `["att-env","att-shared"]`)
...
func TestRunIssuePullRequestsListsLinkedPRsAsJSON(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/api/issues/SILI-2818":
			json.NewEncoder(w).Encode(map[string]any{
				"id":         "issue-uuid",
				"identifier": "SILI-2818",
				"title":      "CLI PR lookup",
			})
		case "/api/issues/issue-uuid/pull-requests":
			json.NewEncoder(w).Encode(map[string]any{
				"pull_requests": []map[string]any{
					{
						"url":    "https://github.com/silieco-ai/silieco/pull/42",
						"number": float64(42),
						"state":  "open",
						"title":  "SILI-2818 add issue PR CLI",
					},
				},
			})
		default:
	...
	r, w, _ := os.Pipe()
	os.Stdout = w
	err := runIssuePullRequests(cmd, []string{"SILI-2818"})
	_ = w.Close()
	os.Stdout = old
	out, _ := io.ReadAll(r)
	if err != nil {
	...
	if want := []string{"/api/issues/SILI-2818", "/api/issues/issue-uuid/pull-requests"}; fmt.Sprint(gotPaths) != fmt.Sprint(want) {
	...
	if pr["url"] != "https://github.com/silieco-ai/silieco/pull/42" || pr["number"] != float64(42) || pr["state"] != "open" || pr["title"] != "SILI-2818 add issue PR CLI" {
...
func TestRunIssueUsageReturnsTokenSummaryAsJSON(t *testing.T) {
	...
	r, w, _ := os.Pipe()
	os.Stdout = w
	err := runIssueUsage(cmd, []string{"SILI-2818"})
	_ = w.Close()
	os.Stdout = old
	out, _ := io.ReadAll(r)
	if err != nil {
	...
	if want := []string{"/api/issues/SILI-2818", "/api/issues/issue-uuid/usage"}; fmt.Sprint(gotPaths) != fmt.Sprint(want) {
		t.Fatalf("paths = %v, want %v", gotPaths, want)
	}
	var payload map[string]any
	if err := json.Unmarshal(out, &payload); err != nil {
...
func TestRunIssuePullRequestsTableIncludesCoreFields(t *testing.T) {
	...
	out, _ := io.ReadAll(r)
	text := string(out)
	for _, want := range []string{"NUMBER", "STATE", "TITLE", "URL", "42", "open", "SILI-2818 add issue PR CLI", "https://github.com/silieco-ai/silieco/pull/42"} {
...
func TestResolveIssueRef(t *testing.T) {
	...
	t.Run("identifier is resolved before prefix lookup", func(t *testing.T) {
		...
		client := cli.NewAPIClient(srv.URL, "ws-1", "test-token")
		got, err := resolveIssueRef(context.Background(), client, "SILI-1852")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if listCalled {
		...
		if got.ID != issue["id"] || got.Display != "SILI-1852" {
			t.Fatalf("got %#v", got)
		}
	})
	...
	t.Run("short UUID prefix is rejected without any HTTP call", func(t *testing.T) {
		...
		if msg := err.Error(); !strings.Contains(msg, "SILI-") {
	...
	t.Run("non-hex gibberish is rejected with key/UUID guidance", func(t *testing.T) {
		// Inputs that are neither SILI-key, full UUID, nor a hex prefix
...
func TestResolveTaskRunID(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "--task") {
		t.Fatalf("expected missing --task error for short prefix, got %v", err)
	}
}
...
func TestRunIssueRunMessagesResolvesShortTaskPrefix(t *testing.T) {
	...
	_ = cmd.Flags().Set("task", "SILI-1852")
	if err := runIssueRunMessages(cmd, []string{"abcd"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if messagePath != "/api/runs/"+taskID+"/messages" {
		t.Fatalf("message path = %q, want canonical run messages path", messagePath)
	}
}
...
func TestResolveAssignee(t *testing.T) {
	...
	// SILI-2165: squad names must resolve to (squad, <id>) so the autopilot
...
// TestResolveAssigneeRespectsKinds covers the SILI-2165 follow-up: callers
// whose target schema is member-or-agent-only (project.lead_type DB CHECK
// at core/migrations/034_projects.up.sql:10, and the subscriber handler's
// isWorkspaceEntity switch at core/internal/handler/handler.go:414) must
...
// silieco-ai/silieco#1620: when one name is a substring of another (e.g.
...
// silieco-ai/silieco#1620: passing a full UUID or its 8-char prefix must
...
func TestResolveAssigneeByIDStrict(t *testing.T) {
	...
	t.Run("full UUID resolves the right agent in a substring-collision workspace", func(t *testing.T) {
		// This is the SILI-1254 scenario: agent "J" is unreachable by name
	...
	// SILI-2165: --assignee-id <squad-uuid> must resolve to (squad, <id>) so
...
// for the SILI-2165 follow-up. Subscriber add/remove and project lead pass
// memberOrAgentKinds because their target schema rejects squads
// (subscriber: core/internal/handler/handler.go:414;
// project: core/migrations/034_projects.up.sql:10). Without this gating,
// `silieco issue subscriber add --user "<SquadName>"` or
// `silieco project create --lead "<SquadName>"` would resolve to
...
func TestRunIssueCommentListFlagGuards(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.setup(cmd)
			err := runIssueCommentList(cmd, []string{"SILI-1"})
			if err == nil {
...
func TestRunIssueCommentList_RootsOnlyPassesThroughWithSince(t *testing.T) {
	...
	if err := runIssueCommentList(cmd, []string{"SILI-1"}); err != nil {
...
// fold default and its --full escape hatch (SILI-3555):
//
...
func TestRunIssueCommentList_ThreadTailPassesThroughAndPrintsReplyCursor(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/comments") {
			...
			w.Header().Set("X-Silieco-Next-Before", "2026-01-01T00:00:00.000000001Z")
			w.Header().Set("X-Silieco-Next-Before-Id", "00000000-0000-0000-0000-000000000999")
			w.Write([]byte("[]"))
			return
		}
		t.Errorf("unexpected request: %s %s", r.Method, r.URL.String())
...
func TestRunIssueCommentList_RecentStillLabelsCursorAsThread(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Silieco-Next-Before", "2026-01-01T00:00:00.000000001Z")
		w.Header().Set("X-Silieco-Next-Before-Id", "00000000-0000-0000-0000-000000000777")
		w.Write([]byte("[]"))
	}))
	defer srv.Close()
...
func TestRunIssueUpdateSendsPosition(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/issues/SILI-1":
			json.NewEncoder(w).Encode(map[string]any{
				"id": "issue-1", "identifier": "SILI-1", "status": "todo",
			})
		case r.Method == http.MethodPut && r.URL.Path == "/api/issues/issue-1":
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
				t.Errorf("decode body: %v", err)
			...
			json.NewEncoder(w).Encode(map[string]any{
				"id": "issue-1", "identifier": "SILI-1", "status": "todo", "priority": "none",
			})
		default:
			http.NotFound(w, r)
		}
...
// are coupled to cobra's wording (pinned at v1.10.2 in core/go.mod).
func TestIssueReorderTargetFlagGroup(t *testing.T) {
	...
	t.Run("two bool targets", func(t *testing.T) {
		cmd := newCmd()
		cmd.SetArgs([]string{"SILI-1", "--top", "--bottom"})
		err := cmd.Execute()
		if err == nil {
	...
	t.Run("before and after together", func(t *testing.T) {
		cmd := newCmd()
		cmd.SetArgs([]string{"SILI-1", "--before", "SILI-2", "--after", "SILI-3"})
		err := cmd.Execute()
		if err == nil {
...
func TestRunIssueReorderRejectsNoOpTargetValues(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.flag, func(t *testing.T) {
			...
			err := runIssueReorder(cmd, []string{"SILI-1"})
			if err == nil {
...
func reorderTestServer(t *testing.T, gotPosition *float64) *httptest.Server {
	...
	a := issue("a-id", "SILI-1", "todo", 1)
	b := issue("b-id", "SILI-2", "todo", 9)
	target := issue("t-id", "SILI-9", "todo", 20)
	c := issue("c-id", "SILI-3", "in_progress", 4)
	byRef := map[string]map[string]any{
		"SILI-1": a, "a-id": a,
		"SILI-2": b, "b-id": b,
		"SILI-9": target, "t-id": target,
		"SILI-3": c, "c-id": c,
	}
...
func TestRunIssueReorderComputesPosition(t *testing.T) {
	tests := []struct {
		...
		{"before another issue", "before", "SILI-2", 5}, // midpoint(a=1, b=9)
		{"after another issue", "after", "SILI-2", 10},  // moves below b (the bottom)
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := runIssueReorder(cmd, []string{"SILI-9"}); err != nil {
...
func TestRunIssueReorderRejectsCrossColumnTarget(t *testing.T) {
	...
	cmd := newIssueReorderTestCmd()
	_ = cmd.Flags().Set("before", "SILI-3") // SILI-3 lives in the in_progress column
	err := runIssueReorder(cmd, []string{"SILI-9"})
	if err == nil {
...
func TestFetchIssueColumnPaginates(t *testing.T) {
	all := []map[string]any{
		mkIssue("i1", "SILI-1", "todo", 1),
		mkIssue("i2", "SILI-2", "todo", 2),
		mkIssue("i3", "SILI-3", "todo", 3),
	}
...
func TestRunIssueReorderNoOpSkipsPut(t *testing.T) {
	...
	a := mkIssue("a-id", "SILI-1", "todo", 0)
	b := mkIssue("b-id", "SILI-2", "todo", 10)
	target := mkIssue("t-id", "SILI-9", "todo", 5)
	byRef := map[string]map[string]any{
		"SILI-1": a, "a-id": a,
		"SILI-2": b, "b-id": b,
		"SILI-9": target, "t-id": target,
...
func TestRunIssueReorderSingleItemColumnValidatesTarget(t *testing.T) {
	// The "todo" column holds only the issue being moved; SILI-3 lives in a
	...
	target := mkIssue("t-id", "SILI-9", "todo", 5)
	other := mkIssue("c-id", "SILI-3", "in_progress", 2)
	byRef := map[string]map[string]any{
		"SILI-9": target, "t-id": target,
		"SILI-3": other, "c-id": other,
	}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/issues" && r.Method == http.MethodGet {
...
func TestRunIssueUpdateOmitsPositionWhenUnset(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/api/issues/SILI-1":
			json.NewEncoder(w).Encode(map[string]any{"id": "issue-1", "identifier": "SILI-1", "status": "todo"})
		case r.Method == http.MethodPut && r.URL.Path == "/api/issues/issue-1":
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
				t.Errorf("decode body: %v", err)
			}
			json.NewEncoder(w).Encode(map[string]any{"id": "issue-1", "identifier": "SILI-1", "title": "Renamed"})
		default:
			http.NotFound(w, r)
		}
	}))
