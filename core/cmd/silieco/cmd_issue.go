...
// scratch paths like /tmp are not. SILI-4252 traced a cross-environment context
...
var issueChildrenCmd = &cobra.Command{
	...
	Short:   "List a task's sub-tasks grouped by stage",
	Args:    exactArgs(1),
...
var issueAssignCmd = &cobra.Command{
	Use:   "assign <id>",
	Short: "Assign a task to a member, agent, or squad",
	Args:  exactArgs(1),
	RunE:  runIssueAssign,
}
...
var issueStatusCmd = &cobra.Command{
	...
	Long: "Change a task's status. Valid statuses: " +
...
var issueReorderCmd = &cobra.Command{
	Use:   "reorder <id>",
	Short: "Move a task within its status column",
	Long: "Reposition a task inside its current status column by computing a new\n" +
		...
		"different column, change its status first with `silieco task status`.",
	Args: exactArgs(1),
...
var issueSubscriberCmd = &cobra.Command{
	Use:   "subscriber",
	Short: "Work with task subscribers",
}

var issueSubscriberListCmd = &cobra.Command{
	Use:   "list <task-id>",
	Short: "List subscribers of a task",
	Args:  exactArgs(1),
...
var issueSubscriberAddCmd = &cobra.Command{
	Use:   "add <task-id>",
	Short: "Subscribe a user or agent to a task (defaults to the caller)",
	Args:  exactArgs(1),
...
var issueSubscriberRemoveCmd = &cobra.Command{
	Use:   "remove <task-id>",
	Short: "Unsubscribe a user or agent from a task (defaults to the caller)",
	Args:  exactArgs(1),
...
var issueRunsCmd = &cobra.Command{
	Use:   "runs <task-id>",
	Short: "List execution history for a task",
	Args:  exactArgs(1),
	RunE:  runIssueRuns,
}
...
var issueUsageCmd = &cobra.Command{
	Use:   "usage <task-id>",
	Short: "Show aggregated token usage for a task",
	Args:  exactArgs(1),
	RunE:  runIssueUsage,
}
...
var issueRerunCmd = &cobra.Command{
	Use:   "rerun <id>",
	Short: "Re-enqueue a task's current agent assignment as a fresh run",
	Args:  exactArgs(1),
	RunE:  runIssueRerun,
}
...
var issueCancelTaskCmd = &cobra.Command{
	...
	Long: "Cancel a single run by its ID. Accepts the short ID prefix shown by `task runs`. " +
		"Use --task to scope short-ID resolution to a specific task when ambiguous. " +
...
func init() {
	...
	issueCreateCmd.Flags().Bool("allow-external-file", false, "Allow --description-file / --attachment to read a path outside the current working directory. Off by default so a stale file from another run/environment can't be picked up (SILI-4252).")
	...
	issueCreateCmd.Flags().String("workflow-instance", "", "SOP workflow run UUID (requires --workflow-stage)")
	issueCreateCmd.Flags().String("workflow-stage", "", "SOP Stage UUID (requires --workflow-instance)")
	...
	issueUpdateCmd.Flags().Bool("allow-external-file", false, "Allow --description-file to read a path outside the current working directory. Off by default so a stale temp file from another run/environment can't be picked up (SILI-4252).")
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-Silieco-Next-Before response header; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-Silieco-Next-Before-Id response header; must be paired with --before.")
	...
	issueCancelTaskCmd.Flags().String("task", "", "Task ID/key to scope short run ID prefix resolution")
	issueCancelTaskCmd.Flags().String("issue", "", "Legacy alias for --task")
	// task run-messages
	...
	issueRunMessagesCmd.Flags().String("task", "", "Task ID/key to scope short run ID prefix resolution")
	issueRunMessagesCmd.Flags().String("issue", "", "Legacy alias for --task")
	...
	issueCommentAddCmd.Flags().Bool("allow-external-file", false, "Allow --content-file / --attachment to read a path outside the current working directory. Off by default so a stale file from another run/environment can't be picked up (SILI-4252).")
...
// --description-file / --content-file (SILI-4252) to a local --attachment path.
...
// only accepts local paths). Each remaining path is run through the SILI-4252
...
func quickCreateAttachmentIDsFromEnv() ([]string, error) {
	raw := strings.TrimSpace(os.Getenv("SILIECO_QUICK_CREATE_ATTACHMENT_IDS"))
	if raw == "" {
		return nil, nil
	}
	var ids []string
	if err := json.Unmarshal([]byte(raw), &ids); err != nil {
		return nil, fmt.Errorf("parse SILIECO_QUICK_CREATE_ATTACHMENT_IDS: %w", err)
	}
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	if hasDesc {
		if err := guardLocalPathLinks(desc, "issue description",
			"Deliver the file itself with `silieco task create --attachment <path>` (repeatable) and drop the link."); err != nil {
			return err
		}
	...
	// Quick-create stamp: when the daemon sets SILIECO_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `silieco task create` call
	...
	if taskID := os.Getenv("SILIECO_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func runIssueUpdate(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("description") || cmd.Flags().Changed("description-stdin") || cmd.Flags().Changed("description-file") {
		...
		if err := guardLocalPathLinks(desc, "issue description",
			"`silieco task update` cannot carry files — deliver the file with `silieco task comment add <task-id> --attachment <path>` instead, and drop the link."); err != nil {
			return err
		}
		body["description"] = desc
	}
...
func reorderTargetNotInColumnError(ctx context.Context, client *cli.APIClient, otherRef, issueRef resolvedID, status string) error {
	if other, err := fetchIssue(ctx, client, otherRef.ID); err == nil {
		if otherStatus := strVal(other, "status"); otherStatus != "" && otherStatus != status {
			return fmt.Errorf("issue %s is in the %q column but %s is in %q; move one with `silieco task status` first, or pick a target in the same column", otherRef.Display, otherStatus, issueRef.Display, status)
		}
	}
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	// validation in handler.ListComments (core/internal/handler/comment.go).
	...
	if nb := respHeaders.Get("X-Silieco-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-Silieco-Next-Before-Id"); nbid != "" {
...
func runIssueCommentAdd(cmd *cobra.Command, args []string) error {
	...
	if err := guardLocalPathLinks(content, "comment body",
		"Deliver the file itself with `silieco task comment add <task-id> --attachment <path>` (repeatable) and drop the link."); err != nil {
		return err
	}
	...
	// the comment is never posted (which would duplicate on retry — SILI-4252).
	pending, err := collectLocalAttachments(cmd, attachments)
	if err != nil {
		return err
	}
...
func runIssueRuns(cmd *cobra.Command, args []string) error {
	...
	if err := client.GetJSON(ctx, "/api/tasks/"+issueRef.ID+"/runs", &runs); err != nil {
...
func runIssueRunMessages(cmd *cobra.Command, args []string) error {
	...
	issueID := ""
	taskInput, _ := cmd.Flags().GetString("task")
	if taskInput == "" {
		taskInput, _ = cmd.Flags().GetString("issue")
	}
	if taskInput != "" {
		issueRef, err := resolveIssueRef(ctx, client, taskInput)
		if err != nil {
	...
	path := "/api/runs/" + url.PathEscape(taskRef.ID) + "/messages"
...
// /api/runs/{runId}/cancel which updates the run row to status=cancelled
...
func runIssueCancelTask(cmd *cobra.Command, args []string) error {
	...
	var result map[string]any
	path := "/api/runs/" + url.PathEscape(taskRef.ID) + "/cancel"
...
// 403 instead of a clean CLI-side resolution error (SILI-2165 follow-up).
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set SILIECO_WORKSPACE_ID")
	}
	...
	// prompt emits "Unrecognized assignee: <SquadName>" (SILI-2165). Callers
...
// from `silieco workspace member list --output json`) should use this instead of
