...
var workspaceCreateCmd = &cobra.Command{
	...
	Long: "Creates a new workspace and adds you as its owner. Both --name and " +
		...
		"this profile — run 'silieco workspace switch <slug>' afterward if you " +
		"want subsequent commands to target the new workspace.",
	Example: "  silieco workspace create --name \"Support Team\" --slug support-team --issue-prefix SUP",
	Args:    cobra.NoArgs,
...
var workspaceMemberInviteCmd = &cobra.Command{
	...
	Long: "Sends a workspace invitation to an email address. The invitee gets a " +
		...
		"UUID, a slug, or a short UUID prefix (≥4 hex chars) as shown in " +
		"'workspace list'; if omitted the current default workspace is used " +
		"(--workspace-id / SILIECO_WORKSPACE_ID / profile default).\n\n" +
...
var workspaceSwitchCmd = &cobra.Command{
	...
	Long: "Sets the default workspace for the current profile after verifying you " +
		...
		"commands without --workspace-id or SILIECO_WORKSPACE_ID will target " +
		"this workspace.\n\n" +
		"Resolution priority (highest to lowest): --workspace-id flag, " +
		"SILIECO_WORKSPACE_ID env, profile default (set by this command).\n\n" +
		"For low-level use, 'silieco config set workspace_id <id>' writes the " +
...
func fetchWorkspaces(ctx context.Context, cmd *cobra.Command) ([]workspaceSummary, error) {
	...
	if token == "" {
		return nil, fmt.Errorf("not authenticated: run 'silieco login' first")
	}
...
func runWorkspaceList(cmd *cobra.Command, _ []string) error {
	...
	if currentID != "" {
		fmt.Fprintln(os.Stderr, "\n* = current default workspace (use 'silieco workspace switch <id|slug|prefix>' to change)")
	} else {
		fmt.Fprintln(os.Stderr, "\nNo default workspace set. Use 'silieco workspace switch <id|slug|prefix>' to pick one.")
	}
...
func resolveWorkspaceByIDOrSlug(workspaces []workspaceSummary, target string) (workspaceSummary, error) {
	...
	return workspaceSummary{}, fmt.Errorf("workspace %q not found or you do not have access; run 'silieco workspace list' to see options", target)
}
...
func runWorkspaceGet(cmd *cobra.Command, args []string) error {
	...
	if wsID == "" {
		return fmt.Errorf("workspace ID is required: pass an id/slug/prefix as argument or set SILIECO_WORKSPACE_ID")
	}
...
// caller actually set, mirroring core/internal/handler/workspace.go's
