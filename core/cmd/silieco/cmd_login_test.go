...
func TestRunLoginTokenAutoWatchesDiscoveredWorkspaces(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Authorization") != "Bearer sili_test_token" {
	...
	cmd := newLoginTestCmd()
	_ = cmd.Flags().Set("token", "sili_test_token")
	...
	if cfg.Token != "sili_test_token" || cfg.ServerURL != srv.URL || cfg.WorkspaceID != "ws-1" {
