...
// loginTokenPrefixes are the token prefixes `silieco login --token` accepts.
// The CLI used to hardcode `sili_` only, which made it impossible to log in
// with a Silieco Cloud Node PAT (`mcn_`) even though the server happily
// authenticates both kinds. Keep this list in sync with the prefix branches
// in core/internal/middleware/auth.go.
var loginTokenPrefixes = []string{"sili_", auth.CloudPATPrefix}
...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate silieco with Silieco",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("SILIECO_TOKEN")); v != "" {
		return v
	}
	...
	// inDaemonManagedExecutionContext already covers the SILIECO_DAEMON_PORT
	// signal for subprocesses that lost SILIECO_AGENT_ID / SILIECO_TASK_ID.
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"SILIECO_APP_URL", "FRONTEND_ORIGIN"} {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'silieco setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthLogin(cmd *cobra.Command, args []string) error {
	if cmd.Flags().Changed("token") {
		...
		// `--token sili_xxx` (space form) is what users actually type — that's
...
func runAuthLoginBrowser(cmd *cobra.Command) error {
	...
	if err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server could not issue an access token for the CLI. Run `silieco login` again.", err)
	}
	...
	if err := patClient.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `silieco login` again.", err)
	}
...
func runAuthLoginToken(cmd *cobra.Command, providedToken string) error {
	...
	if err := client.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Could not sign in with that token — make sure it is valid and not expired, then run `silieco login --token <token>` again.", err)
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'silieco login' to authenticate.")
		return nil
	}
...
<title>Silieco — Authenticated</title>
<style>
