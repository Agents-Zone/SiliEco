...
var loginCmd = &cobra.Command{
	...
	Long:  "Log in to Silieco, then automatically discover and watch all your workspaces.",
	// Up to one positional is accepted so `--token sili_...` / `--token mcn_...`
...
// "prompt me interactively", preserving the legacy `silieco login --token`
// no-value form alongside the documented `--token sili_...` / `--token mcn_...`
// value form.
//
...
func init() {
	...
	loginCmd.Flags().String("token", "", "Authenticate using a personal access token (sili_... user PAT or mcn_... Cloud Node PAT). Pass --token sili_... / --token mcn_... to supply it inline, or --token alone to be prompted interactively.")
	// NoOptDefVal lets `--token` (no value) keep its old prompt-mode behavior
	// while `--token sili_...` / `--token mcn_...` and the `=value` form
...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	if err := autoWatchWorkspaces(cmd); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Run 'silieco workspace list' and 'silieco workspace watch <id>' to set up manually.\n")
		return nil
	}

	fmt.Fprintf(os.Stderr, "\n→ Run 'silieco daemon start' to start your local agent runtime.\n")
	return nil
}
...
func autoWatchWorkspaces(cmd *cobra.Command) error {
	...
	if len(workspaces) > 1 {
		fmt.Fprintln(os.Stderr, "\nUse 'silieco workspace switch <id|slug>' to change the default workspace.")
	}
...
func waitForWorkspaceCreation(cmd *cobra.Command, client *cli.APIClient) ([]struct {
	...
	if appURL == "" {
		...
		fmt.Fprintln(os.Stderr, "\nNo workspaces found.")
		fmt.Fprintln(os.Stderr, "Create a workspace in the web dashboard, then run 'silieco login' again.")
		return nil, nil
	}
	...
	// but it still honors SILIECO_HTTP_TIMEOUT via AtLeastAPITimeout so a user
