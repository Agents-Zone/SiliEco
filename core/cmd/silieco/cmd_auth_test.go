...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers SILIECO_APP_URL", func(t *testing.T) {
		t.Setenv("SILIECO_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://silieco.ai/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `silieco login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}

// TestLoginTokenHelpOutputRendersCleanly renders loginCmd's flag help through
// the same pflag path `silieco login -h` uses (FlagUsagesWrapped) and locks the
...
// right downstream branch is taken: `--token sili_xxx` and `--token=sili_xxx`
...
func TestLoginTokenFlagParsing(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			argv: []string{"--token", "sili_xxx"},
			want: want{changed: true, resolvedToken: "sili_xxx"},
		},
		{
			name: "equals-separated value",
			argv: []string{"--token=sili_yyy"},
			want: want{changed: true, resolvedToken: "sili_yyy"},
		},
		{
...
// `silieco login --token`. The original implementation hardcoded `sili_`
// only, which rejected legitimate Silieco Cloud Node PATs (`mcn_`) at
...
func TestValidateLoginTokenPrefix(t *testing.T) {
	cases := []struct {
		...
		{name: "sili_ PAT", token: "sili_abc123", wantErr: false},
		...
		{name: "case-sensitive: SILI_ rejected", token: "SILI_abc123", wantErr: true},
		{name: "leading whitespace not allowed (callers TrimSpace first)", token: " sili_abc", wantErr: true},
	}
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
	...
	for _, p := range []string{"sili_", "mcn_"} {
