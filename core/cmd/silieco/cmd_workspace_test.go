...
func TestRunWorkspaceCreatePostsWorkspaceAndDoesNotSwitchDefault(t *testing.T) {
	...
	t.Setenv("SILIECO_WORKSPACE_ID", "existing-workspace")
...
func TestRunWorkspaceSwitch(t *testing.T) {
	...
	// Isolate HOME so the test never touches the developer's ~/.silieco.
	t.Setenv("HOME", t.TempDir())
	t.Setenv("SILIECO_SERVER_URL", srv.URL)
	t.Setenv("SILIECO_TOKEN", "test-token")
	...
	t.Run("isolates by profile", func(t *testing.T) {
		...
		wantSuffix := filepath.Join(".silieco", "profiles", "staging", "config.json")
