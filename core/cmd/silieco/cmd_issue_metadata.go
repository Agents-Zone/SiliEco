...
// silieco issue metadata {list|get|set|delete} — KV map attached to each issue
// for agent pipeline state. See core/internal/handler/issue_metadata.go for
// the constraints (key regex, 50-key cap, primitive-only values, 8KB blob).

var issueMetadataCmd = &cobra.Command{
	Use:   "metadata",
	Short: "Manage per-task metadata (KV)",
}

var issueMetadataListCmd = &cobra.Command{
	Use:   "list <issue-id>",
	Short: "List all metadata keys on a task",
	Args:  exactArgs(1),
