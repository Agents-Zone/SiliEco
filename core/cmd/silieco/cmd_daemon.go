...
import (
	...
	"github.com/silieco-ai/silieco/core/internal/daemon"
	logger_pkg "github.com/silieco-ai/silieco/core/internal/logger"
...
var daemonDiskUsageCmd = &cobra.Command{
	...
	Long: "Walks the daemon's workspaces root and reports per-task or per-workspace disk usage.\n" +
		...
		"root — the default root plus each ~/.silieco/profiles/* root, including the Desktop app's dedicated\n" +
		...
		"overridable via SILIECO_GC_ARTIFACT_PATTERNS) so the report stays in sync with what the GC reclaims.\n" +
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: SILIECO_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: SILIECO_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: SILIECO_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: SILIECO_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: SILIECO_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: SILIECO_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: SILIECO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: SILIECO_CODEX_HANDSHAKE_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: SILIECO_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: SILIECO_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: SILIECO_DAEMON_AUTO_UPDATE_INTERVAL)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: SILIECO_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: SILIECO_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: SILIECO_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: SILIECO_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: SILIECO_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: SILIECO_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: SILIECO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: SILIECO_CODEX_HANDSHAKE_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: SILIECO_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: SILIECO_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: SILIECO_DAEMON_AUTO_UPDATE_INTERVAL)")
	...
	df.Bool("all-profiles", false, "Scan every workspace root (default root + all ~/.silieco/profiles/* roots, incl. the Desktop app's) and report a combined total")
...
// Empty profile → ~/.silieco/, named profile → ~/.silieco/profiles/<name>/.
...
func newDaemonLogRotator(logPath string) *lumberjack.Logger {
	return &lumberjack.Logger{
		Filename:   logPath,
		MaxSize:    envPositiveIntOrDefault("SILIECO_DAEMON_LOG_MAX_SIZE_MB", defaultDaemonLogMaxSizeMB),
		MaxBackups: envPositiveIntOrDefault("SILIECO_DAEMON_LOG_MAX_BACKUPS", defaultDaemonLogMaxBackups),
		MaxAge:     envPositiveIntOrDefault("SILIECO_DAEMON_LOG_MAX_AGE_DAYS", defaultDaemonLogMaxAgeDays),
		Compress:   true,
	}
}
...
// requireDaemonAuth fails fast when the user never ran `silieco login`. The
...
// authenticates via the stored config token, never SILIECO_TOKEN.
func requireDaemonAuth(profile string) error {
	...
	if cfg.Token == "" {
		...
		if profile != "" {
			loginHint = fmt.Sprintf("silieco login --profile %s", profile)
		}
...
// SILIECO_SERVER_URL env, then the stored CLI config for the profile.
func resolveDaemonServerURL(cmd *cobra.Command, profile string) string {
	serverURL := cli.FlagOrEnv(cmd, "server-url", "SILIECO_SERVER_URL", "")
	if serverURL == "" {
...
func daemonStartupFailureError(logs daemonStartupLogs, waitErr error, profile, serverURL string) error {
	...
	case strings.Contains(joined, "connection refused") ||
		...
		target := "the Silieco server"
		if serverURL != "" {
			target += " at " + serverURL
		}
		return fmt.Errorf("daemon failed to start: cannot reach %s.\nMake sure the server is running and reachable, then rerun 'silieco daemon start'.\nFull log: %s", target, logs.logPath)
	}
...
func buildDaemonStartArgs(cmd *cobra.Command) []string {
	...
	// (= no cap), so it can override an environment SILIECO_AGENT_TIMEOUT.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	//
	//   --flag  >  SILIECO_… env  >  config.json  >  built-in default
	//
	...
	pollOverride, err := resolveDaemonDurationOverride(pollFlag, "SILIECO_DAEMON_POLL_INTERVAL", fileCfg.PollInterval)
	if err != nil {
		return err
	}
	if pollOverride > 0 {
	...
	heartbeatOverride, err := resolveDaemonDurationOverride(heartbeatFlag, "SILIECO_DAEMON_HEARTBEAT_INTERVAL", fileCfg.HeartbeatInterval)
	if err != nil {
		return err
	}
	...
	agentTimeoutOverride, err := resolveDaemonAgentTimeoutOverride(cmd, "SILIECO_AGENT_TIMEOUT", fileCfg.AgentTimeout)
	if err != nil {
		return err
	}
	...
	semanticOverride, err := resolveDaemonDurationOverride(semanticFlag, "SILIECO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", fileCfg.CodexSemanticInactivityTimeout)
	if err != nil {
		return err
	}
	...
	handshakeOverride, err := resolveDaemonDurationOverride(handshakeFlag, "SILIECO_CODEX_HANDSHAKE_TIMEOUT", fileCfg.CodexHandshakeTimeout)
	if err != nil {
		return err
	}
	...
	if n := resolveDaemonIntOverride(maxFlag, "SILIECO_DAEMON_MAX_CONCURRENT_TASKS", fileCfg.MaxConcurrentTasks); n > 0 {
		overrides.MaxConcurrentTasks = n
	}
	// --no-auto-update / SILIECO_DAEMON_AUTO_UPDATE=false / config.json
	...
	if resolveDaemonDisableAutoUpdate(noAutoUpdateFlag, "SILIECO_DAEMON_AUTO_UPDATE", fileCfg.DisableAutoUpdate) {
		overrides.DisableAutoUpdate = true
	}
	autoUpdateFlag, _ := cmd.Flags().GetDuration("auto-update-interval")
	autoUpdateOverride, err := resolveDaemonDurationOverride(autoUpdateFlag, "SILIECO_DAEMON_AUTO_UPDATE_INTERVAL", fileCfg.AutoUpdateCheckInterval)
	if err != nil {
		return err
	}
	...
	cfg.LaunchedBy = os.Getenv("SILIECO_LAUNCHED_BY")
...
// `silieco auth status`) and refuse to touch the running daemon on failure.
func requireDaemonRestartPreflight(cmd *cobra.Command, profile string) error {
	...
	if err := cli.NewAPIClient(baseURL, "", cfg.Token).GetJSON(ctx, "/api/me", nil); err != nil {
		...
		return fmt.Errorf("refusing to restart: cannot reach the Silieco server at %s (%w); the running daemon was left untouched.\nMake sure the server is running and reachable, then rerun 'silieco daemon restart'", baseURL, err)
	}
	return nil
}
...
//  2. SILIECO_AGENT_TIMEOUT is set -> return nil so LoadConfig reads the
...
//  2. SILIECO_DAEMON_AUTO_UPDATE explicitly set to a falsy value ->
...
// each ~/.silieco/profiles/* root) and renders a per-root breakdown with a
...
// orientation even when empty), then each ~/.silieco/profiles/* root that
// exists on disk, sorted by profile name. Roots that resolve to the same path
// (e.g. when SILIECO_WORKSPACES_ROOT pins every profile to one directory) are
...
func printDiskUsageOtherRootsHint(w io.Writer, report daemon.DiskUsageReport, profile, rootOverride string) {
	...
	fmt.Fprintln(w, "Run 'silieco daemon disk-usage --all-profiles' for a combined total across all roots.")
}
...
func diskUsageProfileSuggestions(currentProfile, currentRoot string) []diskUsageProfileSuggestion {
		...
		if root, err := daemon.ResolveWorkspacesRoot("", ""); err == nil && !samePath(root, currentRoot) {
			if taskCount := countDiskUsageTaskDirs(root); taskCount > 0 {
				out = append(out, diskUsageProfileSuggestion{
					Profile:   "",
					Command:   "silieco daemon disk-usage",
					Root:      root,
					TaskCount: taskCount,
				})
			}
	...
	for _, entry := range entries {
		...
		out = append(out, diskUsageProfileSuggestion{
			Profile:   profile,
			Command:   "silieco --profile " + shellQuoteArg(profile) + " daemon disk-usage",
			Root:      root,
			TaskCount: taskCount,
		})
	}
...
func profilesRootDir() (string, error) {
	...
	return filepath.Join(home, ".silieco", "profiles"), nil
}
