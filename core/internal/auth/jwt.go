...
const defaultJWTSecret = "silieco-dev-secret-change-in-production"
...
// GeneratePATToken creates a new personal access token: "sili_" + 40 random hex chars.
func GeneratePATToken() (string, error) {
	b := make([]byte, 20) // 20 bytes = 40 hex chars
	if _, err := rand.Read(b); err != nil {
		return "", fmt.Errorf("generate PAT token: %w", err)
	}
	return "sili_" + hex.EncodeToString(b), nil
}
