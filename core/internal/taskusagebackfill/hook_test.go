...
//   - run taskusagebackfill.Hook (the migrator hook from SILI-2957);
...
func TestHook_DirectV034Upgrade(t *testing.T) {
	...
	if adminURL == "" {
		adminURL = "postgres://silieco:silieco@localhost:5432/silieco?sslmode=disable"
	}
	...
	tmpDB := fmt.Sprintf("silieco_v034_upgrade_%d", time.Now().UnixNano())
	...
	// Run the SILI-2957 migrator hook. The hook should:
...
func TestHook_FreshDatabaseStampsWatermarkOnly(t *testing.T) {
	...
	tmpDB := fmt.Sprintf("silieco_v034_fresh_%d", time.Now().UnixNano())
	if err := createDatabase(ctx, adminURL, tmpDB); err != nil {
...
// resolveMigrationsDir locates core/migrations relative to this test
...
// which collides with the Go package core/internal/migrations and
...
func resolveMigrationsDir() (string, error) {
	...
	// here = .../core/internal/taskusagebackfill/hook_test.go
