...
type Source struct {
	...
	// one Silieco chat_session via the channel_chat_session_binding.
	ChatID string
...
type MediaRef struct {
	...
	Type MsgType
	// StorageKey locates the persisted object in Silieco object storage.
	StorageKey string
...
// fields. Per the boundary rule (SILI-3515 §2) the struct holds only
...
// webhooks are deliberately NOT modeled here (SILI-3515 decision §6) — an
