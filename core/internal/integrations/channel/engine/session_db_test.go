...
func sessionPersistenceTestDB(t *testing.T) *pgxpool.Pool {
	...
	if dsn == "" {
		dsn = "postgres://silieco:silieco@localhost:5432/silieco?sslmode=disable"
	}
...
func seedSessionPersistenceFixture(t *testing.T, pool *pgxpool.Pool) sessionPersistenceFixture {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id`,
		"Channel media test", fmt.Sprintf("channel-media-%d@silieco.test", suffix)).Scan(&f.userID); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ($1, $2, 'local', 'silieco_daemon', $3)
