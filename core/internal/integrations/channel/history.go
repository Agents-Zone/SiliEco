...
// `silieco chat history` (the channel OVERVIEW: top-level messages + thread
// metadata, not thread contents) and `silieco chat thread [id]` (one thread's
...
// reader"; the agent-facing contract never changes (SILI-3871).
...
// platform so the agent reads a uniform list, like `silieco issue comment list
// --output json`.
type HistoryMessage struct {
	...
	// so the agent can `silieco chat thread <thread_id>` to read its contents.
	// They are absent on a plain message and on thread-read rows.

	// ThreadID is the identifier to pass to `silieco chat thread <id>` to read
