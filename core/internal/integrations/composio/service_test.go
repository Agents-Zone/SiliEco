...
func newTestService(t *testing.T, client SDK, store Store) *Service {
	...
	svc, err := NewService(client, store, Config{
		...
		CallbackBaseURL: "https://silieco.ai",
		FrontendBaseURL: "https://silieco.ai",
...
func TestBeginConnect_MappingAndState(t *testing.T) {
	...
	// composio_user_id == silieco user id
	...
	if !strings.HasPrefix(cb, "https://silieco.ai"+callbackPath+"?state=") {
...
// config are returned (SILI-4009); the rest are dropped from the catalog, and
...
// (SILI-4009). With filtering in place, masking the error would render as a
...
func TestCallbackRedirect(t *testing.T) {
	t.Parallel()
	svc := newTestService(t, &fakeSDK{}, newFakeStore())
	if got := svc.CallbackRedirect("notion", true); got != "https://silieco.ai/settings?tab=integrations&connected=notion" {
		t.Errorf("success redirect = %q", got)
	}
	if got := svc.CallbackRedirect("notion", false); got != "https://silieco.ai/settings?tab=integrations&error=composio_connect_failed" {
