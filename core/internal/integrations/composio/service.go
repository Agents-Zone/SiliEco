// Package composio is the Stage 2 business-integration glue between Silieco and
// the standalone Composio SDK (core/pkg/composio). It owns Silieco semantics:
...
//
// MVP scope (SILI-3720): toolkits are discovered dynamically. The
...
import (
	...
	sdk "github.com/silieco-ai/silieco/core/pkg/composio"
...
type Config struct {
	...
	// trailing slash (e.g. "https://silieco.ai"). The Composio callback
	...
	// browser redirect (e.g. "https://silieco.ai"). May be empty, in which
...
// BeginConnect would succeed. Since SILI-4009 ListToolkits only returns
...
//
// The composio_user_id sent to Composio is the Silieco user id verbatim — the
...
func (s *Service) CompleteCallback(ctx context.Context, state, status, connectedAccountID string) (string, error) {
	...
	// composio_user_id == silieco user id invariant) and was created under the
	...
	if _, err := s.store.UpsertUserComposioConnection(ctx, db.UpsertUserComposioConnectionParams{
		...
		// Invariant: composio_user_id == Silieco user id.
...
// config are filtered out entirely (SILI-4009): a card the user can't act on is
