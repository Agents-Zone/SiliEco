...
func TestRegistrationClient_Begin_HappyPath(t *testing.T) {
	...
	res, err := c.Begin(context.Background(), "Ada - Silieco", "")
	if err != nil {
	...
	if !strings.HasPrefix(q.Get("source"), "go-sdk/silieco") {
		t.Errorf("qr source=%q want go-sdk/silieco", q.Get("source"))
	}
	// The name preset pre-fills the Lark PersonalAgent creation form so
	// the bot defaults to "<agent> - Silieco" rather than the
	// auto-generated "{用户姓名}的智能助手".
	if q.Get("name") != "Ada - Silieco" {
		t.Errorf("qr name=%q want %q", q.Get("name"), "Ada - Silieco")
	}
}
