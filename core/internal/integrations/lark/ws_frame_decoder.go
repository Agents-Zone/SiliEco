...
func (d *LarkJSONFrameDecoder) Decode(payload []byte, inst Installation) (InboundMessage, bool, error) {
	...
	// the binary as a Silieco attachment.
...
// SILI-2671 routing bug). Only when union_id is missing do we fall
...
// see SILI-2671 group-@-mention triage. In a Lark group with several
// Silieco bots, each bot's WS receives the event, and Lark fills
