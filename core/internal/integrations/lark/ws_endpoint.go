...
// SILIECO_LARK_CALLBACK_BASE_URL still overrides every installation when
// set (staging / mock).
...
//
// PersonalAgent compatibility — OPEN RISK (SILI-2671 review thread):
...
// `silieco` -> register a PersonalAgent -> enable WS -> watch logs.
...
func (c HTTPConnectionTokenConfig) withDefaults() HTTPConnectionTokenConfig {
	...
	// (SILIECO_LARK_CALLBACK_BASE_URL, or an httptest URL in tests)
...
func (f *HTTPConnectionTokenFetcher) Endpoint(ctx context.Context, creds InstallationCredentials) (WSEndpoint, error) {
	...
	// the audience Silieco server logs are read by today; if i18n
