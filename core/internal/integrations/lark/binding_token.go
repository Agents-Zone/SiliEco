...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	SiliecoUserID  pgtype.UUID // the installer's Silieco account
...
// ChannelStore so lark_* calls resolve to channel_* rows (SILI-3515).
...
// identity is the supplied siliecoUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Silieco user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, siliecoUserID pgtype.UUID) (RedeemedBindingToken, error) {
	if s.tx == nil {
	...
	// used to reject a non-member redeemer is gone (SILI-3515 §4), so we
	...
	isMember, err := qtx.IsWorkspaceMember(ctx, row.WorkspaceID, siliecoUserID)
	if err != nil {
	...
	_, err = qtx.CreateLarkUserBinding(ctx, CreateUserBindingParams{
		...
		SiliecoUserID:  siliecoUserID,
		InstallationID: row.InstallationID,
		ChannelUserID:  row.ChannelUserID,
	})
	if err != nil {
		// pgx.ErrNoRows here means the conflict row exists but its
		// silieco_user_id differs from ours, so the WHERE clause on
...
// (installation_id, lark_open_id) when silieco_user_id matches (the
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *ChannelStore, p InstallerBindParams) error {
	...
	// (SILI-3515 §4): the installer must be a member of the workspace
	// they are binding into.
	isMember, err := q.IsWorkspaceMember(ctx, p.WorkspaceID, p.SiliecoUserID)
	if err != nil {
...
// open_id) pair and points at a different Silieco user. Account
...
// (SILI-3515 §4 removed the member FK that used to enforce this).
