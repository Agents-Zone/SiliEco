...
// workspace/agent (SILI-3515 §4), so nothing else would.
const (
...
`, ws, agent, app, ccInstaller).Scan(&id); err != nil {
	...
	exec(`INSERT INTO channel_user_binding (workspace_id, silieco_user_id, installation_id, channel_type, channel_user_id)
...
func TestDeleteChannelInstallationsByArchivedRuntimeAgents(t *testing.T) {
	...
	clean := func() {
		...
		_, _ = pool.Exec(ctx, `DELETE FROM channel_user_binding WHERE silieco_user_id = $1`, ccUser)
...
VALUES ($1, $2, 'cc runtime', 'local', 'silieco_daemon')`, ccRuntime, ccWS)
...
func TestDeleteWorkspace_SweepsChannelInstallations(t *testing.T) {
	...
	exec(`INSERT INTO agent_runtime (id, workspace_id, name, runtime_mode, provider)
VALUES ($1, $2, 'cc runtime del', 'local', 'silieco_daemon')`, ccRuntimeDel, ccWSDel)
	exec(`INSERT INTO agent (id, workspace_id, name, runtime_mode, runtime_id)
