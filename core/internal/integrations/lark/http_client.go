...
const (
	...
	// Operators do NOT set SILIECO_LARK_HTTP_BASE_URL to pick a cloud
	...
	// latency from a self-hosted Silieco deployment to feishu.cn.
...
func (c HTTPClientConfig) withDefaults() HTTPClientConfig {
	...
	// non-empty BaseURL (SILIECO_LARK_HTTP_BASE_URL, or an httptest URL
...
// keyed by app_id so a single Silieco server reuses Lark's
...
// cfg.BaseURL (SILIECO_LARK_HTTP_BASE_URL, or an httptest URL in tests)
...
// inverse across WS perspectives — see SILI-2671 triage), so we
...
func bindingPromptTemplate(bindURL string) (string, error) {
	doc := map[string]any{
		...
		"header": map[string]any{
			"template": "blue",
			"title":    map[string]any{"tag": "plain_text", "content": "Silieco"},
		},
		"elements": []any{
			map[string]any{
				"tag": "div",
				"text": map[string]any{
					"tag":     "lark_md",
					"content": "你还没有绑定 Silieco 账户。点击下方按钮完成绑定后即可使用此 Agent。",
				},
			},
			map[string]any{
				"tag": "action",
