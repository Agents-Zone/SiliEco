...
//     and an expiry. Silieco renders the QR, the user scans it in the
...
const (
	...
	// Feishu account. The split-CTA UX (SILI-3083) rendered a QR against
...
type RegistrationConfig struct {
	...
	// telemetry can attribute installs back to Silieco. Empty defaults
	// to "silieco".
	Source string
...
type BeginResult struct {
	DeviceCode string
	// QRCodeURL is the verification_uri_complete with Silieco's `source`
...
type PollResult struct {
	...
	// a Feishu-first begin); after SILI-3083 follow-up it is symmetric,
...
// "<agent> - Silieco" instead of Lark's auto-generated
...
func (c *RegistrationClient) Poll(ctx context.Context, domain, deviceCode string) (*PollResult, error) {
	...
	// because the split-CTA UI (SILI-3083) also begins on
...
// cannot attribute installs back to Silieco in their analytics.
//
