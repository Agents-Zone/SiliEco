...
func TestHTTPClient_SendBindingPromptCard_HappyPath(t *testing.T) {
	...
	if err := c.SendBindingPromptCard(context.Background(), BindingPromptParams{
		...
		BindURL:        "https://silieco.test/lark/bind?token=abc",
	}); err != nil {
	...
	if !strings.Contains(capturedBody["content"], "silieco.test/lark/bind") {
...
// route inbound @-mentions in multi-bot group chats (SILI-2671). The
...
func TestBindingPromptTemplate_Shape(t *testing.T) {
	raw, err := bindingPromptTemplate("https://silieco.test/bind?token=abc")
	if err != nil {
	...
	if btn["url"] != "https://silieco.test/bind?token=abc" {
