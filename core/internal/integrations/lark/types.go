...
// can't accidentally pass a Silieco user UUID where a Lark open_id is
// expected.
type OpenID string

// ChatID identifies a Lark conversation (p2p or group). One ChatID maps
// to one Silieco chat_session via lark_chat_session_binding.
type ChatID string
...
// separate clouds with distinct hosts; a single Silieco deployment serves
...
// SILI-2671 §4.7 (drop-audit policy).
...
const (
	...
	// DropReasonNonWorkspaceMember — the sender resolved to a Silieco
