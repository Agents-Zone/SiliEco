...
func TestUserBindingConfigRoundTrip(t *testing.T) {
	...
	got, err := userBindingFromRow(db.ChannelUserBinding{
		ID:             in.ID,
		WorkspaceID:    in.WorkspaceID,
		SiliecoUserID:  in.SiliecoUserID,
	...
	if got.UnionID != in.UnionID || got.ChannelUserID != in.ChannelUserID || got.SiliecoUserID != in.SiliecoUserID {
