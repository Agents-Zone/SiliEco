...
VALUES ($1, $2, 'rebind runtime', 'local', 'silieco_daemon') ON CONFLICT (id) DO NOTHING`, rbRuntime, rbWS)
...
func TestChannelStore_ReclaimDeadRevokedFences(t *testing.T) {
	...
	clean := func() {
		...
		_, _ = pool.Exec(ctx, `DELETE FROM channel_user_binding WHERE silieco_user_id = $1`, rbUser)
...
`, rbWS, rbAgentA, app, rbInstaller).Scan(&oldID); err != nil {
	...
	seed(`INSERT INTO channel_user_binding (workspace_id, silieco_user_id, installation_id, channel_type, channel_user_id)
