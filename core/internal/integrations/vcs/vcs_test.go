...
func TestForgejoEventKindAndParse(t *testing.T) {
	...
	pr, err := p.ParsePullRequest([]byte(`{
		"action":"closed",
		"pull_request":{"number":7,"title":"Fix SILI-1","state":"closed","merged":true,
...
func TestGitlabParse(t *testing.T) {
	...
	pr, err := p.ParsePullRequest([]byte(`{
		...
		"object_attributes":{"iid":42,"title":"Add SILI-9","state":"merged","action":"merge",
...
func TestGitlabParseTimestamps(t *testing.T) {
	...
	pr, err := p.ParsePullRequest([]byte(`{
		"object_kind":"merge_request",
		"project":{"path_with_namespace":"g/r"},
		"object_attributes":{"iid":1,"title":"SILI-1","state":"opened","action":"open",
