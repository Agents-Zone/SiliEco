...
// account-link reuse path (SILI-3911) is exercised without a database.
...
// existing account link across installations of the same Slack team + Silieco
...
func TestResolveSenderReuse(t *testing.T) {
	...
	t.Run("direct binding resolves without a reuse lookup or write", func(t *testing.T) {
		f := &fakeIdentityQueries{binding: db.ChannelUserBinding{SiliecoUserID: userID}}
	...
	t.Run("unlinked sender reuses a same-team link and materializes it", func(t *testing.T) {
		f := &fakeIdentityQueries{
			...
			reusable: db.ChannelUserBinding{SiliecoUserID: userID, InstallationID: instA},
		}
		got, err := (&identityResolver{q: f}).ResolveSender(context.Background(), inst("T1"), msg)
		if err != nil {
			t.Fatalf("ResolveSender err = %v", err)
		...
		if f.createWith.InstallationID != instB || f.createWith.SiliecoUserID != userID || f.createWith.ChannelUserID != senderID {
	...
	t.Run("directly-bound non-member surfaces not-member", func(t *testing.T) {
		f := &fakeIdentityQueries{binding: db.ChannelUserBinding{SiliecoUserID: userID}, memberErr: pgx.ErrNoRows}
		_, err := (&identityResolver{q: f}).ResolveSender(context.Background(), inst("T1"), msg)
