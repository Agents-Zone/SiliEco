...
// Slack user id is bound to their Silieco account. It mirrors
...
var (
	...
	// different Silieco user (account transfer must go through explicit unbind).
...
// siliecoUserID (taken from the session, never from the token). Returns
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, siliecoUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	if _, err := qtx.CreateChannelUserBinding(ctx, db.CreateChannelUserBindingParams{
		...
		// the ON CONFLICT DO UPDATE WHERE silieco_user_id=… gating rejected it.
		if errors.Is(err, pgx.ErrNoRows) {
