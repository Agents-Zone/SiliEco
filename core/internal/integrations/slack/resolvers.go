...
// (SILI-3874). (SILI-3666 wired the replier; stage 3 had both nil.)
...
func (r *identityResolver) ResolveSender(ctx context.Context, inst engine.ResolvedInstallation, msg channel.InboundMessage) (engine.ResolvedIdentity, error) {
	...
	if err != nil {
		...
		// workspace (SILI-3911): one link per Slack workspace, not per app.
	...
	if reused {
		...
		if _, err := r.q.CreateChannelUserBinding(ctx, db.CreateChannelUserBindingParams{
			WorkspaceID:    inst.WorkspaceID,
			SiliecoUserID:  binding.SiliecoUserID,
			InstallationID: inst.ID,
			ChannelType:    string(TypeSlack),
			ChannelUserID:  senderID,
			Config:         []byte(`{}`),
	...
	return engine.ResolvedIdentity{UserID: binding.SiliecoUserID}, nil
}
...
// Slack workspace need not re-prompt (SILI-3911). ok=false (nil error) means "no
