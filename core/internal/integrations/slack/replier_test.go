...
func TestReply_NeedsBinding_MintsAndPostsPrompt(t *testing.T) {
	...
	wantLink := "<https://silieco.example/slack/bind?token=tok_RAW-123|link your account>"
...
func TestReply_IngestedWithIssue_Confirms(t *testing.T) {
	...
	if !strings.Contains(sender.sent.Text, "SILI-42") || !strings.Contains(sender.sent.Text, "Fix the thing") {
...
func TestIssueCreatedText(t *testing.T) {
	if got := issueCreatedText(engine.Result{IssueIdentifier: "SILI-7", IssueTitle: "Title"}); got != "✅ Created SILI-7 — Title" {
