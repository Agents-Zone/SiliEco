...
// `/issue` cannot work here at all (SILI-3908). Registering `/issue` as a real
...
// The agent turns the prompt into a well-formed `silieco issue create` in the
...
// completion surfaces to the invoker as a Silieco inbox notification through the
...
const (
	taskSlashCommand        = "/task"
	legacyIssueSlashCommand = "/issue"
)

// User-facing ephemeral replies. Kept terse; only the invoker sees them.
const (
	slashUsageText           = "Tell me what to create, e.g. `/task the login button does nothing on Safari`."
	slashQueuedText          = "✅ On it — I'm turning that into a task. You'll get a Silieco notification when it's ready."
	slashNotMemberText       = "You're not a member of this Silieco workspace, so I can't create a task for you."
	slashLinkAccountFallback = "Link your Slack account to Silieco first, then try `/task` again."
	slashInternalErrorText   = "⚠️ Something went wrong creating the task. Please try again."
	slashDisabledText        = "This Slack app isn't connected to Silieco (or was disconnected). Ask a workspace admin to reconnect it."
)
...
type quickCreateEnqueuer interface {
	EnqueueQuickCreateTask(ctx context.Context, workspaceID, requesterID, agentID, squadID pgtype.UUID, prompt, priority, dueDate string, projectID, parentIssueID, workflowInstanceID, workflowStageID pgtype.UUID, attachmentIDs []pgtype.UUID) (db.AgentTaskQueue, error)
}
...
func (p *SlashCommandProcessor) Handle(ctx context.Context, cmd slack.SlashCommand) {
	...
	if !strings.EqualFold(command, taskSlashCommand) &&
		!strings.EqualFold(command, legacyIssueSlashCommand) {
		return
	}
...
func (p *SlashCommandProcessor) process(ctx context.Context, cmd slack.SlashCommand) string {
	...
	if _, err := p.tasks.EnqueueQuickCreateTask(
		...
		pgtype.UUID{}, // no workflow run
		pgtype.UUID{}, // no workflow stage
		nil,           // no attachments
	); err != nil {
...
// resolveUser maps the Slack user id to the bound Silieco user, re-checking
...
func (p *SlashCommandProcessor) resolveUser(ctx context.Context, inst engine.ResolvedInstallation, slackUserID string) (pgtype.UUID, error) {
	...
	return binding.SiliecoUserID, nil
}
...
func (p *SlashCommandProcessor) bindingText(ctx context.Context, inst engine.ResolvedInstallation, slackUserID string) string {
	...
	return "👋 To file issues, link your Slack account to Silieco: <" +
