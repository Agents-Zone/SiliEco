...
// failed) read so the unified `silieco chat history` / `silieco chat thread`
...
// `silieco chat history` (channel overview) and `silieco chat thread [id]`
// (one thread) commands (SILI-3871). Both are scoped to the session's OWN
...
// boundary for `silieco chat thread <id>` (the agent supplies only a
...
// drills into with `silieco chat thread <id>`. Backs `silieco chat history`.
...
// threads) reads its linear conversation. Backs `silieco chat thread [id]`.
...
// agent can drill in with `silieco chat thread <id>`.
...
// dropped (SILI-3931 / #4803). Order: top-level text, then each attachment's
