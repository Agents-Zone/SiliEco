...
// Client handles HTTP communication with the Silieco server daemon API.
type Client struct {
	baseURL string
	token   string
	client  *http.Client
...
// machine-level batch claim (SILI-4257). Unlike the per-runtime claim — which
...
// SILI-1744). Bounding the batch to a few seconds caps that worst-case
...
// ClaimTasks is the machine-level (SILI-4257) batch counterpart of ClaimTask:
...
// daemon must fall back to the legacy per-runtime claim (SILI-4257). The batch
...
// claimTasksLegacy is the pre-batch compatibility fallback (SILI-4257): claim per
...
// (SILI-3284). protocol_family is the provider used for task routing (it
...
// (SILI-2780) without leaving the task stuck if the outage outlives the
// window.
