...
import (
	...
	"github.com/silieco-ai/silieco/core/internal/cli"
	"github.com/silieco-ai/silieco/core/internal/daemon/execenv"
	"github.com/silieco-ai/silieco/core/internal/daemon/repocache"
	"github.com/silieco-ai/silieco/core/internal/selfexec"
	"github.com/silieco-ai/silieco/core/pkg/agent"
	"github.com/silieco-ai/silieco/core/pkg/skillbundle"
...
// on the server's retry allowlist. (SILI-5370)
...
const (
	...
	repoCheckoutModeEnv      = "SILIECO_REPO_CHECKOUT_MODE"
	...
	// server-pushed daemon:pending_work hint triggers (SILI-5444). Short on
...
type terminalTaskReport struct {
	...
	// session because its rollout was not in the store (SILI-5305). The server
...
var (
	...
	// path override (SILI-3284) — a stale or mistyped override must fall back
...
type workspaceState struct {
	...
	// profile list (SILI-3332) as last seen from the server. An on-demand
...
type Daemon struct {
	...
	// (SILI-3284). Populated in registerRuntimesForWorkspace when a profile's
	...
	// runtime (SILI-5439) would otherwise be a data race.
	...
	// (SILI-5439). Guarded by skippedAgentsMu.
	...
	// login-shell probe + version detection instead of one per task (SILI-4486).
	...
	// wsRPC carries generic request/response RPCs (e.g. tasks.claim, SILI-4257)
	...
	// bounces the connection re-probes the batch route (SILI-4257).
	...
	// coalesce and rate-limit server-pushed "heartbeat now" hints (SILI-5444).
	...
	activeCodexStores     map[string]int  // per-issue Codex session store path -> live-task refcount; guards the store from GC mid-task (SILI-4424)
...
// has vanished from disk (SILI-4486).
//
...
//     version — a mismatched {old path, new version} (SILI-4486 review).
...
//     (preserving the ~/.silieco/hooks exclusion and the login-shell fallback).
...
func (d *Daemon) healAgentPath(ctx context.Context, provider, command string) healedAgent {
	...
	// registration path applies (SILI-4486 review).
...
// (SILI-5439): that payload carries built-in runtimes only, so under the
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// fault can strip every SILIECO_* env var from an agent subprocess; the
	...
	// Discover agent CLIs installed after startup (SILI-5439). Separate from the
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		loginHint := "'silieco login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'silieco login --profile %s'", d.cfg.Profile)
		}
...
//
// A round now serves a whole batch of workspace registrations (SILI-5225), so a
...
// found" state before probes were parallelized (SILI-5119). Only fast failures,
...
// previously only visible in the daemon log (SILI-5439).
func (d *Daemon) probeBuiltinRuntime(ctx context.Context, name string, entry AgentEntry) (string, string, bool) {
	...
	for attempts < runtimeVersionProbeAttempts {
		...
		// (SILI-4486) so version detection — and thus staying registered/online —
...
// "no runtime found" state while the probes were still running (SILI-5119).
...
//
// Each probe still self-heals a vanished pinned path (SILI-4486) and re-detects
...
// registerRuntimesForWorkspaceBatch for each workspace (SILI-5225).
...
// (24 workspaces × 5 agents = 120 instead of 5, SILI-5225 / #5837). Beyond the
...
func (d *Daemon) registerRuntimesForWorkspaceBatch(ctx context.Context, workspaceID string, builtins []map[string]string) (*RegisterResponse, string, error) {
	...
	// this host (SILI-3284). This is best-effort: a fetch error (e.g. an older
	...
	// the effective profile set is already current (SILI-3332). An empty string
...
//
// This exists for the CLI-discovery path (SILI-5439), which must not participate
...
// profiles (SILI-3284) and appends a runtime registration entry for each one
...
//
// Returns a content signature of the fetched profile list (SILI-3332). The
...
func (d *Daemon) appendProfileRuntimes(ctx context.Context, workspaceID string, runtimes *[]map[string]string, failedProfiles *[]map[string]string) string {
	...
	for _, profile := range resp.RuntimeProfiles {
		...
		// path override (SILI-3284, `silieco runtime profile set-path`) wins
...
// runtime profile list (SILI-3332). An on-demand refresh diffs this against the
...
// in commits, matching the contract documented in RFC SILI-2414 §4.8.
...
// `silieco repo checkout` accepts project-only URLs without an extra round
...
// runtime profile list (SILI-3332), compares its content signature against
...
// other API call is what surfaces a user-actionable "run silieco login"
...
func (d *Daemon) syncWorkspacesFromAPI(ctx context.Context, reconcileProfiles bool) error {
	...
	// every workspace this sync has to register (SILI-5225). Probing is lazy —
...
// that heartbeat claimed (SILI-5444).
//
...
func (d *Daemon) handleModelList(ctx context.Context, rt Runtime, requestID string) {
	...
	// Self-heal a pinned executable path an in-place upgrade deleted (SILI-4486).
	...
	// picker for claude/codex (SILI-2339).
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Silieco Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/silieco)
...
func (d *Daemon) triggerRestart() {
	...
	// must use the stable <brew-prefix>/bin/silieco symlink instead.
	if isBrewInstall() {
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			newBin = filepath.Join(brewPrefix, "bin", "silieco")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			newBin = filepath.Join(prefix, "bin", "silieco")
		} else {
...
// pollLoop runs the machine-level batch claim poller (SILI-4257): a single
...
//
// This trades the per-runtime isolation the old model gave (SILI-1744) for a
...
// release. Used by pollLoop to expose SILIECO_TASK_SLOT to spawned tasks.
...
func (d *Daemon) handleTask(ctx context.Context, task Task, slot int) {
	...
	if err != nil {
		...
		// SILI-2946: route the bare error string through the canonical
...
func (d *Daemon) reportTaskResult(ctx context.Context, taskID string, result TaskResult, taskLog *slog.Logger) {
	...
	case "completed":
		...
		// SILI-2946: this fallback fires when a server-side complete
	...
	default:
		...
		if failureReason == "" {
			if result.Status == "cancelled" {
				...
				// SILI-2946: classify the agent's comment text so the
...
//
// Confirmed to load their context file, so deliberately absent here. SILI-5392
...
func gateResumeToReusedWorkdir(task *Task, taskCtx *execenv.TaskContextForEnv, envWorkDir string, taskLog *slog.Logger) bool {
	...
	if !reused && task.PriorSessionID != "" {
		...
		// the loss instead of silently restarting (SILI-4424). Set it on BOTH
		// carriers: the notice is rendered from `task` by BuildPrompt (SILI-5377
...
// terminal file raced and dropped the session (SILI-4886). Both proofs this
...
// isolation (SILI-4424) means the rollout may be missing: a migrated legacy home
...
func gateCodexResumeToRolloutPresence(task *Task, taskCtx *execenv.TaskContextForEnv, provider, codexHome string, taskLog *slog.Logger) {
	...
	// loss instead of silently restarting (SILI-4424). Set it on BOTH carriers:
	// the notice is rendered from `task` by BuildPrompt (SILI-5377 moved it out
...
// via gateCodexResumeToRolloutPresence, losing the conversation (SILI-5305). It
...
// verify and return immediately (SILI-5305).
...
func (d *Daemon) ensureTaskSkillBundles(ctx context.Context, task *Task) error {
	...
	for _, ref := range misses {
		...
		if err != nil {
			...
			// guesswork (SILI-5370): size + elapsed separate "this bundle is
...
// across dispatches. (GitHub #4505 / SILI-3650)
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	// here would make SILIECO_WORKSPACE_ID empty in the agent env, and the
	...
	// `silieco repo checkout` would reject project-only URLs that aren't also
	...
	// A custom runtime profile (SILI-3284) overrides the executable path: the
	...
	// codex is never launched under the previous version's policy (SILI-4486).
	...
	if customSpec, isCustom := d.customProfileLaunchForRuntime(task.RuntimeID); isCustom {
		...
		// upgrade deleted (SILI-4486). Only reached when no custom profile owns
	...
	taskCtx := execenv.TaskContextForEnv{
		...
		// SILI-5305: the server sets this when a more recent Codex session was
	...
	// (SILI-4486). A custom codex runtime skips the self-heal, so resolvedVersion
	...
	// instead of silently downgrading a user's isolation opt-in (SILI-4957).
	...
	// of a long-idle issue (SILI-4424). No-op for non-Codex tasks / no stable key.
	if provider == "codex" {
	...
	// /silieco_workspaces/{ws}/{short-id}/workdir hit FileNotFoundError in
	...
	// taxonomy as before — see SILI-2946 for the classifier contract.
	...
	// sessions (SILI-4424 isolates them). Drop the resume before the brief is
	...
	// up stale Silieco instructions (issue id, trigger comment id, reply
	...
	// can call the Silieco API and the local daemon (e.g. `silieco repo checkout`).
	// SILIECO_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	// SILIECO_TOKEN is bound to (agent, task) by the server. Never fall back
	...
	agentEnv := map[string]string{
		"SILIECO_TOKEN":        agentToken,
		"SILIECO_SERVER_URL":   d.cfg.ServerBaseURL,
		"SILIECO_DAEMON_PORT":  fmt.Sprintf("%d", d.cfg.HealthPort),
		"SILIECO_WORKSPACE_ID": task.WorkspaceID,
		"SILIECO_AGENT_NAME":   agentName,
		"SILIECO_AGENT_ID":     task.AgentID,
		"SILIECO_TASK_ID":      task.ID,
		"SILIECO_TASK_SLOT":    strconv.Itoa(slot),
		"TMPDIR":               taskTempDir,
		"TMP":                  taskTempDir,
		"TEMP":                 taskTempDir,
	}
	...
	if task.AutopilotRunID != "" {
		agentEnv["SILIECO_AUTOPILOT_RUN_ID"] = task.AutopilotRunID
	}
	if task.AutopilotID != "" {
		agentEnv["SILIECO_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the silieco CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["SILIECO_QUICK_CREATE_TASK_ID"] = task.ID
		if len(task.QuickCreateAttachmentIDs) > 0 {
			if raw, err := json.Marshal(task.QuickCreateAttachmentIDs); err == nil {
				agentEnv["SILIECO_QUICK_CREATE_ATTACHMENT_IDS"] = string(raw)
			} else {
				taskLog.Warn("quick-create attachment ids: marshal failed; skipping env injection", "error", err)
			}
		}
	}
	// Ensure the silieco CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// silieco binary so that `silieco` commands in the agent always resolve.
	...
	// then the daemon-wide SILIECO_<PROVIDER>_MODEL env var. If
	...
	execOpts := agent.ExecOptions{
		...
		// clears ResumeSessionID but not this (SILI-4424).
	...
	// `silieco issue status` / `silieco issue comment add`, leaving issues
	// stuck in `todo`.
	//
	...
	// SILI-5305: withhold a Codex session whose rollout never reached the per-issue
	...
	// loss (PriorSessionResumeUnavailable, SILI-4424 transparency) even while
	...
	// as session_rollout_missing on the terminal callback (SILI-5305).
	...
	default:
		...
		if failureReason != "" {
			...
			// SILI-2946: classifyPoisonedError only matches the
			...
			// SILI-1949 offline backfill to re-classify after the
			// fact.
...
//     SILI-4910): the platform's own retry is supposed to inherit the session
...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID, codexHome string, msgSeq *atomic.Int32) (agent.Result, int32, error) {
	...
	// would be cut off here regardless of progress (SILI-3064).
	...
	// zero remains authoritative so SILIECO_AGENT_IDLE_WATCHDOG=0 still disables
		...
		for {
			...
			case msg, ok := <-session.Messages:
				...
				case agent.MessageStatus:
					...
					// without context.
					// SILI-5305: pin the resume pointer only once the session's
...
// outside the env root, so isActiveEnvRoot does not cover it (SILI-4424). If a GC
...
// reservation, closing the stat->remove race (SILI-4424).
...
func ensureTaskTempDir(envRoot string, workspaceID string, taskID string) (string, error) {
	...
	dir, err := os.MkdirTemp(socketSafeTempBaseDir(), "silieco-task-")
	if err != nil {
		return "", err
	}
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "SILIECO_") {
		return true
	}
	switch upper {
