...
// same machine shares one UUID stored at `~/.silieco/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.silieco/profiles/*/daemon.id` and returns every
...
// explicitly pins SILIECO_DAEMON_ID to the hostname itself, there's nothing
