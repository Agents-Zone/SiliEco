...
func (d *Daemon) runTaskWakeupConnection(ctx context.Context, runtimeIDs []string, taskWakeups chan<- taskWakeup, runtimeSetCh <-chan struct{}) (time.Duration, error) {
	...
	// this WS connection gets identical capability gating (SILI-4257).
	...
	// Attach the generic WS RPC sender (SILI-4257) to this connection's write
	...
	// route rather than staying on the legacy fallback forever (SILI-4257).
	...
	defer func() {
		...
		// slots (SILI-4257, Sol-Boy review). With the conn closed here,
...
func (d *Daemon) runWSWriter(conn *websocket.Conn, writes <-chan *wsOutbound, done chan<- struct{}) {
	...
	for item := range writes {
		...
		// fallback would double-claim (SILI-4257). beginWrite also marks the
