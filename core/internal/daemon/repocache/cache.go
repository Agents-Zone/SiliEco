...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/silieco_workspaces/.repos)
	logger *slog.Logger
...
type WorktreeParams struct {
	...
	// listed as a writable root (silieco-ai/silieco#2925).
...
const (
	isolatedCheckoutConfigKey   = "silieco.checkout-mode"
	isolatedCheckoutConfigValue = "isolated"
	isolatedCacheRemoteName     = "silieco-cache"
)
...
func (c *Cache) createOrUpdateIsolatedCheckout(barePath, repoURL, checkoutPath, branchName, baseRef string) (string, error) {
	...
	if _, err := os.Stat(checkoutPath); err == nil {
		return "", fmt.Errorf("checkout path already exists and is not a Silieco isolated checkout: %s", checkoutPath)
...
// deleteStaleAgentBranches prunes branches left by earlier Silieco tasks while
...
// siliecoHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const siliecoHookMarker = "# silieco:prepare-commit-msg:co-authored-by"
...
// treats a hook as Silieco-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before siliecoHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	siliecoHookMarker,
	"# Installed by the Silieco daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the Silieco Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# silieco:prepare-commit-msg:co-authored-by
# Silieco: add Co-authored-by trailer for the Silieco Agent.
# Installed by the Silieco daemon. Do not edit — it will be overwritten.
...
TRAILER="Co-authored-by: silieco-agent <github@silieco.ai>"
...
// a Co-authored-by trailer for the Silieco Agent. The hook is installed in the
...
// installed by the Silieco daemon (current or any previously released
