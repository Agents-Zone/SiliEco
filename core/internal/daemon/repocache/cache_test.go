...
// for the Silieco Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: silieco-agent <github@silieco.ai>"
...
// path: a bare cache that already carries the Silieco prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: silieco-agent") {
...
// prepare-commit-msg hook that does NOT include the siliecoHookMarker
// sentinel — only the older `# Installed by the Silieco daemon.` comment.
...
// indefinitely (the exact bug reported in SILI-1704).
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no siliecoHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: silieco-agent") {
...
// without the Silieco marker (e.g. one a user added manually) must be left
