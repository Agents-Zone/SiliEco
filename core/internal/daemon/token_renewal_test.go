...
func TestClient_RenewToken_PostsToCorrectEndpoint(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("Authorization"); got != "Bearer sili_abc" {
			t.Errorf("expected Bearer sili_abc, got %q", got)
		}
...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "silieco login") {
		t.Fatalf("401 warning must tell the user to run 'silieco login', got: %s", out)
	}
}
...
// must-fix from SILI-2744 review: when the daemon starts with an already-
...
// and the operator would never see a "run silieco login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	var buf bytes.Buffer
	d := &Daemon{client: NewClient(srv.URL), logger: captureLogger(&buf)}
	d.client.SetToken("sili_already_revoked")
	...
	if !strings.Contains(out, "silieco login") {
		t.Fatalf("expected the actionable 'run silieco login' hint in the WARN, got: %s", out)
	}
}
...
func TestPreflightAuth_SyncProceedsWhenRenewIsNoOp(t *testing.T) {
	...
	var buf bytes.Buffer
	d := &Daemon{client: NewClient(srv.URL), logger: captureLogger(&buf)}
	d.client.SetToken("sili_healthy")
