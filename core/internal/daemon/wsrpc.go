...
// transport for the same work — that risks a double claim (SILI-4257).
...
// transport (SILI-4257). It correlates responses to requests by request_id over
...
// double-claiming (SILI-4257, Sol-Boy review). sent/cancel race under mu so the
...
type wsRPCClient struct {
	...
	// server deadline still reports back before the daemon gives up (SILI-4257).
	grace time.Duration
}
...
func (c *wsRPCClient) call(ctx context.Context, method string, serverTimeout time.Duration, reqBody, respBody any, requireRPCV1 bool) (int, error) {
	...
	// caller must NOT fall back (that would double-claim, SILI-4257).
	giveUp := func() error {
		if item.cancel() {
	...
	// gives up and falls back to HTTP, or we would double-claim (SILI-4257).
...
// ClaimTasksWSFirst is the WS-first claim policy (SILI-4257): it issues the
