...
const sidecarManifestFile = ".silieco_sidecar_manifest.json"
...
//     so the agent still discovers the Silieco skill, just under a
//     different directory name.
//   - For Silieco-only namespaces (.agent_context/issue_context.md,
//     .silieco/project/resources.json) the caller swallows the error
...
// down a Silieco skill without colliding with a user-installed skill
...
// collision we append `-silieco`, then `-silieco-2`, `-silieco-3`,
...
func allocateCollisionFreeSkillDir(skillsParent, baseSlug string) (slug, dir string, err error) {
	...
	for i := 0; i < maxAttempts; i++ {
		...
		case i == 0:
			candidate = baseSlug
		case i == 1:
			candidate = baseSlug + "-silieco"
		default:
			candidate = fmt.Sprintf("%s-silieco-%d", baseSlug, i)
		}
...
// (.agent_context/, .silieco/, .claude/skills/, .github/skills/,
...
// issue-review-silieco. A managed skill directory is platform-owned — the
