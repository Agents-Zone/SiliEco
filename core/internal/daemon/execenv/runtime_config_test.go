...
// Sub-issue Creation section — after SILI-2538 the platform posts the
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				"`--status backlog` = **wait**",
				"`silieco issue status <child-id> todo`",
				"all `--status todo`",
...
// hardcoded `SILI-` prefix). PR #3055 first downgraded it to a "do NOT
...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		// The pre-SILI-2538 phrasing instructed the agent to compose a
		// parent comment by hand — including a hardcoded `SILI-` prefix
		...
		for _, banned := range []string{
			...
			"silieco issue comment add <parent-id>",
			"with NO `--parent`",
			"link the child as `[SILI-",
			...
			"`silieco issue status <this-issue-id> in_review`",
...
// dedicated protocol step that wrote `silieco issue status <this-issue-id> in_review`;
...
func TestCommentTriggeredProtocolDoesNotForceInReview(t *testing.T) {
	...
	if strings.Contains(out, "`silieco issue status <this-issue-id> in_review`") {
...
// TestPerRunCommentContextStaysOutOfBrief pins SILI-5377: no per-run comment
...
func TestPerRunCommentContextStaysOutOfBrief(t *testing.T) {
	...
	for _, banned := range []string{
		...
		if strings.Contains(out, banned) {
			t.Errorf("brief must not carry per-run comment value %q (SILI-5377)\n---\n%s", banned, out)
		}
	}
...
// Cold-start thread routing moved to the per-turn prompt (SILI-5377); the
...
func TestColdCommentsHintPointsAtTriggeringThread(t *testing.T) {
	...
	if !strings.Contains(hint, "silieco issue comment list "+issueID+" --thread thread-root-1 --tail 30 --output json") {
	...
	if strings.Contains(buildMetaSkillContent("claude", TaskContextForEnv{IssueID: issueID, TriggerCommentID: "trigger-1", TriggerThreadID: "thread-root-1"}), "thread-root-1") {
		t.Error("brief must not carry the per-run thread id (SILI-5377)")
	}
}

// Resumed/no-delta routing moved to the per-turn prompt (SILI-5377).
...
// (SILI-5377) because it is true of one run and false of the next.
func TestSessionContinuityNoticeLivesOutsideBrief(t *testing.T) {
	...
	if strings.Contains(buildMetaSkillContent("codex", lost), "Session Continuity Notice") {
		t.Error("brief must never carry the continuity notice — it is per-run state (SILI-5377)")
	}
}
...
func TestIssueWorkflowHonorsAgentIdentity(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Before step 4, run `silieco issue status " + issueID + " in_progress` unless your Agent Identity forbids issue status changes; if it does, skip it.",
		...
		"When done, run `silieco issue status " + issueID + " in_review` unless your Agent Identity forbids issue status changes; if it does, skip it.",
		"If blocked, run `silieco issue status " + issueID + " blocked` unless your Agent Identity forbids issue status changes.",
	} {
		if !strings.Contains(out, want) {
	...
	for _, banned := range []string{
		"4. Run `silieco issue status " + issueID + " in_progress`\n",
		"5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)",
		"8. When done, run `silieco issue status " + issueID + " in_review`\n",
	} {
		if strings.Contains(out, banned) {
...
func TestSquadLeaderIssueWorkflowKeepsParentInProgress(t *testing.T) {
	...
	for _, want := range []string{
		...
		"do NOT run `silieco issue status " + issueID + " in_review` or `done` on this turn",
	...
	if strings.Contains(out, "When done, run `silieco issue status "+issueID+" in_review`") {
...
// kinds must not inherit it. After SILI-5377 it applies to every issue run,
...
func TestChatOutputDoesNotRequireIssueComment(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"do not call `silieco issue comment add`",
...
// SILI-3605 regression where a review agent surfaced its progress
...
func TestOutputForbidsMidRunProgressComments(t *testing.T) {
	...
	// The `runtime_brief_slim` flag was retired (SILI-4297); there is now a
	// single brief.
	run(t, "brief")
}
...
// Connected Apps moved to the per-turn prompt (SILI-5377): the app set is
...
func TestConnectedAppsBlockLivesOutsideBrief(t *testing.T) {
	...
	if strings.Contains(out, "## Connected Apps") {
		t.Errorf("brief must not carry Connected Apps — it is per-run state (SILI-5377)\n---\n%s", out)
	}
}
...
// SILI-2753), and file present with markers (idempotent second-run replace).

func TestWriteRuntimeConfigFileCreatesMissingFile(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "CLAUDE.md")
	const brief = "# Silieco Agent Runtime\n\nbrief body line"
...
func TestWriteRuntimeConfigFilePreservesUserContent(t *testing.T) {
	...
	// injected marker block; this is the core regression case from SILI-2753.
...
func TestWriteRuntimeConfigFileReplacesExistingBlock(t *testing.T) {
	...
	const newBrief = "## New Silieco brief\n\nfresh body"
...
func TestWriteRuntimeConfigFileIgnoresStrayEndMarkerBeforeBegin(t *testing.T) {
	...
	// real block authored by an earlier Silieco run below.
	const userDoc = "# Repo CLAUDE.md\n\nExample of what Silieco writes:\n" +
		runtimeMarkerEnd + "\n\n# Real config below\n"
	original := userDoc +
		runtimeMarkerBegin + "\nFIRST BRIEF\n" + runtimeMarkerEnd + "\n"
	if err := os.WriteFile(path, []byte(original), 0o644); err != nil {
...
// `codex` run started by the user after a Silieco task must see the same
// file the user wrote.
...
// Cross-thread fan-out moved to the per-turn prompt (SILI-5377).
...
// Single-thread reply cookbook moved to the per-turn prompt (SILI-5377).
	...
	for _, provider := range []string{"claude", "codex"} {
		...
		t.Run(provider, func(t *testing.T) {
			...
			for i, v := range variants {
				...
				if got != want {
					t.Errorf("brief differs for variant %q — per-run state leaked into messages[0] (SILI-5377).\n%s",
...
// TestBriefByteIdenticalAcrossRunsForEveryKind extends the SILI-5377 guarantee
// past issue runs.
//
	...
	for kindName, baseCtx := range kinds {
		...
		t.Run(kindName, func(t *testing.T) {
			...
			for i, v := range variants {
				...
				if got != want {
					t.Errorf("%s brief differs for variant %q — per-run state leaked into the cached prefix (SILI-5377).\n%s",
