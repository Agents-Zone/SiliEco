...
// `--content-stdin` + HEREDOC mandate (#1795 / #1851 / SILI-2904) was kept
...
// when a model wrapped extra flags around the heredoc on `silieco issue
...
func TestBuildCommentReplyInstructionsCodexLinux(t *testing.T) {
	...
	for _, want := range []string{
		"silieco issue comment add " + issueID + " --parent " + triggerID + " --content-file ./reply.md",
...
//   - SILI-2904 / OKK-497: an agent inlined a backtick-wrapped table name into
	...
	for _, host := range []string{"linux", "darwin"} {
		for _, provider := range []string{"claude", "opencode", "openclaw", "hermes", "kimi", "kiro", "cursor"} {
			...
			t.Run(name, func(t *testing.T) {
				...
				// steered at inline `--content "..."` (SILI-2904) and never at
...
func TestBuildCommentReplyInstructionsWindowsUsesContentFile(t *testing.T) {
	...
	for _, provider := range []string{"codex", "claude", "opencode", "openclaw", "hermes", "kimi", "kiro", "cursor"} {
		t.Run(provider+"/windows", func(t *testing.T) {
			got := BuildCommentReplyInstructions(provider, issueID, triggerID)
			for _, want := range []string{
				"silieco issue comment add " + issueID + " --parent " + triggerID + " --content-file",
...
// agent at the per-turn user message instead (SILI-5377).
func TestInjectRuntimeConfigKeepsTriggerCommentOutOfBrief(t *testing.T) {
	...
	if strings.Contains(s, triggerID) {
		t.Errorf("CLAUDE.md must not carry the trigger comment id (SILI-5377)\n---\n%s", s)
	}
	for _, want := range []string{
		"Mode router",
		"`Turn mode: Reply.`",
...
// now ship in the per-turn prompt, so pin the helper directly (SILI-5377).
