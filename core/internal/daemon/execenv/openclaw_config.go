...
// risk surface. Tracked in SILI-5467; this file intentionally keeps the existing
...
func buildPerTaskOpenclawConfig(activePath string, exists bool, snapshotPath string, resolvedList []any, agentsFromRegistry bool, workDir string, managedMcp map[string]any, hasManagedMcp bool, gateway OpenclawGatewayPin) map[string]any {
	...
	// fields the silieco admin explicitly populated — zero strings/ints
...
// `exit status 1` hides (SILI-5422 / #6061). Real stderr always wins — the
...
//
// Input shape mirrors the rest of Silieco: Claude-style
