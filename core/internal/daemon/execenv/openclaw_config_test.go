...
// SILI-2219 DoD calls out: the directory Silieco writes skills into MUST be
...
// the scanner would ever see it; that is why SILI-2213 / #2621 needed a
// follow-up.
func TestPrepareOpenclawSkillWriteMatchesScanPath(t *testing.T) {
	...
	for _, s := range skills {
		...
		if _, err := os.Stat(want); err != nil {
			t.Errorf("openclaw scan target %s missing — Silieco's write path and the openclaw scanner are out of sync: %v", want, err)
		}
	}
}
...
//
// When a silieco agent is configured for gateway-mode openclaw and the
