...
// — users cannot audit or edit the contents from any Silieco UI.
//
// This conflicts with the Silieco daemon's context model. Silieco
...
//     unrelated host-project context into Silieco tasks. The reproduction
//     in github.com/silieco-ai/silieco#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Silieco issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Silieco task
// (and accept the leak risk) can keep the feature enabled by setting
// `SILIECO_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Silieco-owned, user-visible,
...
// SiliecoCodexMemoryEnv is the env var users can set to keep Codex
...
const SiliecoCodexMemoryEnv = "SILIECO_CODEX_MEMORY"

const (
	siliecoMemoryFeatureBeginMarker = "# BEGIN silieco-managed memory-feature (do not edit; regenerated by daemon)"
	siliecoMemoryFeatureEndMarker   = "# END silieco-managed memory-feature"
	siliecoMemoryConfigBeginMarker  = "# BEGIN silieco-managed memory-config (do not edit; regenerated by daemon)"
	siliecoMemoryConfigEndMarker    = "# END silieco-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(siliecoMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(siliecoMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(siliecoMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(siliecoMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(SiliecoCodexMemoryEnv))
...
// renderSiliecoMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderSiliecoMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(siliecoMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(siliecoMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderSiliecoMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderSiliecoMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(siliecoMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(siliecoMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderSiliecoMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderSiliecoMemoryConfigBlock(true))
}
...
//
// When SILIECO_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per SILIECO_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderSiliecoMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderSiliecoMemoryConfigBlock(false))
	}
