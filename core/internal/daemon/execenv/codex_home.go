...
type CodexHomeOptions struct {
	...
	// expose). See prepareCodexSessionsDir (SILI-4424).
	...
	// See prepareCodexSessionsDir (SILI-4424).
	...
	// codexSessionStoreDir and prepareCodexSessionsDir (SILI-4424).
	...
	// darwin danger-full-access. See task_home.go and SILI-4856.
...
// danger-full-access on doubt. See resolveWindowsSandboxState (SILI-4957).
...
// logic unit-testable without faulting the filesystem, and close SILI-4957's
...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// Codex's `initialize` state backfill (SILI-4424). See prepareCodexSessionsDir.
	...
	// stale or absent copy as "unconfigured" and loosen (SILI-4957).
	...
	// `missing field path` and bails out of `thread/start`. Silieco writes the
	...
	if err := ensureCodexSandboxConfig(configFile, policy, opts.CodexVersion, logger); err != nil {
		...
		// configureCodexTaskShellEnvironment then refuses to start (SILI-4957).
	...
	// so cross-task and cross-workspace context leaks (silieco#3130) cannot
...
// enumerated by a plain `codex` run, keeping Silieco task history out of the
// user's own thread list.
const codexSessionStoreRoot = "silieco-sessions"
...
// shared home's silieco-sessions root that have not been touched within
...
// their store trees disjoint so it does not need to (SILI-4424).
//
...
// ~/.codex/sessions.
//
// Background (SILI-4424): Codex 0.143+ backfills a per-home session-state DB by
...
// ~/.codex (SILI-4424, Windows cross-volume).
//
...
func linkCodexSessionsToStore(dst, storeDir, sharedSessions, resumeID string, logger *slog.Logger) error {
	...
	// remaining stat→remove race (SILI-4424).
...
// stat→remove race the mtime refresh alone cannot (SILI-4424).
...
// copy would reintroduce the stall SILI-4424 fixes.
...
//
// Regression for SILI-2646: the prior "don't overwrite" guard left per-task
