...
// The SILI-4899 delivery contract. Two orthogonal properties are pinned here and
...
func TestBriefSurfaceDeliveryPolicy(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"comment": {
			mustHave: []string{"`--attachment <path>` to `silieco issue comment add`"},
			mustNot:  []string{"silieco attachment upload"},
		},
		"assignment": {
			mustHave: []string{"`--attachment <path>` to `silieco issue comment add`"},
			mustNot:  []string{"silieco attachment upload"},
		...
		"chat_direct": {
			mustHave: []string{"`silieco attachment upload <local-path>`"},
			mustNot:  []string{"text-only"},
		},
		// IM surfaces are text-only. The upload command must not appear: it binds
		// to a Silieco chat reply, which an IM reply is not, so suggesting it
		...
		"chat_slack": {
			...
			mustNot:  []string{"run `silieco attachment upload"},
		},
		"chat_feishu": {
		...
		"quickcreate": {
			mustHave: []string{"your stdout is text-only", "`silieco issue create` call itself via `--attachment <path>`"},
			mustNot:  []string{"silieco attachment upload"},
		},
	}
