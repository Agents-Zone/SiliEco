...
func TestCodexShellEnvAllowlistUsesExactTaskAndSafeInheritedNames(t *testing.T) {
	...
	inherited := []string{
		...
		"SILIECO_SERVER_URL=https://wrong.example",
	}
...
set = { FOO = "bar", SILIECO_TOKEN = "stale" }
...
`,
			...
			removed: []string{"legal quoted table", `FOO = "bar"`, `SILIECO_TOKEN = "stale"`},
		},
		{
...
`,
	...
	includeOnly := []string{"CUSTOM_FLAG", "SILIECO_SERVER_URL", "SILIECO_TOKEN", "PATH", "SystemRoot"}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(s, `"SILIECO_*"`) {
				t.Fatalf("managed policy must not use a SILIECO_* wildcard:\n%s", s)
			}
...
func TestEnsureCodexShellEnvPolicyConfigRejectsInvalidInputWithoutWriting(t *testing.T) {
	...
	if err := EnsureCodexShellEnvPolicyConfig(configPath, []string{"SILIECO_TOKEN", "PATH"}, testLogger()); err == nil {
...
func TestEnsureCodexShellEnvPolicyConfigIsIdempotent(t *testing.T) {
	t.Parallel()
	configPath := filepath.Join(t.TempDir(), "config.toml")
	includeOnly := []string{"SILIECO_SERVER_URL", "SILIECO_TOKEN", "PATH"}
	...
	s := string(data)
	assertValidToml(t, s)
	if n := strings.Count(s, siliecoShellEnvBeginMarker); n != 1 {
