...
// subagents. The Silieco daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Silieco runtime
...
//
// Users who explicitly want Codex native subagents inside a Silieco task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `SILIECO_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// SiliecoCodexMultiAgentEnv is the env var users can set to keep Codex
...
const SiliecoCodexMultiAgentEnv = "SILIECO_CODEX_MULTI_AGENT"

// siliecoMultiAgentBeginMarker / siliecoMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	siliecoMultiAgentBeginMarker = "# BEGIN silieco-managed multi-agent (do not edit; regenerated by daemon)"
	siliecoMultiAgentEndMarker   = "# END silieco-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(siliecoMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(siliecoMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(SiliecoCodexMultiAgentEnv))
...
// renderSiliecoMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderSiliecoMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(siliecoMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(siliecoMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderSiliecoMultiAgentBlock(true)
...
//
// When SILIECO_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per SILIECO_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderSiliecoMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
