...
// `no such host` errors when calling out (for example, `silieco issue get`
// hitting the Silieco API). See upstream issue openai/codex#10390.
//
...
// actually reach the Silieco API. On Linux (and on macOS once the upstream
...
//     rejects non-safe mutation commands "by policy" — e.g. `silieco issue
...
//     SILI-4957. A user who has opted into windows.sandbox (via config.toml or a
...
//     danger-full-access so the Silieco CLI can reach the API.
func codexSandboxPolicyFor(goos, detectedVersion string) codexSandboxPolicy {
	...
	if goos == "windows" {
		return codexSandboxPolicy{
			...
			Reason: "codex on windows: compatibility fallback; no native windows.sandbox configured, so workspace-write cannot be enforced (SILI-4957)",
		}
	}
	if goos != "darwin" {
		return codexSandboxPolicy{
...
// confirm the user's intent. See SILI-4957.
...
// danger-full-access compatibility fallback. See SILI-4957.
//
...
func codexSandboxPolicyForWindows(state windowsSandboxConfig) codexSandboxPolicy {
	...
	case windowsSandboxUndecidable:
		return codexSandboxPolicy{
			Mode:          "workspace-write",
			NetworkAccess: true,
			Reason:        "codex on windows: windows.sandbox config undecidable (unreadable/unparseable/invalid); failing closed to workspace-write rather than loosening (SILI-4957)",
		}
	default: // windowsSandboxAbsent
		return codexSandboxPolicy{
			Mode:   "danger-full-access",
			Reason: "codex on windows: compatibility fallback; no native windows.sandbox configured (SILI-4957)",
		}
	}
}
...
// top of it), so config-only detection would miss them — the SILI-4957 review's
// second must-fix. Mirrors the override-parsing shape in core/pkg/agent's
...
// siliecoManagedBeginMarker / siliecoManagedEndMarker delimit the block the
...
const (
	siliecoManagedBeginMarker = "# BEGIN silieco-managed (do not edit; regenerated by daemon)"
	siliecoManagedEndMarker   = "# END silieco-managed"
)

// renderSiliecoManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.silieco]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertSiliecoManagedBlock), avoids both traps.
func renderSiliecoManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(siliecoManagedBeginMarker)
	b.WriteString("\n")
	...
	b.WriteString(siliecoManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(siliecoManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(siliecoManagedEndMarker) + `\n*`)

// upsertSiliecoManagedBlock returns the config content with the silieco-managed
...
// `[permissions.silieco]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderSiliecoManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertSiliecoManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderSiliecoManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the silieco-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertSiliecoManagedBlock(existing, policy)
...
// with core/pkg/agent. The agent package already has a similar parser; we
