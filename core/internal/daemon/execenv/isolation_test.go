...
// exercises the same stdin/stdout protocol as the real silieco helper.
			...
			ProjectResources: []ProjectResourceForEnv{
				{
					...
					ResourceRef:  json.RawMessage(`{"url":"https://github.com/silieco-ai/silieco"}`),
				},
			},
		},
	}, logger)
...
func TestPreparationHelperRoundTripsProjectResources(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(env.WorkDir, ".silieco", "project", "resources.json"))
	if err != nil {
	...
	if resource.ID != "resource-helper-project-resource" ||
		...
		ref.URL != "https://github.com/silieco-ai/silieco" ||
		resource.Label != "Silieco" {
