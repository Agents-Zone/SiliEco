...
// seedLegacySessionsSymlink recreates the pre-SILI-4424 layout: codex-home's
...
func TestExposeResumeRollout_LinksLargeFileWithoutCopying(t *testing.T) {
	...
	// exactly what SILI-4424 forbids.
...
func TestPrepareCodexSessionsDir_LocalDirectoryUsesPerIssueStore(t *testing.T) {
	...
	// are invisible, so `initialize` never enumerates them (the SILI-4424 stall).
	entries, _ := os.ReadDir(sessions)
	if len(entries) != 0 {
...
// the persistent store can't grow forever (SILI-4424).
...
// the store refreshes its activity (SILI-4424).
...
// GC only ever sees, and reclaims, its own (SILI-4424).
...
// character-dropping scheme merged them). SILI-4424.
...
// a lossy sanitizer collapsed (Elon's round-7 repro). SILI-4424.
