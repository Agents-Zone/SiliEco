...
// daemon-spawned Codex process can have SILIECO_TOKEN while `silieco issue ...`
...
const (
	...
	siliecoShellEnvBeginMarker = "# BEGIN silieco-managed shell-environment (do not edit; regenerated by daemon)"
	siliecoShellEnvEndMarker   = "# END silieco-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(siliecoShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(siliecoShellEnvEndMarker) + `\n*`)
...
// custom_env. Inherited SILIECO_* variables are always dropped because they
// belong to the daemon process, not necessarily to this task. Explicit
// SILIECO_* values are safe to include because daemon.go blocklists that
...
func renderSiliecoShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(siliecoShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(siliecoShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderSiliecoShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
