...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/silieco-ai/silieco","ref":"release/v2","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".silieco", "project", "resources.json")
	...
	for _, want := range []string{
		...
		"GitHub repo",
		"https://github.com/silieco-ai/silieco",
		"checkout ref: `release/v2`",
		"default branch hint: `main`",
		".silieco/project/resources.json",
	} {
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".silieco", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestPrepareWithRepoContext(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".silieco" && name != "CLAUDE.md" && name != ".claude" {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"silieco autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `silieco issue get") {
...
// collision-free duplicates (issue-review, issue-review-silieco,
// issue-review-silieco-2, …). Reuse rolls back the prior dispatch's writes
...
func TestReuseRefreshesSkillsWithoutDuplicating(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after re-dispatch the skills dir = %v, want exactly [issue-review] with no -silieco duplicates", names)
	}
...
// next refresh dodges to issue-review-silieco. Reuse must reclaim the
...
func TestReuseReclaimsManagedSkillDirWithStrayAgentFile(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after reuse with a stray agent file the skills dir = %v, want exactly [issue-review] with no -silieco duplicate", names)
	}
...
func TestInjectRuntimeConfigClaude(t *testing.T) {
	...
	s := string(content)
	for _, want := range []string{
		"Silieco Agent Runtime",
		"silieco issue get",
		"silieco issue comment list",
		"Go Conventions",
		"PR Review",
	...
	for _, tc := range providers {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				// Hardened pins (SILI-4140): the mechanism that slipped
				// through in SILI-4091 was a turn ending cleanly with a
				...
				// SILI-5274: persistent local services are allowed only as
				...
				// SILI-5223: the conceptual boundary above was read as
			...
			// SILI-5274 review: with the persistent-service exception in the
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"`silieco <command> --help`",
		"silieco issue get <id> --output json",
		"silieco issue comment list <issue-id>",
		"silieco issue create --title",
		"silieco issue update <id>",
		"--description-file <path>",
		"--parent \"\"",
		"silieco repo checkout <url>",
		"silieco issue status <id> <status>",
		"silieco issue comment add <issue-id>",
		"silieco issue comment add --help",
		"silieco squad member set-role <squad-id>",
	} {
		if !strings.Contains(s, want) {
	...
	for _, banned := range []string{
		"silieco issue list [--status",
		"silieco issue label list",
		"silieco issue subscriber list",
		"silieco label list",
		"silieco workspace member list",
		"silieco agent list",
		"silieco squad list",
		"silieco issue runs",
		"silieco issue run-messages",
		"silieco attachment download",
		"silieco autopilot list",
		"silieco autopilot create",
		"silieco autopilot update",
		"silieco autopilot trigger",
		"silieco autopilot delete",
		"silieco project get",
		"silieco project resource list",
		"silieco issue assign",
		"silieco issue label add",
		"silieco issue label remove",
		"silieco issue subscriber add",
		"silieco issue subscriber remove",
		"silieco issue comment delete",
		"silieco label create",
	} {
		if strings.Contains(s, banned) {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Silieco Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "silieco issue get") {
		t.Error("should reference silieco CLI even without skills")
	}
...
// This test pins the post-SILI-2219 write path; the previous fallback into
...
func TestWriteContextFilesOpenclawNativeSkills(t *testing.T) {
	...
	// The pre-SILI-2219 fallback path must NOT be written: openclaw never scans it.
	if _, err := os.Stat(filepath.Join(dir, ".agent_context", "skills")); !os.IsNotExist(err) {
...
func TestPrepareWithRepoContextOpencode(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".silieco" && name != "AGENTS.md" {
			t.Errorf("unexpected entry in workdir: %s", name)
		}
	}
...
// SILI-1124. Covering this in a test prevents the guidance from decaying back
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `silieco issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"silieco issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `silieco issue comment add`",
...
// provider on every host OS — post-SILI-2904 the corruption is shell-driven, so
...
// substitution in the body (SILI-2904), but the heredoc/flag boundary turned out
// to be its own structural bug: when a model wrapped extra flags around the
// heredoc on `silieco issue create`, the flags were silently swallowed into
...
func TestInjectRuntimeConfigLinuxCommentFormattingEmphasizesFile(t *testing.T) {
	...
	for _, provider := range []string{"codex", "claude", "opencode"} {
		t.Run(provider, func(t *testing.T) {
			...
			// The previous mandate (#1795 / #1851 / SILI-2904) must NOT remain.
			for _, banned := range []string{
...
// non-ASCII as `?` before reaching `silieco.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
func TestPrepareCodexHomeSeedsFromShared(t *testing.T) {
	...
	// shared home (SILI-4424). A fresh home gets an empty local dir.
...
// Regression for SILI-2646: when the user updates `~/.codex/config.toml`
...
`
	...
	for _, marker := range []string{
		siliecoManagedBeginMarker,
		siliecoMultiAgentBeginMarker,
		siliecoMemoryFeatureBeginMarker,
		siliecoMemoryConfigBeginMarker,
	} {
...
// Regression for SILI-2646 (deletion arm): when the user removes a file from
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, siliecoManagedBeginMarker) || !strings.Contains(s, siliecoManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertSiliecoManagedBlock
	// for why.
...
// TestEnsureCodexSandboxConfigWindowsFallsBack pins SILI-4957: when a Windows
// user has not opted into a native Codex sandbox, Codex cannot enforce
// workspace-write and rejects mutation commands (e.g. `silieco issue create`)
...
// TestEnsureCodexSandboxConfigWindowsRespectsUserSandbox pins the SILI-4957
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), siliecoManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.silieco.sandbox_mode and Codex would never see it — see
	// review of SILI-963 PR #1246. The block must be hoisted above any
...
`
	...
	beginIdx := strings.Index(s, siliecoManagedBeginMarker)
	endIdx := strings.Index(s, siliecoManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.silieco]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.silieco]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.silieco content")
	}
	...
	if n := strings.Count(string(data2), siliecoManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.silieco], got:\n%s", s)
	}
	if strings.Count(s, siliecoManagedBeginMarker) != 1 {
...
// into via `-c windows.sandbox=...` args — the second SILI-4957 must-fix, since
...
// TestResolveWindowsSandboxStateFailsClosed pins the SILI-4957 fail-closed
...
// TestPrepareCodexHomeFailsClosedWhenSandboxWriteFails pins the SILI-4957
...
func TestPrepareCodexHomeFailsClosedWhenSandboxWriteFails(t *testing.T) {
	...
	stale := siliecoManagedBeginMarker + "\nsandbox_mode = \"danger-full-access\"\n" + siliecoManagedEndMarker + "\n"
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), siliecoManagedBeginMarker) {
		t.Error("reused config.toml missing silieco-managed block")
	}
}
...
// inside a Silieco task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
// instructions (see SILI-1323 / GH#1576). Two agents were stuck in an infinite
...
// This is the fix for SILI-2168 — squad leaders were posting "Exiting silently"
...
// TestBuildMetaSkillContentEmitsRequestingUser pins SILI-2406's brief
...
// TestBuildMetaSkillContentSanitizesRequestingUserName guards SILI-2406's
...
// TestBuildMetaSkillContentNormalizesDescriptionLineEndings guards SILI-2406's
...
// block. Per SILI-2406 the section is description-driven; emitting just a
...
// Task Initiator moved to the per-turn prompt (SILI-5377): the initiator
...
func TestTaskInitiatorBlockMember(t *testing.T) {
	...
	if strings.Contains(content, "## Task Initiator") {
		t.Errorf("brief must not carry Task Initiator — it is per-run state (SILI-5377)\n---\n%s", content)
	}
}
...
// point; the per-run thread routing lives in the per-turn prompt (SILI-5377).
func TestInjectRuntimeConfigBriefKeepsStaticCatchUpRead(t *testing.T) {
	...
	if !strings.Contains(s, "silieco issue comment list "+issueID+" --roots-only --summary --output json") {
	...
	if strings.Contains(s, triggerID) {
		t.Errorf("brief must not carry the trigger comment id (SILI-5377)\n---\n%s", s)
	}
...
// Resumed/no-delta routing lives in the per-turn prompt (SILI-5377); pin the
...
func TestInjectRuntimeConfigBriefOmitsResumedThreadAnchor(t *testing.T) {
	...
	for _, banned := range []string{triggerID, "thread-root-1", "triggering comment is already included above"} {
		if strings.Contains(s, banned) {
			t.Errorf("brief must not carry per-run resume routing %q (SILI-5377)\n---\n%s", banned, s)
		}
	}
	...
	for _, want := range []string{
		...
		"silieco issue comment list " + issueID + " --thread thread-root-1 --tail 30 --output json",
	} {
...
func TestInjectRuntimeConfigAssignmentTriggerScansRootsFirst(t *testing.T) {
	...
	for _, want := range []string{
		"silieco issue comment list issue-1 --roots-only --summary --output json",
	...
	// inside the step (SILI-5372), so assert the label itself, not the literal
	...
	for _, banned := range []string{
		"silieco issue comment list issue-1 --output json",
...
// TestInjectRuntimeConfigCatchUpScansRootsFirst locks in SILI-5372: the
...
func TestInjectRuntimeConfigCatchUpScansRootsFirst(t *testing.T) {
	...
	for _, want := range []string{
		...
		"silieco issue comment list issue-1 --thread <thread-id> --tail 30 --output json",
	...
	if strings.Contains(s, "silieco issue comment list issue-1 --recent") {
...
// TestInjectRuntimeConfigIssueMetadataSectionScope locks in SILI-2017:
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"silieco issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"silieco issue metadata delete <issue-id> --key <k>",
	}
	...
	withoutSection := wantSection{
		// We can't simply require `silieco issue metadata list` absent
	...
	cases := []struct {
		...
		},
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"silieco issue metadata list issue-md-2 --output json",
				"See the `## Issue Metadata` section above",
				"silieco issue metadata set",
				"silieco issue metadata delete",
				"Before exiting",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_content_file", func(t *testing.T) {
		...
		if !strings.Contains(s, "silieco issue metadata list issue-md-codex --output json") {
		...
		// it moved to the per-turn user message (SILI-5377).
...
// Tests below cover the local_directory flow (SILI-2663): the daemon
