...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Silieco-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN SILIECO-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END SILIECO-RUNTIME -->"
	...
	// (see SILI-2753 review on PR #3438).
	//
...
// defense-in-depth, not the primary guard. See SILI-2645.
...
// writeRuntimeConfigFile writes the Silieco runtime brief to path without
...
// local_directory project resource flow. See SILI-2753.
...
// locateMarkerBlock finds the [start, end) byte range of the Silieco marker
...
//
//   - User content carries a stray `<!-- END SILIECO-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Silieco marker block from the runtime
...
// of Silieco runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Silieco task would pick up
...
// agent about the Silieco runtime environment and available CLI tools.
//
...
// legacy verbose brief; the flag has been retired (SILI-4297) and the slim brief
