...
// checked out on demand by the agent via `silieco repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string // base path for all envs (e.g., ~/silieco_workspaces)
	...
	// ~/.codex cannot see or GC this daemon's stores (SILI-4424).
	Profile      string
	...
	// (SILI-2663). When set, the envRoot/workdir directory is not created.
	LocalWorkDir string
	...
	// override that never lands in config.toml (SILI-4957).
...
type TaskContextForEnv struct {
	...
	// spanning MORE THAN ONE root thread (SILI-4348). When it has >=2 entries the
	...
	// into a user-visible one (SILI-4424). Distinct from an ordinary cold start,
	...
	// policy only: any non-empty value means the reply leaves Silieco for an
	// external channel, so `silieco attachment upload` cannot deliver a file and
	// the Output section says text-only instead (SILI-4899). The orthogonal
	...
	HandoffNote             string // assignment handoff instruction; rendered into issue_context.md (SILI-3375)
...
// on demand via `silieco repo checkout <url>`.
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	// that lose all SILIECO_* env vars AND escape above the workdir. Non-fatal:
	...
	// the set squad-leader reuse targets (SILI-4886). Non-fatal: a write failure
	...
	if params.Provider == "codex" {
		...
		// Codex config so npm/Prisma can write their caches (SILI-4856).
...
type ReuseParams struct {
	...
	// it. Empty means a fresh thread. See prepareCodexSessionsDir (SILI-4424).
	...
	// store into the same profile namespace (SILI-4424).
	Profile string
...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	// a collision-free sibling (issue-review, issue-review-silieco,
	// issue-review-silieco-2, …), accumulating a fresh duplicate on every
	...
	//      refresh back to issue-review-silieco.
	...
	if params.Provider == "codex" {
		...
		// writable_roots on reuse, mirroring the fresh Prepare path (SILI-4856).
...
const ManagedEnvProvenanceManagedBy = "silieco-daemon-managed-env"
...
// not-yet-written file and started a fresh session (SILI-4886). This marker is
