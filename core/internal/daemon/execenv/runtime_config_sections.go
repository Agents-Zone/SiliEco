...
import (
	...
	"github.com/silieco-ai/silieco/core/internal/runtimeapps"
)

// This file holds the runtime brief assembler — the post-SILI-3560 path
...
// verbose brief; the flag was retired in SILI-4297 and this is now the
// only brief.
//
// Layout:
//
...
func writeHeader(b *strings.Builder) {
	b.WriteString("# Silieco Agent Runtime\n\n")
	b.WriteString("You are a coding agent in the Silieco platform. Use the `silieco` CLI to interact with the platform.\n\n")
}
...
//
// SILI-5223: the external-work boundary alone did not stop agents from
...
// land is banned.
//
// SILI-5274 adds one narrow lifetime exception: a user-requested local
...
// current run" was dropped in the SILI-5274 review: with the handoff exception
...
func writeBackgroundTaskSafetySlim(b *strings.Builder) {
	...
	b.WriteString("Silieco marks the task terminal the moment your top-level turn exits — any process, tool call, or subagent owned by this run that is still active is orphaned, its result lost, and the final comment you meant to post after it never sends. There is no background-completion wakeup here.\n\n")
	...
	b.WriteString("- If a tool response says to wait for a future notification/reminder, or that it is running in the background so you can keep working, do not rely on that in Silieco-managed runs — block on the appropriate wait / output / collect operation before exiting.\n")
...
// user message. Both SILI-2645 test-pinned phrases ("apply any per-person
...
// across resumes (SILI-5377). Returns "" when no initiator name resolves.
func BuildTaskInitiatorBlock(initiatorType, initiatorName, initiatorEmail string) string {
	...
	b.WriteString("Attribute this request to that person and apply any per-person privacy or access rules your instructions define — in a workspace many people can reach, the initiator (not the runtime owner) is who you are answering. Your Silieco credentials stay scoped to the runtime owner, so this attribution does not widen what you can read or write — do not assume the initiator can see everything you can.\n\n")
	return b.String()
}
...
// breaking prompt-cache prefix stability across resumes (SILI-5377).
...
// preserved: each `silieco issue …` command name, all three `comment add`
...
// create/update tasks" intro phrase, and `silieco issue comment add
// --help`.
//
// The fold-aware `--full` flag from SILI-3555 is documented inline on the
...
func writeAvailableCommands(b *strings.Builder) {
	...
	b.WriteString("Prefer `--output json` for structured data. The default brief lists only the core agent loop and common issue create/update tasks; for everything else run `silieco --help` or `silieco <command> --help`.\n\n")
	b.WriteString("### Core\n")
	b.WriteString("- `silieco issue get <id> --output json` — full issue.\n")
	b.WriteString("- `silieco issue comment list <issue-id> [--roots-only] [--summary] [--thread <comment-id> [--tail N] | --recent N] [--before <ts> --before-id <uuid>] [--since <RFC3339>] [--full] --output json` — thread-aware comment reads. `--recent N` caps THREADS, not comments: every returned thread carries its root plus EVERY descendant with no per-thread cap, so on an issue with fewer than N root threads it hands you the entire history apart from the resolved threads it folds. `--roots-only` (top-level comments with `reply_count` + `last_activity_at`) and `--summary` (clip each body to a preview) are how you bound a wide read; `--thread <id> --tail N` is how you bound a deep one. Resolved threads come back folded by default on complete-thread reads (default list, `--recent`, `--thread` without `--tail`); pass `--full` to expand. Page older replies / threads with `--before`/`--before-id` (stderr labels: `Next reply cursor`, `Next thread cursor`); `--help` for full semantics.\n")
	b.WriteString("- `silieco issue create --title \"...\" [--description-file <path>] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--stage N] [--project <project-id>] [--workflow-instance <uuid> --workflow-stage <uuid>] [--due-date <RFC3339>] [--attachment <path>]` — create an issue. SOP coordinates must be passed together. For agent-authored long descriptions prefer `--description-file <path>` (heredoc stdin can swallow trailing flags, #4182). Write that file inside your working directory (e.g. `./description.md`), never `/tmp` or shared paths, and treat a failed write as fatal — the CLI rejects a path outside the workdir so a stale file from another run can't leak in (SILI-4252).\n")
	b.WriteString("- `silieco issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--assignee X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--workflow-instance <uuid> --workflow-stage <uuid>] [--due-date <RFC3339>]` — update fields; pass `--parent \"\"` to clear parent, or both SOP flags as empty strings to detach from a workflow run.\n")
	b.WriteString("- `silieco issue status <id> <status>` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled).\n")
	b.WriteString("- `silieco issue children <id> [--output json]` — list a parent's sub-issues grouped by stage.\n")
	b.WriteString("- `silieco issue comment add <issue-id> [--content \"...\" | --content-file <path> | --content-stdin] [--parent <comment-id>] [--attachment <path>]` — post a comment. Agent-authored bodies MUST use `--content-file`. `silieco issue comment add --help` for full flags.\n")
	b.WriteString("- `silieco issue metadata list <issue-id> [--output json]` — list KV metadata.\n")
	b.WriteString("- `silieco issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]` — pin or overwrite a key.\n")
	b.WriteString("- `silieco issue metadata delete <issue-id> --key <k>` — remove a key.\n")
	b.WriteString("- `silieco repo checkout <url> [--ref <branch-or-sha>]` — repository checkout on a dedicated branch.\n\n")
	b.WriteString("### Squad maintenance\n")
	b.WriteString("- `silieco squad member set-role <squad-id> --member-id <id> --member-type <agent|member> --role <role> [--output json]` — change role in place (use this instead of remove+add).\n\n")
}
...
// every CLI other than `silieco issue create`, so listing more would just
// tempt the model to bend the guardrail.
func writeAvailableCommandsQuickCreate(b *strings.Builder) {
	b.WriteString("## Available Commands\n\n")
	b.WriteString("**Use `--output json` for structured data.** For anything beyond `issue create`, run `silieco --help` or `silieco <command> --help`.\n\n")
	b.WriteString("### Core\n")
	b.WriteString("- `silieco issue create --title \"...\" [--description \"...\" | --description-file <path> | --description-stdin] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--stage N] [--project <project-id>] [--workflow-instance <uuid> --workflow-stage <uuid>] [--due-date <RFC3339>] [--attachment <path>]` — Create a new issue; SOP coordinates must be passed together and `--attachment` may be repeated. For agent-authored long descriptions, prefer `--description-file <path>` over `--description-stdin` (flags after a HEREDOC terminator can be silently swallowed, #4182). Write that file inside your working directory (e.g. `./description.md`), never `/tmp` or shared paths, and treat a failed write as fatal — the CLI rejects a path outside the workdir so a stale file from another run can't leak in (SILI-4252).\n\n")
}
...
func writeCommentFormatting(b *strings.Builder) {
	...
	if runtimeGOOS == "windows" {
		b.WriteString("On Windows, **always write the comment body to a UTF-8 file with your file-write tool first, then post it with `--content-file <path>`** — do NOT pipe via `--content-stdin` (PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to a native command, silently dropping non-ASCII characters as `?` before they reach `silieco.exe`). Never use inline `--content` for agent-authored comments. Write that file inside your working directory (`./reply.md`), never `/tmp` or shared paths — the CLI rejects a `--content-file` path outside the workdir so another run's stale file can't leak in (SILI-4252). Keep the same `--parent` value from the trigger comment when replying. Delete the temp file (`Remove-Item ./reply.md`) after posting; do not rely on `\\n` escapes.\n\n")
		return
	}
	b.WriteString("For issue comments, **always write the comment body to a UTF-8 file with your file-write tool first, then post it with `--content-file <path>`**. Never use inline `--content` for agent-authored comments — the shell rewrites backticks / `$()` / quotes in the body (SILI-2904). Never use `--content-stdin` with a HEREDOC alongside other flags either — the heredoc/flag boundary is fragile and flags get silently swallowed (#4182). Write that file inside your working directory (`./reply.md`), never `/tmp` or shared paths — the CLI rejects a `--content-file` path outside the workdir so another run's stale file can't leak in (SILI-4252). Keep the same `--parent` value from the trigger comment when replying. Delete the temp file (`rm ./reply.md`) after posting; do not rely on `\\n` escapes.\n\n")
}
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Available in this workspace — `silieco repo checkout <url> [--ref <branch-or-sha>]` to fetch (creates a repository checkout on a dedicated branch).\n\n")
...
func writeProjectContext(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if len(ctx.ProjectResources) > 0 {
		b.WriteString("Project resources (also written to `.silieco/project/resources.json`):\n\n")
		for _, r := range ctx.ProjectResources {
			fmt.Fprintf(b, "- %s\n", formatProjectResource(r))
		}
		b.WriteString("\nResources are pointers — open them only when relevant to the task. ")
		b.WriteString("For `github_repo` resources, use `silieco repo checkout <url>` to fetch the code. Add `--ref <branch-or-sha>` when a task or handoff names an exact revision.\n\n")
	} else {
...
func writeIssueMetadata(b *strings.Builder) {
	...
	b.WriteString("- **Write on exit.** Pin only if BOTH: (a) materially important to this issue, AND (b) a future run is likely to re-read it. Otherwise leave the bag alone. Stale keys: overwrite with the new value or `silieco issue metadata delete`.\n")
...
// disclose it in its reply (SILI-4424).
//
...
// the brief broke prompt-cache prefix stability across resumes (SILI-5377).
...
func writeWorkflowChat(b *strings.Builder) {
	...
	b.WriteString("- You have full access to the `silieco` CLI to look up issues, workspace info, members, agents, etc.\n")
	b.WriteString("- If asked about issues, use `silieco issue list --output json` or `silieco issue get <id> --output json`\n")
	b.WriteString("- If asked about the workspace, use `silieco workspace get --output json`\n")
	b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
	b.WriteString("- If the task requires code changes, use `silieco repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
...
func writeWorkflowQuickCreate(b *strings.Builder) {
	b.WriteString("**This task was triggered by quick-create.** There is NO existing Silieco issue. Follow the field and output rules in the user message you just received; ignore the default assignment-task workflow.\n\n")
	b.WriteString("Hard guardrails (apply even if the user message is missing):\n")
	b.WriteString("- Run exactly one `silieco issue create` invocation, then exit.\n")
	b.WriteString("- Do NOT call `silieco issue get`, `silieco issue status`, or `silieco issue comment add` for this task — there is no issue to query, transition, or comment on. The platform writes the user's success/failure inbox notification automatically based on whether `silieco issue create` succeeded.\n")
...
func writeWorkflowAutopilot(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("**This task was triggered by an Autopilot in run-only mode.** There is no assigned Silieco issue for this run.\n\n")
	...
	if ctx.AutopilotID != "" {
		fmt.Fprintf(b, "- Run `silieco autopilot get %s --output json` if you need the full autopilot configuration\n", ctx.AutopilotID)
	}
	b.WriteString("- Complete the autopilot instructions directly\n")
	b.WriteString("- Do not run `silieco issue get`, `silieco issue comment add`, or `silieco issue status` for this run unless the autopilot instructions explicitly tell you to create or update an issue\n\n")
}
...
// history (SILI-5377). So nothing here may depend on which trigger fired this
...
//
// Step 3 asks for a roots scan first, not `--recent 10` (SILI-5372). `--recent N`
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if ctx.IsSquadLeader {
		fmt.Fprintf(b, "5. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `silieco squad activity %s no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently): post it with `silieco issue comment add %s` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID, ctx.IssueID)
	} else {
		fmt.Fprintf(b, "5. **Post your final results as a comment — this step is mandatory**: post it with `silieco issue comment add %s` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered. In Reply mode this step is conditional on the reply rule below.\n", ctx.IssueID)
	}
	b.WriteString("6. Before exiting: only if this run produced a fact that clears the high bar (important AND likely to be re-read by future runs on this same issue, e.g. a new PR URL or deploy URL), or you noticed a metadata key from entry that is now stale, pin or clear it via `silieco issue metadata set`/`delete`. Most runs write nothing here — that is the expected outcome, not a gap. When in doubt, do not write. See the `## Issue Metadata` section above for the full bar.\n\n")

	b.WriteString("**Ownership mode only — you own the issue status this run**\n\n")
	fmt.Fprintf(b, "- Before step 4, run `silieco issue status %s in_progress` unless your Agent Identity forbids issue status changes; if it does, skip it.\n", ctx.IssueID)
	if ctx.IsSquadLeader {
		fmt.Fprintf(b, "- After this initial dispatch, leave the parent issue `in_progress` — do NOT run `silieco issue status %s in_review` or `done` on this turn. Dispatching members is not completion. You will be re-triggered when members post updates or a stage closes; only then, if the overall goal is met, move the parent to `in_review`.\n", ctx.IssueID)
	} else {
		fmt.Fprintf(b, "- When done, run `silieco issue status %s in_review` unless your Agent Identity forbids issue status changes; if it does, skip it.\n", ctx.IssueID)
	}
	fmt.Fprintf(b, "- If blocked, run `silieco issue status %s blocked` unless your Agent Identity forbids issue status changes. Post a comment explaining the blocker unless your Agent Identity forbids issue comments.\n\n", ctx.IssueID)
...
func writeSubIssueCreation(b *strings.Builder) {
	...
	b.WriteString("**Choosing `--status` when creating sub-issues.** `--status todo` = **start now** (default — agent assignees fire immediately). `--status backlog` = **wait**, then promote later with `silieco issue status <child-id> todo`. Parallel children: all `--status todo`. Strict serial 1→2→3: only Step 1 `todo`, Steps 2/3 `--status backlog` from the start.\n\n")
	b.WriteString("**Ordering with stages.** For phased plans, group children with `--stage <N>` (N ≥ 1) instead of hand-promoting the backlog chain — stage members run together, and the parent wakes once per stage. Use `--stage k --status backlog` for later stages, then `silieco issue children <id>` to inspect groupings before promoting. Reach for stages whenever a plan has more than one step or a step must wait for a group.\n\n")
}
...
func writeMentions(b *strings.Builder) {
	...
	b.WriteString("- `[SILI-123](mention://issue/<issue-id>)` — clickable link (no side effect)\n")
...
func writeAttachments(b *strings.Builder) {
	...
	b.WriteString("When a task includes attachment IDs and you need the files, inspect `silieco attachment --help` and use the authenticated CLI path. Do not open Silieco resource URLs directly.\n")
	// Closes the inbound half of the SILI-4899 loop: an attachment the agent
...
// writeAlwaysUseCLI emits the "must go through the silieco CLI" guardrail
// (compressed).
func writeAlwaysUseCLI(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `silieco` CLI\n\n")
	b.WriteString("Access Silieco platform resources (issues, comments, attachments, files) only through the `silieco` CLI — never `curl` / `wget`. For any operation the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}

// writeDeliveryInvariant emits the always-on delivery contract, shared by every
// task kind.
//
// SILI-4899: agents were writing runtime-local paths into deliverables as
...
func writeOutput(b *strings.Builder, kind taskKind, ctx TaskContextForEnv) {
	...
	case kindQuickCreate:
		b.WriteString("This is a quick-create task. There is NO existing issue to comment on. Your final stdout is captured automatically and the platform writes the user's success/failure inbox notification based on whether `silieco issue create` succeeded.\n\n")
		b.WriteString("- Do NOT call `silieco issue comment add` — the issue you just created has no conversation context for this run.\n")
		b.WriteString("- Print exactly one final line: `Created <identifier-or-id>: <title>` after a successful `silieco issue create`. Use the created issue's `identifier` from JSON output when available; otherwise use its `id`. Do not assume any workspace issue prefix such as `SILI-`; workspaces can use custom prefixes.\n")
		...
		b.WriteString("**Delivering files here:** your stdout is text-only. A file that belongs to the new issue goes on the `silieco issue create` call itself via `--attachment <path>`; never put its path in the description or in your stdout line.\n")
	case kindChat:
		b.WriteString("This is a chat session. Your reply is delivered directly to the chat window the user is reading.\n\n")
		// Two-layer channel policy (SILI-4899). This is the DELIVERY layer: any
		// non-empty channel type means the reply leaves Silieco for an external
		...
		if ctx.ChatChannelType != "" {
			fmt.Fprintf(b, "**Delivering files here:** this %s conversation is text-only — Silieco cannot push a file you produced back into it. `silieco attachment upload` does NOT apply: it binds to a Silieco chat reply, which this is not. Say in words what you produced and where it can be obtained; never upload and then write as though the file arrived, and never link its local path.\n", ChannelDisplayName(ctx.ChatChannelType))
		} else {
			b.WriteString("**Delivering files here:** run `silieco attachment upload <local-path>` — it binds the file to your reply and it renders as an attachment card. That command is the ONLY way a file reaches the user; a path written into your reply text is not.\n")
		}
	default:
		if ctx.IsSquadLeader {
			b.WriteString("⚠️ **Final results MUST be delivered via `silieco issue comment add`** — unless your outcome is `no_action`. When you evaluate a trigger and decide no action is needed, calling `silieco squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment that announces no_action, acknowledges another agent, or says you are exiting silently — such comments are noise. For all other outcomes (`action`, `failed`), a comment is still mandatory.\n\n")
		} else {
			b.WriteString("⚠️ **Final results MUST be delivered via `silieco issue comment add`.** The user does NOT see your terminal output, assistant chat text, or run logs — only comments on the issue. A task that finishes without a result comment is invisible to the user, even if the work itself was correct.\n\n")
		}
		...
		b.WriteString("**Delivering files here:** pass `--attachment <path>` to `silieco issue comment add` (repeatable). The file uploads and renders on the comment; that is the only way a screenshot or artifact reaches the reader.\n")
	}
	b.WriteString("\n")
	writeDeliveryInvariant(b)
}

// buildMetaSkillContentSlim is the post-SILI-3560 brief assembler.
// Called from buildMetaSkillContent (runtime_config.go). The
// `runtime_brief_slim` flag that once gated it was retired in SILI-4297.
//
...
func buildMetaSkillContentSlim(provider string, ctx TaskContextForEnv) string {
	...
	// the per-turn user message (daemon.BuildPrompt) instead. See SILI-5377.
	writeHeader(&b)
