...
type AgentEntry struct {
	...
	// Command is the bare command name or SILIECO_*_PATH value that Path was
	...
	// Daemon.resolveAgentEntry and SILI-4486.
	Command string
...
type Runtime struct {
	...
	// workspace custom runtime profile (SILI-3284). It links the runtime row
...
type Task struct {
	...
	PriorSessionResumeUnavailable bool                   `json:"prior_session_resume_unavailable,omitempty"` // SILI-5305: server signals a more recent Codex session was withheld (rollout missing) and PriorSessionID (if any) is an older fallback; the run must disclose the continuity gap even if that older session resumes cleanly. Absent/false on old servers.
	TriggerCommentID              string                 `json:"trigger_comment_id,omitempty"`               // comment that triggered this task
	CoalescedCommentIDs           []string               `json:"coalesced_comment_ids,omitempty"`            // SILI-4195: earlier comments folded into this run while it was still queued; the agent must address these in addition to the (newest) triggering comment. Empty for old servers / non-merged runs
	CoalescedComments             []CoalescedCommentData `json:"coalesced_comments,omitempty"`               // SILI-4195: full detail of the folded comments (thread_id/author/created_at/content) so the prompt can address each without assuming a shared thread. Empty for old servers / non-merged runs
	...
	ChatMessageAttachments        []ChatAttachmentMeta   `json:"chat_message_attachments,omitempty"`         // attachments linked to the chat message; agent uses these to `silieco attachment download <id>`
	...
	WorkflowInstanceID            string                 `json:"workflow_instance_id,omitempty"`             // SOP run selected for a quick-created or issue-bound task
	...
	ParentIssueIdentifier string `json:"parent_issue_identifier,omitempty"` // human-readable identifier (e.g. SILI-123) of the quick-create parent issue, used in prompt context
	...
	// identity, not a credential. See SILI-2645.
	...
	// The daemon injects it into the spawned agent as SILIECO_TOKEN so the
	...
	// daemon must not fall back to its own token. See SILI-3292.
...
// run `silieco attachment download <id>` instead of guessing from a
...
// (SILI-4195). The prompt embeds each one directly so the agent addresses every
...
type TaskResult struct {
	...
	// session because its rollout was not in the store (SILI-5305). Forwarded to
