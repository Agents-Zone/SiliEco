...
func TestPatternsFromEnv_DefaultsWhenUnset(t *testing.T) {
	t.Setenv("SILIECO_GC_ARTIFACT_PATTERNS", "")
	defaults := []string{"node_modules", ".next", ".turbo"}
	got := patternsFromEnv("SILIECO_GC_ARTIFACT_PATTERNS", defaults)
...
func TestPatternsFromEnv_DropsSeparatorBearingEntries(t *testing.T) {
	t.Setenv("SILIECO_GC_ARTIFACT_PATTERNS", "node_modules, .next ,foo/bar, ../etc, ,target")
	got := patternsFromEnv("SILIECO_GC_ARTIFACT_PATTERNS", nil)
...
func TestIsOfficialCloudServer(t *testing.T) {
	for _, tc := range []struct {
		...
		{"canonical cloud https", "https://api.silieco.ai", true},
		{"canonical cloud with trailing slash stripped", "https://api.silieco.ai/", true},
		{"canonical cloud case-insensitive", "https://API.Silieco.AI", true},
		{"cloud over plain http (unusual but match host)", "http://api.silieco.ai", true},
		...
		{"third-party host is self-host", "https://silieco.example.com", false},
		// Staging / preview / future subdomains deliberately follow the
		// safer self-host default until explicitly opted in.
		{"silieco.ai apex is not the api host", "https://silieco.ai", false},
		{"staging subdomain is self-host", "https://staging.silieco.ai", false},
		{"preview subdomain is self-host", "https://api-preview.silieco.ai", false},
...
func stageFakeAgent(t *testing.T) string {
	...
	t.Setenv("SILIECO_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	...
	t.Setenv("SILIECO_DAEMON_AUTO_UPDATE", "")
	return binDir
}
...
func TestLoadConfig_DiscoversQwenCode(t *testing.T) {
	...
	t.Setenv("SHELL", "/usr/bin/fish")
	t.Setenv("SILIECO_QWEN_MODEL", "qwen3.8-max-preview")
	t.Setenv("SILIECO_QWEN_ARGS", "--verbose --foo=bar")
...
func TestLoadConfig_SkipsSiliecoHooksShadowingAgentBinaries(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("POSIX shell not available on Windows")
	}
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	hooksDir := filepath.Join(home, ".silieco", "hooks")
...
func TestLoadConfig_SkipsSiliecoHooksFromLoginShellFallback(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("POSIX shell not available on Windows")
	}
	sh := "/bin/sh"
...
// SILI-2381: a daemon pointed at any non-cloud server URL must default
...
func TestLoadConfig_CodexHandshakeTimeout(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("SILIECO_CODEX_HANDSHAKE_TIMEOUT", "")
	...
	t.Setenv("SILIECO_CODEX_HANDSHAKE_TIMEOUT", "47s")
	...
	t.Setenv("SILIECO_CODEX_HANDSHAKE_TIMEOUT", "0")
	cfg, err = LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
...
func TestLoadConfig_OpenCodeIdleWatchdog(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("SILIECO_OPENCODE_IDLE_WATCHDOG", "")
	...
	t.Setenv("SILIECO_OPENCODE_IDLE_WATCHDOG", "7m")
	cfg, err = LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
	...
	t.Setenv("SILIECO_OPENCODE_IDLE_WATCHDOG", "0")
	cfg, err = LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
...
// daemon pointed at Silieco's hosted cloud keeps the historical opt-in
...
func TestLoadConfig_AutoUpdateDefault_CloudOn(t *testing.T) {
	stageFakeAgent(t)
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "wss://api.silieco.ai/ws",
		WorkspacesRoot: t.TempDir(),
	})
	if err != nil {
		t.Fatalf("LoadConfig: %v", err)
	...
	if !cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = false for Silieco Cloud server, want true")
	}
}
...
func TestLoadConfig_AutoUpdateEnv_ForcesOnForSelfHost(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("SILIECO_DAEMON_AUTO_UPDATE", "true")
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
	...
	if !cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = false after explicit SILIECO_DAEMON_AUTO_UPDATE=true, want true")
	}
}
...
func TestLoadConfig_AutoUpdateEnv_ForcesOffForCloud(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("SILIECO_DAEMON_AUTO_UPDATE", "false")
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "https://api.silieco.ai",
		WorkspacesRoot: t.TempDir(),
	})
	if err != nil {
		t.Fatalf("LoadConfig: %v", err)
	...
	if cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = true after explicit SILIECO_DAEMON_AUTO_UPDATE=false, want false")
	}
}
...
// resolvable via the daemon's PATH (or pinned to an explicit SILIECO_*_PATH),
...
func TestLoadConfig_UsesCodexDesktopAppBundleFallback(t *testing.T) {
	...
	t.Setenv("SHELL", filepath.Join(t.TempDir(), "fish"))
	t.Setenv("SILIECO_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	t.Setenv("SILIECO_CODEX_MODEL", "gpt-5")
	pinNonCodexAgentsToMissingPaths(t)
...
// Silieco must resolve the bundled CLI under ChatGPT.app (and prefer it over
...
func TestLoadConfig_CodexDesktopFallbackDoesNotOverrideExplicitPath(t *testing.T) {
	...
	t.Setenv("SHELL", filepath.Join(t.TempDir(), "fish"))
	t.Setenv("SILIECO_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	t.Setenv("SILIECO_CODEX_PATH", filepath.Join(t.TempDir(), "missing-codex"))
	pinNonCodexAgentsToMissingPaths(t)
	fakeClaude := filepath.Join(t.TempDir(), "claude")
	if err := os.WriteFile(fakeClaude, []byte("#!/bin/sh\nexit 0\n"), 0o755); err != nil {
		t.Fatalf("write fake claude: %v", err)
	}
	t.Setenv("SILIECO_CLAUDE_PATH", fakeClaude)
	...
	if got, ok := cfg.Agents["codex"]; ok {
		t.Fatalf("explicit missing SILIECO_CODEX_PATH should not fall back to Desktop bundle, got %#v", got)
	}
}
...
func pinNonCodexAgentsToMissingPaths(t *testing.T) {
	...
	for _, name := range []string{
		"SILIECO_CLAUDE_PATH",
		"SILIECO_OPENCODE_PATH",
		"SILIECO_OPENCLAW_PATH",
		"SILIECO_HERMES_PATH",
		"SILIECO_PI_PATH",
		"SILIECO_CURSOR_PATH",
		"SILIECO_COPILOT_PATH",
		"SILIECO_KIMI_PATH",
		"SILIECO_KIRO_PATH",
		"SILIECO_GROK_PATH",
	} {
...
func TestApplyOpenclawOverride_DoesNothingWhenNil(t *testing.T) {
	...
	t.Setenv("SILIECO_OPENCLAW_PATH", "/before/openclaw")
	t.Setenv("OPENCLAW_STATE_DIR", "/before/state")

	applyOpenclawOverride(nil)

	if got := os.Getenv("SILIECO_OPENCLAW_PATH"); got != "/before/openclaw" {
		t.Errorf("SILIECO_OPENCLAW_PATH mutated: got %q, want /before/openclaw", got)
	}
...
func TestApplyOpenclawOverride_SetsBothWhenEnvUnset(t *testing.T) {
	t.Setenv("SILIECO_OPENCLAW_PATH", "")
	t.Setenv("OPENCLAW_STATE_DIR", "")
	os.Unsetenv("SILIECO_OPENCLAW_PATH")
	...
	if got := os.Getenv("SILIECO_OPENCLAW_PATH"); got != "/from/config/openclaw" {
		t.Errorf("SILIECO_OPENCLAW_PATH: got %q, want /from/config/openclaw", got)
	}
...
// SILIECO_OPENCLAW_PATH already in their environment must not see the
...
func TestApplyOpenclawOverride_EnvWinsOverConfig(t *testing.T) {
	...
	t.Setenv("SILIECO_OPENCLAW_PATH", "/from/env/openclaw")
	...
	if got := os.Getenv("SILIECO_OPENCLAW_PATH"); got != "/from/env/openclaw" {
		t.Errorf("SILIECO_OPENCLAW_PATH: env should win, got %q want /from/env/openclaw", got)
	}
...
// must not have their SILIECO_OPENCLAW_PATH discovery path forcibly
...
func TestApplyOpenclawOverride_PartialFields_OnlySetsConfigured(t *testing.T) {
	...
	applyOpenclawOverride(&cli.OpenClawOverride{
		StateDir: "/from/config/state",
		// BinaryPath intentionally empty — must NOT call Setenv("SILIECO_OPENCLAW_PATH", "")
	})

	if _, set := os.LookupEnv("SILIECO_OPENCLAW_PATH"); set {
		t.Errorf("SILIECO_OPENCLAW_PATH should remain unset when BinaryPath is empty; got %q", os.Getenv("SILIECO_OPENCLAW_PATH"))
	}
	if got := os.Getenv("OPENCLAW_STATE_DIR"); got != "/from/config/state" {
		t.Errorf("OPENCLAW_STATE_DIR: got %q, want /from/config/state", got)
	}
...
func TestLoadConfig_BackendOverrides_MalformedConfigFileNonFatal(t *testing.T) {
	...
	cfgDir := filepath.Join(homeDir, ".silieco")
