...
const (
	...
	// never killed merely for running long (SILI-3064). Operators who want a
	// hard ceiling for cost/resource control can set SILIECO_AGENT_TIMEOUT.
	...
	// daemon-visible activity — see SILI-2300. 30 min keeps the safety net for
	// truly stuck runs (dockerd hang) while leaving headroom for long writes.
	// Set SILIECO_AGENT_IDLE_WATCHDOG=0 to disable.
	...
	// that stuck-tool case (SILI-3064). Set SILIECO_AGENT_TOOL_WATCHDOG=0 to
...
// SILIECO_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // silieco CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/silieco_workspaces)
	...
	GCCodexSessionTTL              time.Duration         // reclaim a per-issue Codex session store (~/.codex/silieco-sessions/<agent>/<issue>) untouched for at least this long, so a done/abandoned issue's conversation history does not accumulate forever (default: 14d, set 0 to disable)
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on Silieco Cloud, false on self-host)
	...
	// executable path to use for that profile on THIS machine (SILI-3284).
	// Sourced from the local CLI config (cli.CLIConfig.ProfileCommandOverrides),
	// written by `silieco runtime profile set-path`. appendProfileRuntimes
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("SILIECO_SERVER_URL", DefaultServerURL)
	...
	//
	//   - SILIECO_OPENCLAW_PATH: read by probe() via envOrDefault for the
	//     binary lookup; pre-existing path.
	//   - OPENCLAW_STATE_DIR:    OpenClaw's own env var; the daemon already
	//     forwards it to spawned children via mergeEnv (core/pkg/agent/...).
	//
	// Precedence is "env wins over config wins over default" — same shape
	// users already get with SILIECO_OPENCLAW_PATH today. We achieve it with
	...
	if cliCfg, err := cli.LoadCLIConfigForProfile(overrides.Profile); err != nil {
		...
		// Per-machine custom-runtime command path overrides (SILI-3284).
	...
	// can re-run the same discovery on a live daemon (SILI-5439).
	...
	claudeArgs, err := shellArgsFromEnv("SILIECO_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("SILIECO_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	codebuddyArgs, err := shellArgsFromEnv("SILIECO_CODEBUDDY_ARGS")
	if err != nil {
		return Config{}, err
	}
	qwenArgs, err := shellArgsFromEnv("SILIECO_QWEN_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("SILIECO_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("SILIECO_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("SILIECO_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("SILIECO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexHandshakeTimeout, err := durationFromEnv("SILIECO_CODEX_HANDSHAKE_TIMEOUT", DefaultCodexHandshakeTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// SILIECO_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("SILIECO_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}
	// SILIECO_OPENCODE_IDLE_WATCHDOG narrows the no-message window for
	// OpenCode's streamed model responses. Zero removes the provider-specific
	// override and falls back to SILIECO_AGENT_IDLE_WATCHDOG; positive values
	// cannot extend the global bound, and the global zero still disables the
	// whole mechanism.
	openCodeIdleWatchdog, err := durationFromEnv("SILIECO_OPENCODE_IDLE_WATCHDOG", DefaultOpenCodeIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	// SILIECO_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop; any
	// positive duration overrides DefaultAgentToolWatchdog.
	agentToolWatchdog, err := durationFromEnv("SILIECO_AGENT_TOOL_WATCHDOG", DefaultAgentToolWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("SILIECO_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via SILIECO_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("SILIECO_DAEMON_ID"))
	...
	// — e.g. ~/.silieco/daemon.id (default) plus ~/.silieco/profiles/<x>/
	...
	// the user explicitly pins SILIECO_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("SILIECO_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("SILIECO_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/silieco_workspaces or ~/silieco_workspaces_<profile>)
	...
	keepEnv := os.Getenv("SILIECO_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("SILIECO_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("SILIECO_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("SILIECO_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("SILIECO_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("SILIECO_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("SILIECO_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcCodexSessionTTL, err := durationFromEnv("SILIECO_GC_CODEX_SESSION_TTL", DefaultGCCodexSessionTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("SILIECO_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: default -> env override -> CLI override.
	//
	// Default is opt-in on Silieco Cloud (api.silieco.ai) and opt-out for
	...
	// auto-update off by default for self-host avoids both footguns (SILI-2381).
	// Operators on either side can flip the default with SILIECO_DAEMON_AUTO_UPDATE.
	autoUpdateEnabled := isOfficialCloudServer(serverBaseURL)
	if v := strings.TrimSpace(os.Getenv("SILIECO_DAEMON_AUTO_UPDATE")); v != "" {
	...
	autoUpdateInterval, err := durationFromEnv("SILIECO_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
...
// officialCloudHost is the hostname of Silieco's hosted cloud. It's the only
// origin we treat as "official" for the auto-update default — staging,
// preview, and any future *.silieco.ai subdomains are deliberately excluded
// so they inherit the safer self-host default until explicitly opted in.
const officialCloudHost = "api.silieco.ai"

// isOfficialCloudServer reports whether the resolved server base URL points
// at Silieco's hosted cloud. Used to pick the auto-update default: cloud
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid SILIECO_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("SILIECO_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
// SILIECO_WORKSPACES_ROOT env > default ($HOME/silieco_workspaces, or
// $HOME/silieco_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `silieco daemon disk-usage`) use this directly so they pick the same
// directory the running daemon would have picked.
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("SILIECO_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set SILIECO_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "silieco_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "silieco_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("SILIECO_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// When ~/.silieco/hooks shadows a real agent binary, skip that hooks directory:
...
func resolveAgentExecutablePath(cmd string) (string, error) {
	...
	if isInSiliecoHooksDir(resolved) {
		if unshadowed, err := lookPathExcludingSiliecoHooks(cmd); err == nil {
...
// deleted the old versioned directory the path pointed into (SILI-4486).
...
// probe() order in LoadConfig: exec.LookPath (with the ~/.silieco/hooks
...
func reresolveAgentCommand(cmd string) (string, bool) {
	...
	// this — an operator-pinned SILIECO_*_PATH that no longer exists should
...
func lookPathExcludingSiliecoHooks(cmd string) (string, error) {
	for _, dir := range filepath.SplitList(os.Getenv("PATH")) {
		if dir == "" {
			dir = "."
		}
		if isSiliecoHooksDir(dir) {
			continue
		}
...
func isInSiliecoHooksDir(path string) bool {
	if path == "" {
		return false
	}
	return isSiliecoHooksDir(filepath.Dir(path))
}

func isSiliecoHooksDir(dir string) bool {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return false
	}
	return samePathDir(dir, filepath.Join(home, ".silieco", "hooks"))
}
...
// before any SILIECO_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom SILIECO_*_PATH values never reach this
...
//  6. if the resolved path lives in ~/.silieco/hooks, searches the same
...
func buildLoginShellResolveScript(names []string) string {
	...
	b.WriteString("  hc=\"\"\n")
	b.WriteString("  if [ -n \"${HOME:-}\" ]; then hd=\"$HOME/.silieco/hooks\"; hc=$(cd \"$hd\" 2>/dev/null && pwd -P) || hc=\"\"; fi\n")
...
//
//   - The two vars touched (SILIECO_OPENCLAW_PATH, OPENCLAW_STATE_DIR) are
...
func applyOpenclawOverride(oc *cli.OpenClawOverride) {
	...
	if oc.BinaryPath != "" {
		if _, set := os.LookupEnv("SILIECO_OPENCLAW_PATH"); !set {
			_ = os.Setenv("SILIECO_OPENCLAW_PATH", oc.BinaryPath)
		}
	}
	if oc.StateDir != "" {
