...
// and the user has to restart the daemon — the bug SILI-3332 is about.
...
// the user-visible fix for SILI-3332: a profile created via the web UI on an
...
func TestRefreshWorkspaceRuntimeProfiles_NewProfileTriggersReregister(t *testing.T) {
	...
	// SILI-3332 review (concern 1): the drift path MUST NOT call recover-
...
// is the targeted regression for SILI-3332 review concern #1: a daemon that
...
func TestRefreshWorkspaceRuntimeProfiles_DriftWithRunningRuntimeSkipsOrphanRecovery(t *testing.T) {
	...
	// The whole point of SILI-3332 review concern #1 is that adding a
...
// targeted regression for SILI-3332 review concern #2: when a custom-only
