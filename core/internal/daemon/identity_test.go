...
func TestEnsureDaemonID_Persists(t *testing.T) {
	...
	path := filepath.Join(home, ".silieco", "daemon.id")
...
func TestEnsureDaemonID_SharedAcrossProfiles(t *testing.T) {
	...
	// only source of truth is ~/.silieco/daemon.id.
	profileFile := filepath.Join(home, ".silieco", "profiles", "staging", "daemon.id")
...
func TestEnsureDaemonID_PromotesPreChangeProfileFile(t *testing.T) {
	...
	profileDir := filepath.Join(home, ".silieco", "profiles", "staging")
	...
	data, err := os.ReadFile(filepath.Join(home, ".silieco", "daemon.id"))
	if err != nil {
...
func TestEnsureDaemonID_RegeneratesCorruptFile(t *testing.T) {
	...
	dir := filepath.Join(home, ".silieco")
...
func TestLegacyDaemonUUIDs_ScansProfileDirs(t *testing.T) {
	...
	for name, id := range map[string]string{"prod": uuidA, "desktop-silieco": uuidB} {
		dir := filepath.Join(home, ".silieco", "profiles", name)
		if err := os.MkdirAll(dir, 0o755); err != nil {
	...
	corruptDir := filepath.Join(home, ".silieco", "profiles", "corrupt")
