...
func TestAuth_ValidToken(t *testing.T) {
	...
	if gotEmail != "test@silieco.ai" {
		t.Fatalf("expected X-User-Email 'test@silieco.ai', got '%s'", gotEmail)
	}
}
...
func TestAuth_InvalidPAT(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/api/me", nil)
	req.Header.Set("Authorization", "Bearer sili_invalid_token_here")
	w := httptest.NewRecorder()
	handler.ServeHTTP(w, req)
...
// guarantee a client carrying a normal sili_ PAT could supply a forged
...
func TestAuth_PATCacheHit(t *testing.T) {
	...
	const rawToken = "sili_cache_hit_test_token"
...
// as the daemon side: with no SILIECO_CLOUD_FLEET_URL configured, an
// mcn_ bearer token must be rejected with 401 at the prefix branch.
// We don't fall through — an mcn_ string can't be a valid sili_ PAT or
