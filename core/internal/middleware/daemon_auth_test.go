...
// daemon traffic today uses sili_ PATs (mdt_ minting isn't wired up yet), so
...
func TestDaemonAuth_PATCacheHit(t *testing.T) {
	...
	const rawToken = "sili_daemon_pat_cache_hit_test"
	hash := auth.HashToken(rawToken)
...
// behaviour when SILIECO_CLOUD_FLEET_URL is empty: an mcn_ token MUST
// be rejected at the prefix branch with 401, not silently fall
// through to the sili_/JWT paths (an mcn_ string would never match a
