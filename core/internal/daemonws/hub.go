...
// RPCHandler processes a generic daemon:rpc_request (SILI-4257). It dispatches
...
// (SILI-4257). Like SetHeartbeatHandler it is wired after handler construction.
...
// scheduled tick (SILI-5444). Best-effort like every other hub notification: the
...
func (c *client) readPump() {
	...
	// runtime_id set (SILI-4257), well above the tiny heartbeat/wakeup frames.
...
// handleRPCFrame processes a generic daemon:rpc_request (SILI-4257): it runs the
...
func (c *client) handleRPCFrame(raw json.RawMessage) {
	...
	go func() {
		...
		// timed out and fallen back to HTTP (SILI-4257). The daemon waits a grace
