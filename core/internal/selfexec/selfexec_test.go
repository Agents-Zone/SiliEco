...
func TestResolveWithFallsBackToArgv0(t *testing.T) {
	...
	t.Run("absolute path", func(t *testing.T) {
		want := writeTestExecutable(t, t.TempDir(), "silieco")
	...
	t.Run("relative path", func(t *testing.T) {
		dir := t.TempDir()
		want := writeTestExecutable(t, dir, "silieco")
		t.Chdir(dir)
	...
	t.Run("PATH command", func(t *testing.T) {
		...
		got, err := resolveWith(failExecutable, []string{"silieco"})
		if err != nil {
			t.Fatalf("resolveWith() error = %v", err)
		}
		if got != want {
...
func TestResolveWithRejectsInvalidFallback(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing executable", args: []string{"silieco-does-not-exist"}, want: "silieco-does-not-exist"},
	}
	...
	if runtime.GOOS != "windows" {
		t.Run("non-executable file", func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "silieco")
