...
func seedOriginatorFanout(t *testing.T, pool *pgxpool.Pool) (memberCommentID, agentCommentID, taskAID, userID, workspaceUUID pgtype.UUID) {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ('Resolve Originator User', 'resolve-originator-fanout@silieco.test')
		RETURNING id
	...
	t.Cleanup(func() {
		pool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'resolve-originator-fanout@silieco.test'`)
	})
...
// the Stage-completion cascade (SILI-4302; raised by Bohan). Closing the last
...
func TestAttributionForIssueTask_SystemCommentFallsThroughToIssueProvenance(t *testing.T) {
	...
	// WorkspaceID is set so the workspace-scoped trigger-comment lookup (SILI-4252)
...
// the SILI-4305 fix: an agent that creates an issue through the ordinary
...
// consistency guarantee from SILI-4305: the exported OriginatorForIssueTask
...
func TestEnqueueTaskForIssueStoresRuntimeMCPOverlayInQueuedRow(t *testing.T) {
	...
	email := fmt.Sprintf("runtime-overlay-insert-%d@silieco.test", suffix)
