...
// test for SILI-4525 §5: a rerun whose operator cannot invoke the resolved target
...
// SILI-4525 §3: a MANUAL "run now" admits on the CURRENT clicker's invoke
...
func TestAutopilotDispatchAdmitsClickerNotCreator(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('AP Creator', $1) RETURNING id`,
		fmt.Sprintf("apc-%d@silieco.test", time.Now().UnixNano())).Scan(&apCreatorID); err != nil {
