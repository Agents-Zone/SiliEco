...
```bash
silieco agent get <agent-id> --output json      # full persisted agent record
silieco agent skills list <agent-id> --output json   # current skill bindings
silieco agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
`POST /api/agents` (`silieco agent create`). At task claim time the daemon
...
`silieco agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
silieco agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
silieco agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
silieco agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
silieco agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`silieco agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`silieco agent env set`), which carries the same
...
```bash
silieco agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
silieco agent update <agent-id> --mcp-config-stdin --output json
silieco agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `silieco agent create` — inserts a new agent row.
- `silieco agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `silieco agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `silieco agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
