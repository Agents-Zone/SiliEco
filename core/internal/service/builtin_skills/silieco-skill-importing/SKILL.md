---
name: silieco-skill-importing
description: "Use when a user provides a skill URL, slug, or clear intent to import/install a specific skill into the current Silieco workspace. Teaches the workspace import API/CLI path (POST /api/skills/import), the supported URL source families, --on-conflict fail|overwrite|rename|skip behavior and structured import results, additive agent binding vs replace-all, and the reserved SKILL.md supporting-file rule. Do not use it to decide which skill the user needs, and never treat an external local installer like npx skills add as the final Silieco install."
user-invocable: false
allowed-tools: Bash(silieco *)
---

# Importing skills into Silieco

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Silieco workspace.
...
A skill is installed for Silieco only when it exists in the current workspace's
...
```bash
silieco skill import --url <url> --output json              # hosted source
silieco skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Silieco workspace DB, so Silieco cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `silieco skill import --url <url> --output json`:

```bash
silieco skill import --url clawhub.ai/owner/skill --output json
silieco skill import --url skills.sh/owner/repo/skill --output json
silieco skill import --url github.com/owner/repo --output json
silieco skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
silieco skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`silieco skill import --file <path> --output json` imports a skill from a local
...
```bash
silieco skill import --url <url> --output json
```
...
```bash
silieco agent skills add <agent-id> --skill-ids <skill-id> --output json
silieco agent skills list <agent-id> --output json
```

After the final `silieco agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`silieco agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`silieco agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `silieco skill import --url <url>` is equivalent to
...
silieco skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
silieco skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
silieco skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
silieco skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
silieco skill get <skill-id> --output json
```
...
```bash
silieco skill list --output json
silieco skill get <skill-id> --output json
```
...
Incorrect (bypasses Silieco):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Silieco cannot manage it as a workspace skill.
...
```bash
silieco skill import --url https://skills.sh/owner/repo/skill --output json
```
...
- `references/skill-importing-source-map.md` — every behavior above mapped to
  `file:line` in `core/`, plus the verification command to re-derive the lines.
