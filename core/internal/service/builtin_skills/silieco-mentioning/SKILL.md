...
This skill states WHAT a mention link does in the Silieco backend, traced to
...
The parser (`util.MentionRe` in `core/internal/util/mention.go`) accepts
...
- a person → `silieco workspace member list --output json` → use `user_id`
- an agent → `silieco agent list --output json` → use `id`
- a squad  → `silieco squad list --output json` → use `id`
...
(`core/internal/handler/comment.go`); the comment path folds that result into
...
mention in the same comment still fires normally (SILI-5411): a comment reading
...
`core/internal/handler/comment.go`, where the explicit-mention branch is
...
- **A private agent you cannot access:** skipped — the mention path gates on
  ...
  one; the child-done wake is ungated — see the silieco-squads skill).

One nuance for automation (SILI-4857): when an UNATTRIBUTED autopilot run (a
...
Correct:
  1. `silieco workspace member list --output json`  → Alice's `user_id` = 7f3a…
