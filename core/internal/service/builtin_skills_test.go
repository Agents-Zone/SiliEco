...
func TestBuiltinSkillsConformToTemplate(t *testing.T) {
	...
	for _, skill := range skills {
		t.Run(skill.Name, func(t *testing.T) {
			// The silieco- prefix keeps the on-disk slug from colliding with a
			// user-authored workspace skill.
			if !strings.HasPrefix(skill.Name, "silieco-") {
				t.Errorf("skill name %q must carry the silieco- prefix", skill.Name)
			}
...
// TestBuiltinSkillsFrontmatterIsStrictYAML is the regression guard for SILI-3100
...
// must declare user-invocable:false and fence itself to the silieco CLI. New
// contract skills should copy this shape.
func TestMentioningSkillFollowsContractFrontmatter(t *testing.T) {
	skill, ok := findSkill(t, "silieco-mentioning")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); got != "Bash(silieco *)" {
		t.Errorf("allowed-tools = %q, want Bash(silieco *) (fence the skill to the CLI it teaches)", got)
	}
}
...
func TestWorkingOnIssuesSkillCoversIssueLoopContracts(t *testing.T) {
	skill, ok := findSkill(t, "silieco-working-on-issues")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); !strings.Contains(got, "Bash(silieco *)") {
		t.Errorf("allowed-tools = %q, want access to the Silieco CLI", got)
	}
	...
	mustContain := []string{
		...
		"open or update a PR before posting the final Silieco issue comment",
	...
	mustNotContain := []string{
		...
		"silieco issue get <issue-id> --output json",
		"silieco issue metadata list <issue-id> --output json",
		"silieco issue comment list <issue-id> --thread <trigger-comment-id>",
		"silieco issue comment add <issue-id> --parent <trigger-comment-id>",
	}
...
func TestSkillImportingSkillCoversWorkspaceImportContracts(t *testing.T) {
	skill, ok := findSkill(t, "silieco-skill-importing")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustNotContain := []string{
		"silieco agent skills set <agent-id> --skill-ids <skill-id>",
...
func TestCreatingAgentsSkillCoversAgentCreationContracts(t *testing.T) {
	skill, ok := findSkill(t, "silieco-creating-agents")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"silieco agent create --name <name> --runtime-id <runtime-id>",
		...
		"silieco agent skills add <agent-id> --skill-ids <skill-id> --output json",
		"silieco agent skills list <agent-id> --output json",
		"silieco agent get <agent-id> --output json",
		"255",
...
func TestSquadsSkillCoversLeaderRoutingContract(t *testing.T) {
	skill, ok := findSkill(t, "silieco-squads")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
...
func TestAutopilotsSkillCoversDispatchAndSideEffects(t *testing.T) {
	skill, ok := findSkill(t, "silieco-autopilots")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"create_issue",
		"run_only",
		"silieco autopilot trigger-add <autopilot-id> --kind schedule",
		"silieco autopilot trigger <autopilot-id> --output json",
		"Do not run `trigger`",
		"webhook tokens",
		"{{date}}",
...
func TestRuntimesAndReposSkillCoversClaimAndCheckoutChain(t *testing.T) {
	skill, ok := findSkill(t, "silieco-runtimes-and-repos")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"silieco runtime list --output json",
...
func TestProjectsAndResourcesSkillCoversDurableContext(t *testing.T) {
	skill, ok := findSkill(t, "silieco-projects-and-resources")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"silieco project resource add <project-id> --type local_directory",
