...
type IssueService struct {
	...
	// cmd/core/router.go after construction; nil in tests / self-hosted
...
type IssueCreateParams struct {
	...
	// WorkflowInstanceID and WorkflowStageID are an atomic SOP assignment.
	// They are either both NULL or both valid, and a valid assignment must
	// resolve to the same project and workspace as the task.
	WorkflowInstanceID pgtype.UUID
	WorkflowStageID    pgtype.UUID
...
// ErrWorkflowAssignmentInvalid signals that the supplied SOP run and stage
// are incomplete, closed, cross-workspace, cross-project, or do not belong to
// the same immutable workflow version.
var ErrWorkflowAssignmentInvalid = errors.New("invalid workflow assignment")

// ErrWorkflowProjectRequired signals that an SOP task was submitted without a
// project. Plain workspace tasks remain valid with all three fields unset.
var ErrWorkflowProjectRequired = errors.New("workflow assignment requires a project")
...
func (s *IssueService) Create(ctx context.Context, p IssueCreateParams, opts IssueCreateOpts) (IssueCreateResult, error) {
	...
	if workflowInstanceID.Valid != workflowStageID.Valid {
		return IssueCreateResult{}, ErrWorkflowAssignmentInvalid
	}
	if workflowInstanceID.Valid {
		if !projectID.Valid {
			return IssueCreateResult{}, ErrWorkflowProjectRequired
		}
		instance, err := qtx.GetWorkflowInstanceInWorkspace(ctx, db.GetWorkflowInstanceInWorkspaceParams{
			ID:          workflowInstanceID,
			WorkspaceID: p.WorkspaceID,
		})
		if err != nil || instance.Status == "completed" || instance.Status == "cancelled" {
			return IssueCreateResult{}, ErrWorkflowAssignmentInvalid
		}
		if !instance.ProjectID.Valid || instance.ProjectID != projectID {
			return IssueCreateResult{}, ErrWorkflowAssignmentInvalid
		}
		if _, err := qtx.GetWorkflowStageInVersion(ctx, db.GetWorkflowStageInVersionParams{
			ID:                workflowStageID,
...
func classifyOrigin(issue db.Issue, opts IssueCreateOpts) (source, taskID, autopilotRunID string) {
	...
	case "quick_create", "agent_create":
		...
		// (agent_create is the ordinary agent `issue create` path, SILI-4305);
