...
import (
	...
	"github.com/silieco-ai/silieco/core/internal/dispatch"
	"github.com/silieco-ai/silieco/core/internal/events"
	"github.com/silieco-ai/silieco/core/internal/issueguard"
	"github.com/silieco-ai/silieco/core/internal/issueposition"
...
// audit display (SILI-4302 §7). Cosmetic fields (title / description / issue title
...
// publish (SILI-4302 §3.4), recording the publisher and the effective config. Shared
...
// from SILI-1899 — without it a paused laptop / offline daemon causes scheduled
...
// from SILI-2429: Autopilot-on-squad ≈ Autopilot-on-leader), with the same
...
// (authorization) and accountable human (SILI-4302 §4), across both execution modes.
...
//
// When the autopilot is assigned to a squad (Path A from SILI-2429), the
...
func (s *AutopilotService) dispatchCreateIssue(ctx context.Context, ap db.Autopilot, run *db.AutopilotRun, triggerTimezone string, actorUserID pgtype.UUID) error {
	...
	*run = updatedRun
	...
	// SILI-2429); agent-assigned autopilots go through the standard issue
	...
	// triggering member (originator == accountable == actor, SILI-4302 §4). Schedule /
	...
	if ap.AssigneeType == "squad" {
		...
		// that no longer pass (SILI-3963 / SILI-4525).
...
type errDispatchSkipped struct {
	...
	// carried through to the response (SILI-4525) — never reverse-engineered from
...
// trigger time (Path A from SILI-2429). The same archived / runtime-bound /
...
func (s *AutopilotService) dispatchRunOnly(ctx context.Context, ap db.Autopilot, run *db.AutopilotRun, actorUserID pgtype.UUID) error {
	...
	// carries their authorization context) and accountable (SILI-4302 §4). A
	...
	// it) — trigger_owner, resolved from run.TriggerID (SILI-4302; Elon must-fix) —
	...
	// workspace is fail-closed (SILI-4302 §3.5).
...
// shouldSkipDispatch is the pre-flight admission check from SILI-1899.
...
func (s *AutopilotService) shouldSkipDispatch(ctx context.Context, ap db.Autopilot, actorUserID pgtype.UUID) (string, dispatch.ReasonCode, bool) {
	...
	// Invocation gate at the autopilot layer (SILI-3963 / SILI-4525). The
...
// the agent's own state rather than the human-readable reason string (SILI-4525).
...
// substring ("offline at dispatch time" is how the SILI-1899 alert fires).
//
...
func formatAdmissionReason(ap db.Autopilot, raw string) string {
	...
	default:
		...
		// preserving the legacy "at dispatch time" suffix from SILI-1899
...
// need to backfill — see RFC §4.e (SILI-2429).
...
// target agent under the invocation-permission model (SILI-3963). It mirrors
...
// invoke the target agent (SILI-4525). A MANUAL "run now" (actorUserID valid) is
...
// under the invocation-permission model (SILI-3963). It mirrors
