...
func seedAttributionFixture(t *testing.T, pool *pgxpool.Pool) (workspaceID, userID, agentID, issueID string) {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Attr User', $1) RETURNING id`,
		fmt.Sprintf("attr-%d@silieco.test", suffix)).Scan(&userID); err != nil {
...
// the Phase 1 foundation (SILI-4302 §11): a member-assigned run must land with a
...
func TestEnqueueTaskForIssueStampsDirectHumanAttribution(t *testing.T) {
	...
	// SILI-4302 §11 invariant at the DB layer: a non-NULL originator implies the
...
// the assign/promote actor fix (SILI-4302 §4): when a member assigns an issue that
...
func TestEnqueueTaskForIssueWithHandoffAttributesToActor(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Actor', $1) RETURNING id`,
		fmt.Sprintf("actor-%d@silieco.test", suffix)).Scan(&actorID); err != nil {
...
// SILI-4302 one-way invariant across the comment-coalescing merge (main #5192 ×
...
func TestMergeCommentIntoPendingTask_KeepsAccountableEqualsOriginator(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Attr User B', $1) RETURNING id`,
		fmt.Sprintf("attr-b-%d@silieco.test", time.Now().UnixNano())).Scan(&userB); err != nil {
...
// with no error, exactly as a fresh enqueue would (SILI-4302).
...
// CHECK (SILI-4302): a write in the ENFORCED regime (originator_source non-NULL — every
...
func TestAttributionInvariantCheck_RejectsBypass(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Check B', $1) RETURNING id`,
		fmt.Sprintf("check-b-%d@silieco.test", time.Now().UnixNano())).Scan(&userB); err != nil {
		t.Fatalf("seed user B: %v", err)
	}
	t.Cleanup(func() { pool.Exec(context.Background(), `DELETE FROM "user" WHERE id = $1`, userB) })
	if _, err := pool.Exec(ctx, `
...
// phase of the two-phase rollout (SILI-4302). Once the out-of-band backfill is complete,
...
// trigger_owner (SILI-4302; Bohan): an autopilot schedule/webhook run is accountable
...
func seedExtraMember(t *testing.T, pool *pgxpool.Pool, workspaceID, label string) string {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id`,
		label, fmt.Sprintf("%s-%d@silieco.test", label, suffix)).Scan(&userID); err != nil {
...
// every trigger together (SILI-4302).
...
// rule_owner (SILI-4302 §3.4): an autopilot-origin issue's run has NO authorizing
...
// left without an accountable human (SILI-4302 §3.5).
...
// published rule owned by someone else (SILI-4302 §4).
func TestDispatchRunOnlyManualStampsDirectHuman(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Trigger', $1) RETURNING id`,
		fmt.Sprintf("trigger-%d@silieco.test", time.Now().UnixNano())).Scan(&actorID); err != nil {
...
// (SILI-4302). The resolver-level before/after and per-trigger isolation are covered by
...
func TestEnqueueTaskForIssueAutopilotManualStampsDirectHuman(t *testing.T) {
	...
	var actorID string
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Trigger', $1) RETURNING id`,
		fmt.Sprintf("trig2-%d@silieco.test", time.Now().UnixNano())).Scan(&actorID); err != nil {
		t.Fatalf("seed actor: %v", err)
	}
	t.Cleanup(func() { pool.Exec(context.Background(), `DELETE FROM "user" WHERE id = $1`, actorID) })
...
// item (SILI-4302 §3.4): republishing a rule (as a trigger edit / archive / system
...
func TestRecordAutopilotRuleVersionRepublishReattributes(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Editor', $1) RETURNING id`,
		fmt.Sprintf("editor-%d@silieco.test", time.Now().UnixNano())).Scan(&memberB); err != nil {
...
// creating a NULL-accountable task (SILI-4302 §3.5, Elon must-fix 1).
...
func TestRerunIssueAttributesToRerunningMember(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Rerunner', $1) RETURNING id`,
		fmt.Sprintf("rerunner-%d@silieco.test", time.Now().UnixNano())).Scan(&rerunnerID); err != nil {
...
// way it does for autopilot_run / issue_assignment (SILI-4302 §2, Elon 2nd-round).
