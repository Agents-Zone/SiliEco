...
// transient provider stream cut (SILI-4910): first run + immediate retry + one
...
func TestTaskFailureClassifiers(t *testing.T) {
	cases := []struct {
		...
		// Transient mid-stream provider disconnect (SILI-4910): retryable, and
...
// regression for SILI-5370. It walks the exact chain FailTask runs for a task
