...
func NewEmailService() *EmailService {
	...
	default:
		fmt.Println("EmailService: DEV mode — codes printed to stdout (set SILIECO_DEV_VERIFICATION_CODE in .env for a fixed local code)")
	}
...
func (s *EmailService) SendVerificationCode(to, code string) error {
	...
	if s.smtpHost != "" {
		return s.sendSMTP(to, "Your Silieco verification code", body)
	}
	if s.client == nil {
	...
	params := &resend.SendEmailRequest{
		From:    s.fromEmail,
		To:      []string{to},
		Subject: "Your Silieco verification code",
		Html:    body,
	}
...
func buildInvitationParams(from, to, inviterName, workspaceName, inviteURL string) *resend.SendEmailRequest {
	...
	return &resend.SendEmailRequest{
		From:    from,
		To:      []string{to},
		Subject: fmt.Sprintf("%s invited you to %s on Silieco", subjectInviter, subjectWorkspace),
		Html: fmt.Sprintf(
			`<div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
				<h2>You're invited to join %s</h2>
				<p><strong>%s</strong> invited you to collaborate in the <strong>%s</strong> workspace on Silieco.</p>
