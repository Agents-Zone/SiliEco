...
// cmd/core/autopilot_scheduler.go goroutine (SILI-3551).
//
// Design highlights:
//
...
// could leak a scheduled occurrence (SILI-3551 acceptance ③).
func autopilotPlansForScope(cache *autopilotScheduleCache) func(
	...
	return func(ctx context.Context, scope Scope, now time.Time, latest LatestPlanInfo) ([]time.Time, error) {
		...
		//      on SILI-3551 dev: without it, a trigger that fired at
...
func autopilotHandler(
	...
	return func(ctx context.Context, in HandlerInput) (HandlerResult, error) {
		...
		// showing a "next run" that has already fired (SILI-3749). The
...
// (SILI-3749) would reappear at the top-of-period boundary. Returns
