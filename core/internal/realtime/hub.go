...
// loadTrustedProxies reads the same SILIECO_TRUSTED_PROXIES env var the rest of
// the server uses (see cmd/core/router.go and handler.Config.TrustedProxies),
...
func loadTrustedProxies() []netip.Prefix {
	raw := strings.TrimSpace(os.Getenv("SILIECO_TRUSTED_PROXIES"))
	if raw == "" {
		return nil
	}
...
// shared SILIECO_TRUSTED_PROXIES value in here at startup.
...
func authenticateToken(tokenStr string, pr PATResolver, ctx context.Context) (string, string) {
	if strings.HasPrefix(tokenStr, "sili_") {
		if pr == nil {
