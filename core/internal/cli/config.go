...
const defaultCLIConfigPath = ".silieco/config.json"
...
type CLIConfig struct {
	...
	// SILIECO_DAEMON_DEVICE_NAME env, this field, os.Hostname().
	...
	// SILIECO_AGENT_RUNTIME_NAME env, this field, the built-in
	// DefaultRuntimeName.
	...
	// SILIECO_DAEMON_MAX_CONCURRENT_TASKS env, this field, default.
	...
	// SILIECO_DAEMON_POLL_INTERVAL env, this field, DefaultPollInterval.
	...
	// flag, SILIECO_DAEMON_HEARTBEAT_INTERVAL env, this field,
	...
	// explicit 0), SILIECO_AGENT_TIMEOUT env, this field,
	// DefaultAgentTimeout.
	...
	// SILIECO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT env, this field,
	...
	// SILIECO_CODEX_HANDSHAKE_TIMEOUT env, this field,
	...
	// already resolves to enabled on Silieco Cloud. Absent / false means
	// "let env/default decide". Resolution precedence:
	// --no-auto-update flag, SILIECO_DAEMON_AUTO_UPDATE=false env, this
	...
	// --auto-update-interval flag, SILIECO_DAEMON_AUTO_UPDATE_INTERVAL
	...
	// profile_id -> absolute executable path (SILI-3284). A workspace custom
	...
	// `silieco runtime profile set-path`; the daemon prefers it over the
...
//
//	BinaryPath: SILIECO_OPENCLAW_PATH (env)  > backends.openclaw.binary_path > PATH lookup
...
// SILIECO_OPENCLAW_STATE_DIR. Rationale: OpenClaw already honors its own env
...
// OpenClaw's own env, SILIECO_OPENCLAW_STATE_DIR can be layered on top
...
// An empty profile returns the default path (~/.silieco/config.json).
// A named profile returns ~/.silieco/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".silieco", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.silieco/. A named profile returns ~/.silieco/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".silieco"), nil
	}
	return filepath.Join(home, ".silieco", "profiles", profile), nil
}
