...
func TestCLIConfig_BackwardCompat_OldFileLoadsWithNilBackends(t *testing.T) {
	...
	cfgDir := filepath.Join(tmp, ".silieco")
	if err := os.MkdirAll(cfgDir, 0o755); err != nil {
		t.Fatal(err)
	}
	historical := `{
  "server_url": "https://api.silieco.ai",
  "app_url": "https://silieco.ai",
  "workspace_id": "ws-123",
  "token": "sili_abcdef"
}`
	...
	if cfg.ServerURL != "https://api.silieco.ai" {
		t.Errorf("ServerURL: got %q, want historical value", cfg.ServerURL)
	}
	if cfg.Token != "sili_abcdef" {
...
func TestCLIConfig_BackwardCompat_NilBackendsOmittedFromJSON(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(tmp, ".silieco", "config.json"))
	if err != nil {
		t.Fatal(err)
	}
	if string(data) == "" {
...
func TestCLIConfig_ProfileCommandOverrides_OmittedWhenEmpty(t *testing.T) {
	...
	cfg := CLIConfig{ServerURL: "https://api.silieco.ai", Token: "sili_xyz"}
	if err := SaveCLIConfig(cfg); err != nil {
		t.Fatal(err)
	}
