...
func TestClassifyNetworkError(t *testing.T) {
	cases := []struct {
		...
		{"dns", &net.DNSError{Err: "no such host", Name: "api.silieco.ai", IsNotFound: true}, KindNetworkDNS},
		...
		{"x509 hostname", x509.HostnameError{Host: "api.silieco.ai"}, KindNetworkTLS},
		{"timeout string fallback", errors.New("Get \"https://x\": net/http: request canceled (Client.Timeout exceeded)"), KindNetworkTimeout},
		{"dns string fallback", errors.New("dial tcp: lookup api.silieco.ai: no such host"), KindNetworkDNS},
...
func TestFormatErrorNetwork(t *testing.T) {
	...
	raw := errors.New("Get \"https://api.silieco.ai/api/issues/abc\": context deadline exceeded")
	...
	if strings.Contains(got, "api.silieco.ai") || strings.Contains(got, "resolve issue") {
...
func TestHTTPTimeout(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("SILIECO_HTTP_TIMEOUT", tc.val)
...
func TestFormatErrorActionableHints(t *testing.T) {
	cases := []struct {
		...
		{401, []string{"silieco login", "self-hosted", "administrator"}, []string{"silieco login", "自托管", "管理员"}},
...
// `silieco login` failure guidance visible without losing classification.
func TestUserMessageError(t *testing.T) {
	withLang(t, "en_US.UTF-8")
	const hint = "Could not sign in with that token — make sure it is valid and not expired, then run `silieco login --token <token>` again."
	...
	t.Run("wrapped NetworkError classifies as network", func(t *testing.T) {
		...
		err := WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `silieco login` again.", underlying)
