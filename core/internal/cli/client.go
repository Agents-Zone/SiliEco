...
// Set by the silieco binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Silieco server API.
...
// reading a capped slice of the body. Every Silieco API helper funnels its
...
// It can be overridden with the SILIECO_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring SILIECO_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("SILIECO_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors SILIECO_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects SILIECO_HTTP_TIMEOUT.
...
// signature that is re-minted on every request (SILI-5372 / GitHub #5999). The
// CLI never hands an attachment URL to a native loader — `silieco attachment
