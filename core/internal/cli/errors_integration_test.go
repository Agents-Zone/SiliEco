...
func TestHelperStatusErrorsAreClassified(t *testing.T) {
	...
	statusCases := []struct {
		...
		{http.StatusUnauthorized, ExitAuth, "silieco login"},
...
// context that truncated a longer SILIECO_HTTP_TIMEOUT.
func TestCommandContextNotTruncatedBelowHTTPTimeout(t *testing.T) {
	...
	t.Setenv("SILIECO_HTTP_TIMEOUT", "400ms")
...
// SILIECO_HTTP_TIMEOUT and the AtLeast floor.
func TestAPITimeoutRespectsEnv(t *testing.T) {
	t.Run("default", func(t *testing.T) {
		t.Setenv("SILIECO_HTTP_TIMEOUT", "")
	...
	t.Run("AtLeast budget wins when larger than floor", func(t *testing.T) {
		t.Setenv("SILIECO_HTTP_TIMEOUT", "120s")
	...
	// responsive, but it must still honor SILIECO_HTTP_TIMEOUT (it is not a
