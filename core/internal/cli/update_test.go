...
func TestReleaseAssetCandidates(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			goarch:        "arm64",
			wantAssets: []string{
				"silieco-cli-1.2.3-darwin-arm64.tar.gz",
				"silieco_darwin_arm64.tar.gz",
			},
		},
		{
			...
			goarch:        "amd64",
			wantAssets: []string{
				"silieco-cli-1.2.3-linux-amd64.tar.gz",
				"silieco_linux_amd64.tar.gz",
			},
		},
		{
			...
			goarch:        "amd64",
			wantAssets: []string{
				"silieco-cli-1.2.3-windows-amd64.zip",
				"silieco_windows_amd64.zip",
			},
		},
	}
...
func TestFindReleaseAsset(t *testing.T) {
	t.Run("prefers versioned asset when both names exist", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "silieco_darwin_amd64.tar.gz", BrowserDownloadURL: "old"},
			{Name: "silieco-cli-1.2.3-darwin-amd64.tar.gz", BrowserDownloadURL: "new"},
		}
		...
		if got.Name != "silieco-cli-1.2.3-darwin-amd64.tar.gz" {
	...
	t.Run("falls back to legacy asset when versioned is absent", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "silieco_linux_amd64.tar.gz", BrowserDownloadURL: "old"},
		}
		...
		if got.Name != "silieco_linux_amd64.tar.gz" {
			t.Fatalf("asset mismatch: got %q", got.Name)
		}
	})
...
func TestFindChecksumManifestAsset(t *testing.T) {
	t.Run("finds checksums.txt among assets", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "silieco-cli-1.2.3-darwin-arm64.tar.gz"},
			{Name: "checksums.txt", BrowserDownloadURL: "https://example/checksums.txt"},
			{Name: "silieco-cli-1.2.3-linux-amd64.tar.gz"},
		}
...
func TestParseChecksumManifest(t *testing.T) {
	manifest := []byte(strings.Join([]string{
		...
		"",
		"aaaa1111  silieco-cli-1.2.3-darwin-arm64.tar.gz",
		"bbbb2222  silieco-cli-1.2.3-darwin-amd64.tar.gz",
		"cccc3333\tmulti-tab-separator.tar.gz",
		"DDDD4444  silieco_linux_amd64.tar.gz",
	}, "\n"))

	t.Run("returns lowercase sha for matched entry", func(t *testing.T) {
		got, err := parseChecksumManifest(manifest, "silieco-cli-1.2.3-darwin-arm64.tar.gz")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if got != "aaaa1111" {
	...
	t.Run("downcases an uppercase entry", func(t *testing.T) {
		got, err := parseChecksumManifest(manifest, "silieco_linux_amd64.tar.gz")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if got != "dddd4444" {
	...
	t.Run("skips blank lines and comments", func(t *testing.T) {
		...
		if _, err := parseChecksumManifest(manifest, "silieco-cli-1.2.3-darwin-arm64.tar.gz"); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
}
