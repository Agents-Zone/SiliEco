...
// *NetworkError / *HTTPError), so command-level guidance — e.g. a `silieco
...
var kindMessages = map[ErrorKind][2]string{
	KindNetworkTimeout: {
		"Request timed out: the server did not respond in time. Check your network connection or try again later. You can raise the limit with SILIECO_HTTP_TIMEOUT.",
		"请求超时：服务器未在规定时间内响应。请检查网络连接或稍后重试。可通过 SILIECO_HTTP_TIMEOUT 调高超时时间。",
	},
	KindNetworkDNS: {
		"Could not resolve the Silieco server address. Check your network connection or the --server-url setting.",
		"无法解析 Silieco 服务器地址。请检查网络连接或 --server-url 配置。",
	},
	KindNetworkRefused: {
		"Could not connect to the Silieco server. Make sure the server address is correct and reachable.",
		"无法连接到 Silieco 服务器。请确认服务器地址正确且网络可达。",
	},
	KindNetworkTLS: {
		"Could not establish a secure connection to the Silieco server (TLS/certificate error). Check your system clock and CA certificates.",
		"无法与 Silieco 服务器建立安全连接（TLS/证书错误）。请检查系统时间和 CA 证书。",
	},
	KindNetworkOffline: {
		"Could not reach the Silieco server. Check your network connection.",
		"无法访问 Silieco 服务器。请检查网络连接。",
	},
	KindAuthRequired: {
		"Your session has expired or you are not signed in. Run `silieco login` to sign in again. On a self-hosted or non-OAuth setup, ask your administrator for valid credentials.",
		"登录已过期或尚未登录。请运行 `silieco login` 重新登录。自托管或非 OAuth 场景请联系管理员获取有效凭证。",
	},
	KindForbidden: {
	...
	KindServerError: {
		"The Silieco service is temporarily unavailable (server error). Please try again later; if it persists, contact support. Re-run with --debug to see the raw server response.",
		"Silieco 服务暂时不可用（服务器错误）。请稍后重试；若持续出现请联系支持。可加 --debug 查看服务器原始响应。",
	},
	KindUnknown: {
...
// true (or SILIECO_DEBUG is set) it additionally prints the full original
...
// details from typed errors, for --debug / SILIECO_DEBUG output.
...
// debugEnabled reports whether SILIECO_DEBUG requests debug output.
func debugEnabled() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("SILIECO_DEBUG"))) {
	case "", "0", "false", "no", "off":
		return false
	default:
		return true
