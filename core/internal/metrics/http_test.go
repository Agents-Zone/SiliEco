...
func TestHTTPMiddlewareUsesRoutePatternLabels(t *testing.T) {
	...
	for _, want := range []string{
		`silieco_http_requests_total{method="GET",route="/api/issues/{id}",status="201"} 1`,
		`silieco_build_info{commit="abc123",version="v-test"} 1`,
	} {
...
func TestHTTPMiddlewareRecordsDaemonWorkspaceResponseSizeByStatus(t *testing.T) {
	...
	body := metricsRec.Body.String()
	for _, want := range []string{
		`silieco_http_daemon_workspace_response_size_bytes_count{status="200"} 1`,
		`silieco_http_daemon_workspace_response_size_bytes_sum{status="200"} 2`,
		`silieco_http_daemon_workspace_response_size_bytes_count{status="304"} 1`,
		`silieco_http_daemon_workspace_response_size_bytes_sum{status="304"} 0`,
	} {
		if !strings.Contains(body, want) {
			t.Fatalf("metrics body missing %q\n%s", want, body)
		}
...
func TestMetricsHandlerOnlyServesMetricsPath(t *testing.T) {
	...
	if body, _ := io.ReadAll(rec.Body); !strings.Contains(string(body), "silieco_build_info") {
