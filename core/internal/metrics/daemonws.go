...
func NewDaemonWSCollector(m *daemonws.Metrics) *DaemonWSCollector {
	return &DaemonWSCollector{
		...
		wakeupDeliveredTotal: prometheus.NewDesc("silieco_daemonws_wakeup_delivered_total", "Total daemon wakeup local delivery attempts.", []string{"result"}, nil),
	}
}

func newDaemonWSDesc(name, help string) *prometheus.Desc {
	return prometheus.NewDesc("silieco_daemonws_"+name, help, nil, nil)
}
