...
// PostHog. As of SILI-4127 every server-side event is metrics-only, so none of
...
func TestRecordEventSkipsPostHogForMetricsOnly(t *testing.T) {
	...
	// Product-behaviour event: since SILI-4127 it is metrics-only too — the
	...
	if len(spy.names) != 0 {
		t.Fatalf("workspace_created shipped to PostHog, want 0 (metrics-only since SILI-4127): %v", spy.names)
	}
	if metrics.SumAllCounters(m) <= before {
