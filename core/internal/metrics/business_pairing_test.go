...
// core/internal/analytics/events.go has a paired Prometheus counter
...
import (
	...
	"github.com/silieco-ai/silieco/core/internal/metrics"
)
...
// core/internal/handler and core/internal/service and asserts that every
...
// repoRoot returns the absolute path to core/. The test sources live in
// core/internal/metrics/ so two parents up is the server root.
func repoRoot(t *testing.T) string {
	_, thisFile, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatalf("runtime.Caller failed")
	}
	// .../core/internal/metrics/business_pairing_test.go → .../server
...
// and resolve the alias for `core/internal/metrics` per-file, so any
...
// and cmd/core/ uses one of the two names, and goimports/`gofmt`
