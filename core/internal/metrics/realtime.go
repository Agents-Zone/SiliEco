...
func NewRealtimeCollector(m *realtime.Metrics) *RealtimeCollector {
	return &RealtimeCollector{
		...
		redisMirrorErrors:   prometheus.NewDesc("silieco_realtime_redis_mirror_errors_total", "Total Redis mirror write errors by the realtime relay.", []string{"target"}, nil),
...
func newRealtimeDesc(name, help string) *prometheus.Desc {
	return prometheus.NewDesc("silieco_realtime_"+name, help, nil, nil)
}
