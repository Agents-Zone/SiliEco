...
import (
	...
	"github.com/silieco-ai/silieco/core/internal/agenttmpl"
...
type CreateAgentFromTemplateRequest struct {
	...
	// (SILI-3963). When permission_mode is present it is authoritative and
...
func (h *Handler) CreateAgentFromTemplate(w http.ResponseWriter, r *http.Request) {
	...
	// Resolve invocation permission (SILI-3963) — mirrors CreateAgent so the
	...
	// Persist the invocation allow-list (SILI-3963) inside the same tx as the
	...
	// did this; the template path was diverging until SILI-4010).
	...
	// Reflect the invocation-permission state we just persisted (SILI-4010).
