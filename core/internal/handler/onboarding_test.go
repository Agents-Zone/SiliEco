...
func TestJoinCloudWaitlistRecordsEmailAndReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-ok@silieco.ai")
...
func TestJoinCloudWaitlistAllowsEmptyReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-noreason@silieco.ai")
...
func TestJoinCloudWaitlistMissingEmailReturns400(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-missing@silieco.ai")
...
func TestJoinCloudWaitlistRejectsOverlongReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-long@silieco.ai")
...
func TestJoinCloudWaitlistSecondCallOverwrites(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-overwrite@silieco.ai")
...
func TestBootstrapOnboardingRuntimeCreatesSingleGuideIssue(t *testing.T) {
	...
	if avatarURL == nil || *avatarURL != onboardingAssistantAvatarURL {
		t.Fatalf("agent avatar_url = %v, want seeded Sili Agent avatar", avatarURL)
	}
...
func TestBootstrapOnboardingRuntime_WithStarterPrompt(t *testing.T) {
	...
	const wantPrompt = "Introduce Silieco to me, please."
	body := map[string]string{
		"workspace_id":   testWorkspaceID,
		"runtime_id":     testRuntimeID,
...
func TestBootstrapOnboardingNoRuntimeCreatesSingleGuideIssue(t *testing.T) {
	...
	for _, want := range []string{
		"Try Silieco first",
		"https://silieco.ai/docs/install-agent-runtime",
...
// since SILI-4127, so PatchOnboarding's emissions are observable ONLY
...
// The in-flow questionnaire is role + use_case only (SILI-5159): its
...
func TestPatchOnboardingSplitsQuestionnaireAndSourceEvents(t *testing.T) {
	userID := newWaitlistTestUser(t, "onboarding-event-split@silieco.ai")

	m := obsmetrics.NewBusinessMetrics()
	h := *testHandler
	h.Metrics = m

	const questionnaireCounter = "silieco_onboarding_questionnaire_submitted_total"
	const sourceCounter = "silieco_onboarding_source_submitted_total"
...
func TestPatchOnboardingAllSkippedResolvesBothCounters(t *testing.T) {
	userID := newWaitlistTestUser(t, "onboarding-source-skip@silieco.ai")
	...
	if got := counterValue(t, m, "silieco_onboarding_questionnaire_submitted_total"); got != 1 {
		t.Fatalf("expected questionnaire counter 1 for all-skip, got %v", got)
	}
	if got := counterValue(t, m, "silieco_onboarding_source_submitted_total"); got != 1 {
