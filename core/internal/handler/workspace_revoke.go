...
func (h *Handler) revokeAndRemoveMember(ctx context.Context, workspaceID, userID, memberID, archivedBy pgtype.UUID) (revocationResult, error) {
	...
	// a removed member's IM bindings vanished automatically. SILI-3515 §4 dropped
	...
	if err := qtx.DeleteChannelUserBindingsByWorkspaceMember(ctx, db.DeleteChannelUserBindingsByWorkspaceMemberParams{
		WorkspaceID:   workspaceID,
		SiliecoUserID: userID,
	}); err != nil {
		return empty, err
	}

	// agent_invocation_target carries member-target grants with NO database FK
	// (SILI-3963 keeps the new table FK-free, matching the SILI-3515 channel
	...
	// Group-chat participant rows are application-managed and intentionally
	// have no database foreign keys. Remove the departing member here so a
	// kick or leave/rejoin cycle cannot retain access to old transcripts.
	if err := qtx.DeleteMemberFromGroupChatsInWorkspace(ctx, db.DeleteMemberFromGroupChatsInWorkspaceParams{
		WorkspaceID:   workspaceID,
