...
func TestGetConfigIncludesRuntimeAuthConfig(t *testing.T) {
	...
	t.Setenv("SILIECO_PUBLIC_URL", "https://api.example.com/")
	t.Setenv("SILIECO_APP_URL", "https://app.example.com/")
...
func TestGetConfigUsesAppURLForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("SILIECO_PUBLIC_URL", "")
	t.Setenv("SILIECO_APP_URL", "https://silieco.internal.example/")
	...
	if cfg.DaemonServerURL != "https://silieco.internal.example" {
		t.Fatalf("daemon_server_url: want same-origin URL, got %q", cfg.DaemonServerURL)
	}
	if cfg.DaemonAppURL != "https://silieco.internal.example" {
...
func TestGetConfigUsesFrontendOriginForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("SILIECO_PUBLIC_URL", "")
	t.Setenv("SILIECO_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://silieco.internal.example/")
...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	t.Setenv("SILIECO_PUBLIC_URL", "https://api.silieco.ai")
	t.Setenv("SILIECO_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://silieco.ai")
...
// frontend is silieco.ai, but the deployment does not set SILIECO_PUBLIC_URL to
// the api host. Previously this fell through to the same-origin branch and
// emitted daemon_server_url=https://silieco.ai, which the dialog turned into
// `silieco setup self-host --server-url https://silieco.ai` — pointing the
...
// setup URLs are omitted and the dialog falls back to `silieco setup`.
...
// cloud frontend when it is configured through SILIECO_APP_URL.
func TestGetConfigOmitsCloudDaemonSetupForConfiguredAppURL(t *testing.T) {
	t.Setenv("SILIECO_PUBLIC_URL", "")
	t.Setenv("SILIECO_APP_URL", "https://silieco.ai")
...
func TestURLHostEqualsCanonicalizesCommonHostForms(t *testing.T) {
	tests := []struct {
		...
		{name: "full URL", raw: "https://api.silieco.ai", want: true},
		{name: "bare host", raw: "api.silieco.ai", want: true},
		{name: "host port", raw: "api.silieco.ai:8080", want: true},
		{name: "trailing dot", raw: "https://api.silieco.ai.", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlHostEquals(tt.raw, "api.silieco.ai"); got != tt.want {
...
func TestGetConfigExposesServerVersion(t *testing.T) {
	...
	t.Setenv("SILIECO_APP_URL", "https://silieco.self-hosted.example")
	t.Setenv("FRONTEND_ORIGIN", "")
...
// suppressed on the managed cloud (frontend host silieco.ai) even when the
...
func TestGetConfigOmitsServerVersionOnOfficialCloud(t *testing.T) {
	...
	// Official cloud: frontend host silieco.ai -> version omitted.
	t.Setenv("SILIECO_APP_URL", "https://silieco.ai")
	t.Setenv("FRONTEND_ORIGIN", "")
	w := httptest.NewRecorder()
	testHandler.GetConfig(w, req)
