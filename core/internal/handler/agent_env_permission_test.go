...
// (SILI-5438), workspace owner/admin must keep working, and a NULL
...
// TestAgentEnv_AgentOwnerMemberCanRevealAndUpdate is the SILI-5438 fix:
...
func TestAgentEnv_AgentOwnerMemberCanRevealAndUpdate(t *testing.T) {
	...
	agentID, ownerUserID := agentEnvOwnerFixture(t, "env-owner-member-agent", "env-owner-member@silieco.test")
...
// SILI-5438 at exactly one agent: ownership, not membership, is what
...
func TestAgentEnv_UnrelatedMemberForbidden(t *testing.T) {
	...
	agentID, _ := agentEnvOwnerFixture(t, "env-unrelated-target-agent", "env-target-owner@silieco.test")
	strangerID := createPermissionTestMember(t, "env-stranger@silieco.test")
...
func TestAgentEnv_WorkspaceRolesUnchanged(t *testing.T) {
	...
	agentID, _ := agentEnvOwnerFixture(t, "env-role-check-agent", "env-role-check-owner@silieco.test")

	adminID := createPermissionTestMember(t, "env-admin@silieco.test")
...
// TestAgentEnv_AgentActorRejectedForOwnedAgent is the boundary SILI-5438
...
func TestAgentEnv_AgentActorRejectedForOwnedAgent(t *testing.T) {
	...
	targetID, ownerUserID := agentEnvOwnerFixture(t, "env-actor-target-agent", "env-actor-owner@silieco.test")
