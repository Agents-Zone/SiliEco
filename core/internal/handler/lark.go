...
//     SILIECO_LARK_SECRET_KEY) AND the APIClient.IsConfigured signal
...
// can unbind the bot they bound (SILI-4213). When the bound agent has
...
// Silieco account. The token only proves "this open_id requested
...
func (h *Handler) RedeemLarkBindingToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, lark.ErrBindingAlreadyAssigned):
			writeError(w, http.StatusConflict, "this Lark account is already bound to a different Silieco user")
...
// workspace admin (SILI-4213). The agent_id query param picks which
// Silieco Agent the new Bot will be bound to; the agent must belong to
...
// without exposing it to unrelated members (SILI-4213). Unknown /
