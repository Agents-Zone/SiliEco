...
VALUES ($1, 123456789, 'silieco-ai', 'silieco', 3366, 987654321, 'abc123', 15368, 'completed', now())
`, wsID); err != nil {
...
VALUES ($1, 123456789, 'silieco-ai', 'silieco', 5265,
	'Workspace cleanup snapshot', 'open', 'https://github.com/silieco-ai/silieco/pull/5265',
	now(), now(), 'head-a')
RETURNING id
...
`, wsID, testUserID); err != nil {
	...
	t.Run("normalizes valid repos", func(t *testing.T) {
		...
		req := newRequest("PATCH", "/api/workspaces/"+wsID, map[string]any{
			"repos": []map[string]any{
				{
					"url":         "  https://github.com/silieco-ai/silieco.git  ",
				...
				{
					"url": "git@github.com:silieco-ai/silieco-cloud.git",
				},
			},
		})
		req = withURLParam(req, "id", wsID)
		...
		if repos[0].URL != "https://github.com/silieco-ai/silieco.git" || repos[0].Description != "main monorepo" {
			t.Fatalf("first repo not normalized: %+v", repos[0])
		}
		if repos[1].URL != "git@github.com:silieco-ai/silieco-cloud.git" {
...
`, wsID, testUserID); err != nil {
	...
	targetEmail := fmt.Sprintf("revocation-%s@silieco.ai", slug)
...
VALUES ($1, $2, 'Target Runtime', 'local', 'silieco_daemon', 'online', '', '{}'::jsonb, $3, now())
RETURNING id
...
// replacement for the channel_user_binding member-FK cascade (SILI-3515 §4):
...
func TestDeleteMember_PrunesChannelUserBindings(t *testing.T) {
	...
	// channel_* rows have no FK to workspace (SILI-3515 §4), so the fixture's
...
`, fx.WorkspaceID, fx.AgentID, appID, testUserID).Scan(&installID); err != nil {
	...
	if _, err := testPool.Exec(ctx, `
INSERT INTO channel_user_binding (workspace_id, silieco_user_id, installation_id, channel_type, channel_user_id)
...
VALUES ($1, $2, 'Other Runtime', 'local', 'silieco_daemon', 'online', '', '{}'::jsonb, $3, now())
RETURNING id
...
`, wsID, testUserID); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id
`, "Revocation No Runtimes Target", "revocation-no-runtimes@silieco.ai").Scan(&targetUserID); err != nil {
		t.Fatalf("create target user: %v", err)
	}
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workspace WHERE id = $1`, wsID)
