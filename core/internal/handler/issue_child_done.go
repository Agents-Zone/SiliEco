...
// self-mention loops, planner ping-pong, and accidental `SILI-` prefix
...
//     unwanted auto-activation reported in #4320 / SILI-3497. A parked
...
//     comment entirely (Bohan's call on SILI-2538) also sidesteps the
//     mention question — no comment, no mention, no inbox row.
//   - the completion must close a STAGE barrier (SILI-3508). Sub-issues under
...
// parent assignee — Bohan's product call on SILI-2538 ("system child-done
...
func (h *Handler) notifyParentOfChildDone(ctx context.Context, prev, issue db.Issue) {
	...
	// activation reported in #4320 / SILI-3497. Skip the whole notification so
	...
	// whole notification (comment + mention + inbox row) — SILI-2538.
	if parent.AssigneeType.Valid && parent.AssigneeType.String == "member" {
		return
	}

	// Stage barrier (SILI-3508 / discussion #4320). The notification + assignee
...
// dedup, and the outcome depended on issue_ids order (SILI-4155). Aggregating
...
func (h *Handler) postChildDoneComment(ctx context.Context, parent, completed db.Issue, children []db.Issue, staged bool, closedStage int32, batch bool) {
	...
	if staged {
		...
		if batch {
			content = fmt.Sprintf(
				"%sAll sub-issues are complete — they just finished together in a batch update, most recently [%s](mention://issue/%s) — \"%s\". Continue the parent: synthesize the children's results and move it forward, or — if nothing remains — run `silieco issue status %s in_review` to mark the parent ready for review.",
				mentionPrefix, identifier, childID, title, parentID,
			)
		} else {
			content = fmt.Sprintf(
				"%sAll sub-issues are complete — the last one, [%s](mention://issue/%s) — \"%s\", just finished. Continue the parent: synthesize the children's results and move it forward, or — if nothing remains — run `silieco issue status %s in_review` to mark the parent ready for review.",
				mentionPrefix, identifier, childID, title, parentID,
			)
		}
	}
...
//     mid-workflow (SILI-4062 / #4927). The message now names both possibilities
...
//     parent silently stalled in in_progress (SILI-3969). The squad path now
//     mirrors the agent path (SILI-2808): always dispatch, bounded only by
//     idempotency.
...
// stranded those parents (SILI-2808). Runaway re-triggering is prevented by
...
//     to its own squad" pattern (SILI-3969).
...
//     parents advanced fine (SILI-4063 / GH #4928). Removed so agent and squad
