...
// full CreateComment behavior for the scenario reported in SILI-4015:
//
...
// the private-leader case of the SILI-4015 regression. The default agent
...
func TestCreateComment_WorkerAgentCommentWakesPrivateSquadLeader_MUL4015(t *testing.T) {
	...
	leaderID := privateAgent("SILI-4015 Private Leader")
	workerID := privateAgent("SILI-4015 Private Worker")

	// Squad with the private leader.
	var squadID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO squad (workspace_id, name, description, leader_id, creator_id)
		VALUES ($1, 'SILI-4015 Private Squad', '', $2, $3)
		RETURNING id
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, 'member', $2, 'private squad worker-comment SILI-4015', 'squad', $3)
		RETURNING id
	...
	if leaderTasksQueued != 1 {
		t.Fatalf("after worker done: expected 1 queued leader task for private L, got %d — leader→worker→leader loop broken for private leader (SILI-4015)",
			leaderTasksQueued)
	}
}
