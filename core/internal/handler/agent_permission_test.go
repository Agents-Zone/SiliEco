...
// legacy-visibility mapping (SILI-3963) at the API layer — the same mapping the
...
func TestCanInvokeAgent_PublicToMemberWhitelist(t *testing.T) {
	...
	allowedMember := createPermissionTestMember(t, "perm-allowed-member@silieco.test")
	otherMember := createPermissionTestMember(t, "perm-other-member@silieco.test")
...
// --- SILI-3963 follow-up: stackable / mixed / batch-replaced targets -------
...
func TestCanInvokeAgent_MixedMemberAndTeamTargets(t *testing.T) {
	...
	memberA := createPermissionTestMember(t, "perm-mix-a@silieco.test")
	memberB := createPermissionTestMember(t, "perm-mix-b@silieco.test")
...
func TestUpdateAgent_BatchReplaceOverlappingMembers(t *testing.T) {
	...
	memberA := createPermissionTestMember(t, "perm-batch-a@silieco.test")
	memberB := createPermissionTestMember(t, "perm-batch-b@silieco.test")
	memberC := createPermissionTestMember(t, "perm-batch-c@silieco.test")
...
func TestUpdateAgent_WorkspaceStacksWithMembersThenNarrowed(t *testing.T) {
	...
	memberA := createPermissionTestMember(t, "perm-stack-a@silieco.test")
	memberB := createPermissionTestMember(t, "perm-stack-b@silieco.test")
	...
	memberC := createPermissionTestMember(t, "perm-stack-c@silieco.test")
	w := httptest.NewRecorder()
	r := newRequest("PUT", "/api/agents/"+agentID, map[string]any{
		"permission_mode": "public_to",
		"invocation_targets": []map[string]any{
...
// TestCreateAgent_EmptyPublicToNormalizesToWorkspace locks the SILI-3963 review
...
func TestCreateAgent_EmptyPublicToNormalizesToWorkspace(t *testing.T) {
	...
	someMember := createPermissionTestMember(t, "perm-emptypublic-m@silieco.test")
...
// product-approved exception (SILI-3963): a system / no-human-originator trigger
...
func TestCanInvokeAgent_SystemWorkspaceExceptionAndMemberFailClosed(t *testing.T) {
	...
	memberX := createPermissionTestMember(t, "perm-sys-failclosed@silieco.test")
...
// TestRevokeMember_ClearsInvocationTargets is the SILI-3963 review regression:
...
func TestRevokeMember_ClearsInvocationTargets(t *testing.T) {
	...
	memberX := createPermissionTestMember(t, "perm-revoke-x@silieco.test")
...
// fix (SILI-3963): removing a user from ONE workspace must only prune their
...
func TestRevokeMember_InvocationTargetCleanupIsWorkspaceScoped(t *testing.T) {
	...
	userX := createPermissionTestMember(t, "perm-xws@silieco.test")
...
func TestUpdateAgent_AccessChangeIsOwnerOnly(t *testing.T) {
	...
	adminID := createPermissionTestAdmin(t, "perm-access-admin@silieco.test")
...
func TestUpdateAgent_LegacyVisibilityNoOpForMemberOnlyPublicTo(t *testing.T) {
	...
	memberX := createPermissionTestMember(t, "perm-legacyvis-x@silieco.test")
	agentID := createPublicToAgentWithTargets(t, "legacy-vis-member-only-agent", []map[string]any{
		{"target_type": "member", "target_id": memberX},
	})
	adminID := createPermissionTestAdmin(t, "perm-legacyvis-admin@silieco.test")
