...
// and — the core of SILI-4062 — must NOT assert finality when no later stage
...
func TestStageAdvanceInstruction(t *testing.T) {
	...
	t.Run("no created next stage does not assert finality", func(t *testing.T) {
		...
		// Regression guard for SILI-4062: an intermediate stage in a lazily
		...
		if !strings.Contains(got, "silieco issue status "+parentID+" in_review") {
