...
// channel table-of-contents (`silieco chat history`), Thread reads one thread's
// messages (`silieco chat thread [id]`). Both are scoped server-side to the
// session's own channel (SILI-3871).
...
// GetChatChannelHistory serves `silieco chat history` — the channel overview:
// recent top-level messages, each thread tagged with its id + reply count (no
// thread contents). The agent drills into a thread with `silieco chat thread`.
...
// GetChatThread serves `silieco chat thread [id]` — one thread's messages. With
...
// sili_ PAT leaves X-Actor-Source empty and does NOT strip a client-forged
