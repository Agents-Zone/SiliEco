...
func TestProjectResourceLifecycle(t *testing.T) {
	...
	if ref.URL != "https://github.com/silieco-ai/silieco" {
...
func TestProjectResourceAcceptsSSHRepoURLs(t *testing.T) {
	...
	cases := []struct {
		...
		{"scp-like", "git@github.com:silieco-ai/silieco.git"},
		{"ssh-scheme", "ssh://git@github.com/silieco-ai/silieco.git"},
	}
...
func TestIsValidGitRepoURL(t *testing.T) {
	good := []string{
		...
		"https://github.com/silieco-ai/silieco.git",
		"http://github.example.com/x/y",
		"ssh://git@github.com/silieco-ai/silieco.git",
		"ssh://git@github.com:22/silieco-ai/silieco.git",
		"git@github.com:silieco-ai/silieco.git",
		"git@gitlab.example.com:group/sub/repo.git",
	}
	bad := []string{
		"",
		"not-a-url",
		"github.com/silieco-ai/silieco", // no scheme, no scp-style colon
		"https://",                      // empty host
...
// local_directory resource type added in SILI-2662. Unlike github_repo, the
...
func TestCreateProjectAttachesResources(t *testing.T) {
	...
	req := newRequest("POST", "/api/projects?workspace_id="+testWorkspaceID, map[string]any{
		...
		"resources": []map[string]any{
			{
				"resource_type": "github_repo",
				"resource_ref":  map[string]any{"url": "https://github.com/silieco-ai/silieco"},
			},
		},
	})
	testHandler.CreateProject(w, req)
...
func TestProjectResourceCountBreadcrumb(t *testing.T) {
	...
	req = newRequest("POST", "/api/projects/"+project.ID+"/resources", map[string]any{
		"resource_type": "github_repo",
		"resource_ref":  map[string]any{"url": "https://github.com/silieco-ai/breadcrumb"},
	})
	req = withURLParam(req, "id", project.ID)
	testHandler.CreateProjectResource(w, req)
	if w.Code != http.StatusCreated {
...
func TestCreateProjectWithResourcesEchoesCount(t *testing.T) {
	...
	req := newRequest("POST", "/api/projects?workspace_id="+testWorkspaceID, map[string]any{
		...
		"resources": []map[string]any{
			{
				"resource_type": "github_repo",
				"resource_ref":  map[string]any{"url": "https://github.com/silieco-ai/echo-count"},
			},
		},
	})
	testHandler.CreateProject(w, req)
...
// TestProjectResourceUpdateLifecycle covers the PUT endpoint added in SILI-2662:
