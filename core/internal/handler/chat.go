...
func (h *Handler) CreateChatSession(w http.ResponseWriter, r *http.Request) {
	...
	if req.Kind == "group" {
		h.createGroupChatSession(w, r, userID, workspaceID, req)
		return
	}
	if req.Kind != "" && req.Kind != "direct" {
	...
	// invoke permission (SILI-3963), not the softer view gate. Agent-to-agent
...
func (h *Handler) createGroupChatSession(w http.ResponseWriter, r *http.Request, userID, workspaceID string, req CreateChatSessionRequest) {
	...
	memberIDs := []pgtype.UUID{creatorUUID}
	seenMembers := map[string]struct{}{userID: {}}
	for _, raw := range req.MemberIDs {
		id, ok := parseUUIDOrBadRequest(w, strings.TrimSpace(raw), "member_id")
		if !ok {
			return
		}
		key := uuidToString(id)
		...
		if _, err := h.Queries.GetMemberByUserAndWorkspace(r.Context(), db.GetMemberByUserAndWorkspaceParams{
			UserID: id, WorkspaceID: workspaceUUID,
		}); err != nil {
			writeError(w, http.StatusBadRequest, "member is not in this workspace")
			return
		}
	...
	seenAgents := make(map[string]struct{}, len(req.AgentIDs))
	for _, raw := range req.AgentIDs {
		id, ok := parseUUIDOrBadRequest(w, strings.TrimSpace(raw), "agent_id")
		if !ok {
			return
		}
		key := uuidToString(id)
		...
		agent, err := h.Queries.GetAgentInWorkspace(r.Context(), db.GetAgentInWorkspaceParams{
			ID: id, WorkspaceID: workspaceUUID,
		})
		if err != nil || agent.ArchivedAt.Valid {
			writeError(w, http.StatusBadRequest, "agent is not available in this workspace")
			return
		}
	...
	session, err := qtx.CreateGroupChatSession(r.Context(), db.CreateGroupChatSessionParams{
		WorkspaceID: workspaceUUID,
	...
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to create group chat")
		return
	}
	for _, id := range memberIDs {
		if err := qtx.AddChatSessionParticipant(r.Context(), db.AddChatSessionParticipantParams{
			ChatSessionID: session.ID, ParticipantType: "member", ParticipantID: id, CreatedBy: creatorUUID,
		}); err != nil {
	...
	for _, id := range agentIDs {
		if err := qtx.AddChatSessionParticipant(r.Context(), db.AddChatSessionParticipantParams{
			ChatSessionID: session.ID, ParticipantType: "agent", ParticipantID: id, CreatedBy: creatorUUID,
		}); err != nil {
			writeError(w, http.StatusInternalServerError, "failed to add group agents")
			return
		}
	}
	if err := tx.Commit(r.Context()); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to commit group chat")
		return
	}
	resp := chatSessionToResponse(session)
	resp.Participants = h.chatSessionParticipants(r.Context(), session.ID)
	h.publishChat(protocol.EventChatSessionUpdated, workspaceID, "member", userID, "", protocol.ChatSessionUpdatedPayload{
		ChatSessionID: uuidToString(session.ID), Title: session.Title, UpdatedAt: timestampToString(session.UpdatedAt), ParticipantsChanged: true,
	})
	writeJSON(w, http.StatusCreated, resp)
}

type UpdateGroupChatParticipantsRequest struct {
	AgentIDs  []string `json:"agent_ids"`
	MemberIDs []string `json:"member_ids"`
}

// UpdateGroupChatParticipants adds workspace-owned identities to a group's
// roster. The first release is intentionally additive: removing a participant
// also requires revoking an already-open realtime scope, so that lifecycle is
// deferred until the hub can enforce it atomically. Only the creator or a
// workspace owner/admin may invite participants.
func (h *Handler) UpdateGroupChatParticipants(w http.ResponseWriter, r *http.Request) {
	userID, ok := requireUserID(w, r)
	if !ok {
		return
	}
	...
	if session.Kind != "group" {
		writeError(w, http.StatusBadRequest, "participants can only be updated for group chats")
		return
	}
	...
	if uuidToString(session.CreatorID) != userID && member.Role != "owner" && member.Role != "admin" {
		writeError(w, http.StatusForbidden, "only the group creator or a workspace admin can update participants")
		return
	}

	var req UpdateGroupChatParticipantsRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		writeError(w, http.StatusBadRequest, "invalid request body")
		return
	}
	...
	memberIDs := []pgtype.UUID{session.CreatorID}
	seenMembers := map[string]struct{}{uuidToString(session.CreatorID): {}}
	for _, raw := range req.MemberIDs {
		id, valid := parseUUIDOrBadRequest(w, strings.TrimSpace(raw), "member_id")
		if !valid {
			return
		}
		key := uuidToString(id)
	...
	seenAgents := make(map[string]struct{}, len(req.AgentIDs))
	for _, raw := range req.AgentIDs {
		id, valid := parseUUIDOrBadRequest(w, strings.TrimSpace(raw), "agent_id")
		if !valid {
			return
		}
		key := uuidToString(id)
	...
	if _, err := qtx.LockChatSessionForRuntimeBind(r.Context(), session.ID); err != nil {
		writeError(w, http.StatusNotFound, "group chat not found")
		return
	}
	actorUUID := parseUUID(userID)
	for _, id := range memberIDs {
		if err := qtx.AddChatSessionParticipant(r.Context(), db.AddChatSessionParticipantParams{
			ChatSessionID: session.ID, ParticipantType: "member", ParticipantID: id, CreatedBy: actorUUID,
		}); err != nil {
	...
	for _, id := range agentIDs {
		if err := qtx.AddChatSessionParticipant(r.Context(), db.AddChatSessionParticipantParams{
			ChatSessionID: session.ID, ParticipantType: "agent", ParticipantID: id, CreatedBy: actorUUID,
		}); err != nil {
			writeError(w, http.StatusInternalServerError, "failed to update group agents")
			return
		}
	}
	if err := tx.Commit(r.Context()); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to commit group participant update")
		return
	}

	resp := chatSessionToResponse(session)
	resp.Participants = h.chatSessionParticipants(r.Context(), session.ID)
	// Workspace-scoped (empty routing ChatSessionID): newly invited members are
	// not subscribed to this chat yet, so a chat-scoped event could never tell
	// their session list to discover it.
	h.publishChat(protocol.EventChatSessionUpdated, workspaceID, "member", userID, "", protocol.ChatSessionUpdatedPayload{
		ChatSessionID: uuidToString(session.ID), Title: session.Title, UpdatedAt: timestampToString(session.UpdatedAt), ParticipantsChanged: true,
	})
...
func (h *Handler) ListChatSessions(w http.ResponseWriter, r *http.Request) {
	...
	if status == "all" {
		...
		for _, s := range rows {
			...
			if s.Kind == "group" {
				item.Participants = h.chatSessionParticipants(r.Context(), s.ID)
			}
...
func (h *Handler) loadChatSessionForUser(w http.ResponseWriter, r *http.Request, userID, workspaceID, sessionID string) (db.ChatSession, bool) {
	...
	if session.Kind == "group" {
		isParticipant, err := h.Queries.IsChatSessionMemberParticipant(r.Context(), db.IsChatSessionMemberParticipantParams{
			ChatSessionID: session.ID,
			ParticipantID: parseUUID(userID),
		})
		if err != nil || !isParticipant {
			writeError(w, http.StatusForbidden, "not a participant in this group chat")
			return db.ChatSession{}, false
		}
	} else if uuidToString(session.CreatorID) != userID {
...
// stuck unread badge — after the user archived it (SILI-4372). Dropping the
...
func (h *Handler) DeleteChatSession(w http.ResponseWriter, r *http.Request) {
	...
	// ON DELETE CASCADE; SILI-3515 §4 dropped every channel_* foreign key, so
	...
	if err := qtx.DeleteChatSessionParticipants(r.Context(), session.ID); err != nil {
	...
	if session.Kind == "direct" && session.AgentID.Valid {
...
func (h *Handler) SendChatMessage(w http.ResponseWriter, r *http.Request) {
	...
	if session.Kind == "group" {
		h.sendGroupChatMessage(w, r, userID, workspaceID, session, req, attachmentIDs)
		return
	}
	...
	// gateChatSessionForUser (SILI-4525). canAccessPrivateAgent lets a workspace
	...
	// BEFORE we insert the new row. This scopes LLM auto-titling (SILI-4295) to
	...
	// Persist the whole turn atomically (SILI-4351): the owning task, the user
	...
	// First user message → kick off best-effort LLM auto-titling (SILI-4295).
...
func (h *Handler) sendGroupChatMessage(
	...
	for _, mention := range util.ParseMentions(req.Content) {
		...
		inGroup, err := h.Queries.IsChatSessionAgentParticipant(r.Context(), db.IsChatSessionAgentParticipantParams{
			ChatSessionID: session.ID,
			ParticipantID: agentID,
		})
		if err != nil || !inGroup {
			writeError(w, http.StatusBadRequest, "mentioned agent is not in this group chat")
			return
		}
		agent, err := h.Queries.GetAgentInWorkspace(r.Context(), db.GetAgentInWorkspaceParams{
			ID: agentID, WorkspaceID: workspaceUUID,
		})
		if err != nil || agent.ArchivedAt.Valid {
			writeError(w, http.StatusConflict, "mentioned agent is unavailable")
			return
		}
		if !agent.RuntimeID.Valid {
			writeError(w, http.StatusConflict, "mentioned agent has no runtime")
			return
		}
		if !h.canInvokeAgent(r.Context(), agent, actorType, actorID, h.invokeOriginatorFromRequest(r, actorType, actorID), workspaceID) {
			h.writeDispatchBlocked(w, http.StatusForbidden, ReasonInvocationNotAllowed)
	...
	sent, err := h.TaskService.SendGroupChatMessage(
		r.Context(), session, parseUUID(userID), req.Content, attachmentIDs,
		actorType, parseUUID(actorID), agents,
	)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to send group chat message: "+err.Error())
		return
	}
	...
	taskIDs := make([]string, 0, len(sent.Tasks))
...
func (h *Handler) MarkChatSessionRead(w http.ResponseWriter, r *http.Request) {
	...
	var err error
	if session.Kind == "group" {
		err = h.Queries.MarkGroupChatSessionRead(r.Context(), db.MarkGroupChatSessionReadParams{
			ChatSessionID: session.ID, ParticipantID: parseUUID(userID),
		})
	} else {
		err = h.Queries.MarkChatSessionRead(r.Context(), session.ID)
	}
	if err != nil {
...
// has no FK (SILI-3515) and no reaper, so a restore left behind keeps the user's
...
func (h *Handler) ListPendingChatTasks(w http.ResponseWriter, r *http.Request) {
	...
	// scan on this hot path (SILI-4159).
...
// agent never sees a true from a task on that agent (SILI-4159). An empty
...
// alone is exactly what 404'd these tasks before (SILI-2827).
//
...
type ChatLastMessage struct {
	...
	// English fallback content the server stores (SILI-4351).
...
type ChatMessageResponse struct {
	...
	// direct-chat turn that produced no text reply (SILI-4351). Additive:
	...
	// agent can `silieco attachment download <id>` rather than guessing
...
func (h *Handler) chatSessionParticipants(ctx context.Context, sessionID pgtype.UUID) []ChatParticipantResponse {
	rows, err := h.Queries.ListChatSessionParticipants(ctx, sessionID)
	if err != nil {
		return []ChatParticipantResponse{}
	}
	result := make([]ChatParticipantResponse, 0, len(rows))
	for _, row := range rows {
		result = append(result, ChatParticipantResponse{Type: row.ParticipantType, ID: uuidToString(row.ParticipantID)})
	}
	return result
}

func chatMessageToResponse(m db.ChatMessage, attachments []AttachmentResponse) ChatMessageResponse {
	mentionedAgentIDs := make([]string, 0, len(m.MentionedAgentIds))
	for _, id := range m.MentionedAgentIds {
		mentionedAgentIDs = append(mentionedAgentIDs, uuidToString(id))
	}
	return ChatMessageResponse{
		...
		SenderAgentID:     uuidToPtr(m.SenderAgentID),
...
// never surprises an older client into a broken render (SILI-4351).
