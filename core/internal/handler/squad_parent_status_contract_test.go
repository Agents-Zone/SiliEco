...
func TestSquadAssignedLeaderCanWrapUpOnCommentTurn(t *testing.T) {
	...
	combined := briefing + "\n" + brief
	if !strings.Contains(combined, "silieco issue status <issue-id> in_review") {
...
// scope fix (SILI-3724 path): the issue belongs to a plain agent and this squad
...
func TestGuestLeaderCannotChangeStatusOnCommentTurn(t *testing.T) {
	...
	for _, want := range []string{
		...
		"never run `silieco issue status` on it",
	} {
