...
// TestCreateIssue_AgentCreate_StampsActingTaskOrigin locks the SILI-4305 fix at
...
func TestCreateIssue_AgentCreate_StampsActingTaskOrigin(t *testing.T) {
	...
	req := newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title": "Agent-created via normal create (SILI-4305)",
	})
...
// regression for SILI-4305: the agent_create stamp must only ride a genuine
...
func TestCreateIssue_NoAgentCreateStampForMemberOrForgedAgent(t *testing.T) {
	...
	assertNoAgentOrigin := func(t *testing.T, mutate func(*http.Request)) {
		...
		w := httptest.NewRecorder()
		req := newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
			"title": "No agent_create stamp expected (SILI-4305)",
		})
		if mutate != nil {
			mutate(req)
		}
