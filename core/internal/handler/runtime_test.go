...
func TestResolveViewingTZ(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Resolve', 'tz-resolve@silieco.ai', 'Asia/Tokyo') RETURNING id`,
	).Scan(&userID); err != nil {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email)
		 VALUES ('TZ Bare', 'tz-bare@silieco.ai') RETURNING id`,
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Bad', 'tz-bad@silieco.ai', 'Bad/Zone') RETURNING id`,
