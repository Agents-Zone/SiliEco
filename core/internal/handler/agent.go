...
type AgentResponse struct {
	...
	// same path. agent-actor tokens are denied there. See SILI-2600.
	HasCustomEnv      bool   `json:"has_custom_env"`
	CustomEnvKeyCount int    `json:"custom_env_key_count"`
	McpConfigRedacted bool   `json:"mcp_config_redacted"`
	Visibility        string `json:"visibility"`
	// PermissionMode is the invocation-permission mode (SILI-3963):
	...
	// per-model; the API never normalizes across providers. See SILI-2339.
	...
	// Composio connection (SILI-3963; no longer gated on originator == owner).
...
type AgentTaskResponse struct {
	...
	// withheld because its rollout was missing (SILI-5305); PriorSessionID (if
	...
	CoalescedCommentIDs      []string               `json:"coalesced_comment_ids,omitempty"`       // SILI-4195: earlier comments folded into this run when it had not yet started, so a single run still covers every deliberate comment; trigger_comment_id is the newest. Surfaced so the UI can show which comments a run covered. omitempty so old clients ignore it
	CoalescedComments        []CoalescedCommentData `json:"coalesced_comments,omitempty"`          // SILI-4195: full detail (thread_id/author/created_at/content) of the folded comments, so the daemon prompt can address each without assuming they share the triggering thread. omitempty so old clients ignore it
	...
	ChatKind                 string                 `json:"chat_kind,omitempty"`                   // direct or group
	ChatChannelType          string                 `json:"chat_channel_type,omitempty"`           // "slack" when the chat session is backed by an IM channel; empty for a web-only chat. Makes the agent channel-aware (read history from the channel, not Silieco)
	ChatInThread             bool                   `json:"chat_in_thread,omitempty"`              // true when the latest @mention was a thread reply; tells the agent to start with `silieco chat thread` vs `silieco chat history`
	ChatMessage              string                 `json:"chat_message,omitempty"`                // user message for chat tasks
	ChatMessageAttachments   []ChatAttachmentMeta   `json:"chat_message_attachments,omitempty"`    // attachments on the user message — agent calls `silieco attachment download <id>` per entry
	...
	WorkflowInstanceID       string                 `json:"workflow_instance_id,omitempty"`        // SOP run selected in the create flow
	WorkflowStageID          string                 `json:"workflow_stage_id,omitempty"`           // Stage selected within WorkflowInstanceID
	...
	ParentIssueIdentifier    string                 `json:"parent_issue_identifier,omitempty"`     // human-readable identifier (e.g. SILI-123) of the quick-create parent issue, resolved on claim for prompt context
	...
	// the owner. The agent's effective Silieco credentials stay owner-scoped —
	// this is an attested identity, not a credential. See SILI-2645.
	...
	// (SILI-4302 §9): the source label + precise flag, the initiator (accountable)
	...
	// SILIECO_TOKEN in the agent process environment. The server binds it to
...
// (SILI-4302 §9). Source/Precise/Evidence/lineage come straight from the row (pure);
...
// `silieco attachment download <id>` rather than guessing from the
...
// into a not-yet-started run (SILI-4195) so the daemon can embed it directly in
...
func taskToResponse(t db.AgentTaskQueue, workspaceID string) AgentTaskResponse {
	...
	return AgentTaskResponse{
		...
		// hydrated separately on user-facing surfaces (SILI-4302 §9).
...
// lock-step with core/internal/daemon/execenv/git.go:shortID — both
...
func (h *Handler) ListAgents(w http.ResponseWriter, r *http.Request) {
	...
	// never serialized on agent resources anymore (SILI-2600); see the
	...
	for _, a := range agents {
		...
		// same lateral-movement vector SILI-2600 closed for custom_env.
		...
		// (SILI-3963), so surfacing the slugs to admins gives them nothing
...
func (h *Handler) GetAgent(w http.ResponseWriter, r *http.Request) {
	...
	// in SILI-2600; secrets are now fetched via GET /api/agents/{id}/env).
	userID := requestUserID(r)
...
type CreateAgentRequest struct {
	...
	// inputs (SILI-3963). When permission_mode is present it is authoritative
	...
	ThinkingLevel      string                     `json:"thinking_level"`
	ServiceTier        string                     `json:"service_tier"`
	// ComposioToolkitAllowlist seeds the per-task overlay gate (SILI-3869). On
...
func (h *Handler) CreateAgent(w http.ResponseWriter, r *http.Request) {
	...
	// Resolve invocation permission (SILI-3963). permission_mode is
	...
	// Per-model gaps are enforced by the daemon at execution time (SILI-2339):
...
// messaging its creator, not the creator prompting the agent (SILI-4230). Best
...
type UpdateAgentRequest struct {
	...
	// secret values with literal `****`. See SILI-2600.
	CustomArgs *[]string        `json:"custom_args"`
	McpConfig  *json.RawMessage `json:"mcp_config"`
	Visibility *string          `json:"visibility"`
	// PermissionMode + InvocationTargets are the invocation-permission inputs
	// (SILI-3963). Owner-only writes (like composio_toolkit_allowlist): a
	...
	MaxConcurrentTasks *int32                      `json:"max_concurrent_tasks"`
	Model              *string                     `json:"model"`
	// ThinkingLevel is treated as a tri-state per-SILI-2339:
	...
	// representation as both). SILI-3869.
	ComposioToolkitAllowlist *[]string `json:"composio_toolkit_allowlist"`
}
...
// compatibility with workspaces that flipped the setting before SILI-2600
...
// resource response (see SILI-2600), so this predicate is shared only by
...
// target's mcp_config from the mutation response. SILI-2600.
//
...
func (h *Handler) UpdateAgent(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := rawFields["custom_env"]; ok {
		writeError(w, http.StatusBadRequest, "custom_env is no longer accepted on this endpoint; use PUT /api/agents/{id}/env (or `silieco agent env set`)")
		return
	}
	...
	// Invocation permission (SILI-3963). OWNER-ONLY write: access is the one
	...
	// thinking_level handling (SILI-2339). Tri-state semantics:
	...
	// composio_toolkit_allowlist handling (SILI-3869). Tri-state semantics
	...
	// Composio overlay uses the agent OWNER's connection (SILI-3963), so an
	...
	// Invocation targets (SILI-3963): replace wholesale when the owner touched
...
// keep working; SILI-5436 tracks moving it to a dedicated lazy endpoint.
