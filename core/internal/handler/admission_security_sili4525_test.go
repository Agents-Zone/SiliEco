...
func seedSecurityTestOwner(t *testing.T, label string) string {
	...
	if err := testPool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id`,
		label, fmt.Sprintf("%s-%d@silieco.test", label, time.Now().UnixNano())).Scan(&ownerID); err != nil {
...
// TestSendChatMessage_InvokeRevokedAfterSessionCreate is the SILI-4525 must-fix 3
...
// TestRerunIssue_PrivateHistoricalAgent is the SILI-4525 must-fix 3 rerun
