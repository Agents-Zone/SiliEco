...
// (Silieco Cloud) MUST use the Redis-backed implementation, otherwise the
...
// review on silieco-ai/silieco#2009).
...
type ModelListRequest struct {
	...
	// instead of a live daemon round trip (SILI-5444). Purely informational —
...
// SILI-2339). nil means "no picker for this model"; the UI hides the
...
func (h *Handler) ReportModelListResult(w http.ResponseWriter, r *http.Request) {
	...
	if body.Status == "completed" {
		...
		// (SILI-5444). A cache write failure is not the daemon's problem — the
