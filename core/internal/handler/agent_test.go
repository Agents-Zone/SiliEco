...
func TestListWorkspaceWorkingAgents(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ('Working Agents Outsider', 'working-agents-outsider-' || gen_random_uuid()::text || '@silieco.test')
		RETURNING id
...
// SILI-2600: the generic agent resource response NEVER carries the
...
// TestAgentEnv_TaskTokenActorSource locks in the post-SILI-2600 attack
...
func TestAgentResponseShape_HasNoLegacyEnvFields(t *testing.T) {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		case "custom_env", "custom_env_redacted", "custom_env_redacted_reason":
			t.Errorf("AgentResponse must not carry %q field (SILI-2600)", tag)
		}
	}
}

// TestUpdateAgent_RedactsMcpConfigForAgentActor closes the second leg
// of SILI-2600 review #2: an agent process with a task token (or with
