...
func TestCreateAutopilot_SquadPrivateLeader_OwnerAllowed(t *testing.T) {
	...
	// The AGENT OWNER creates the autopilot — allowed under SILI-3963 (workspace
...
func TestTriggerAutopilot_SquadPrivateLeader_OwnerCanDispatch(t *testing.T) {
	...
	// Create autopilot as the AGENT OWNER (SILI-3963: only owner/allow-listed
	...
	// invoke permission (SILI-4525), so the owner (who can invoke the private
...
// SILI-4525 fork fix: manual "run now" admits on the CURRENT clicker, not the
