...
type DaemonRegisterRequest struct {
	...
	CLIVersion      string   `json:"cli_version"` // silieco CLI version
	...
	Runtimes        []struct {
		...
		// runtime_profile (SILI-3284). Empty = built-in runtime (legacy path).
...
// shared custom name (SILI-4217) when the machine is already named, so adding a
...
func (h *Handler) DaemonRegister(w http.ResponseWriter, r *http.Request) {
	...
	for _, runtime := range req.Runtimes {
		...
		// and go (SILI-4217). Shared with the failed-profile path below.
	...
	for _, failed := range req.FailedProfiles {
		...
		// doesn't drag the machine title back to the hostname (SILI-4217).
...
// cmd/core/runtime_sweeper.go). That ordering is safe and desirable:
...
// stale threshold (150s in cmd/core/runtime_sweeper.go) MINUS one daemon
...
// handlers so the batch path can't silently drop surviving comments (SILI-4257).
...
// ClaimTasksByRuntime is the machine-level (SILI-4257) batch claim endpoint. A
...
func (h *Handler) ClaimTasksByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	// Machine-level ownership (SILI-4257 review): the batch claim is scoped to a
	...
	// max_tasks semantics (SILI-4257 review): a negative count is malformed; zero
	...
	// Parse + de-dup requested ids with the NON-panicking parser (SILI-4257
...
// (ClaimTasksByRuntime, SILI-4257) so both build byte-identical payloads and
...
func (h *Handler) buildClaimedTaskResponse(r *http.Request, task *db.AgentTaskQueue, runtime db.AgentRuntime, runtimeID, runtimeWorkspaceID string) (resp AgentTaskResponse, deliveredCommentIDs []pgtype.UUID, agentSkillCount, builtinSkillCount int, failure *claimBuildFailure) {
	...
	if task.IssueID.Valid {
		if issue, err := h.Queries.GetIssue(r.Context(), task.IssueID); err == nil {
			...
			// plain agent — the SILI-3724 case), sub-issue done callback,
			...
			if resp.Agent != nil && task.IsLeaderTask && task.SquadID.Valid {
				if squad, err := h.Queries.GetSquadInWorkspace(r.Context(), db.GetSquadInWorkspaceParams{
					...
					// (see above) and therefore also fires on the SILI-3724 path,
			...
			if issue.WorkflowInstanceID.Valid && issue.WorkflowStageID.Valid {
				resp.WorkflowInstanceID = uuidToString(issue.WorkflowInstanceID)
				resp.WorkflowStageID = uuidToString(issue.WorkflowStageID)
			}
			if issue.ProjectID.Valid {
				...
				if rows := h.listProjectResourcesForProject(r.Context(), issue.ProjectID); len(rows) > 0 {
					...
					for _, row := range rows {
						...
						// so `silieco repo checkout` and the meta-skill render
		...
		// Workspace-scoped load (SILI-4252) so a foreign comment UUID resolves to
		...
		if effectiveTriggerUUID.Valid {
			...
			// to runtime.WorkspaceID below, so this is the right tenant (SILI-4252).
			if comment, err := h.Queries.GetCommentInWorkspace(r.Context(), db.GetCommentInWorkspaceParams{
				...
				// also capture the id and email. See SILI-2645.
		...
		if task.RerunOfTaskID.Valid {
			...
			// on the same issue must never hijack the resume (SILI-4869). The
			...
			if src, err := h.Queries.GetAgentTask(r.Context(), task.RerunOfTaskID); err == nil {
				...
				// SILI-5305: if the source task withheld its Codex session because
			...
			// SILI-5305: if the most recent terminal task withheld its Codex
	...
	if task.ChatSessionID.Valid {
		if cs, err := h.Queries.GetChatSession(r.Context(), task.ChatSessionID); err == nil {
			...
			// prompt (SILI-4230). The is_agent_intro column stays true for the
			...
			// agent keeps repeating the same introduction (SILI-4259).
			if cs.IsAgentIntro {
			...
			// is operating inside an IM conversation and not the Silieco web app
			// (SILI-3871). Empty for a web-only chat session.
			//
			...
			// mis-flag made the brief inject `silieco attachment upload` guidance
			// into a conversation that cannot carry attachments at all (SILI-4899).
			//
			// ChatInThread stays Slack-only on purpose. It selects between
			// `silieco chat history` and `silieco chat thread`, and those two
			...
			if !task.ForceFreshSession {
				...
				// SILI-5305: if the most recent terminal task on this chat session
			...
			// tasks have owned their single message since SILI-4351; channel
			...
			// (SILI-2968: "看上海天气" then "还有青岛" must both be delivered) —
			// so a rolling deploy never replays their history. Attachments are
			// collected per included message so the agent can
			// `silieco attachment download <id>` (the inline markdown URL is
			// signed + 30-min expiring on the CDN).
			var unanswered []db.ChatMessage
			var inputLoadErr error
			if cs.Kind == "group" && task.TriggerChatMessageID.Valid {
				var history []db.ChatMessage
				history, inputLoadErr = h.Queries.ListChatMessages(r.Context(), cs.ID)
				if inputLoadErr == nil {
					resp.ChatMessage = h.groupChatPromptTranscript(r.Context(), history, task.TriggerChatMessageID)
					trigger, err := h.Queries.GetChatMessage(r.Context(), task.TriggerChatMessageID)
					if err != nil {
						inputLoadErr = err
					} else if trigger.ChatSessionID != cs.ID {
						inputLoadErr = errors.New("group chat trigger message belongs to another session")
					} else {
						unanswered = []db.ChatMessage{trigger}
					}
				}
			...
			// task on a transient DB error (SILI-4351 review).
			if inputLoadErr != nil {
			...
			for _, m := range unanswered {
				if cs.Kind != "group" && strings.TrimSpace(m.Content) != "" {
			...
			// rather than run the agent with nothing to answer (SILI-4351).
	...
	// Handoff note (SILI-3375) is populated by taskToResponse (the shared mapper
	...
	if task.Context != nil && !task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid {
		...
		if json.Unmarshal(task.Context, &qc) == nil && qc.Type == service.QuickCreateContextType {
			...
			resp.WorkflowInstanceID = qc.WorkflowInstanceID
			resp.WorkflowStageID = qc.WorkflowStageID
			...
			// the project, and `silieco repo checkout` sees the project's
			...
			// human-readable identifier (e.g. SILI-123) the agent will
	...
	// as the only authority for SILIECO_WORKSPACE_ID in the agent env. An
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	// owner). The daemon will inject this as SILIECO_TOKEN into the agent
	...
	// fall back to a member/owner credential. SILI-3292.
...
// groupChatPromptTranscript renders a bounded, speaker-labelled snapshot
// through the exact message that triggered a group-chat run. Messages sent
// while the agent is already queued/running are intentionally excluded, so a
// run receives an immutable conversational cause rather than a moving target.
func (h *Handler) groupChatPromptTranscript(ctx context.Context, messages []db.ChatMessage, triggerID pgtype.UUID) string {
	end := len(messages)
	for i, message := range messages {
		if message.ID == triggerID {
			end = i + 1
			break
		}
	}
	...
	for _, message := range messages[start:end] {
		...
		if message.SenderUserID.Valid {
			id := uuidToString(message.SenderUserID)
			label = userNames[id]
			if label == "" {
				if user, err := h.Queries.GetUser(ctx, message.SenderUserID); err == nil && strings.TrimSpace(user.Name) != "" {
					label = user.Name
				} else {
					label = "Member " + id[:min(8, len(id))]
				}
				userNames[id] = label
			}
			kind = "human"
		} else if message.SenderAgentID.Valid {
			id := uuidToString(message.SenderAgentID)
			label = agentNames[id]
			if label == "" {
				if agent, err := h.Queries.GetAgent(ctx, message.SenderAgentID); err == nil && strings.TrimSpace(agent.Name) != "" {
					label = agent.Name
				} else {
					label = "Agent " + id[:min(8, len(id))]
				}
				agentNames[id] = label
			}
			kind = "agent"
		} else if message.Role == "assistant" {
			label, kind = "Agent", "agent"
		}
		lines = append(lines, fmt.Sprintf("\n%s (%s):\n%s\n", label, kind, content))
	}

	const header = "Recent group conversation (oldest to newest):\n"
	total := len(header)
	for _, line := range lines {
		total += len(line)
	}
	omitted := false
	for len(lines) > 1 && total > 20000 {
		total -= len(lines[0])
		lines = lines[1:]
		omitted = true
	}
	...
	if omitted {
		b.WriteString("[Earlier context omitted because the group transcript reached its size limit.]\n")
	}
	for _, line := range lines {
		b.WriteString(line)
	}
...
// run that covers a debounced burst (SILI-2968) must deliver every one of
...
type TaskCompleteRequest struct {
	...
	// because its rollout was missing (SILI-5305). Clear the resume pointer and
...
func (h *Handler) CompleteTask(w http.ResponseWriter, r *http.Request) {
	...
	// SILI-5305: SessionRolloutMissing is applied inside CompleteTask's terminal
	...
	if err != nil {
		...
		// including the single chat outcome row — lands exactly once (SILI-4351).
	...
	// SILI-4195: guarantee at-least-once processing. If a member posted a
...
// comments a completing run did NOT deliver (SILI-4195).
//
...
// (SILI-4195 review round-3 must-fix). A timestamp anchor cannot tell a
...
//     This closes SILI-4304: an explicit agent→agent @mention that landed while
...
func (h *Handler) reconcileCommentsOnCompletion(ctx context.Context, task *db.AgentTaskQueue) {
	...
	for i := range comments {
		...
		if c.ParentID.Valid {
			...
			// UUID rather than changing behavior (SILI-4252).
		...
		// originator, not the immediate agent principal (SILI-3963).
		...
		if actorType != "member" {
			...
			// SILI-4857: this is the deferred replay of an already-accepted delegation
		...
		// boundary the old member-only filter protected (SILI-4304). Member
...
// produced by an EXPLICIT @agent / @squad mention (SILI-4304). It is applied to
...
// folded into a not-yet-started run (SILI-4195) so the claim response can embed
...
func (h *Handler) buildCoalescedCommentData(ctx context.Context, workspaceID pgtype.UUID, ids []pgtype.UUID) []CoalescedCommentData {
	...
	for _, id := range ids {
		...
		// (SILI-4252). Matches this function's documented skip-on-missing rule.
...
func (h *Handler) ReportTaskUsage(w http.ResponseWriter, r *http.Request) {
	...
	for _, u := range req.Usage {
		...
		// (e.g. volatile values poisoning the cacheable prefix). SILI-3887.
...
func (h *Handler) FailTask(w http.ResponseWriter, r *http.Request) {
	...
	// SILI-5305: SessionRolloutMissing is applied inside FailTask's terminal
	...
	if err != nil {
		...
		// exactly once (SILI-5305). An invalid request body still returns 400 above.
