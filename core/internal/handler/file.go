...
type AttachmentResponse struct {
	...
	// SILIECO_LOCAL_UPLOAD_BASE_URL), and otherwise prefixing
	// SILIECO_PUBLIC_URL onto the stable per-attachment endpoint that the
	...
	//     resolved host).
	//
	// SILI-3192 — fixes the Desktop / mobile-webview regression where the
...
//
// SILI-5372 / GitHub #5999. A CloudFront-signed DownloadURL is ~800 chars, of
...
// browser resource fetch on every supported client" (SILI-3192).
//
// Decision:
//
...
//     leaking into `a.Url` (the original SILI-3130 bug).
//
...
//     proxies on every request, anchored on `SILIECO_PUBLIC_URL` so the
//     persisted URL keeps working for clients that don't share the
//     document origin (Desktop / mobile webview).
//
//  3. Last-resort fallback (no `SILIECO_PUBLIC_URL` configured): emit
...
//     already broken before SILI-3192 and stay broken here, but we
...
func (h *Handler) UploadFile(w http.ResponseWriter, r *http.Request) {
	...
	if workspaceID != "" {
		...
		if taskID := r.FormValue("task_id"); taskID != "" {
			...
			// and strips a client-forged X-Task-ID. A normal JWT / `sili_` PAT
...
func (h *Handler) GetAttachmentByID(w http.ResponseWriter, r *http.Request) {
	...
	case h.CFSigner == nil && mode == attachmentDownloadModeProxy:
		...
		// leaving the user with no file (SILI-5292). Mint a
...
// core/internal/handler/reserved_slugs.json + scripts/generate-reserved-slugs.mjs).
