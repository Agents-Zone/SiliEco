...
func (h *Handler) RegisterSlackBYO(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, slack.ErrTeamOwnedByAnotherWorkspace):
			writeError(w, http.StatusConflict, "this Slack app is already connected to a different Silieco workspace — disconnect it there before connecting it here")
		default:
...
// id carried by the token to the logged-in Silieco user. The redeemer's identity
...
func (h *Handler) RedeemSlackBindingToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, slack.ErrBindingAlreadyAssigned):
			writeError(w, http.StatusConflict, "this Slack account is already bound to a different Silieco user")
