...
func nextThreadCursor(w *httptest.ResponseRecorder) (string, string) {
	return w.Header().Get("X-Silieco-Next-Before"), w.Header().Get("X-Silieco-Next-Before-Id")
}
...
// Elon's nit on PR #2787 / SILI-2340: a `recent + since` page whose every
...
// SILI-2421 contract: `--thread X --tail N` returns the thread root + the N
...
func TestListComments_ThreadTailCursorScrollsOlderReplies(t *testing.T) {
	...
	// round-trip. (SILI-2421 review: probe `reply_limit + 1` to detect
...
func TestListComments_ThreadTailWithSinceFiltersAfterTail(t *testing.T) {
	...
	t.Run("tail overflow with since past oldest retained reply suppresses cursor", func(t *testing.T) {
		// Recreate Elon's SILI-2421 v2 case: long thread, tail=2 overflows
