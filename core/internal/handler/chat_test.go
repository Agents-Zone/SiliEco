...
// replacement for the channel_* chat_session-FK cascade (SILI-3515 §4): deleting a
...
func TestDeleteChatSession_PrunesChannelRows(t *testing.T) {
	...
	// channel_* rows have no FK to chat_session/workspace (SILI-3515 §4), so
...
// severs the external-channel link (SILI-4372): the channel engine resolves
...
func TestSetChatSessionArchived_ClearsChannelBinding(t *testing.T) {
	...
	// channel_* rows have no FK to chat_session/workspace (SILI-3515 §4), so they
...
// TestListChatSessions_ArchivedSessionReportsZeroUnread pins the SILI-4360 fix:
