...
func TestExtractIdentifiers(t *testing.T) {
	cases := []struct {
		...
		{
			name: "branch_name",
			in:   []string{"", "", "sili-1510/fix-login"},
			want: []string{"SILI-1510"},
		},
		{
			name: "title_and_body",
			in:   []string{"Fix SILI-82", "Closes SILI-1510 and ABC-7", ""},
			want: []string{"SILI-82", "SILI-1510", "ABC-7"},
		},
		{
			name: "dedupe_across_fields",
			in:   []string{"SILI-1", "SILI-1 again", "sili-1/branch"},
			want: []string{"SILI-1"},
		},
		{
...
func TestExtractClosingIdentifiers(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			want: []string{"SILI-1", "SILI-2", "SILI-3", "SILI-4", "SILI-5", "SILI-6", "SILI-7", "SILI-8", "SILI-9"},
		},
		{
			name: "case_insensitive_and_colon",
			in:   []string{"CLOSES: SILI-1", "Fixes:SILI-2 resolves   SILI-3"},
			want: []string{"SILI-1", "SILI-2", "SILI-3"},
		},
		{
		...
		},
		{
			...
			// "Fix login SILI-1" — keyword present but the identifier is
			// not adjacent. Consistent with GitHub's closing-keyword
			// grammar; matches via extractIdentifiers for linking only.
			in:   []string{"Fix login SILI-1", ""},
			want: []string{},
		},
		{
			name: "dedupe_across_fields",
			in:   []string{"Closes SILI-1", "fixes sili-1"},
			want: []string{"SILI-1"},
		},
		{
			name: "no_match_on_disclosed_or_foreclose",
			// Word-boundary guards against keyword fragments embedded
			// in larger words ("Disclosed SILI-1", "Foreclose SILI-1").
			in:   []string{"Disclosed SILI-1 in foreclose SILI-2", ""},
			want: []string{},
		},
	}
	for _, tc := range cases {
...
func TestGitHubConnectRepositoryReturnTarget(t *testing.T) {
	t.Setenv("GITHUB_APP_SLUG", "silieco-test")
	t.Setenv("GITHUB_WEBHOOK_SECRET", "test-secret-123")
	wsID := "11111111-2222-3333-4444-555555555555"
...
func TestGitHubSetupCallbackRepositoryReturnTarget(t *testing.T) {
	t.Setenv("GITHUB_WEBHOOK_SECRET", "test-secret-123")
	t.Setenv("FRONTEND_ORIGIN", "https://app.silieco.test/")
	wsID := "11111111-2222-3333-4444-555555555555"
	...
	if got := rec.Header().Get("Location"); got != "https://app.silieco.test/settings?tab=repositories&github_error=bad_installation_id" {
...
// from GitHub issue silieco-ai/silieco#3264: a PR that mentions three issues
...
// that a bare "SILI-X: foo" title (no closing keyword) links but never
// auto-completes. The user must write `Closes SILI-X` somewhere if they want
...
//
//  1. PR A declares closing intent (`Closes SILI-X`) and is opened.
...
// SILI-3739: a PR that only mentions an issue identifier in its body (no closing
...
// in SILI-2413: the endpoint is now reachable by any workspace member, but
...
// introduced in SILI-2413: GET installations runs under
...
`, "GitHub Routes Role Gating", slug, "github routes role gating", "GRG").Scan(&wsID); err != nil {
	...
	mkUser := func(t *testing.T, label string) string {
		t.Helper()
		var id string
		email := fmt.Sprintf("github-routes-%s-%s@silieco.ai", slug, label)
		if err := testPool.QueryRow(ctx, `
...
`, wsID, m.userID, m.role); err != nil {
	...
	// core/cmd/core/router.go for the workspace-scoped GitHub routes.
...
// TestWebhook_MergedPR_ChildWithParent_NotifiesParent guards the SILI-2538
...
func TestWebhook_MergedPR_ChildWithParent_NotifiesParent(t *testing.T) {
	...
	// issue_child_done_test.go (SILI-2538 Option C).
...
// the assertion that nails down the bug fix for SILI-3078.
...
// SILI-3078: when the setup callback persists a row with the "unknown"
...
// TestWebhook_PullRequest_FansOutToBoundWorkspaces is the SILI-4343 change: one
