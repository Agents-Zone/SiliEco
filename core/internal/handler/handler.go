...
import (
	...
	composio "github.com/silieco-ai/silieco/core/internal/integrations/composio"
	"github.com/silieco-ai/silieco/core/internal/integrations/ghsnapshot"
	...
	"github.com/silieco-ai/silieco/core/pkg/llm"
)
...
type Config struct {
	...
	// SILIECO_VCS_SECRET_KEY is set. It is the product boundary: the feature is
	// intended for self-hosted Silieco only (where Silieco and the Git instance
	...
	// SILIECO_VCS_INTEGRATION_ENABLED; the self-host compose defaults it on.
	VCSIntegrationEnabled bool
	// PublicURL is the absolute base URL the API is reachable at from the
	// public internet, with no trailing slash (e.g. "https://silieco.ai").
	...
	// SILIECO_TRUSTED_PROXIES env var (comma-separated CIDRs, e.g.
	...
	// LLM* configure the basic LLM API layer (SILI-4238). They back the
	...
	// SILI-4309; LLM access is internal-only now. When both LLMAPIKey and
	// LLMBaseURL are empty the layer is disabled and callers fall back
	// silently (see maybeGenerateChatTitleAsync).
	//   - LLMAPIKey       -> SILIECO_LLM_API_KEY
	//   - LLMBaseURL       -> SILIECO_LLM_BASE_URL (OpenAI or any compatible gateway)
	//   - LLMDefaultModel  -> SILIECO_LLM_DEFAULT_MODEL (used when a request omits `model`)
	LLMAPIKey       string
	LLMBaseURL      string
...
// immediately instead of on the daemon's next scheduled tick (SILI-5444).
...
type Handler struct {
	...
	// heartbeat-carried requests (SILI-5444). Optional: when nil,
	...
	// (stale-while-revalidate, SILI-5444). Nil-safe: every call site treats a nil
	...
	// (SILIECO_LARK_SECRET_KEY) is unset; the corresponding HTTP
	// handlers return 503 in that case so a misconfigured self-host
	// deployment surfaces a clear error instead of silently using a
	// zero key. Wired in cmd/core/router.go after handler.New.
	...
	// SILIECO_LARK_SECRET_KEY is set; tests that need a no-op
	...
	// Composio integration (SILI-3720). Nil when COMPOSIO_API_KEY is unset;
	// the composio HTTP handlers return 503 in that case. Wired in
	// cmd/core/router.go after handler.New.
	...
	// (SILI-3620 generalized the Feishu-only Hub into this channel-agnostic
	...
	// (Feishu when SILIECO_LARK_SECRET_KEY is set). The router does NOT
	...
	// (uploaded-but-unbound object reclaim). Built in cmd/core/router.go
	...
	// + app tokens (SILI-3666). Nil unless SILIECO_SLACK_SECRET_KEY is set.
	SlackInstall *slack.InstallService
	// SlackBindingTokens mints/redeems the user-binding tokens behind the
	// "link your Slack account" prompt (SILI-3666). Nil unless Slack is
	// configured (SILIECO_SLACK_SECRET_KEY set).
	SlackBindingTokens *slack.BindingTokenService
	// SlackHistory backs the agent-facing `silieco chat history` command: it
	// reads a chat session's bound Slack conversation on demand (SILI-3871). Nil
	...
	// LLM is the basic LLM API layer (SILI-4238): a thin wrapper over the
	...
	// SILI-4309, so it is internal-only now. Always non-nil (New builds it from
	...
	// SILIECO_VCS_SECRET_KEY is unset; the connect/webhook handlers return 503
	...
	// cmd/core/router.go after New.
	VCSSecretBox *secretbox.Box
	// PRRefresh drives the GitHub API snapshot pipeline for PR cards (SILI-5265):
	...
	// Wired in cmd/core/router.go after New.
...
func New(queries *db.Queries, txStarter txStarter, hub *realtime.Hub, bus *events.Bus, emailService *service.EmailService, store storage.Storage, cfSigner *auth.CloudFrontSigner, analyticsClient analytics.Client, cfg Config, daemonHubs ...*daemonws.Hub) *Handler {
	...
	// GitHub API snapshot pipeline for PR cards (SILI-5265). Built
...
// `omitempty` JSON fields drop out cleanly (SILI-4195).
...
// stripped by the agent process. This is the path SILI-2600 relies on to
