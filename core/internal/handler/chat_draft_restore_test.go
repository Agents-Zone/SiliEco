...
func TestChatDraftRestores_NonCreatorForbidden(t *testing.T) {
	...
	if err := testPool.QueryRow(context.Background(), `
		...
		RETURNING id
	`, fmt.Sprintf("draft-restore-other-%d@silieco.ai", time.Now().UnixNano())).Scan(&otherUserID); err != nil {
...
// chat_draft_restore has no chat_session FK (SILI-3515), so nothing but
