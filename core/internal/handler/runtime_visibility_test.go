...
// (SILI-2062) acceptance criteria: workspace owner / admin can use any
...
func runtimeVisibilityFixture(t *testing.T) (runtimeID, runtimeOwnerID, plainMemberID string) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ('Runtime Owner', 'runtime-owner@silieco.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'runtime-owner@silieco.test'`)
	})
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Plain Runtime Member', 'plain-runtime-member@silieco.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'plain-runtime-member@silieco.test'`)
	})
...
func TestCreateAgent_RejectsPrivateRuntimeForNonOwner(t *testing.T) {
	...
	// Plain member: this is the hole SILI-2062 closes — must be 403.
	w = httptest.NewRecorder()
