...
// autopilotDelegationFixture builds the SILI-4857 create_issue scenario: a
...
func newAutopilotDelegationFixture(t *testing.T, targetAgentID, autopilotCreatorUserID, originType string) autopilotDelegationFixture {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, 'SILI-4857 delegation', $2, 'create_issue', 'member', $3) RETURNING id
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, 'agent', $2, 'SILI-4857 delegation issue', 'agent', $2, $3, $4, $5)
		RETURNING id
...
// TestAutopilotDelegationAuthority_LineageBinding is the SILI-4857 fix, guarded by
...
// TestCreateComment_AutopilotLeaderMentionEnqueuesPrivateWorker is the SILI-4857
...
func TestCreateComment_AutopilotLeaderMentionEnqueuesPrivateWorker(t *testing.T) {
	...
	// the gate only, never written onto the delegated task's originator (SILI-4302).
...
func seedBareIssue(t *testing.T, creatorAgentID string) string {
	...
	if err := testPool.QueryRow(context.Background(), `
		...
		VALUES ($1, 'agent', $2, 'SILI-4857 unrelated issue', $3) RETURNING id
...
// SILI-4857 must-fix #1 (review round 2): when the mentioned target was BUSY at
...
// TestUpdateComment_AutopilotAuthorityReStampedToEditingTask is the SILI-4857
...
func TestCreateComment_AutopilotWorkerResultWakesSquadLeader(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO autopilot (workspace_id, title, assignee_id, execution_mode, created_by_type, created_by_id)
		VALUES ($1, 'SILI-4857 squad', $2, 'create_issue', 'member', $3) RETURNING id
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, 'SILI-4857 Squad', '', $2, $3) RETURNING id
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (workspace_id, creator_type, creator_id, title, assignee_type, assignee_id, number, origin_type, origin_id)
		VALUES ($1, 'agent', $2, 'SILI-4857 squad issue', 'squad', $3, $4, 'autopilot', $5) RETURNING id
...
// TestUpdateComment_AdminEditOfAgentCommentClearsStaleLineage is the SILI-4857
...
func TestUpdateComment_AdminEditOfAgentCommentClearsStaleLineage(t *testing.T) {
	...
	adminID := createPermissionTestAdmin(t, "mul4857-edit-admin@silieco.test")
