...
type AutopilotResponse struct {
	...
	// AssigneeType is "agent" or "squad". Path A from SILI-2429: when set
	...
	// and fall back to attempting the action). See SILI-3807.
	...
	// re-grant). Nil when built without a caller in context. See SILI-3807.
...
type AutopilotTriggerResponse struct {
	...
	// SILIECO_PUBLIC_URL setting. Nil when the server has no public URL
...
func autopilotToResponse(a db.Autopilot, subscribers []db.AutopilotSubscriber) AutopilotResponse {
	...
	if assigneeType == "" {
		// Older rows pre-SILI-2429 may surface as "" against an out-of-date
...
func runToResponse(r db.AutopilotRun) AutopilotRunResponse {
	...
	return AutopilotRunResponse{
		...
		// "run now" handler injects from the typed dispatch outcome (SILI-4525).
...
type CreateAutopilotRequest struct {
	...
	// compatibility with desktop clients shipped before SILI-2429.
	AssigneeType       *string           `json:"assignee_type"`
	AssigneeID         string            `json:"assignee_id"`
	ExecutionMode      string            `json:"execution_mode"`
	IssueTitleTemplate *string           `json:"issue_title_template"`
...
func (h *Handler) GetAutopilot(w http.ResponseWriter, r *http.Request) {
	...
	if member, err := h.getWorkspaceMember(r.Context(), requestUserID(r), workspaceID); err == nil {
		...
		// write but cannot re-grant (SILI-3807).
...
func (h *Handler) CreateAutopilot(w http.ResponseWriter, r *http.Request) {
	...
	// human at dispatch time (SILI-4302 §3.4).
...
func (h *Handler) UpdateAutopilot(w http.ResponseWriter, r *http.Request) {
	...
	// leave accountability with the previous publisher (SILI-4302 §3.4).
...
// transfers accountability to the editor (SILI-4302 §3.4; boundary pinned with Elon):
//
...
// publish (SILI-4302 §3.4). Thin handler wrapper over service.RecordAutopilotRuleVersion
...
func (h *Handler) DeleteAutopilot(w http.ResponseWriter, r *http.Request) {
	...
	// Archiving is a substantive status change (SILI-4302 §3.4), so republish the
...
// (privilege escalation). See SILI-3807.
...
func (h *Handler) CreateAutopilotTrigger(w http.ResponseWriter, r *http.Request) {
	...
	// create (SILI-4302 §3.4). Resolved here so both the webhook and schedule create
	...
	trigger, err := qtx.CreateAutopilotTrigger(r.Context(), db.CreateAutopilotTriggerParams{
		...
		// (source=trigger_owner, SILI-4302).
...
// SILI-4302 §3.4, published by publisherID) commit together — a version-write failure
...
func (h *Handler) createWebhookTriggerWithMintedToken(
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		trigger, err := qtx.CreateAutopilotTrigger(ctx, db.CreateAutopilotTriggerParams{
			...
			// later substantive edit (source=trigger_owner, SILI-4302).
			PublishedByType: pgtype.Text{String: "member", Valid: publisherID.Valid},
			PublishedByID:   publisherID,
		})
		if err != nil {
...
func (h *Handler) UpdateAutopilotTrigger(w http.ResponseWriter, r *http.Request) {
	...
	// no-op PATCH changes nothing — neither should move responsibility (SILI-4302; the
...
func (h *Handler) DeleteAutopilotTrigger(w http.ResponseWriter, r *http.Request) {
	...
	// Removing a trigger changes what fires — a substantive publish (SILI-4302 §3.4).
...
func (h *Handler) TriggerAutopilot(w http.ResponseWriter, r *http.Request) {
	...
	// direct_human to the triggering member (SILI-4302 §4). Resolve the actor the
	...
	// Carry the typed admission reason (decided at its source, SILI-4525) straight
