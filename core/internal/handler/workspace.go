...
func (h *Handler) DeleteWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	// away, the restore has no FK to follow it (SILI-3515) and no reaper, and the
	...
	if err := qtx.DeleteChatSessionParticipantsByWorkspace(r.Context(), requester.WorkspaceID); err != nil {
		slog.Warn("delete workspace chat participants failed", append(logger.RequestAttrs(r), "error", err, "workspace_id", workspaceID)...)
		writeError(w, http.StatusInternalServerError, "failed to delete workspace")
		return
	}
