...
// identifier (NOT a hardcoded `SILI-` prefix — that was the bug PR #2918
...
// only mention we ever inject (see SILI-2538 Option C — covered separately
...
func TestChildDoneNotifiesParent(t *testing.T) {
	...
	if strings.Contains(content, "SILI-") {
		t.Errorf("comment must not hardcode SILI- prefix, got: %s", content)
	}
...
// captures the "reopen + done counts as a new event" line from SILI-2538.
...
// SILI-3497. No system comment, no trigger, until the user explicitly moves
...
// TestChildDoneMentionsParentAssignee_Agent verifies the SILI-2538 Option C
...
// TestChildDoneSkippedWhenParentMember verifies the SILI-2538 follow-up: a
...
// assignees (see core/internal/handler/issue.go), so the fixture must
...
// parent agent must still be triggered (SILI-2808). A child finishing and
...
// routes through the agent path, which (post-SILI-2808) has no self-trigger
...
// behaves identically: SILI-3969 removed its old same-squad / shared-leader
...
// guard was removed in SILI-3969: waking the leader on the PARENT is a serial
...
func TestChildDoneWakesLeaderWhenParentAndChildSquadsShareLeader(t *testing.T) {
	...
	if got := countPendingTasksForAgent(t, fx.parent.ID, parentSquad.LeaderID); got != 1 {
		t.Errorf("expected 1 pending leader task on parent (shared-leader guard removed, SILI-3969), got %d", got)
	}
}

// TestChildDoneWakesLeaderWhenChildIsSameSquad — the SILI-3969 repro. Parent
...
func TestChildDoneWakesLeaderWhenChildIsSameSquad(t *testing.T) {
	...
	if got := countPendingTasksForAgent(t, fx.parent.ID, sq.LeaderID); got != 1 {
		t.Errorf("expected 1 pending leader task for same-squad child (SILI-3969), got %d", got)
	}
}

// TestStageLeaderPrepareTimeoutRetryCanAdvanceNextStage covers the full server
// half of SILI-4923's recovery chain: a stage barrier wakes the squad leader,
