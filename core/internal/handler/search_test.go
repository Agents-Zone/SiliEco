...
func TestBuildSearchQuery_WithNumber(t *testing.T) {
	query, args := buildSearchQuery("SILI-42", []string{"SILI-42"}, 42, true, false)
...
// SILI-4059 fix: every EXISTS / correlated subquery over `comment` MUST
...
func TestBuildSearchQuery_CommentSubqueryWorkspaceScope(t *testing.T) {
	...
	if scopedCount < fromCount {
		t.Errorf("single-term query has %d comment subqueries but only %d workspace_id filters — %d unscoped subquery(ies) will trigger the SILI-4059 global-hash plan",
