...
func privateAgentTestFixture(t *testing.T) (agentID, ownerID, memberID string) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ('Private Agent Owner', 'private-agent-owner@silieco.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'private-agent-owner@silieco.test'`)
	})
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Plain Member', 'plain-member@silieco.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'plain-member@silieco.test'`)
	})
...
func TestCreateIssue_AssignToPrivateAgentForbidsPlainMember(t *testing.T) {
	...
	// the invocation-permission model (SILI-3963) — admin/owner status no
...
func TestMentionAgent_RejectsCrossWorkspaceAgentUUID(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Foreign Owner', 'cross-ws-foreign@silieco.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'cross-ws-foreign@silieco.test'`)
	})
	...
	// Silieco's mention format is markdown-linked: [@Name](mention://agent/<uuid>).
...
func TestShouldEnqueueOnComment_PrivateAgentGate(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			actorID:   testUserID,
			want:      false,
			reason:    "SILI-3963: workspace owners/admins no longer bypass a private agent's invocation gate",
		},
		{
			...
			actorID:   agentID,
			want:      false,
			reason:    "SILI-3963: A2A is judged by the top-of-chain originator; a private agent denies an agent actor with no owner/allow-listed originator",
		},
	}
