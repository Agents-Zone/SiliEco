...
func TestCommentEnqueueRaceDifferentHeadNotCoalesced(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, 1, 'silieco-ai', 'silieco', 999313, 'review PR', 'open', 'https://example.test/pr', now(), now(), $2)
		RETURNING id
...
// bypass the atomic re-attribution a queued fold requires — SILI-4302). Only
...
func TestCommentEnqueueRaceQueuedWinnerReattributesOriginator(t *testing.T) {
	...
	var m2 string
	if err := testPool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ('Race M2', 'race-m2-999315@silieco.test') RETURNING id`).Scan(&m2); err != nil {
...
func TestCommentEnqueueRaceNewerDifferentHeadNotDeferred(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO github_pull_request (workspace_id, installation_id, repo_owner, repo_name, pr_number, title, state, html_url, pr_created_at, pr_updated_at, head_sha)
		VALUES ($1, 1, 'silieco-ai', 'silieco', 999317, 'review PR', 'open', 'https://example.test/pr', now(), now(), $2)
		RETURNING id
	`, testWorkspaceID, dupRaceHeadB).Scan(&prID); err != nil {
		t.Fatalf("seed PR: %v", err)
	}
...
func TestCommentEnqueueRaceMixedCoveringAndNewerNotDeferred(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO github_pull_request (workspace_id, installation_id, repo_owner, repo_name, pr_number, title, state, html_url, pr_created_at, pr_updated_at, head_sha)
		VALUES ($1, 1, 'silieco-ai', 'silieco', 999318, 'review PR', 'open', 'https://example.test/pr', now(), now(), $2)
		RETURNING id
	`, testWorkspaceID, dupRaceHeadB).Scan(&prID); err != nil {
		t.Fatalf("seed PR: %v", err)
	}
...
func seedDupRacePR(t *testing.T, issueID string, prNumber int) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO github_pull_request (workspace_id, installation_id, repo_owner, repo_name, pr_number, title, state, html_url, pr_created_at, pr_updated_at, head_sha)
		VALUES ($1, 1, 'silieco-ai', 'silieco', $2, 'review PR', 'open', 'https://example.test/pr', now(), now(), $3)
		RETURNING id
