...
// failure before the user actually has to re-run `silieco login`.
...
//
// Only sili_ PATs may be renewed: a cookie/JWT session has no PAT row to
// extend, and an mat_ task token is single-purpose and short-lived. mcn_
// cloud-node PATs are owned by Silieco Cloud Fleet, not us — we don't even
...
func (h *Handler) RenewCurrentPersonalAccessToken(w http.ResponseWriter, r *http.Request) {
	...
	if rawToken == "" || rawToken == authHeader || !strings.HasPrefix(rawToken, "sili_") {
