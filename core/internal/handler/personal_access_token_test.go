...
func TestRenewPAT_RejectsNonPATAuthHeader(t *testing.T) {
	tests := []struct {
		...
		{"empty", ""},
		{"missing bearer prefix", "sili_abc123"},
...
// idempotency that the SILI-2744 review flagged: when N callers race to
...
func TestRenewPAT_RejectsTokenBelongingToDifferentUser(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, $2)
		RETURNING id
	`, "Other User", "other-renew@silieco.ai").Scan(&otherUserID); err != nil {
