...
// TestClaimTaskByRuntime_ChatIntroGateClearsAfterUserReplies pins the SILI-4259
...
// for SILI-2542: before this fix the field was never plumbed through, so
...
//
// This is the acceptance path from SILI-975: hostname drift must no longer
...
// Regression test for SILI-1198: comment-triggered tasks that finish without
...
// `silieco issue comment add` call) left the user staring at a "Completed"
// badge with no reply.
func TestCompleteTask_CommentTriggered_SynthesizesCommentWhenAgentSilent(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (workspace_id, title, status, priority, creator_id, creator_type, number, position)
		VALUES ($1, 'sili-3310 agent output fixture', 'in_progress', 'none', $2, 'member', 3310, 0)
		RETURNING id
	`, testWorkspaceID, testUserID).Scan(&issueID); err != nil {
		t.Fatalf("setup: create issue: %v", err)
	}
	...
	agentFinalOutput := fmt.Sprintf(
		"sure, see SILI-3310, issue/SILI-3310, feature/SILI-3310, and [SILI-3310](mention://issue/%s)",
		issueID,
	)
...
func TestCompleteTask_CommentTriggered_SkipsSynthesisWhenAgentAlreadyCommented(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (workspace_id, title, status, priority, creator_id, creator_type, number, position)
		VALUES ($1, 'sili-1198 dedup fixture', 'in_progress', 'none', $2, 'member', 81199, 0)
		RETURNING id
	`, testWorkspaceID, testUserID).Scan(&issueID); err != nil {
		t.Fatalf("setup: create issue: %v", err)
	}
...
// TestClaimTask_ManualRetryReusesWorkdir is the SILI-4869 claim-layer contract: a
...
// regression the SILI-2968 debounce exposed: when several user messages are
...
// TestClaimTask_ChatPopulatesInitiator verifies SILI-2645 for chat tasks: the
// claim response surfaces the STORED task initiator (initiator_user_id captured
// at enqueue), NOT chat_session.creator_id. This is the SILI-2645 review fix: for
...
func TestClaimTask_ChatPopulatesInitiator(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email) VALUES ('Installer User', 'installer-test@silieco.ai')
		RETURNING id
...
func createEphemeralUser(t *testing.T, label string) string {
	t.Helper()
	email := fmt.Sprintf("membership-cache-%s-%s@silieco.ai", label, uuid.NewString())
	var userID string
	if err := testPool.QueryRow(context.Background(), `
...
// TestClaimTaskByRuntime_CommentTaskPopulatesInitiator verifies SILI-2645: the
