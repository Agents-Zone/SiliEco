...
func newDownloadRouter() http.Handler {
	// Mirrors the production router after SILI-3130: the download
...
// is the regression test for SILI-3130: a markdown image rendered as
...
// SILI-3192 — buildMarkdownURL must emit a durable, absolute-when-possible
...
func TestBuildMarkdownURL_PublicCdnAbsoluteURLReusedVerbatim(t *testing.T) {
	...
	testHandler.cfg.PublicURL = "https://api.silieco.test"
	testHandler.CFSigner = nil
	...
	id := seedAttachmentURL(t, "https://cdn.silieco.test/uploads/abc.png", "abc.png", "image/png", 1)
	att, err := testHandler.Queries.GetAttachment(context.Background(), db.GetAttachmentParams{
		ID:          parseUUID(id),
		WorkspaceID: parseUUID(testWorkspaceID),
	})
	...
	resp := testHandler.attachmentToResponse(att, attachmentURLModeSigned)
	if resp.MarkdownURL != "https://cdn.silieco.test/uploads/abc.png" {
		t.Fatalf("markdown_url = %q, want raw a.Url passthrough", resp.MarkdownURL)
	}
}

// SILI-3192 review must-fix 1 — `att.url` for a private S3 / R2 / MinIO
...
func TestBuildMarkdownURL_PrivateBucketWithoutCdnDomainRoutesThroughAPIEndpoint(t *testing.T) {
	...
	resp := testHandler.attachmentToResponse(att, attachmentURLModeSigned)
	want := "https://api.silieco.test/api/attachments/" + id + "/download"
...
func TestIsDurablePublicURL(t *testing.T) {
	cases := []struct {
		...
		{"absolute https no signature", "https://cdn.silieco.test/foo.png", true},
		{"absolute http no signature", "http://cdn.silieco.test/foo.png", true},
		...
		{"unsupported scheme", "ftp://core/foo", false},
...
// policy the /api/attachments download endpoint uses. See SILI-3821 / #4477.
