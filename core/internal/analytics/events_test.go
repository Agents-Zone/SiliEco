...
func TestIsMetricsOnly(t *testing.T) {
	// As of SILI-4127, PostHog is retired for server-side product analytics:
	...
	for _, name := range []string{
		...
		if !IsMetricsOnly(name) {
			t.Errorf("IsMetricsOnly(%q) = false, want true (server events stay out of PostHog since SILI-4127)", name)
		}
	}
