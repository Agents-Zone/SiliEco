...
//
// As of SILI-4127, PostHog is no longer used for server-side product analytics:
...
var metricsOnlyEvents = map[string]struct{}{
	...
	// (SILI-4127); the PostHog copy was redundant.
	EventSignup:                        {},
...
// not be sent to PostHog. As of SILI-4127 this is true for every server-side
...
// `silieco_onboarding_started_total{platform=...}` label via the fixed
...
// flow (SILI-5159): it is asked by the workspace backfill prompt after
...
// (SILI-4127); the per-user source value reaches analytics through the
