...
// is read from the same task-local sessions Codex actually wrote to (SILI-4424).
...
func TestCodexDeliverableOutputExcludesNarration(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// still stream to the timeline the Silieco UI renders.
...
// codexRuntimeBriefCanary stands in for the Silieco runtime brief the daemon
// would inline if developerInstructions were ever wired back up.
const codexRuntimeBriefCanary = "SILIECO-RUNTIME-BRIEF-CANARY"

// assertNoDeveloperInstructions pins the SILI-5392 contract: Codex loads the
...
func assertNoDeveloperInstructions(t *testing.T, params map[string]any) {
	...
	if got != nil {
		t.Errorf("developerInstructions = %v, want null: the runtime brief is delivered via the workdir AGENTS.md, not inline (SILI-5392)", got)
	}
}
...
// TestCodexExecuteRetriesAfterModelCatalogRefreshFailure covers the SILI-5110
...
// daemon reuses for the Windows sandbox decision (SILI-4957): a shell-quoted
...
func TestBuildCodexArgsPreservesCustomMcpOverridesWhenUnmanaged(t *testing.T) {
	...
	// because before SILI-2764 there was no MCP Tab. When the agent has
...
func TestEnsureCodexMcpConfigEmptyClearsBlock(t *testing.T) {
	...
	got := string(data)
	if strings.Contains(got, siliecoCodexMcpBeginMarker) {
...
func TestEnsureCodexMcpConfigWritesManagedBlock(t *testing.T) {
	...
	if !strings.Contains(got, siliecoCodexMcpBeginMarker) || !strings.Contains(got, siliecoCodexMcpEndMarker) {
