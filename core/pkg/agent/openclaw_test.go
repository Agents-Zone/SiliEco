...
// embedded-mode escape hatch. Silieco historically hard-coded `--local` so
...
func TestOpenclawProcessOutputExtractsModelFromAgentMeta(t *testing.T) {
	...
	result := openclawResult{
		...
		Meta: openclawMeta{
			DurationMs: 9501,
			AgentMeta: map[string]any{
				"sessionId": "silieco-1776752018613706000",
				"provider":  "deepseek",
				"model":     "deepseek-chat",
				"usage": map[string]any{
					"input":      float64(414),
	...
	if res.sessionID != "silieco-1776752018613706000" {
