...
// qwenBlockedArgs are owned by Silieco. Qwen accepts the task prompt and stream
// protocol as flags, so custom args must not replace either. Model/session are
// also selected by Silieco, and safe mode disables the QWEN.md context file.
...
func buildQwenArgs(prompt string, opts ExecOptions, logger *slog.Logger) []string {
	...
	// mechanism; this keeps Qwen headless runs consistent (SILI-5134).
