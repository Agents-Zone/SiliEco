...
func (b *claudeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	go func() {
		...
		if sawAsyncLaunch {
			completionGuardError = "claude launched an async background task; Silieco-managed runs require foreground execution"
		}
...
func buildClaudeArgs(opts ExecOptions, logger *slog.Logger) []string {
	...
	// turn. Verified against Claude Code 2.1.220 (SILI-5392).
...
var resumeRejectedPhrases = []string{
	...
	// Reported verbatim in silieco-ai/silieco#5704 against Claude Code
...
func claudeRootSudoPreflight(args, env []string) error {
	...
	return fmt.Errorf("Claude Code refuses bypassPermissions under root/sudo privileges. Run the Silieco daemon as a non-root user, or set IS_SANDBOX=1 if running in a genuine container/sandbox")
}
...
func mergeEnv(base []string, extra map[string]string) []string {
	...
	for _, entry := range base {
		...
		// SILIECO_* in the daemon's own environment is not task context. Drop
		// the inherited namespace for every backend and append only the values
		// daemon.go explicitly assembled for this task below.
		if isFilteredChildEnvKey(key) || strings.HasPrefix(strings.ToUpper(key), "SILIECO_") {
			continue
		}
...
func writeMcpConfigToTemp(raw json.RawMessage) (string, error) {
	dir, err := os.MkdirTemp("", "silieco-mcp-*")
	if err != nil {
...
func cleanupMcpConfigTemp(path string) {
	...
	if strings.HasPrefix(filepath.Base(dir), "silieco-mcp-") {
		_ = os.RemoveAll(dir)
		return
	}
	_ = os.Remove(path)
...
// wedged by a bun regression (SILI-3812) — would otherwise stall the whole
