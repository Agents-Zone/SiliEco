...
// write (GitHub silieco#5300). It prefers a session-scoped grant over a
...
func TestHermesClientAutoApprovesPermissionRequest(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			// exactly "allow_once"; this is the silieco#5300 regression
...
// failures from GitHub silieco#5862.
//
...
//     completed the requested work, then Silieco suddenly flips the same
...
// second pollution shape from GitHub silieco#5862. Hermes echoes a
...
// silieco#1952 scenario: the upstream LLM returns HTTP 429 (rate
...
// fix for GitHub silieco#1952: a hermes run that hits a 429 (or any
...
// GitHub silieco#4010: session/resume succeeds and echoes back the
...
// fix for GitHub silieco#4010: when a resumed session turns out to be
...
// regression GPT-Boy flagged on the silieco#1952 fix: a per-attempt
...
// TestHermesSkipsRedundantSetModelWhenAlreadyCurrent pins the SILI-5029 fix:
