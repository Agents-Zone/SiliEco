...
// downgrading a user's isolation opt-in (SILI-4957).
...
const (
	siliecoCodexMcpBeginMarker = "# BEGIN silieco-managed mcp_servers (do not edit; regenerated by daemon)"
	siliecoCodexMcpEndMarker   = "# END silieco-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(siliecoCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(siliecoCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = siliecoCodexMcpBeginMarker + "\n" + siliecoCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(siliecoCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(siliecoCodexMcpEndMarker)
	sb.WriteString("\n")
...
func normalizeCodexMcpServerConfig(server map[string]any) map[string]any {
	...
	for k, v := range server {
		switch {
		case isSiliecoMcpSelectorKey(k):
			continue
		case k == "type":
			continue
		case k == "headers":
...
func isSiliecoMcpSelectorKey(k string) bool {
	switch k {
...
func (b *codexBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	go func() {
		...
		for attempt := 1; attempt <= 2; attempt++ {
			...
			// thread that never produced a turn (SILI-5110). The first non-pin
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "silieco-agent-sdk",
				"title":   "Silieco Agent SDK",
				"version": "0.2.0",
			},
		...
		// this covers the ones only the live resume reveals (SILI-4424).
		...
		// SILI-2339 — Trump's constraint that the three injection points
		...
		// there rather than in the shared ~/.codex/sessions (SILI-4424).
...
func (c *codexClient) startOrResumeThread(ctx context.Context, opts ExecOptions, logger *slog.Logger) (string, bool, error) {
	if priorThreadID := opts.ResumeSessionID; priorThreadID != "" {
		...
		// agent's thinking_level since. See SILI-2339 — Elon flagged that
...
// (per Trump's SILI-2339 review constraint).
//
