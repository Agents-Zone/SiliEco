...
func TestOpencodeEventParsingToolUseFixture(t *testing.T) {
	...
	line := `{"type":"tool_use","timestamp":1775117187163,"sessionID":"ses_abc","part":{"id":"prt_123","messageID":"msg_456","sessionID":"ses_abc","type":"tool","tool":"bash","callID":"call_BHA1","state":{"status":"completed","input":{"command":"pwd","description":"Prints current working directory path"},"output":"/tmp/silieco\n","metadata":{"exit":0},"time":{"start":1775117187092,"end":1775117187162}}}}`
	...
	if output, ok := event.Part.State.Output.(string); !ok || output != "/tmp/silieco\n" {
...
func TestResolveOpenCodeNativeFromShimResolvesNpmShim(t *testing.T) {
	...
	// Reporter's exact layout from silieco#1717.
...
// SILI-2416: OpenCode resolves its AGENTS.md walk-up and .opencode/skills
...
// TestOpencodeBackendNeverEmitsPromptFlag pins SILI-5392: `opencode run` has no
...
func TestOpencodeBackendNeverEmitsPromptFlag(t *testing.T) {
	...
	const brief = "the entire silieco runtime brief"
...
// override it would re-introduce the SILI-2416 regression.
