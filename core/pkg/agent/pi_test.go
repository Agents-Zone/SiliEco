...
func TestBuildPiArgsIgnoresSystemPrompt(t *testing.T) {
	args := buildPiArgs("hello world", "/tmp/s.jsonl", ExecOptions{
		SystemPrompt: "the entire silieco runtime brief",
	}, slog.Default())
	...
	for _, a := range args {
		...
		if a == "the entire silieco runtime brief" {
...
func TestStripPiToolCallMarkup(t *testing.T) {
	tests := map[string]string{
		...
		`before response:bash{command:<|"|>silieco issue comment list issue-id --all --output json<|"|>} after`: "before  after",
