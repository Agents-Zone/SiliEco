...
func TestMergeEnvFiltersClaudeCodeVars(t *testing.T) {
	...
	env := mergeEnv([]string{
		...
		"SILIECO_SERVER_URL=https://daemon.example",
	}, map[string]string{
		"FOO":                "bar",
	...
	if !found["SILIECO_SERVER_URL=https://task.example"] || !found["SILIECO_TOKEN=mat_task"] {
		t.Fatalf("expected explicit task SILIECO_* values to be appended, got %v", env)
	}
}
...
func TestResumeWasRejected(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Verbatim from silieco-ai/silieco#5704 (Claude Code 2.1.207,
...
func TestBuildClaudeArgsManagedSkillSettingsWins(t *testing.T) {
	args := buildClaudeArgs(ExecOptions{
		ClaudeSettingsPath: "/tmp/silieco-claude-settings.json",
	...
	if !strings.Contains(joined, "--settings /tmp/silieco-claude-settings.json") {
