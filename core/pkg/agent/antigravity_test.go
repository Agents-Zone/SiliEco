...
func TestBuildAntigravityArgsNoCapUsesLargePrintTimeout(t *testing.T) {
	...
	// minutes and kills any run whose build/tests outlive it (SILI-3570). "No cap"
...
// the sequence that made a stalled turn look "completed" (SILI-3570).
...
// SILI-3570: agy aborts a long turn by printing its timeout sentinel and exiting
...
// persisted/API value, or the daemon-wide SILIECO_ANTIGRAVITY_MODEL default —
...
// case (SILI-3726). The real reply lives only in the conversation transcript,
...
// guard for SILI-3726: agy 1.0.14 can complete a turn with empty stdout while the
