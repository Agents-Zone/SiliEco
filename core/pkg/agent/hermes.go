...
func (b *hermesBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	go func() {
		...
		// behaviour. See SILI-5029 / NousResearch/hermes-agent#59089.
...
// file write on the Hermes ACP runtime (GitHub silieco#5300).
...
// GitHub silieco#5300 it refuses to auto-select a permanent "allow_always"
...
func (c *hermesClient) extractPromptResult(data json.RawMessage) {
	...
	// usage map and Silieco's Usage/cost dashboards stay at zero.
...
// task was wrongly marked failed (issue #5509 / SILI-4860). Accept both shapes.
...
// even when Silieco did not pass an explicit agent.model override.
...
// silieco#5862 — do not gate matching on length).
