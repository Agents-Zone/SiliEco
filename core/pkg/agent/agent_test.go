...
// TestDetectVersionTimesOutOnHang guards SILI-3812: a CLI whose `--version`
...
func TestDetectVersionTimesOutOnHang(t *testing.T) {
	...
	case <-time.After(10 * time.Second):
		t.Fatal("DetectVersion did not return: version probe is unbounded (regression of SILI-3812)")
	}
}
...
func TestRunContextPositiveTimeoutHasDeadline(t *testing.T) {
	...
	// absolute cap operators can still set via SILIECO_AGENT_TIMEOUT).
