...
type ExecOptions struct {
	Cwd   string
	Model string
	// SystemPrompt carries the Silieco runtime brief for the few providers
	...
	// restarting. Currently honoured by the codex backend (SILI-4424).
	ResumeExpected bool
	...
	// the field rather than fail (so SILI-2339 can grow runtime support
	...
	// see core/internal/daemon/execenv/openclaw_config.go). Other backends
...
// events is never killed merely for running long (SILI-3064). The caller owns
...
// be based on a backend Silieco officially supports.
