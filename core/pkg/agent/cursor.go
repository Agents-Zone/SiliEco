	...
	go func() {
		...
		for scanner.Scan() {
			...
			case "thinking":
				...
				// exact failure mode this fix exists to prevent (SILI-5231).
				switch evt.Subtype {
				case "delta":
			...
			case "tool_call":
				...
				// can force-stop it as falsely stuck (SILI-5231 review).
				switch evt.Subtype {
				case "started":
			...
			default:
				...
				// distinguish that from a tool-free run (SILI-5434).
				//
				...
				// upstream additions is the failure mode SILI-5231 already
				// fixed once.
...
//     builds (SILI-5434 review). Not reproduced locally, so they are listed
...
// tool_use_count=0 and no diagnostic whatsoever (SILI-5434).
//
