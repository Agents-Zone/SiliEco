...
const (
	...
	// Workflow events. Workflow definitions are versioned SOPs; instances are
	// concrete runs of a published version and tasks may move between stages
	// without changing their canonical task status.
	EventWorkflowCreated         = "workflow:created"
	EventWorkflowUpdated         = "workflow:updated"
	EventWorkflowInstanceCreated = "workflow_instance:created"
	EventWorkflowInstanceUpdated = "workflow_instance:updated"
	EventWorkflowTaskChanged     = "workflow_task:changed"
	...
	// dead wait to an interactive UI flow (SILI-5444). The hint carries no work
	...
	// connection (SILI-4257). The daemon sends EventDaemonRPCRequest with a
	...
	// Slack installation lifecycle (SILI-3666). Same semantics as the Lark
