...
const (
	...
	// request/response RPCs over the WebSocket control connection (SILI-4257).
...
type RPCRequestPayload struct {
	...
	// out waiting and fallen back to HTTP (SILI-4257). 0 means no server-side
...
const (
	...
	// than a silently-dropped turn (SILI-4351).
...
//
// MessageKind is additive (SILI-4351): older clients ignore it and fall back to
...
type ChatSessionUpdatedPayload struct {
	...
	// ParticipantsChanged turns this into a workspace-level roster
	// invalidation so newly invited members discover the session immediately.
	ParticipantsChanged bool `json:"participants_changed,omitempty"`
}
