...
// SILIECO_*_CONFIG / SILIECO_*_FILE knobs documented in .env.example.
const EnvFlagFile = "SILIECO_FEATURE_FLAGS_FILE"
...
// NewServiceFromEnv constructs a Service wired with the standard silieco
// config sources, in order of decreasing precedence:
//
//  1. EnvProvider (FF_<KEY> overrides — Ops kill switches, fastest path).
//  2. StaticProvider loaded from the YAML file at SILIECO_FEATURE_FLAGS_FILE
//     (when the env var is set and the file exists).
//
// When SILIECO_FEATURE_FLAGS_FILE is unset, the Service still works — the
