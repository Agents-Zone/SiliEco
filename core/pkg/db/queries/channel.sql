-- Platform-agnostic inbound channel queries (SILI-3515). These operate on
...
--
-- No foreign keys exist on these tables (SILI-3515 §4): the integrity the
...
-- re-connecting it — even to represent a DIFFERENT agent in the SAME Silieco
...
-- the conflict update to the SAME Silieco workspace: a team already owned by a
...
-- old catch-all "connected to a different Silieco workspace". Meant to be read
...
-- installation (channel_* has no FK/cascade, SILI-3515 §4). Returns the removed id
...
-- Application-layer replacement for the (deliberately absent, SILI-3515 §4)
...
-- channel_installation has no FK (SILI-3515 §4), so unlike the old
...
-- Boot path for the channel-agnostic engine Supervisor (SILI-3620): every
...
-- gone (channel_installation has no FK, SILI-3515 §4), matching the old ON
...
-- to a Silieco user. The old composite member-FK is gone, so this no
...
-- before calling. ON CONFLICT DO UPDATE is still gated on silieco_user_id
...
INSERT INTO channel_user_binding (
    workspace_id, silieco_user_id, installation_id,
...
ON CONFLICT (installation_id, channel_user_id) DO UPDATE SET
    ...
    bound_at = now()
WHERE channel_user_binding.silieco_user_id = EXCLUDED.silieco_user_id
RETURNING *;

-- name: GetChannelUserBindingByUserID :one
-- The inbound identity lookup: does this platform user id map to a Silieco
...
-- Cross-installation account-link reuse (SILI-3911). When a platform user
-- messages an installation they have NOT linked, but the SAME user id is already
-- bound to ANOTHER installation in the SAME Silieco workspace + SAME Slack team,
...
-- different Silieco workspaces, and a user may hold different Silieco accounts in
...
DELETE FROM channel_user_binding
WHERE workspace_id = $1 AND silieco_user_id = $2;

-- name: DeleteChannelUserBindingsByInstallation :exec
-- Application-layer integrity (schema has no FK/cascade, SILI-3515 §4): drop
...
-- Application-layer stand-in for the old ON DELETE SET NULL (SILI-3515 §4,
...
-- Application-layer integrity (channel_* has no FK/cascade, SILI-3515 §4): drop the
