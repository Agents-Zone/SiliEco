...
const addChatSessionParticipant = `-- name: AddChatSessionParticipant :exec
...
func (q *Queries) AddChatSessionParticipant(ctx context.Context, arg AddChatSessionParticipantParams) error {
	_, err := q.db.Exec(ctx, addChatSessionParticipant,
		arg.ChatSessionID,
		arg.ParticipantType,
		arg.ParticipantID,
		arg.CreatedBy,
...
RETURNING id, chat_session_id, role, content, task_id, created_at, failure_reason, elapsed_ms, message_kind, channel_media_pending_until, channel_ingested, sender_user_id, sender_agent_id, mentioned_agent_ids
`
...
type CreateChatMessageParams struct {
	...
	MentionedAgentIds       []pgtype.UUID `json:"mentioned_agent_ids"`
}
...
RETURNING id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, last_read_at, is_agent_intro, pinned_at, project_id, kind
`
...
const createGroupChatTask = `-- name: CreateGroupChatTask :one
INSERT INTO agent_task_queue (
    agent_id, runtime_id, issue_id, status, priority, chat_session_id,
...
type CreateGroupChatTaskParams struct {
	AgentID              pgtype.UUID `json:"agent_id"`
	RuntimeID            pgtype.UUID `json:"runtime_id"`
	ChatSessionID        pgtype.UUID `json:"chat_session_id"`
	InitiatorUserID      pgtype.UUID `json:"initiator_user_id"`
...
func (q *Queries) CreateGroupChatTask(ctx context.Context, arg CreateGroupChatTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, createGroupChatTask,
		arg.AgentID,
		arg.RuntimeID,
		arg.ChatSessionID,
		arg.InitiatorUserID,
...
const deleteChatSessionParticipants = `-- name: DeleteChatSessionParticipants :exec
DELETE FROM chat_session_participant WHERE chat_session_id = $1
`

func (q *Queries) DeleteChatSessionParticipants(ctx context.Context, chatSessionID pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteChatSessionParticipants, chatSessionID)
	return err
}

const deleteChatSessionParticipantsByWorkspace = `-- name: DeleteChatSessionParticipantsByWorkspace :exec
...
func (q *Queries) DeleteChatSessionParticipantsByWorkspace(ctx context.Context, workspaceID pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteChatSessionParticipantsByWorkspace, workspaceID)
	return err
}

const deleteMemberFromGroupChatsInWorkspace = `-- name: DeleteMemberFromGroupChatsInWorkspace :exec
DELETE FROM chat_session_participant
...
type DeleteMemberFromGroupChatsInWorkspaceParams struct {
	ParticipantID pgtype.UUID `json:"participant_id"`
	WorkspaceID   pgtype.UUID `json:"workspace_id"`
}

func (q *Queries) DeleteMemberFromGroupChatsInWorkspace(ctx context.Context, arg DeleteMemberFromGroupChatsInWorkspaceParams) error {
	_, err := q.db.Exec(ctx, deleteMemberFromGroupChatsInWorkspace, arg.ParticipantID, arg.WorkspaceID)
	return err
}
...
SELECT id, chat_session_id, role, content, task_id, created_at, failure_reason, elapsed_ms, message_kind, channel_media_pending_until, channel_ingested, sender_user_id, sender_agent_id, mentioned_agent_ids FROM chat_message
WHERE id = $1
`
...
SELECT id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, last_read_at, is_agent_intro, pinned_at, project_id, kind FROM chat_session
WHERE id = $1
`
...
func (q *Queries) HasPendingChatTasksByCreator(ctx context.Context, arg HasPendingChatTasksByCreatorParams) (bool, error) {
	row := q.db.QueryRow(ctx, hasPendingChatTasksByCreator, arg.CreatorID, arg.WorkspaceID, arg.AgentIds)
	var has_pending bool
	err := row.Scan(&has_pending)
	return has_pending, err
}

const isChatSessionAgentParticipant = `-- name: IsChatSessionAgentParticipant :one
SELECT EXISTS (
...
type IsChatSessionAgentParticipantParams struct {
	ChatSessionID pgtype.UUID `json:"chat_session_id"`
	ParticipantID pgtype.UUID `json:"participant_id"`
}

func (q *Queries) IsChatSessionAgentParticipant(ctx context.Context, arg IsChatSessionAgentParticipantParams) (bool, error) {
	row := q.db.QueryRow(ctx, isChatSessionAgentParticipant, arg.ChatSessionID, arg.ParticipantID)
	var is_participant bool
	err := row.Scan(&is_participant)
	return is_participant, err
}

const isChatSessionMemberParticipant = `-- name: IsChatSessionMemberParticipant :one
SELECT EXISTS (
    SELECT 1 FROM chat_session_participant
    WHERE chat_session_id = $1
...
type IsChatSessionMemberParticipantParams struct {
	ChatSessionID pgtype.UUID `json:"chat_session_id"`
	ParticipantID pgtype.UUID `json:"participant_id"`
}

func (q *Queries) IsChatSessionMemberParticipant(ctx context.Context, arg IsChatSessionMemberParticipantParams) (bool, error) {
	row := q.db.QueryRow(ctx, isChatSessionMemberParticipant, arg.ChatSessionID, arg.ParticipantID)
	var is_participant bool
	err := row.Scan(&is_participant)
	return is_participant, err
}
...
SELECT cs.id, cs.workspace_id, cs.agent_id, cs.creator_id, cs.title, cs.session_id, cs.work_dir, cs.status, cs.created_at, cs.updated_at, cs.unread_since, cs.runtime_id, cs.last_read_at, cs.is_agent_intro, cs.pinned_at, cs.project_id, cs.kind,
       CASE WHEN cs.status = 'archived' THEN 0
            ELSE (SELECT count(*) FROM chat_message m
                    WHERE m.chat_session_id = cs.id
                      AND (
                        (cs.kind = 'direct' AND m.role = 'assistant')
                        OR (cs.kind = 'group' AND (m.sender_user_id IS NULL OR m.sender_user_id <> $1))
                      )
...
func (q *Queries) ListAllChatSessionsByCreator(ctx context.Context, arg ListAllChatSessionsByCreatorParams) ([]ListAllChatSessionsByCreatorRow, error) {
	rows, err := q.db.Query(ctx, listAllChatSessionsByCreator, arg.ParticipantID, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListChatSessionsByCreator(ctx context.Context, arg ListChatSessionsByCreatorParams) ([]ListChatSessionsByCreatorRow, error) {
	rows, err := q.db.Query(ctx, listChatSessionsByCreator, arg.ParticipantID, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListPendingChatTasksByCreator(ctx context.Context, arg ListPendingChatTasksByCreatorParams) ([]ListPendingChatTasksByCreatorRow, error) {
	rows, err := q.db.Query(ctx, listPendingChatTasksByCreator, arg.WorkspaceID, arg.ParticipantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const markGroupChatSessionRead = `-- name: MarkGroupChatSessionRead :exec
...
type MarkGroupChatSessionReadParams struct {
	ChatSessionID pgtype.UUID `json:"chat_session_id"`
	ParticipantID pgtype.UUID `json:"participant_id"`
}

func (q *Queries) MarkGroupChatSessionRead(ctx context.Context, arg MarkGroupChatSessionReadParams) error {
	_, err := q.db.Exec(ctx, markGroupChatSessionRead, arg.ChatSessionID, arg.ParticipantID)
	return err
}
