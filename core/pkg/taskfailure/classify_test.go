...
// sample taken from SILI-1949's db-boy production analysis (top error
// prefixes from `agent_task_queue.error` over a 7-day window). When
// SILI-1949's SQL grows a new rule, add a fixture here so the in-flight
...
func TestClassifyRules(t *testing.T) {
	...
	cases := []struct {
		...
		// SILI-5370: every Go-side context deadline used to land in
...
// overlapping rules. These cases caught regressions during SILI-2946 PR1
...
// TestNormalizeDaemonReason is the mixed-version regression for SILI-5370.
//
...
func TestNormalizeDaemonReason(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:   "pre-SILI-1949 coarse reason is upgraded",
			reason: "agent_error",
			raw:    legacyErr,
			want:   ReasonSkillBundleUnavailable,
		},
