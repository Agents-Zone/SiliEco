...
// regex `(^|[^0-9])5[0-9][0-9]([^0-9]|$)` from SILI-1949 — keeps phrases
...
// — SILI-4910 — and these regexes never route into it.) The 5xx bucket was
// already anchored for exactly this reason (SILI-1949); these codes were not.
var (
...
// empty input.
//
// The rule order mirrors the SQL CASE expression in SILI-1949
...
// 11 backend wrappers (core/pkg/agent/*) all surface upstream API
...
func Classify(rawError string) Reason {
	...
	//    and terminating the task (SILI-4910). Checked before rule 13 so the
	...
	//    awaiting headers` variant. Before SILI-5370 these all landed in
	...
	//    Mirror these substrings into the SILI-1949 offline backfill SQL.
	case containsAny(lower,
		"stream disconnected",
		"connection closed",
		"mid-response",
	...
	//    strings come from core/pkg/agent/*.go wrappers and are
	//    stable.
	case containsAny(lower,
...
// legacySkillBundlePrefix is the exact wrapper a pre-SILI-5370 daemon put on a
...
// pre-SILI-1949 coarse agent_error. None of them carries information that
...
//
// Why this exists (SILI-5370): installed daemons upgrade on their own cadence,
