...
import { sanitizeNextUrl, useAuthStore } from "@silieco/core/auth";
import { workspaceKeys } from "@silieco/core/workspace/queries";
import { paths, resolvePostAuthDestination } from "@silieco/core/paths";
import { api } from "@silieco/core/api";
import { validateCliCallback, redirectToCliCallback } from "@silieco/views/auth";
import {
  Card,
  CardHeader,
  CardTitle,
...
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    // headless/WSL2 `silieco login` can receive the JWT after browser-based
    ...
    if (cliCallback) {
      ...
      api
        ...
        .then(({ token }) => {
          ...
          window.location.href = `silieco://auth/callback?token=${encodeURIComponent(token)}`;
        })
        .catch((err) => {
          setError(err instanceof Error ? err.message : "Login failed");
        });
      ...
      loginWithGoogle(code, redirectUri)
        .then(async (loggedInUser) => {
          ...
          //    login on silieco.ai instead of clicking the email link,
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-display-sm">Opening Silieco</CardTitle>
            <CardDescription>
              You should see a prompt to open the Silieco desktop app. If
          ...
          <CardContent className="flex justify-center">
            <Button
              variant="outline"
              onClick={() => {
                window.location.href = `silieco://auth/callback?token=${encodeURIComponent(desktopToken)}`;
              }}
            >
              Open Silieco Desktop
            </Button>
          </CardContent>
        </Card>
      </div>
