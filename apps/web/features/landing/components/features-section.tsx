...
import { StatusIcon, PriorityIcon } from "@silieco/views/issues/components";
import { STATUS_CONFIG } from "@silieco/core/issues/config/status";
import { PRIORITY_CONFIG } from "@silieco/core/issues/config/priority";
import type { IssueStatus, IssuePriority } from "@silieco/core/types";
    ...
    <div className="relative aspect-video overflow-hidden rounded-lg border bg-background text-foreground shadow-2xl">
      ...
      <div className="flex h-10 shrink-0 items-center border-b bg-background px-4 text-body">
        <div className="flex items-center gap-1.5 min-w-0 text-caption">
          <span className="text-muted-foreground">Silieco Demo</span>
          <ChevronRight className="h-3 w-3 text-muted-foreground/50 shrink-0" />
          <span className="text-muted-foreground">SILI-18</span>
          <ChevronRight className="h-3 w-3 text-muted-foreground/50 shrink-0" />
...
const mockToolCalls = [
  ...
  { type: "tool_use" as const, tool: "Read", summary: "core/internal/handler/task.go" },
  { type: "tool_result" as const, preview: "func (h *TaskHandler) Create(w http.ResponseWriter, r *http.Request) { …" },
  { type: "tool_use" as const, tool: "Edit", summary: "core/internal/handler/task.go: replace writeJSON error calls" },
  ...
  { type: "tool_use" as const, tool: "Read", summary: "core/internal/handler/comment.go" },
  ...
  { type: "tool_result" as const, preview: "ok  \tgithub.com/silieco/core/internal/handler\t0.847s" },
];

const mockTaskHistory = [
  { status: "completed" as const, title: "Set up error response types", duration: "2m 14s" },
  { status: "completed" as const, title: "Migrate task handler", duration: "3m 41s" },
