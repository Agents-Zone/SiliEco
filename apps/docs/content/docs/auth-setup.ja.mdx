...
Silieco はデフォルトでメール認証コードによるログインを使い、Google OAuth を追加することもできます。既存のユーザーはいつでも再ログインできます。サインアップ制限は、新しいアカウントを作成できるかどうかだけを決めます。

## メール認証コード

ユーザーがメールアドレスを入力すると、Silieco は 6 桁の認証コードを送信します。コードは 10 分間有効で、検証に成功するとブラウザにログイン cookie が発行されます。
...
公開インスタンスで固定認証コードを有効にしないでください。プロダクションの組み合わせは `APP_ENV=production` と空の `SILIECO_DEV_VERIFICATION_CODE` です。
</Callout>

## Google ログイン

1. [Google Cloud Console](https://console.cloud.google.com/) で OAuth 2.0 client を作成します。
2. Silieco フロントエンドのコールバックアドレスを **Authorized redirect URIs** に追加します:
...
- 相手がすでに Silieco アカウントを持っている場合は、ログインして招待を受諾できます。
