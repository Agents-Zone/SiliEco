---
title: 환경 변수
description: 자체 호스팅 Silieco에서 자주 사용하는 서버, 저장소, 연동, 런타임 설정입니다.
---

import { Callout } from "fumadocs-ui/components/callout";

Silieco는 프로세스를 시작할 때 환경 변수를 읽습니다. 수정 후에는 보통 해당 API, Web 또는 데몬을 다시 시작해야 합니다. Docker Compose의 `docker compose restart`는 `.env`를 다시 읽지 않으므로 `up -d`로 컨테이너를 다시 만들어야 적용됩니다.

이 페이지에는 배포자를 위한 설정만 나열하며 테스트 변수와 내부 Run 변수는 다루지 않습니다. 여기서는 그룹별 참조를 제공하고 전체 배포 단계는 [자체 호스팅 빠른 시작](/self-host-quickstart)을 참고하세요.
...
프로덕션 환경에서 기본 `JWT_SECRET`을 사용하거나 `SILIECO_DEV_VERIFICATION_CODE`를 설정하지 마세요.
</Callout>
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `DATABASE_URL` | 로컬 `silieco` 데이터베이스 | PostgreSQL 연결 주소 |
...
| `SILIECO_SHUTDOWN_HOLD_DURATION` | `0` | 종료 신호를 받은 뒤 graceful shutdown을 시작하기 전 대기 시간 |
...
| `SILIECO_APP_URL` | `FRONTEND_ORIGIN`으로 fallback | 사용자가 접근할 수 있는 Web 주소. CLI 로그인과 계정 연결 링크에 사용 |
| `SILIECO_PUBLIC_URL` | 비어 있음 | 공개 API 주소. webhook URL과 런타임 연결 안내에 사용 |
...
| `RESEND_FROM_EMAIL` | `noreply@silieco.ai` | 발신 주소. 인증된 도메인에 속해야 함 |
...
| `SILIECO_DEV_VERIFICATION_CODE` | 비어 있음 | production이 아닌 환경에서 사용하는 고정 6자리 테스트 인증 코드 |

allowlist의 정확한 판단 순서는 [로그인과 가입](/auth-setup#가입-범위-제어)을 참고하세요.

## 첨부 파일 저장소

`S3_BUCKET`을 설정하지 않으면 Silieco가 로컬 디스크를 사용합니다.
...
| `SILIECO_TRUSTED_PROXIES` | 비어 있음 | 자동화 webhook과 실시간 연결에서 사용할 신뢰 proxy CIDR |
...
| Feishu | `SILIECO_LARK_SECRET_KEY` | base64로 인코딩한 32바이트 자격 증명 암호화 키 |
| Slack | `SILIECO_SLACK_SECRET_KEY` | base64로 인코딩한 32바이트 token 암호화 키 |
| Composio | `COMPOSIO_API_KEY` | Composio 도구 연결 활성화 |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | callback API 주소. `SILIECO_PUBLIC_URL`로 fallback 가능 |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state 서명 키. `JWT_SECRET`에서 파생 가능 |
| 자체 호스팅 Git | `SILIECO_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab 연동 스위치. compose에서는 기본 활성화 |
| 자체 호스팅 Git | `SILIECO_VCS_SECRET_KEY` | base64로 인코딩한 32바이트 암호화 키(`openssl rand -base64 32`). 없으면 전체 기능을 사용할 수 없음 |
...
이 설정 그룹은 대화 제목 같은 서버 측 보조 생성 기능에 사용됩니다. 에이전트가 Run를 실행할 때 사용하는 AI 코딩 도구의 자격 증명이 아닙니다.

| 변수 | 기본값 | 설명 |
|---|---|---|
| `SILIECO_LLM_API_KEY` | 비어 있음 | OpenAI 호환 API key |
| `SILIECO_LLM_BASE_URL` | 비어 있음 | OpenAI 호환 endpoint |
| `SILIECO_LLM_DEFAULT_MODEL` | 내장 기본값 | 요청에 모델이 지정되지 않았을 때 사용 |
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `SILIECO_SERVER_URL` | `ws://localhost:8080/ws` | Silieco API / WebSocket 주소. `http(s)`도 허용 |
| `SILIECO_DAEMON_DEVICE_NAME` | host 이름 | 런타임 목록의 기기 이름 |
| `SILIECO_AGENT_RUNTIME_NAME` | `Local Agent` | 런타임 표시 이름 |
| `SILIECO_DAEMON_POLL_INTERVAL` | `30s` | wakeup 이벤트가 없을 때 실행 Run polling 간격 |
| `SILIECO_DAEMON_HEARTBEAT_INTERVAL` | `15s` | heartbeat 간격 |
| `SILIECO_DAEMON_MAX_CONCURRENT_TASKS` | `20` | 데몬 하나의 동시 실행 Run 상한 |
| `SILIECO_AGENT_TIMEOUT` | `0` | 단일 실행의 절대 시간 제한. `0`은 제한 없음 |
| `SILIECO_AGENT_IDLE_WATCHDOG` | `30m` | 출력과 도구 실행이 모두 없을 때의 무응답 상한 |
| `SILIECO_AGENT_TOOL_WATCHDOG` | `2h` | 단일 도구 호출이 계속 무응답인 시간 상한 |
| `SILIECO_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode 전용 무응답 기준값 |
| `SILIECO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex 의미 활동 없음 기준값 |
| `SILIECO_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server 시작 handshake 상한 |
| `SILIECO_DAEMON_AUTO_UPDATE` | Cloud `true`, 자체 호스팅 `false` | CLI 자동 확인 및 업데이트 여부 |
| `SILIECO_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | 업데이트 확인 간격 |
| `SILIECO_WORKSPACES_ROOT` | `~/silieco_workspaces` | 실행 Run 작업 디렉터리의 루트 |
| `SILIECO_KEEP_ENV_AFTER_TASK` | `false` | 디버깅을 위해 실행 Run 디렉터리 유지 |

각 AI 코딩 도구는 `SILIECO_<PROVIDER>_PATH`와 `SILIECO_<PROVIDER>_MODEL`로 명령 경로와 기본 모델을 덮어쓸 수 있습니다. 컴퓨터 전체 기본 인수 `SILIECO_<PROVIDER>_ARGS`는 현재 Claude Code, Codex, CodeBuddy, Qwen Code 네 도구를 지원합니다. 해당 변수는 `SILIECO_CLAUDE_ARGS`, `SILIECO_CODEX_ARGS`, `SILIECO_CODEBUDDY_ARGS`, `SILIECO_QWEN_ARGS`입니다. 예시:
...
우선순위는 명령줄 flag → 환경 변수 → `~/.silieco/config.json` → 내장 기본값입니다. watchdog 동작은 [데몬과 런타임](/daemon-runtimes)을 참고하세요.

### 데몬 설정 영속화

자주 사용하는 데몬 측 설정은 shell 환경 변수에 의존하지 않고 `~/.silieco/config.json`에 기록할 수도 있습니다. 이름 있는 profile의 설정 파일은 `~/.silieco/profiles/<name>/config.json`에 있습니다.
...
| key | 기본값 | 설명 |
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Silieco API / WebSocket 주소 |
...
| `max_concurrent_tasks` | `20` | 동시 실행 Run 상한. `0` 또는 빈 값은 설정되지 않음을 의미 |
| `poll_interval` | `30s` | 실행 Run polling 간격 |
...
- 빈 문자열을 전달하면 저장된 값을 지우고 환경 변수 또는 내장 기본값으로 돌아갑니다. 예: `silieco config set poll_interval ""`
