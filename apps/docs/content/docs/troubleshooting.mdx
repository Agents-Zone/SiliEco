...
First work out which layer the problem lives in: the Silieco service, the daemon, the runtime, or the AI coding tool. These commands are usually enough to surface the first meaningful error:

```bash
silieco version
silieco auth status
silieco daemon status --output json
silieco daemon logs --lines 100
```
...
- The daemon is connected to the wrong Silieco service.
...
Self-hosted deployments should also request the API's `/health` from the **executing computer** — testing on the server itself cannot expose DNS, TLS, or firewall problems on the executing computer's side. To change the address, rerun `silieco setup self-host`, or check the current profile's `server_url`:

```bash
silieco config show
```

## Task does not start executing

Open the task's **execution log** and check the run's current status and what it is waiting for.
...
An agent runs at most 6 runs concurrently by default; a single daemon runs at most 20. At the limit, new runs stay in the queue until a running one finishes. Runs also keep queueing while the runtime is offline; they only fail after going unclaimed for more than 2 hours.

```bash
silieco daemon status --output json
silieco agent get <agent-id>
silieco task runs <task-id>
```

If the runtime list is missing an expected tool, first confirm the tool runs and is signed in under the same system account and `PATH`, then run `silieco daemon restart`.

### `waiting_local_directory` status

This means another in-flight run is using the same local directory. Silieco waits for the directory lock to be released so two agents never modify the same files at once.

Usually you just wait for the earlier run to finish. If it is stuck, stop it from its execution log, or pick a different local directory for the current agent. This directory mutex lives in the daemon's memory — no lock file is written to disk. If you suspect stale lock state, `silieco daemon restart` releases it; there is nothing to delete manually.
...
When `SMTP_HOST` and Resend are both configured, Silieco prefers SMTP. For configuration, see [Sign-in and signup](/auth-setup#email-verification-codes).
...
If it is another Silieco checkout, run `make stop` in that directory first. Otherwise stop the offending program normally, or change the current service's port. Public ports 80/443 are listened on by a reverse proxy such as Caddy or Nginx.

## Log locations

| Component | How to view |
| --- | --- |
| Background daemon | `silieco daemon logs --lines 100` |
| Follow daemon logs live | `silieco daemon logs --follow` |
| Default profile's log file | `~/.silieco/daemon.log` |
| Default profile's startup or crash log | `~/.silieco/daemon.err.log` |
| Named profiles | the matching logs under `~/.silieco/profiles/<name>/` |
...
If you still cannot pin it down, search for the problem or report it on [GitHub Issues](https://github.com/silieco-ai/silieco/issues).

## Next steps

- [Daemon and runtimes](/daemon-runtimes) — how runtimes register and report online status.
- [Runs](/runs) — status, timeout, and failure reference.
