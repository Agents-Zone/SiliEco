---
title: 環境変数
description: セルフホスト Silieco でよく使うサーバー、ストレージ、連携、ランタイムの設定。
---

import { Callout } from "fumadocs-ui/components/callout";

Silieco はプロセス起動時に環境変数を読み込みます。変更後は、該当する API・Web・デーモンのプロセスを再起動してください。Docker Compose の `docker compose restart` は `.env` を再読み込みしません。`up -d` でコンテナを再作成して初めて反映されます。

このページにはデプロイ運用者向けの設定のみを載せています。テスト用変数や内部Run用の変数は扱いません。グループ別のリファレンスとして使い、完全なデプロイ手順は[セルフホストクイックスタート](/self-host-quickstart)を参照してください。
...
プロダクションでデフォルトの `JWT_SECRET` を使わないでください。`SILIECO_DEV_VERIFICATION_CODE` も設定しないでください。
</Callout>
...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `DATABASE_URL` | ローカルの `silieco` データベース | PostgreSQL 接続文字列 |
...
| `SILIECO_SHUTDOWN_HOLD_DURATION` | `0` | 終了シグナル受信後、グレースフルシャットダウンを開始するまでの待機時間 |
...
| `SILIECO_APP_URL` | `FRONTEND_ORIGIN` にフォールバック | ユーザーが到達できる Web アドレス。CLI ログインとアカウント連携リンクが使います |
| `SILIECO_PUBLIC_URL` | 空 | 公開 API アドレス。webhook URL とランタイム接続案内に使われます |
...
| `RESEND_FROM_EMAIL` | `noreply@silieco.ai` | 送信元アドレス。検証済みドメインに属している必要があります |
...
| `SILIECO_DEV_VERIFICATION_CODE` | 空 | 非 production 環境で使う固定 6 桁のテスト認証コード |

allowlist の正確な判定順序は[ログインとサインアップ](/auth-setup#サインアップ制限)を参照してください。

## 添付ファイルストレージ

`S3_BUCKET` が未設定の場合、Silieco はローカルディスクを使います。
...
| `SILIECO_TRUSTED_PROXIES` | 空 | オートパイロット webhook とリアルタイム接続が使う信頼済みプロキシ CIDR |
...
| Lark | `SILIECO_LARK_SECRET_KEY` | base64 エンコードされた 32 バイトの資格情報暗号化キー |
| Slack | `SILIECO_SLACK_SECRET_KEY` | base64 エンコードされた 32 バイトの token 暗号化キー |
| Composio | `COMPOSIO_API_KEY` | Composio ツール接続を有効化 |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | コールバック API アドレス。`SILIECO_PUBLIC_URL` にフォールバック可能 |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state の署名シークレット。`JWT_SECRET` から派生可能 |
| セルフホスト Git | `SILIECO_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab 連携のスイッチ。compose ではデフォルトで有効 |
| セルフホスト Git | `SILIECO_VCS_SECRET_KEY` | base64 エンコードされた 32 バイトの暗号化キー（`openssl rand -base64 32`）。未設定の場合、この機能全体が利用できません |
...
このグループは、会話タイトルなどサーバー側の補助生成機能の設定です。エージェントがRunを実行するときに使う AI コーディングツールの資格情報ではありません。

| 変数 | デフォルト | 説明 |
|---|---|---|
| `SILIECO_LLM_API_KEY` | 空 | OpenAI 互換の API key |
| `SILIECO_LLM_BASE_URL` | 空 | OpenAI 互換の endpoint |
| `SILIECO_LLM_DEFAULT_MODEL` | 組み込みデフォルト | リクエストがモデルを指定しない場合に使用 |
...
| 変数 | デフォルト | 説明 |
|---|---|---|
| `SILIECO_SERVER_URL` | `ws://localhost:8080/ws` | Silieco API / WebSocket アドレス。`http(s)` も受け付けます |
| `SILIECO_DAEMON_DEVICE_NAME` | ホスト名 | ランタイム一覧に表示されるデバイス名 |
| `SILIECO_AGENT_RUNTIME_NAME` | `Local Agent` | ランタイムの表示名 |
| `SILIECO_DAEMON_POLL_INTERVAL` | `30s` | ウェイクイベントがないときのRunポーリング間隔 |
| `SILIECO_DAEMON_HEARTBEAT_INTERVAL` | `15s` | ハートビート間隔 |
| `SILIECO_DAEMON_MAX_CONCURRENT_TASKS` | `20` | デーモン 1 つあたりの同時実行Run上限 |
| `SILIECO_AGENT_TIMEOUT` | `0` | 1 回の実行の絶対時間上限。`0` は上限なし |
| `SILIECO_AGENT_IDLE_WATCHDOG` | `30m` | 出力もツール実行もない状態の静穏上限 |
| `SILIECO_AGENT_TOOL_WATCHDOG` | `2h` | 単一のツール呼び出しが静穏なままでいられる上限 |
| `SILIECO_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode 専用の静穏しきい値 |
| `SILIECO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex のセマンティック静穏しきい値 |
| `SILIECO_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server 起動ハンドシェイクの上限 |
| `SILIECO_DAEMON_AUTO_UPDATE` | Cloud は `true`、セルフホストは `false` | CLI の更新を自動でチェックして適用するか |
| `SILIECO_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | 更新チェックの間隔 |
| `SILIECO_WORKSPACES_ROOT` | `~/silieco_workspaces` | Run作業ディレクトリのルート |
| `SILIECO_KEEP_ENV_AFTER_TASK` | `false` | デバッグ用にRunディレクトリを保持 |

各 AI コーディングツールは `SILIECO_<PROVIDER>_PATH` と `SILIECO_<PROVIDER>_MODEL` でコマンドパスとデフォルトモデルを上書きできます。マシン全体のデフォルト引数 `SILIECO_<PROVIDER>_ARGS` は、現在 Claude Code、Codex、CodeBuddy、Qwen Code の 4 ツールに対応しています。対応する変数は `SILIECO_CLAUDE_ARGS`、`SILIECO_CODEX_ARGS`、`SILIECO_CODEBUDDY_ARGS`、`SILIECO_QWEN_ARGS` です。例:
...
優先順位はコマンドラインフラグ → 環境変数 → `~/.silieco/config.json` → 組み込みデフォルトです。watchdog の動作は[デーモンとランタイム](/daemon-runtimes)を参照してください。

### デーモン設定の永続化

デーモン側のよく使う設定は、シェルの環境変数に頼らず `~/.silieco/config.json` に書き込むこともできます。名前付きプロファイルの設定ファイルは `~/.silieco/profiles/<name>/config.json` にあります:
...
| キー | デフォルト | 説明 |
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Silieco API / WebSocket アドレス |
...
| `max_concurrent_tasks` | `20` | 同時実行Run上限。`0` または空は未設定を意味します |
| `poll_interval` | `30s` | Runのポーリング間隔 |
...
- 空文字列を渡すと永続化された値がクリアされ、環境変数または組み込みデフォルトに戻ります。例: `silieco config set poll_interval ""`。
