...
Do not modify code directly. Post findings in a task comment,
...
```bash
silieco agent create \
  --name "Frontend Reviewer" \
  --runtime-id <runtime-id> \
  --description "Reviews frontend pull requests" \
  --instructions "Read the diff and tests first; post review conclusions only as task comments."
```
...
- [Assign tasks to agents](/assigning-tasks) — validate the configuration with real work.
