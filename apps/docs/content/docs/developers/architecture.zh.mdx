---
title: 项目架构
description: 了解 Silieco 的客户端、Go 服务、执行守护进程和共享前端包如何协作。
---

Silieco 由一个 Go backend、多种客户端和运行在执行电脑上的守护进程组成。PostgreSQL 保存协作数据；守护进程领取 task，并调用本机的 AI 编程工具。
...
面向产品用户的解释见 [Silieco 的工作方式](/how-silieco-works)。本页关注代码如何分层。

## 仓库结构

| 目录 | 职责 | 主要技术 |
| --- | --- | --- |
| `core/` | API、认证、任务调度、集成、CLI 和守护进程 | Go、Chi、sqlc、gorilla/websocket |
...
Mobile 不复用这些 React 页面。它可以导入 `@silieco/core` 的类型和纯函数，但拥有自己的 UI、query key、状态、实时订阅和发布流程。
