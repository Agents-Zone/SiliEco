---
title: 参与开发
description: 配置 Silieco 本地开发环境，运行测试，并按仓库约定提交改动。
---

import { Callout } from "fumadocs-ui/components/callout";

Silieco 是 Go backend 与 pnpm monorepo。最简单的本地启动方式是 `make dev`；它会为当前 checkout 准备环境、数据库和 migration，再启动 Web 与 API。
...
版本以根目录 `package.json`、`core/go.mod` 和 CI workflow 为准。
...
Migration 位于 `core/migrations/`，查询位于 `core/pkg/db/queries/`。
...
5. 修改查询后运行 `make sqlc`，提交生成的 `core/pkg/db/generated/` 变化；
6. 不直接编辑 sqlc 生成文件。
