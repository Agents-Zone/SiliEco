...
写 Silieco 代码、改翻译、写中文产品文案，都从这一页查。
...
| `packages/ui` | 无业务依赖 | `@silieco/core`、业务逻辑 |
...
| `apps/mobile/` | `@silieco/core` 的类型和纯函数 | Web/Desktop 的 React 页面、store 和平台实现 |
...
`AgentRuntime.name` 是 daemon 的原始技术名（例如 `Codex (host)`），用户别名在 `custom_name`。用户可见文案不得直接渲染 `runtime.name`，要用共享 helper，保证别名和 provider 一致（SILI-5248、#5260）：
...
每个 issue 有人类可读的编号，比如 `SILI-123`：工作区 `issue_prefix`（大写字母和数字，通常 3 个字符，最长 10 个）+ 流水号。工作区管理员可以在**设置 → 通用**中修改前缀；修改会让所有现有 issue 重新编号，外部引用——PR 标题、分支名、文档与聊天里的链接——里的旧前缀会失效。
...
- **实体（typed entity）** —— 有 URL、有数据库 row、是 API 响应里某种 type 的东西。中文里**用小写英文**呈现，视觉上像类型名，告诉读者"这是 Silieco 系统里的特定实体"。
...
`issue` / `skill` / `task` 是 Silieco 的核心实体。schema 字段、API 字段、产品 UI 标签都用英文。中文里采用**混合规则** —— 词出现在哪里决定怎么写：
...
| 类别 | 词 |
| --- | --- |
| 品牌 | **Silieco**、GitHub、Slack、Google、Anthropic、OpenAI、Claude、Codex、Cursor、Linear、Jira |
