---
title: 開発に参加する
description: Silieco のローカル開発環境を構築し、テストを実行して、リポジトリの規約に従って変更を提出します。
---

import { Callout } from "fumadocs-ui/components/callout";

Silieco は Go バックエンドと pnpm monorepo で構成されています。ローカルで起動する最も簡単な方法は `make dev` です。現在の checkout 用に環境、データベース、migration を準備し、Web と API を起動します。
...
バージョンについては、ルートの `package.json`、`core/go.mod`、CI workflow を信頼できる情報源としてください。
...
Migration は `core/migrations/`、クエリは `core/pkg/db/queries/` にあります。
...
5. クエリ変更後に `make sqlc` を実行し、生成された `core/pkg/db/generated/` の変更をコミットします。
