...
If you write Silieco code, change a translation, or write Chinese product copy, this is the page to reference.
...
| `packages/ui` | nothing | `@silieco/core`, business logic |
...
| `apps/mobile/` | types and pure functions from `@silieco/core` | Web/Desktop React pages, stores, and platform implementations |
...
`AgentRuntime.name` is the daemon's raw technical name (e.g. `Codex (host)`); the user's alias lives in `custom_name`. User-visible copy must never render `runtime.name` directly — use the shared helpers so aliases and provider stay consistent (SILI-5248, #5260):
...
Silieco's product nouns split into two categories:

- **Entity** — has a URL, a database row, an API type. In Chinese text, render as **lowercase English** so it visually reads like a type name and signals "this is a Silieco system entity".
...
`issue` / `skill` / `task` are Silieco's core entities. They have schema columns, API fields, and product UI labels that are all English. In Chinese text, they follow a **mixed rule** — what to use depends on where the word appears:
...
- **`skill`**: Silieco-specific concept with no established Chinese term.
...
| Category | Terms |
| --- | --- |
| Brands | **Silieco**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
