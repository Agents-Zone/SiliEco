---
title: プロジェクトアーキテクチャ
description: Silieco のクライアント、Go サービス、実行デーモン、共有フロントエンドパッケージがどのように連携するかを説明します。
---

Silieco は Go バックエンド、複数のクライアント、実行用コンピューター上で動作するデーモンで構成されています。PostgreSQL がコラボレーションデータを保存し、デーモンがタスクを取得してローカルの AI コーディングツールを呼び出します。
...
プロダクト利用者向けの説明は [Silieco の仕組み](/how-silieco-works)を参照してください。このページではコードの階層構造に焦点を当てます。

## リポジトリ構成

| ディレクトリ | 責務 | 主な技術 |
| --- | --- | --- |
| `core/` | API、認証、タスクスケジューリング、インテグレーション、CLI、デーモン | Go、Chi、sqlc、gorilla/websocket |
...
Mobile はこれらの React ページを再利用しません。`@silieco/core` の型や純粋関数をインポートできますが、UI、query key、状態、リアルタイム購読、リリースフローは独自に持ちます。
...
Silieco には異なる 2 つの WebSocket パスがあります。
