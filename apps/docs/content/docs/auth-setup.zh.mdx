...
Silieco 默认使用邮箱验证码登录，也可以增加 Google OAuth。已经存在的用户始终可以再次登录；注册限制只决定是否允许创建新账号。

## 邮箱验证码

用户输入邮箱后，Silieco 会发送一个 6 位验证码。验证码 10 分钟内有效，验证成功后浏览器获得登录 cookie。
...
不要在公网实例中启用固定验证码。生产环境的组合是 `APP_ENV=production` 加空的 `SILIECO_DEV_VERIFICATION_CODE`。
</Callout>

## Google 登录

1. 在 [Google Cloud Console](https://console.cloud.google.com/) 创建 OAuth 2.0 client。
2. 把 Silieco 前端的回调地址加入 **Authorized redirect URIs**：
