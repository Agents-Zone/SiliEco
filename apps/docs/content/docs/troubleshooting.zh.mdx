...
先确认问题发生在哪一层：Silieco 服务、守护进程、运行时，还是 AI 编程工具。下面几条命令通常足以找到第一条有效错误：
...
自托管部署还需要从**执行电脑**请求 API 的 `/health`——服务器本机测试无法暴露执行电脑侧的 DNS、TLS 或防火墙问题。需要修改地址时，重新运行 `silieco setup self-host`，或检查当前 profile 的 `server_url`：
...
打开 task 的**执行日志**，先看执行的当前状态和等待原因。

### `queued` 状态

`queued` 表示执行还在等待运行时领取。依次检查：
...
智能体默认最多同时执行 6 条执行；单个守护进程默认最多同时执行 20 条。达到上限时，新执行会留在队列中，等已有执行结束后再开始。运行时离线时，执行也会继续排队；超过 2 小时仍未领取才会失败。
...
如果运行时列表缺少预期工具，先在同一个系统账号和 `PATH` 中确认工具可以运行并且已经登录，再执行 `silieco daemon restart`。

### `waiting_local_directory` 状态

这表示另一条在途执行正在使用同一个本地目录。Silieco 会等待目录锁释放，避免两个智能体同时修改同一份文件。

通常只需等前一条执行结束。若前一条已经卡住，可以从它的执行日志中停止；也可以为当前智能体选择其他本地目录。这个目录互斥由守护进程在内存中维护，不写磁盘锁文件；怀疑锁状态异常时运行 `silieco daemon restart` 即可释放，没有需要手动删除的文件。
...
先在执行电脑的终端中直接运行同一款工具。工具自身也无法开始时，先修复工具登录或配置，再从执行日志重试执行。

## 实时更新失效

Task仍能执行、但评论和状态不能实时出现，通常是 WebSocket 没有连通。
...
`SMTP_HOST` 和 Resend 同时配置时，Silieco 会优先使用 SMTP。配置方式见[登录与注册](/auth-setup#邮箱验证码)。
...
Usage 页面读取按小时汇总的数据，不直接读取每条执行的原始用量。先确认原始数据和汇总表：
...
如果它是另一个 Silieco checkout，先在那个目录运行 `make stop`。否则正常停止占用端口的程序，或修改当前服务端口。公网 80/443 由 Caddy、Nginx 等反向代理监听。

## 日志位置

| 组件 | 查看方式 |
| --- | --- |
| 后台守护进程 | `silieco daemon logs --lines 100` |
| 实时跟随守护进程日志 | `silieco daemon logs --follow` |
| 默认 profile 的日志文件 | `~/.silieco/daemon.log` |
| 默认 profile 的启动或崩溃日志 | `~/.silieco/daemon.err.log` |
| 命名 profile | `~/.silieco/profiles/<name>/` 下的对应日志 |
...
仍然无法定位时，到 [GitHub Issues](https://github.com/silieco-ai/silieco/issues) 搜索已有问题或提交问题报告。

## 接下来

- [守护进程与运行时](/daemon-runtimes) — 运行时注册与在线状态机制。
- [执行](/runs) — 状态、超时与失败原因参考。
