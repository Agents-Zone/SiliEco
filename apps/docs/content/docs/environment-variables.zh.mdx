---
title: 环境变量
description: 自托管 Silieco 常用的服务端、存储、集成和运行时配置。
---

import { Callout } from "fumadocs-ui/components/callout";

Silieco 在进程启动时读取环境变量。修改后通常需要重启对应的 API、Web 或守护进程。Docker Compose 的 `docker compose restart` 不会重新读取 `.env`，要用 `up -d` 重建容器才能生效。

本页只列面向部署者的配置；测试变量和内部Task变量不在这里展开。本页是分组参考；完整部署步骤见[自托管快速上手](/self-host-quickstart)。
...
不要在生产环境使用默认的 `JWT_SECRET`，也不要设置 `SILIECO_DEV_VERIFICATION_CODE`。
</Callout>
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `DATABASE_URL` | 本地 `silieco` 数据库 | PostgreSQL 连接地址 |
...
| `LOG_LEVEL` | 应用默认 | 日志级别 |
| `SILIECO_SHUTDOWN_HOLD_DURATION` | `0` | 收到终止信号后，开始优雅退出前等待多久 |
...
| `SILIECO_APP_URL` | 回退到 `FRONTEND_ORIGIN` | 用户可访问的 Web 地址；CLI 登录和账号绑定链接使用它 |
| `SILIECO_PUBLIC_URL` | 空 | 公网 API 地址；用于 webhook URL 和运行时连接说明 |
...
| `RESEND_FROM_EMAIL` | `noreply@silieco.ai` | 发件地址，必须属于已验证域名 |
...
| `SILIECO_DEV_VERIFICATION_CODE` | 空 | 非 production 环境使用的固定 6 位测试验证码 |

白名单的准确判断顺序见[登录与注册](/auth-setup#注册范围控制)。

## 附件存储

没有设置 `S3_BUCKET` 时，Silieco 使用本地磁盘。
...
| `SILIECO_TRUSTED_PROXIES` | 空 | 自动化 webhook 与实时连接使用的可信代理 CIDR |
...
| 飞书 | `SILIECO_LARK_SECRET_KEY` | base64 编码的 32 字节凭据加密密钥 |
| Slack | `SILIECO_SLACK_SECRET_KEY` | base64 编码的 32 字节 token 加密密钥 |
| Composio | `COMPOSIO_API_KEY` | 启用 Composio 工具连接 |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | 回调 API 地址；可回退到 `SILIECO_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state 签名密钥；可从 `JWT_SECRET` 派生 |
| 自托管 Git | `SILIECO_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab 集成开关，compose 默认开启 |
| 自托管 Git | `SILIECO_VCS_SECRET_KEY` | base64 编码的 32 字节加密密钥（`openssl rand -base64 32`）；未配置时该功能整体不可用 |
...
这组配置用于服务端的辅助生成能力，例如对话标题；它不是智能体执行时使用的 AI 编程工具凭据。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `SILIECO_LLM_API_KEY` | 空 | OpenAI 兼容 API key |
| `SILIECO_LLM_BASE_URL` | 空 | OpenAI 兼容 endpoint |
| `SILIECO_LLM_DEFAULT_MODEL` | 内置默认 | 请求未指定模型时使用 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `SILIECO_SERVER_URL` | `ws://localhost:8080/ws` | Silieco API / WebSocket 地址，也接受 `http(s)` |
| `SILIECO_DAEMON_DEVICE_NAME` | 主机名 | 运行时列表中的设备名 |
| `SILIECO_AGENT_RUNTIME_NAME` | `Local Agent` | 运行时显示名 |
| `SILIECO_DAEMON_POLL_INTERVAL` | `30s` | 没有唤醒事件时的执行轮询间隔 |
| `SILIECO_DAEMON_HEARTBEAT_INTERVAL` | `15s` | 心跳间隔 |
| `SILIECO_DAEMON_MAX_CONCURRENT_TASKS` | `20` | 单个守护进程的并发执行上限 |
| `SILIECO_AGENT_TIMEOUT` | `0` | 单次执行的绝对时限；`0` 表示不设置 |
| `SILIECO_AGENT_IDLE_WATCHDOG` | `30m` | 没有输出且没有工具执行时的静默上限 |
| `SILIECO_AGENT_TOOL_WATCHDOG` | `2h` | 单个工具调用持续静默的上限 |
| `SILIECO_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode 专用静默阈值 |
| `SILIECO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex 语义静默阈值 |
| `SILIECO_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server 启动握手上限 |
| `SILIECO_DAEMON_AUTO_UPDATE` | Cloud `true`；自托管 `false` | 是否自动检查并更新 CLI |
| `SILIECO_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | 检查更新的间隔 |
| `SILIECO_WORKSPACES_ROOT` | `~/silieco_workspaces` | 执行工作目录的根目录 |
| `SILIECO_KEEP_ENV_AFTER_TASK` | `false` | 保留执行目录用于调试 |

各 AI 编程工具可以使用 `SILIECO_<PROVIDER>_PATH` 和 `SILIECO_<PROVIDER>_MODEL` 覆盖命令路径与默认模型。全机默认参数 `SILIECO_<PROVIDER>_ARGS` 目前支持 Claude Code、Codex、CodeBuddy 和 Qwen Code 四款工具。对应变量是 `SILIECO_CLAUDE_ARGS`、`SILIECO_CODEX_ARGS`、`SILIECO_CODEBUDDY_ARGS` 和 `SILIECO_QWEN_ARGS`。例如：
...
优先级为命令行 flag → 环境变量 → `~/.silieco/config.json` → 内置默认。watchdog 的行为见[守护进程与运行时](/daemon-runtimes)。

### 守护进程配置的持久化

守护进程侧的常用配置也可以写入 `~/.silieco/config.json`，不必依赖 shell 环境变量；命名 profile 的配置文件在 `~/.silieco/profiles/<name>/config.json`：
...
| 键 | 默认值 | 说明 |
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Silieco API / WebSocket 地址 |
| `app_url` | 空 | 浏览器登录使用的 Web 地址 |
| `workspace_id` | 空 | 默认工作区 |
| `device_name` | 主机名 | 运行时列表中的设备名 |
| `runtime_name` | `Local Agent` | 运行时显示名 |
| `max_concurrent_tasks` | `20` | 并发执行上限；`0` 或留空表示未设置 |
| `poll_interval` | `30s` | 执行轮询间隔 |
...
- 传空字符串清除已持久化的值，回到环境变量或内置默认，例如 `silieco config set poll_interval ""`。
