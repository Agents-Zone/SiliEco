...
description: Connect a self-hosted Forgejo, Gitea, or GitLab instance per workspace so pull/merge requests that reference a task identifier auto-attach to that task, move it to Done on merge, and show CI status.
---

import { Callout } from "fumadocs-ui/components/callout";

<Callout type="warn">
**Self-hosted Silieco only.** This integration is available only when you run
Silieco yourself; it is not offered on Silieco Cloud. "Self-hosted" here means
**Silieco** is self-hosted — typically so it can reach a Git instance on your own
network. A server operator must enable it (`SILIECO_VCS_INTEGRATION_ENABLED=true`)
and set `SILIECO_VCS_SECRET_KEY`; until then the section does not appear in
Settings → Integrations.
</Callout>

Silieco connects to a self-hosted Git provider per workspace: **Forgejo**, **Gitea**, or **GitLab**. Once connected, any pull request (GitLab: merge request) whose branch name, title, or body contains a task identifier (for example `SILI-123`) is **auto-linked** to that [task](/tasks), appears under **Pull requests** in the task sidebar, and — when merged with a closing keyword — moves the task to **Done**. CI for the head commit shows as a checks bar on the card.
...
```sh
SILIECO_VCS_INTEGRATION_ENABLED=true
```
...
```sh
SILIECO_VCS_SECRET_KEY=<base64 32-byte key>
```
...
Set `SILIECO_PUBLIC_URL` to the server's public base URL so Silieco can show a ready-to-paste webhook URL. Without it, the UI shows only the webhook path and you prepend your own origin.
...
2. In Silieco, open **Settings → Integrations → Git providers**.
3. Choose the **provider**, enter the instance URL (for example `https://forgejo.example.com`) and the access token, then **Connect**. Silieco validates the token against the instance before saving.
...
Silieco authenticates every delivery against the stored secret, so a webhook without the matching secret is rejected.

## What gets mirrored

- **Pull / merge requests** — open, closed, merged, and draft state, author, branch, and (where the provider supplies them) diff stats.
- **Task links** — identifiers in the title, body, or branch link the PR to the task. A closing keyword (`Closes`/`Fixes`/`Resolves SILI-123`) on a merged PR moves the task to **Done** once no linked PR is still open.
