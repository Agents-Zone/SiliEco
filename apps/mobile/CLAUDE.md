...
- `import type` from `@silieco/core/types/*` (zero runtime coupling)
- Pure functions from `@silieco/core/`
...
- **Main CI** (`.github/workflows/ci.yml`) excludes mobile via `--filter='!@silieco/mobile'`. Mobile failures do NOT block web/desktop PRs.
...
1. **Read the payload.** Find the event in `@silieco/core/types/events.ts`. Note the fields; decide if patch is possible (full object) or invalidate is required (just an id).
