...
export interface MentionChip {
   ...
   *  human identifier (e.g. "SILI-123"), which is what the chip + the
...
function MentionChipView({
  ...
  // Issue chips show the bare identifier (e.g. "SILI-123") — no leading @.
...
function AttachmentChipView({ item, onRemove, onRetry }: AttachmentChipProps) {
  ...
  const onPress = () => {
    ...
    if (isImage) {
      ...
      // `/api/attachments/{id}/download` (SILI-2976). RN's `Linking.openURL`
