...
describe("worktree-dev-env", () => {
  ...
  it("never reuses 5173 even when the offset is 0", () => {
    // POSIX cksum("/tmp/silieco-3494") === 1189739000, % 1000 === 0
    expect(offsetForPath("/tmp/silieco-3494")).toBe(0);
    expect(rendererPortForPath("/tmp/silieco-3494")).toBe(5174);
    expect(rendererPortForPath("/tmp/silieco-3494")).not.toBe(5173);
  });

  it("suffix is '<folder>-<offset>' so it stays recognizable and unique", () => {
    expect(appSuffixForPath("/work/SILI-3724_Desktop")).toBe(
      `sili-3724-desktop-${offsetForPath("/work/SILI-3724_Desktop")}`,
    );
  ...
  it("disambiguates worktrees that share a folder name at different paths", () => {
    // Same basename "silieco", different parent dirs → different offsets/suffixes,
    // so each gets its own single-instance lock.
    expect(offsetForPath("/tmp/a/silieco")).not.toBe(offsetForPath("/tmp/b/silieco"));
    expect(appSuffixForPath("/tmp/a/silieco")).not.toBe(
      appSuffixForPath("/tmp/b/silieco"),
    );
  });
