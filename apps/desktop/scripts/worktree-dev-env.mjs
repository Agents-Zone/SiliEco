...
// (keyed by the app name "Silieco Canary"). The env hooks to override both
...
// "/tmp/silieco-3494" is 1189739000, and 1189739000 % 1000 === 0). Range 5174–6173.
...
// The dev app then shows e.g. "Silieco Canary sili-3724-194" in Cmd+Tab and gets
...
// a basename at different paths (e.g. /a/silieco vs /b/silieco) or whose names
...
// the primary checkout keeps the unchanged 5173 / "Silieco Canary" defaults.
  ...
  if (log) {
    console.log(
      `[dev:desktop] worktree isolation → renderer port ${env.DESKTOP_RENDERER_PORT}, ` +
        `app "Silieco Canary ${env.DESKTOP_APP_SUFFIX}"`,
    );
  }
  return env;
}
