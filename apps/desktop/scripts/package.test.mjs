...
describe("deriveVersion (real git describe)", () => {
  ...
  function initRepo() {
    const dir = mkdtempSync(join(tmpdir(), "silieco-desktop-ver-"));
    repos.push(dir);
    const run = (...args) =>
      execFileSync("git", args, { cwd: dir, encoding: "utf-8" });
    run("init", "-q");
    run("config", "user.email", "test@silieco.ai");
...
describe("electron-builder.yml packaging config", () => {
  // Regression guard for github.com/silieco-ai/silieco/issues/5595. The
