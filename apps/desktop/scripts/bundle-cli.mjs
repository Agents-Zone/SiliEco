#!/usr/bin/env node
// Builds the `silieco` CLI from core/cmd/silieco and copies the binary
...
//
// ldflags mirror `make build` so `silieco --version` reports a meaningful
// version / commit / date.
//
...
function binaryNameForPlatform(platform) {
  return platform === "win32" ? "silieco.exe" : "silieco";
}
...
const srcBinary = join(coreDir, "bin", `${goos}-${goarch}`, binName);
...
if (hasGo()) {
  ...
  await mkdir(join(coreDir, "bin", `${goos}-${goarch}`), { recursive: true });
  execFileSync(
    "go",
    [
      "build",
