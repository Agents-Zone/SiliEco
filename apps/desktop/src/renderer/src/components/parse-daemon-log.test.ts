...
describe("parseLogLine", () => {
  ...
  it("falls back to raw rendering for non-matching lines (panic stack frame)", () => {
    const r = parseLogLine("\tat github.com/silieco/foo (line 42)", 1);
    expect(r.timestamp).toBeNull();
    expect(r.level).toBeNull();
    expect(r.message).toBe("\tat github.com/silieco/foo (line 42)");
    expect(r.fields).toEqual({});
    expect(r.raw).toBe("\tat github.com/silieco/foo (line 42)");
  });
