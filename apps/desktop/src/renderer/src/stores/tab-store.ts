...
import { createPersistStorage, defaultStorage } from "@silieco/core/platform";
import { createSafeId } from "@silieco/core/utils";
import { isReservedSlug } from "@silieco/core/paths";

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------
//
// SILI-4741 Phase 2: tabs are *sessions*, not routers. A TabSession is pure
...
// render time via `routeIconForPath` (@silieco/views/layout), which shares the
// route → icon map in `@silieco/core/paths` with the sidebar nav — so the two
 ...
 * `<Navigate to="issues">` index redirect (SILI-4741 invariant 1: the router
...
export function sanitizeTabPath(path: string): string | null {
  ...
  if (segments.length === 1) {
    return `/${firstSegment}/tasks${suffix}`;
  }
  return path;
}
  ...
  persist(
    (set, get) => ({
      ...
      switchWorkspace(slug, openPath) {
        ...
        if (!existing) {
          ...
          const tab = makeSession(seedPath, "Tasks");
          set({
        ...
        if (desiredPath) {
          ...
          if (clean) {
            ...
            const tab = makeSession(clean, "Tasks");
            set({
              activeWorkspaceSlug: slug,
              byWorkspace: {
                ...byWorkspace,
    ...
    }),
    {
      ...
      migrate: (persistedState, version) => {
        ...
        // v3 → v4: Tab → TabSession (SILI-4741). One-time import of legacy
