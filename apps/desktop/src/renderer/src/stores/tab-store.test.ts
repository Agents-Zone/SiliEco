...
describe("sanitizeTabPath", () => {
  ...
  it("rejects paths whose first segment is a reserved slug (missing workspace prefix)", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    expect(sanitizeTabPath("/tasks")).toBeNull();
  ...
  it("normalizes a bare workspace url to its default surface (replaces the in-router index redirect)", () => {
    expect(sanitizeTabPath("/acme")).toBe("/acme/tasks");
    expect(sanitizeTabPath("/acme?welcome=1")).toBe("/acme/tasks?welcome=1");
  });
});

describe("resourceKeyForUrl", () => {
  it("is the pathname — search and hash are view state, not identity", () => {
    expect(resourceKeyForUrl("/acme/tasks")).toBe("/acme/tasks");
    expect(resourceKeyForUrl("/acme/tasks?filter=a")).toBe("/acme/tasks");
    expect(resourceKeyForUrl("/acme/tasks#anchor")).toBe("/acme/tasks");
    expect(resourceKeyForUrl("/acme/tasks?filter=a#x")).toBe("/acme/tasks");
  });
});
...
describe("migrateV1ToV2", () => {
  it("groups v1 flat tabs by workspace slug", () => {
    const v1 = {
      tabs: [
        { id: "t1", path: "/acme/tasks", title: "Issues", icon: "ListTodo" },
        { id: "t2", path: "/acme/projects", title: "Projects", icon: "FolderKanban" },
        { id: "t3", path: "/butter/tasks", title: "Issues", icon: "ListTodo" },
      ],
      activeTabId: "t2",
    };
  ...
  it("drops tabs at root / transition / reserved-slug paths", () => {
    const v1 = {
      tabs: [
        ...
        { id: "t4", path: "/acme/tasks", title: "Issues", icon: "ListTodo" },
      ],
      activeTabId: "t1",
    };
    const v2 = migrateV1ToV2(v1);
...
describe("useTabStore actions", () => {
  it("switchWorkspace creates a new group with a default tab on first entry", () => {
    ...
    expect(s.byWorkspace.acme.tabs[0].url).toBe("/acme/tasks");
    expect(s.byWorkspace.acme.tabs[0].resourceKey).toBe("/acme/tasks");
  ...
  it("switchWorkspace with openPath dedupes into an existing tab with the same resourceKey", () => {
    ...
    expect(activeTab?.url).toBe("/acme/tasks");
  });
  ...
  it("switchWorkspace with openPath not matching any tab adds a new tab", () => {
    ...
    expect(activeTab?.url).toBe("/acme/tasks/bug-42");
  });
  ...
  it("openTab with a different query focuses the existing tab and keeps its url (RFC §8.2 semantic change)", () => {
    const store = useTabStore.getState();
    store.switchWorkspace("acme"); // default tab at /acme/tasks
    const defaultTabId = useTabStore.getState().byWorkspace.acme.tabs[0].id;

    const id = store.openTab("/acme/tasks?filter=urgent", "Issues");
...
describe("navigateActiveSession", () => {
  ...
  it("rejects cross-workspace urls (those go through switchWorkspace)", () => {
    ...
    store.navigateActiveSession("/butter/tasks");
...
describe("commitScrollMemento", () => {
  ...
  it("keeps other routes' entries when one route commits", () => {
    ...
    store.commitScrollMemento(tabId, "/acme/tasks/bug-42", {
      main: { top: 120, height: 3000 },
    });

    expect(useTabStore.getState().byWorkspace.acme.tabs[0].memento.scroll).toEqual({
      "/acme/tasks::list": { top: 500, height: 8000 },
      "/acme/tasks/bug-42::main": { top: 120, height: 3000 },
    });
  });
...
describe("migrateV2ToV3", () => {
  it("adds pinned=false to every persisted tab", () => {
    ...
    expect(v3.byWorkspace.acme.tabs).toEqual([
      { id: "t1", path: "/acme/tasks", title: "Issues", icon: "ListTodo", pinned: false },
...
describe("migrateV3ToV4 (legacy view-state import, SILI-4741)", () => {
    ...
    expect(v4.byWorkspace.acme.tabs).toEqual([
      {
        ...
        title: "Issues",
        pinned: true,
        history: { stack: ["/acme/tasks"], index: 0 },
        memento: { scroll: {} },
      },
    ]);
  });
...
describe("mergePersistedTabs (rehydration, SILI-4370)", () => {
  const emptyState = (): {
