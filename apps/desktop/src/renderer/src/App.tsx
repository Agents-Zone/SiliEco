...
import { CoreProvider } from "@silieco/core/platform";
import { pickLocale, type SupportedLocale } from "@silieco/core/i18n";
import { useAuthStore } from "@silieco/core/auth";
import { useWelcomeStore } from "@silieco/core/onboarding";
import { workspaceKeys, workspaceListOptions } from "@silieco/core/workspace/queries";
import { api } from "@silieco/core/api";
import { useHasOnboarded } from "@silieco/core/paths";
import { setCurrentWorkspace } from "@silieco/core/platform";
import { ThemeProvider } from "@silieco/ui/components/common/theme-provider";
import { SiliecoIcon } from "@silieco/ui/components/common/silieco-icon";
import { Toaster } from "@silieco/ui/components/ui/sonner";
...
import { captureEvent } from "@silieco/core/analytics";
import { RESOURCES } from "@silieco/views/locales";
  ...
  if (isLoading) {
    return (
      <div className="flex h-screen items-center justify-center">
        <SiliecoIcon className="size-6 animate-pulse" />
      </div>
    );
  }
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.silieco config).
  useEffect(() => {
    if (!runtimeConfig) return;
    window.daemonAPI.setTargetApiUrl(runtimeConfig.apiUrl);
  }, [runtimeConfig]);

  // Listen for invite IDs delivered via deep link (silieco://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (silieco://auth/callback?token=...).
  ...
  useEffect(() => {
    if (!user) return;
    const token = localStorage.getItem("silieco_token");
    if (!token) return;
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-body text-muted-foreground">
          Silieco Desktop could not load <code>~/.silieco/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
