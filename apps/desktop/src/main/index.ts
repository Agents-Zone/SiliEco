...
// silieco CLI can find agent binaries like claude/codex/opencode. Must run
...
function handleDeepLink(url: string): void {
  try {
    ...
    // silieco://auth/callback?token=<jwt>
...
function installLocaleRefresh(window: BrowserWindow): void {
  // Electron has no dedicated OS-language event. Check whenever any Silieco
...
// single-worktree case still lands at "Silieco Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Silieco Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Silieco Canary";
...
if (is.dev) {
  ...
  // to "Silieco", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.silieco.desktop.dev" : "ai.silieco.desktop",
    );
