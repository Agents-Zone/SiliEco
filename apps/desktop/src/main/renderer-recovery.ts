...
function rendererRecoveryDetail(payload: ReloadPromptPayload) {
  const guidance = [
    "Click Reload to refresh this window and keep using Silieco.",
  ...
  if (payload.kind === "unresponsive") {
    guidance.push(
      "For macOS reports, an Activity Monitor sample of the Silieco Helper (Renderer) process helps us find what blocked the app.",
    );
  }
