...
const PREFS_PATH = join(homedir(), ".silieco", "desktop_prefs.json");
...
// `silieco daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in core/cmd/silieco/cmd_daemon.go) to
...
// Keep the Go impl in sync: core/cmd/silieco/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".silieco", "profiles", profile)
    : join(homedir(), ".silieco");
}
...
// Sidecar file that records which Silieco user the cached PAT in config.json
 ...
 * a stuck start is an auth problem. Hits the same endpoint `silieco auth status`
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["silieco.exe"] : ["silieco"];
 ...
 *   to `apps/desktop/resources/bin/silieco`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Silieco.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "silieco.exe" : "silieco";
...
/**
 * Returns a usable `silieco` binary path. Priority:
 ...
 *   5. `silieco` on PATH (dev convenience / user-installed via brew).
 ...
 * daemon needs a PAT (or `sili_` / `mdt_` token) because JWTs expire in 30
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Silieco Desktop" }),
  });
  if (!res.ok) {
  ...
  if (typeof data.token !== "string" || !data.token.startsWith("sili_")) {
 ...
 * - If the profile already has a cached PAT (`sili_...`) AND the sidecar user
...
async function syncToken(
  ...
  const sameUserWithCachedPat =
    ...
    config.token.startsWith("sili_");

  let finalToken: string;
  if (tokenFromRenderer.startsWith("sili_")) {
...
async function savePrefs(prefs: DaemonPrefs): Promise<void> {
  const dir = join(homedir(), ".silieco");
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, SILIECO_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "silieco CLI is not installed" };
