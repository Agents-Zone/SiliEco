const RELEASE_ARCHIVE_PREFIX = "silieco-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `silieco-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `silieco_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `silieco_${os}_${mappedArch}.${ext}`;
