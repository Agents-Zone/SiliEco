...
describe("runtime config", () => {
  it("uses local Silieco defaults without a desktop.json file", () => {
  ...
  it("strips the leading api. label when deriving appUrl", () => {
    expect(
      parseRuntimeConfig(
        JSON.stringify({ schemaVersion: 1, apiUrl: "https://api.silieco.ai" }),
      ),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.silieco.ai",
      wsUrl: "wss://api.silieco.ai/ws",
  ...
  it("rejects non-http api schemes", () => {
    expect(() =>
      parseRuntimeConfig(JSON.stringify({ schemaVersion: 1, apiUrl: "file:///tmp/silieco" })),
  ...
  it("derives dev appUrl by stripping the leading api. label", () => {
    ...
    // public web host, not the api host. Silieco's convention exposes the
    // api at `api.<web-host>`, so stripping the leading label gives the
    // right web origin without a separate VITE_APP_URL.
    expect(
      runtimeConfigFromDevEnv({ apiUrl: "https://api.test.silieco.ai" }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.silieco.ai",
      wsUrl: "wss://api.test.silieco.ai/ws",
      appUrl: "https://test.silieco.ai",
    });
  });
  ...
  it("dev VITE_APP_URL still wins over apiUrl-derived value", () => {
    expect(
      runtimeConfigFromDevEnv({
        apiUrl: "https://api.test.silieco.ai",
        appUrl: "https://staging.silieco.ai",
      }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.silieco.ai",
