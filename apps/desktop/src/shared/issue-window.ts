export const ISSUE_WINDOW_ARGUMENT_PREFIX = "--silieco-issue-window=";
...
export function parseIssueWindowRequest(value: unknown): IssueWindowContext | null {
  ...
  const title = rawTitle.slice(0, MAX_ISSUE_WINDOW_TITLE_LENGTH) || "Task";
...
export function parseIssueWindowPath(
  ...
  let url: URL;
  try {
    url = new URL(value, "https://desktop.silieco.invalid");
  } catch {
    return null;
  }
  if (url.origin !== "https://desktop.silieco.invalid") return null;

  const segments = url.pathname.split("/");
  if (
    segments.length !== 4 ||
    segments[0] !== "" ||
    (segments[2] !== "tasks" && segments[2] !== "issues")
  ) {
    return null;
  }
  ...
  const path = `/${workspaceSlug}/tasks/${encodeURIComponent(issueId)}${url.search}${url.hash}`;
