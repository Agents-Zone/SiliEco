...
通过可复用的 SOP 协调人类员工与智能体员工完成工作，让一把手掌握从目标到验收的全过程。
...
Silieco 是一个 SOP 执行平台，通过可复用的 SOP 协调人类员工与智能体员工完成工作。

平台围绕三类角色运行：

* **SOP Designer**：设计和迭代可复用的 SOP，包括阶段、输入、输出、所需 skills 和评审关口。

* **SOP Executors**：人类员工、智能体员工和蜂群共同完成分配的 task，提交产物、更新进度并报告阻塞。

* **一把手**：设定业务目标和优先级，审查进度、阻塞、关键决策和结果，掌握从目标到验收的全过程。

团队可以从一句目标描述开始，把工作拆成 Task，将 Task 分配给人、Agent 或蜂群，\
并让执行过程、讨论、决策、评审和结果持续沉淀在同一份协作记录中。

## 演示待办

* 从观众视角展示一条正在执行的 SOP：业务目标、当前阶段、task、输入、产出、进度和阻塞。

* 展示每类角色的工作：SOP Executors 完成工作并更新进度；一把手审查进度、阻塞、关键决策和验收结果。

* 从 SOP 执行过渡到 SOP 设计：展示 SOP Designer 如何根据运行记录调整阶段、task、输入、评审关口和智能体职责。

* 创建两个标杆 SOP：
  * **支付异常订单查单与商户回复**：从异常发生，到查单、银行催办、商户回复、结案和复盘，提升运营质量。
  * **新支付服务 MVP 验证**：从机会判断，到目标客户与使用场景、原型或接入准备、测试、MVP 验证，以及继续、调整或停止的决策，加快新业务响应。
