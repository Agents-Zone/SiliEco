...
This guide documents the local development workflow for contributors working on the Silieco codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=silieco`
...
POSTGRES_DB=silieco
POSTGRES_PORT=5432
DATABASE_URL=postgres://silieco:silieco@localhost:5432/silieco?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=silieco_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://silieco:silieco@localhost:5432/silieco_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../silieco-feature -b feat/my-change main
cd ../silieco-feature
make dev
```
...
- main checkout
  ...
  - frontend: `3000`
- worktree checkout
  - database: `silieco_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`silieco login`).
...
whatever server is configured in `~/.silieco/config.json`. That's fine for
...
Example: worktree at `../silieco-feat-auth` produces profile
`dev-silieco_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `SILIECO_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.silieco/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `silieco` commands automatically use the same
...
rm -rf "$HOME/.silieco/profiles/$PROFILE"
```
...
1. Compiles the `silieco` CLI from `core/cmd/silieco` into
   `apps/desktop/resources/bin/silieco`
...
backend logs. If you set `SILIECO_DEV_VERIFICATION_CODE=888888` before starting
...
- `DESKTOP_APP_SUFFIX` = `<folder>-<offset>` — its own single-instance lock /
  `userData`, and an app named `Silieco Canary <folder>-<offset>` so it is
  distinguishable in Cmd+Tab. The offset keeps it unique across worktrees that
  share a folder name at different paths.

The primary checkout is left untouched (`5173`, `Silieco Canary`). Set either
...
Nothing in this flow touches the system-installed `silieco` or the default
`~/.silieco/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.silieco/config.json` | `~/.silieco/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.silieco/daemon.pid` | `~/.silieco/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/silieco_workspaces/` | `~/silieco_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `silieco_<slug>_<hash>` |
| Desktop profile | `desktop-api.silieco.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U silieco -d postgres -At -c "select datname from pg_database order by datname;"
```
