...
This document is designed for AI agents to execute. Follow these steps exactly to deploy a local Silieco instance and connect to it.
...
Wait for the server output `✓ Silieco server is running and CLI is ready!` before running `silieco setup self-host`.

**Expected result:**
- Frontend at http://localhost:3000
- Backend at http://localhost:8080
- `silieco` CLI installed and configured for localhost
...
The `silieco setup self-host` command will:
...
2. Run `make selfhost`
3. Run `silieco setup self-host --port <PORT> --frontend-port <FRONTEND_PORT>`

## Troubleshooting

- **Backend not ready:** `docker compose -f docker-compose.selfhost.yml logs backend`
- **Frontend not ready:** `docker compose -f docker-compose.selfhost.yml logs frontend`
- **Daemon issues:** `silieco daemon logs`
