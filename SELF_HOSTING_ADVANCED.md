...
This document covers advanced configuration for self-hosted Silieco deployments. For the quick start guide, see [SELF_HOSTING.md](SELF_HOSTING.md).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string | `postgres://silieco:silieco@localhost:5432/silieco?sslmode=disable` |
...
Silieco supports two email backends. `SMTP_HOST` takes priority when set; otherwise `RESEND_API_KEY` is used. With neither configured, verification codes are printed to the server log — copy them from there to log in.
...
| `RESEND_FROM_EMAIL` | Sender email address (default: `noreply@silieco.ai`) |
...
> **Note:** If neither Resend nor SMTP is configured, generated verification codes are printed to backend logs — copy them from there to log in. A fixed local testing code (e.g. `888888`) is **opt-in only**: set `SILIECO_DEV_VERIFICATION_CODE=888888` in `.env` and keep `APP_ENV` non-production. The Docker self-host stack pins `APP_ENV=production`, so the shortcut is ignored there. **Never enable a fixed code on a publicly reachable instance.**
...
If the frontend and backend are served from different hostnames, `COOKIE_DOMAIN` is **required**, not optional: the browser must be able to read the `silieco_csrf` cookie from the page's own origin to send the `X-CSRF-Token` header, and without it every write request fails with `403 {"error":"CSRF validation failed"}`. Scope it to the narrowest parent domain that covers both hosts — it also shares the `silieco_auth` session cookie with every host under that domain. See [Reverse Proxy](#reverse-proxy) for the full split-domain configuration and its trust requirements.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `SILIECO_SERVER_URL` | `ws://localhost:8080/ws` | WebSocket URL for daemon → server connection |
| `SILIECO_APP_URL` | `http://localhost:3000` | Frontend URL for CLI login flow |
| `SILIECO_DAEMON_POLL_INTERVAL` | `3s` | How often the daemon polls for tasks |
| `SILIECO_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat frequency |
...
| `SILIECO_COPILOT_PATH` | Custom path to the `copilot` (GitHub Copilot CLI) binary |
...
| `SILIECO_GROK_PATH` | Custom path to the `grok` binary |
| `SILIECO_GROK_MODEL` | Override the Grok model used (e.g. `grok-4.5`) |

## Database Setup

Silieco requires PostgreSQL 17 with the pgvector extension.
...
The Usage and Runtime dashboards read from `task_usage_hourly`, a derived table populated by `rollup_task_usage_hourly()`. As of SILI-2957 the backend runs this rollup **in-process** on every replica via a DB-backed scheduler (`sys_cron_executions`); a fresh self-host install needs no operator action — the bundled `pgvector/pgvector:pg17` image works without changes.
...
External cron / systemd / Kubernetes `CronJob` setups that call `SELECT rollup_task_usage_hourly()` directly are also still valid — they were the only option before SILI-2957 and remain a supported compatibility path. They are no longer the recommended setup; new deployments should rely on the in-process scheduler.
...
# Kubernetes
kubectl -n silieco exec deploy/silieco-backend -- \
  ./backfill_task_usage_hourly --sleep-between-slices=2s
```
...
DATABASE_URL="your-database-url" ./core/bin/migrate up

# Start the backend server
DATABASE_URL="your-database-url" PORT=8080 JWT_SECRET="your-secret" ./core/bin/server
```
...
```bash
METRICS_ADDR=127.0.0.1:9090 ./core/bin/server
...
Pin `SILIECO_IMAGE_TAG` in `.env` to an exact release like `v0.2.4` if you want to stay on a specific version. Migrations run automatically on backend startup. They are idempotent — running them multiple times has no effect.
